...
namespace VGMToolbox.format
{
    public abstract class MpegStream
    {
        ...
        public struct DemuxOptionsStruct
        {
            ...
            public List<string> ExtractedFileList { set; get; }
            public bool ExtractInMemoryStream { get; set; }
            public List<MemoryStream> ExtractedMemoryStream { get; set; }
        }
        ...
        protected virtual void DoFinalTasks(FileStream sourceFileStream, Dictionary<uint, Stream> outputFiles, bool addHeader)
        {

        }

        protected virtual bool ReadBlock(FileStream fs, byte[] currentBlockId, long currentOffset)
        {
            return false;
        }
        protected virtual void TryDecrypt(byte[] block)
        {
        ...
        public virtual void DemultiplexStreams(DemuxOptionsStruct demuxOptions)
        {
            using (FileStream fs = File.OpenRead(this.FilePath))
            {
                ...
                Dictionary<uint, Stream> streamOutputWriters = new Dictionary<uint, Stream>();
                            ...
                            if (BlockIdDictionary.ContainsKey(currentBlockIdVal))
                            {
                                ...
                                switch (blockStruct.SizeType)
                                {
                                    ...
                                    case PacketSizeType.SizeBytes:
                                        ...
                                        if(!ReadBlock(fs, currentBlockId, currentOffset))
                                        {
                                                ...
                                                (demuxOptions.ExtractVideo && this.IsThisAVideoBlock(currentBlockId)))
                                            {
                                                ...
                                                if (!streamOutputWriters.ContainsKey(currentStreamKey))
                                                {
                                                    ...
                                                    if(string.IsNullOrEmpty(demuxOptions.OutputPath))
                                                        outputFileName = Path.Combine(Path.GetDirectoryName(this.FilePath), outputFileName);
                                                    else
                                                        outputFileName = Path.Combine(demuxOptions.OutputPath, outputFileName);

                                                    // add an output stream for writing
                                                    if(!demuxOptions.ExtractInMemoryStream)
                                                        streamOutputWriters[currentStreamKey] = new FileStream(outputFileName, FileMode.Create, FileAccess.ReadWrite);
                                                    else
                                                    {
                                                        streamOutputWriters[currentStreamKey] = new MemoryStream((int)fileSize);
                                                    }
                                                    demuxOptions.ExtractedFileList.Add(outputFileName);
                                                }
                                                ...
                                                else
                                                {
                                                    ...
                                                    if (cutSize > 0)
                                                    {
                                                        byte[] block = ParseFile.ParseSimpleOffset(fs, currentOffset + currentBlockId.Length + blockSizeArray.Length + videoBlockSkipSize, (int)(blockSize - videoBlockSkipSize));
                                                        TryDecrypt(block);
                                                        streamOutputWriters[currentStreamKey].Write(block, 0, cutSize);
                                                    }
    #if DEBUG
                                                    //else
                                                    //{
                ...
                foreach(var stream in streamOutputWriters)
                {
                    if(stream.Value is MemoryStream)
                    {
                        demuxOptions.ExtractedMemoryStream.Add(stream.Value as MemoryStream);
                    }
                }
        ...
        private void closeAllWriters(Dictionary<uint, Stream> writers)
        {
            //////////////////////////
            // close all open writers
            //////////////////////////
            ...
            {
                if (writers[b].CanRead && writers[b] is FileStream)
                {
                    writers[b].Close();
                    writers[b].Dispose();
                }
