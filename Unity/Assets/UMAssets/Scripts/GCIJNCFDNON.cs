...
public class GCIJNCFDNON_SceneInfo
{
	...
	{
		CIOECGOMILE.HHCJCDFCLOB.AHEFHIMGIBI_PlayerData.PNLOINMCCKH_Scene.OPIBAPEGCLA[BCCHOBPJJKE_SceneId - 1].LHMOAJAIJCO_New = false;
	}
	...
	public void KHEKNNFCAOI(int PPFNGGCBJKC_Id_, byte[] KBOLNIBLIND_Mb_, byte[] ODKMKEHJOCK_Sb_, int JPIPENJGGDD_Mlt_/* = 0*/, int IELENGDJPHF_Ulk_/* = 0*/, int MJBODMOLOBC_Luck_/* = 0*/, bool _CADENLBDAEB_IsNew/* = false*/, long NPHOIEOPIJO_Date_/* = 0*/, int BLHGJPIFKCL_Leaf_/* = 0*/)
	{
		...
		MLIBEPGADJH_Scene.KKLDOOJBJMN s = db.ECNHDEHADGL_Scene.CDENCMNHNGA_table[PPFNGGCBJKC_Id_ - 1];
		...
		EKLIPGELKCL_Rarity = (byte)((JPIPENJGGDD_Mlt_ > 0 || JOKJBMJBLBB_Single) ? JKGFBFPIMGA_Rarity + 1 : JKGFBFPIMGA_Rarity);
		...
		CIEOBFIIPLD_Level = KBOLNIBLIND_Mb_ == null ? 1 : CEDHHAGBIBA.OGPFNHOKONH_GetNumBitActive(KBOLNIBLIND_Mb_) + 1;
	...
	public void JNNHIDMNBFG(string IDMLDEJMMED_Name/* = ""*/, int _DMNIMMGGJJJ_Leaf/* = -1*/)
	{
		...
		{
			_DMNIMMGGJJJ_Leaf = CIOECGOMILE.HHCJCDFCLOB.AHEFHIMGIBI_PlayerData.PNLOINMCCKH_Scene.OPIBAPEGCLA[BCCHOBPJJKE_SceneId - 1].DMNIMMGGJJJ_Leaf;
		}
	...
	public void HCDGELDHFHB()
	{
		...
		{
			CIEOBFIIPLD_Level = CEDHHAGBIBA.OGPFNHOKONH_GetNumBitActive(KBOLNIBLIND_Mb) + 1;
			if (a != 0)
			{
				DDEDANKHHPN_SkillLevel = s.MGKDKGOACDB_GetSkillLevel(IMMAOANGPNK.HHCJCDFCLOB.NKEBMCIMJND_Database.JEMMMJEJLNL_Board, KBOLNIBLIND_Mb);
				DHEFMEGKKDN_CenterSkillRank = IMMAOANGPNK.HHCJCDFCLOB.NKEBMCIMJND_Database.FOFADHAENKC_Skill.COLCPGFABLP_CenterSkills[a - 1].JGNHOGKKPDM;
	...
	public static StatusData JPCAIAAOOLN(int _BCCHOBPJJKE_SceneId, byte[] KBOLNIBLIND, byte[] ODKMKEHJOCK, int JPIPENJGGDD, int DPCIPDAECEA/* = 0*/)
	{
	...
	public bool OEIFKIPOHME(int OIPCCBHIKIA, int HMFFHLPNMPH)
	{
		if (OIEHPHINMIO(OIPCCBHIKIA) != HINAICIJJJC.LAHFPKEJLCA_3)
			return false;
		MLIBEPGADJH_Scene.KKLDOOJBJMN scene = IMMAOANGPNK.HHCJCDFCLOB.NKEBMCIMJND_Database.ECNHDEHADGL_Scene.CDENCMNHNGA_table[BCCHOBPJJKE_SceneId - 1];
		MMPBPOIFDAF_Scene.PMKOFEIONEG scene2 = CIOECGOMILE.HHCJCDFCLOB.AHEFHIMGIBI_PlayerData.PNLOINMCCKH_Scene.OPIBAPEGCLA[BCCHOBPJJKE_SceneId - 1];
	...
	public int KPCLNEADGEM(int OIPCCBHIKIA)
	{
		...
		if (a < HINAICIJJJC.LAHFPKEJLCA_3 || a > HINAICIJJJC.LBODGCPCGHA_4)
			return 0;
		MMPBPOIFDAF_Scene.PMKOFEIONEG scene = CIOECGOMILE.HHCJCDFCLOB.AHEFHIMGIBI_PlayerData.PNLOINMCCKH_Scene.OPIBAPEGCLA[BCCHOBPJJKE_SceneId - 1];
		MLIBEPGADJH_Scene.KKLDOOJBJMN dbScene = IMMAOANGPNK.HHCJCDFCLOB.NKEBMCIMJND_Database.ECNHDEHADGL_Scene.CDENCMNHNGA_table[BCCHOBPJJKE_SceneId - 1];
	...
	public int MCDPPBBLDKA(int OIPCCBHIKIA)
	{
		if (OIEHPHINMIO(OIPCCBHIKIA) < HINAICIJJJC.LAHFPKEJLCA_3/*3*/ || OIEHPHINMIO(OIPCCBHIKIA) > HINAICIJJJC.LBODGCPCGHA_4/*4*/)
			return 0;
		return CIOECGOMILE.HHCJCDFCLOB.AHEFHIMGIBI_PlayerData.PNLOINMCCKH_Scene.OPIBAPEGCLA[BCCHOBPJJKE_SceneId - 1].CDOBCKMHAOK_Inf;
	}
	...
	{
		MLIBEPGADJH_Scene.KKLDOOJBJMN k = IMMAOANGPNK.HHCJCDFCLOB.NKEBMCIMJND_Database.ECNHDEHADGL_Scene.CDENCMNHNGA_table[BCCHOBPJJKE_SceneId - 1];
		return k.NGGBHLCKJGO(IMMAOANGPNK.HHCJCDFCLOB.NKEBMCIMJND_Database.JEMMMJEJLNL_Board, KBOLNIBLIND_Mb, ODKMKEHJOCK_Sb, JPIPENJGGDD_NumBoard, IMMAOANGPNK.HHCJCDFCLOB.NKEBMCIMJND_Database.HNMMJINNHII_Game.GENHLFPKOEE(k.EKLIPGELKCL_Rarity, k.MCCIFLKCNKO_Feed), CIOECGOMILE.HHCJCDFCLOB.AHEFHIMGIBI_PlayerData.PNLOINMCCKH_Scene.OPIBAPEGCLA[BCCHOBPJJKE_SceneId - 1].CDOBCKMHAOK_Inf);
	}
	...
	public int FGMPFHOEPEL_GetNumUnlockedStory()
	{
		if (JKGFBFPIMGA_Rarity < 6)
			return 0;
		return IMMAOANGPNK.HHCJCDFCLOB.NKEBMCIMJND_Database.ECNHDEHADGL_Scene.CDENCMNHNGA_table[BCCHOBPJJKE_SceneId - 1].JGKOJEOLMIL_GetNumUnlockedStory(IMMAOANGPNK.HHCJCDFCLOB.NKEBMCIMJND_Database.JEMMMJEJLNL_Board, KBOLNIBLIND_Mb);
	}
	...
	public static int CGJCEHGFHMA(int _BCCHOBPJJKE_SceneId)
	{
		...
		MMPBPOIFDAF_Scene.PMKOFEIONEG saveScene = CIOECGOMILE.HHCJCDFCLOB.AHEFHIMGIBI_PlayerData.PNLOINMCCKH_Scene.OPIBAPEGCLA[_BCCHOBPJJKE_SceneId - 1];
	...
	public static int JLNGOOGHCNA(int _BCCHOBPJJKE_SceneId, int JKGFBFPIMGA, bool OOCNLBJJBKI, int JPIPENJGGDD)
	{
	...
	public void OHJIKMCAOLE()
	{
		if(BPJEOPHAJPP_PrevTotalPoint < CGJCEHGFHMA())
		{
			...
			{
				OCLHKHAMDHF_Episode.JEHNEEBBDBO_EpisodeInfo ep = CIOECGOMILE.HHCJCDFCLOB.AHEFHIMGIBI_PlayerData.NGHJPEIKLJL_Episode.BBAJKJPKOHD_EpisodeList[scene.KELFCMEOPPM_Ep - 1];
		...
		CIOECGOMILE.HHCJCDFCLOB.AHEFHIMGIBI_PlayerData.PNLOINMCCKH_Scene.OPIBAPEGCLA[BCCHOBPJJKE_SceneId - 1].IDBDAPPJOND();
	}
