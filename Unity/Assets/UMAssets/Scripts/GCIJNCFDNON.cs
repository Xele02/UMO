...
public class GCIJNCFDNON_SceneInfo
{
	...
	{
		if(FAPMGGOMCOE(OIPCCBHIKIA) != HINAICIJJJC.HJNNKCMLGFL_0 && KBOLNIBLIND != null)
		{
			if (KBOLNIBLIND.Length <= (OIPCCBHIKIA >> 3))
				return;
			if(LHMDABPNDDH == HINAICIJJJC.JIKCABGFIEG_2)
			{
				KBOLNIBLIND[OIPCCBHIKIA >> 3] |= (byte)(1 << (OIPCCBHIKIA & 7));
			}
			else
			{
				KBOLNIBLIND[OIPCCBHIKIA >> 3] &= (byte)~(1 << (OIPCCBHIKIA & 7));
	...
	public void GIAPABMCNOC(int OIPCCBHIKIA, HINAICIJJJC LHMDABPNDDH)
	{
		if ((OIEHPHINMIO(OIPCCBHIKIA) <= HINAICIJJJC.LBODGCPCGHA_4 && ((1 << (int)OIEHPHINMIO(OIPCCBHIKIA)) & 0x19U) != 0)
			|| ODKMKEHJOCK == null)
			return;
		if (ODKMKEHJOCK.Length <= OIPCCBHIKIA >> 3)
			return;
		if (LHMDABPNDDH == HINAICIJJJC.JIKCABGFIEG_2)
		{
			ODKMKEHJOCK[OIPCCBHIKIA >> 3] |= (byte)(1 << (OIPCCBHIKIA & 7));
		}
		else
		{
			ODKMKEHJOCK[OIPCCBHIKIA >> 3] &= (byte)~(1 << (OIPCCBHIKIA & 7));
	...
	public void EJLGAMEIMEG(MNDAMOGGJBJ KNAELKENHPF, List<int> FHHIONFFABP)
	{
		...
		{
			if(KNAELKENHPF.MHGAOHAHFDN[i].HMFFHLPNMPH > 0)
			{
				l1.Add(KNAELKENHPF.MHGAOHAHFDN[i].PPFNGGCBJKC_Id);
				l2.Add(KNAELKENHPF.MHGAOHAHFDN[i].HMFFHLPNMPH);
			}
		}
