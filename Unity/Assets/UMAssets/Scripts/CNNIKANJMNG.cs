...
public class CNNIKANJMNG
{
			...
			if(GNEIHIIJMJK_FriendReq.NFEAMMJIMPG.MDIBIIHAAPN_NextPage < 1)
			{
				if(!PFFJNEFNAMI)
				{
					...
					req2.BHFHGFKBOHH_OnSuccess = (CACGCMBKHDI_Request NHECPMNKEFK) =>
					{
						...
						if(r.NFEAMMJIMPG.DHDHHHOAIKF.Count == 0)
						{
						...
						else
						{
							...
							for(int i_ = 0; i_ < r.NFEAMMJIMPG.DHDHHHOAIKF.Count; i_++)
							{
								l_.Add(r.NFEAMMJIMPG.DHDHHHOAIKF[i_].EHGBICNIBKE_PlayerId);
							}
							FGEIGGNCGGD(l_, CJHEHIMLGGL, 0, r.NFEAMMJIMPG.DHDHHHOAIKF, KLMFJJCNBIP, IDAEHNGOKAE, JGKOLBLPMPG);
						}
					};
					yield break;
				}
				if(MIINNCFHBCL.Count != 0)
				{
					...
					{
						//0x175F8A0
						IBIGBMDANNM item_ = APGCNBONNPE_GetPlayerInfo(DEPGBBJMFED, HHNFHJCAPJO);
	...
	private IBIGBMDANNM APGCNBONNPE_GetPlayerInfo(string DEPGBBJMFED, int HHNFHJCAPJO)
	{
		...
		res.KNIFCANOHOC_Score = IKJBJNJLMKH(res.AHEFHIMGIBI_PlayerData, DEPGBBJMFED, HHNFHJCAPJO);
		res.HMLEDBJDCAF_PreciseScore = LEIMLIFCHCD(res.AHEFHIMGIBI_PlayerData, DEPGBBJMFED, HHNFHJCAPJO);
		res.LHMDABPNDDH_Type = IBIGBMDANNM.LJJOIIAEICI.CCAPCGPIIPF_Guest;
		return res;
	}

	//// RVA: 0x175DEAC Offset: 0x175DEAC VA: 0x175DEAC
	private int IKJBJNJLMKH(BBHNACPENDM_ServerSaveData _AHEFHIMGIBI_PlayerData, string DEPGBBJMFED, int HHNFHJCAPJO)
	{
	...
	private double LEIMLIFCHCD(BBHNACPENDM_ServerSaveData _AHEFHIMGIBI_PlayerData, string DEPGBBJMFED, int HHNFHJCAPJO)
	{
		string a = NGNLPIBPHJH.DEPGBBJMFED;
		if (a != DEPGBBJMFED)
			return 0;
