...
public class LOBDIAABMKG
{
	...
	{
		if (INDDJNMPONH_Type == GCAHJLOGMCI.KNMMOMEHDON_GachaType.DLOPEFGOAPD_10_PassGacha)
			...
			if(summonType == _BJLONGBNPCI_SummonType || AJOAFPDICDK_NoLabelCheck_)
			{
				...
				if(ok)
				{
					...
					if(KACECFNECON_extra.HNKHCIDOKFF_PlateBgId < 1)
					{
						...
						{
							if(INDDJNMPONH_Type == GCAHJLOGMCI.KNMMOMEHDON_GachaType.GENEIBGNMPH_3/*3*/)
							{
								//
							}
	...
	private bool HJLKMDHCGFP(KOPCFBCDBPC CCBEKGNDDBE, JBHCLFDBPKP BKGFCEIFMNF, GCAHJLOGMCI.NFCAJPIJFAM_SummonType _BJLONGBNPCI_SummonType)
	{
		...
		int stepIdx = CCBEKGNDDBE.LKHAAGIJEPG_player_status.DBNAGGGJDAB_CurrentStepIndex;
		long viewOrder = 0;
		if(KACECFNECON_extra != null)
		...
		if(CCBEKGNDDBE.BMFEGOMNECF_Steps[stepIdx - 1].FHBJOLPCAPN_MaxCount < 1)
			return true;
		return CCBEKGNDDBE.LKHAAGIJEPG_player_status.LPIKIBKFOJE_current_step_count < CCBEKGNDDBE.BMFEGOMNECF_Steps[stepIdx - 1].FHBJOLPCAPN_MaxCount;
	}

	// // RVA: 0x10C6E60 Offset: 0x10C6E60 VA: 0x10C6E60
	public bool KHEKNNFCAOI_Init(List<KBPDNHOKEKD_ProductId> BBKDLIPKADG, int _MGBDCFIKBPM_Serie, GCAHJLOGMCI.KNMMOMEHDON_GachaType _INDDJNMPONH_Type, bool _IAHLNPMFJMH_Tutorial, int APHNELOFGAK_CurrencyId_)
	{
	...
	public bool KIAIFPFBGJC(GCAHJLOGMCI.NFCAJPIJFAM_SummonType _BJLONGBNPCI_SummonType, CACGCMBKHDI_Request.HDHIKGLMOGF _BHFHGFKBOHH_OnSuccess, DJBHIFLHJLK HDFGHFOCHKE, DJBHIFLHJLK _JGKOLBLPMPG_OnFail)
	{
		...
		if(_BJLONGBNPCI_SummonType == GCAHJLOGMCI.NFCAJPIJFAM_SummonType.OBLEFFEJGIJ_8)
		{
			...
			HFPHHNDMLKE.BHFHGFKBOHH_OnSuccess = (CACGCMBKHDI_Request JPDFGNJPGHL) =>
			{
				...
				req.MMEBLOIJBKE_UniqueKey = NECDFDNBHFK_StepData.FJGCDPLCIAK_UniqueKey;
				req.NNEGBDKOAHN_Hash = NECDFDNBHFK_StepData.LKHAAGIJEPG_player_status.ENJCKADOFBD_NextPurchaseHash;
		...
		else
		{
			...
			PDHHLMOCEDK.BHFHGFKBOHH_OnSuccess = (CACGCMBKHDI_Request JPCBBHOLPGI) =>
			{
				...
				{
					//0x10CC1A4
					if(INDDJNMPONH_Type == GCAHJLOGMCI.KNMMOMEHDON_GachaType.GENEIBGNMPH_3)
					{
	...
	private IEnumerator BAKEDPGFOLF_Coroutine_GetStepUpItemSetRecord(IMCBBOAFION _BHFHGFKBOHH_OnSuccess, DJBHIFLHJLK _MOBEEPPKFLG_OnFail)
	{
		...
		PNLGHFCFPPK_Request.MMEBLOIJBKE_UniqueKey = NECDFDNBHFK_StepData.FJGCDPLCIAK_UniqueKey;
	...
	public int IEEHJPABKMP_GetOwnedQuantity()
	{
		if (MHKCPJDNJKI_products[3] == null)
		{
			if (MHKCPJDNJKI_products[1] == null)
				return 0;
			return MHKCPJDNJKI_products[1].HMFDJHEEGNN_BuyLimit;
		}
		return MHKCPJDNJKI_products[3].GIEBJDKLCDH_BoughtQuantity;
	}
	...
	public int GBAMENANDAN_GetMaxLimit()
	{
		...
		return MHKCPJDNJKI_products[3].HMFDJHEEGNN_BuyLimit;
	}
	...
	public int GECLFOEDJEI_GetOwnedQuantity()
	{
		if(MHKCPJDNJKI_products[4] == null)
		{
			if (MHKCPJDNJKI_products[2] == null)
				return 0;
			return MHKCPJDNJKI_products[2].GIEBJDKLCDH_BoughtQuantity;
		}
		return MHKCPJDNJKI_products[4].GIEBJDKLCDH_BoughtQuantity;
	}
	...
	public int IBNBABPKLAA_GetMaxLimit()
	{
		if(MHKCPJDNJKI_products[4] == null)
		{
			if (MHKCPJDNJKI_products[2] == null)
				return 0;
			return MHKCPJDNJKI_products[2].HMFDJHEEGNN_BuyLimit;
		}
		return MHKCPJDNJKI_products[4].HMFDJHEEGNN_BuyLimit;
	}
	...
	public void EIHCALCFNEJ(LOBDIAABMKG DPBDFPPMIPH)
	{
		for(int i = 0; i < 11; i++)
		{
			if(MHKCPJDNJKI_products[i] == null)
			{
				if(DPBDFPPMIPH.MHKCPJDNJKI_products[i] != null)
				{
					MHKCPJDNJKI_products[i] = DPBDFPPMIPH.MHKCPJDNJKI_products[i];
				}
			}
			if(DHIACJMOEBH[i] == 0)
			{
