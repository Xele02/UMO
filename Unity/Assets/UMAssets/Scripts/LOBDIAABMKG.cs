...
public class LOBDIAABMKG
{
	...
	public MMNNAPPLHFM CHNFEEOJJCO(int CPNGJMKFCJI)
	{
		...
		{
			return NECDFDNBHFK_StepData.BMFEGOMNECF_Step.Find((MMNNAPPLHFM GHPLINIACBB) =>
			{
				//0x10CB54C
	...
	public int FKKCFICCGMM(int APHNELOFGAK)
	{
		...
		if(INDDJNMPONH_Category == GCAHJLOGMCI.KNMMOMEHDON_GachaType.OOABDNHIEFK_9)
		{
			for(int i = 0; i < IMMAOANGPNK.HHCJCDFCLOB.NKEBMCIMJND_Database.NBKNAAPBFFL_BonusVc.CDENCMNHNGA.Count; i++)
			{
				...
				if(dbVc.CPGFOBNKKBF_CurrencyId == APHNELOFGAK)
				{
	...
	private bool HJLKMDHCGFP(KOPCFBCDBPC CCBEKGNDDBE, JBHCLFDBPKP BKGFCEIFMNF, GCAHJLOGMCI.NFCAJPIJFAM_SummonType BJLONGBNPCI)
	{
		...
		if(CCBEKGNDDBE.BMFEGOMNECF_Step[stepIdx - 1].FHBJOLPCAPN_MaxCount < 1)
			return true;
		return CCBEKGNDDBE.LKHAAGIJEPG_PlayerStatus.LPIKIBKFOJE_CurrentStepCount < CCBEKGNDDBE.BMFEGOMNECF_Step[stepIdx - 1].FHBJOLPCAPN_MaxCount;
	}
	...
	public bool KHEKNNFCAOI_Init(KOPCFBCDBPC CCBEKGNDDBE, JBHCLFDBPKP BKGFCEIFMNF, int MGBDCFIKBPM, GCAHJLOGMCI.KNMMOMEHDON_GachaType INDDJNMPONH, int APHNELOFGAK)
	{
		MHKCPJDNJKI_Summons.Clear();
		LNPCOGEJGLL.Clear();
		DHIACJMOEBH.Clear();
			...
			PDHHLMOCEDK.BHFHGFKBOHH_OnSuccess = (CACGCMBKHDI_Request JPCBBHOLPGI) =>
			{
				...
				req.BHFHGFKBOHH_OnSuccess = (CACGCMBKHDI_Request GCAKIEEGMDD) =>
				{
					...
					if(INDDJNMPONH_Category == GCAHJLOGMCI.KNMMOMEHDON_GachaType.GENEIBGNMPH_3)
					{
						...
						{
							CIOECGOMILE.HHCJCDFCLOB.AHEFHIMGIBI_PlayerData.KCCLEHLLOFG_Common.BKCJPIPJCCM_StaminaLotDone = IMMAOANGPNK.HHCJCDFCLOB.NKEBMCIMJND_Database.GDEKCOOBLMA_System.NCEMAEDMJLO_GetBeginnerGachaVersion(NKGJPJPHLIF.HHCJCDFCLOB.IBLPICFDGOF_ServerRequester.FJDBNGEPKHL.KMEFBNBFJHI_GetServerTime());
						}
					}
					if(FJAOAGNFABN_HasOneDay)
					{
	...
	public bool IJJOGGEBBPF(int APHNELOFGAK)
	{
		...
		for(int i = 0; i < IMMAOANGPNK.HHCJCDFCLOB.NKEBMCIMJND_Database.NBKNAAPBFFL_BonusVc.CDENCMNHNGA.Count; i++)
		{
			if(IMMAOANGPNK.HHCJCDFCLOB.NKEBMCIMJND_Database.NBKNAAPBFFL_BonusVc.CDENCMNHNGA[i].CPGFOBNKKBF_CurrencyId == APHNELOFGAK)
			{
				if(time < IMMAOANGPNK.HHCJCDFCLOB.NKEBMCIMJND_Database.NBKNAAPBFFL_BonusVc.CDENCMNHNGA[i].KMENGHEAIOC)
					return false;
				if(time >= IMMAOANGPNK.HHCJCDFCLOB.NKEBMCIMJND_Database.NBKNAAPBFFL_BonusVc.CDENCMNHNGA[i].KMENGHEAIOC)
					return false;
				return true;
