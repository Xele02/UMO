...
public class LOBDIAABMKG
{
	...
	public MMNNAPPLHFM CHNFEEOJJCO(int CPNGJMKFCJI)
	{
		if(NECDFDNBHFK_StepData != null)
		{
			...
			{
				//0x10CB54C
				return _GHPLINIACBB_x.AGBCJMMMLON_StepIndex == CPNGJMKFCJI;
			});
		}
		return null;
	}
	...
	{
		return MHKCPJDNJKI_Summons[(int)_BJLONGBNPCI_SummonType];
	}
	...
	public int OMNAPCHLBHF(GCAHJLOGMCI.NFCAJPIJFAM_SummonType _BJLONGBNPCI_SummonType)
	{
		...
		if (NECDFDNBHFK_StepData == null)
			return 0;
		return NECDFDNBHFK_StepData.LKPHIGAFJKD_VirtualCurrency.PPFNGGCBJKC_Id;
	}

	// // RVA: 0x10C4BF0 Offset: 0x10C4BF0 VA: 0x10C4BF0
	public int FKKCFICCGMM(int _APHNELOFGAK_CurrencyId)
	{
		if (INDDJNMPONH_Category == GCAHJLOGMCI.KNMMOMEHDON_GachaType.DLOPEFGOAPD_10_PassGacha)
		...
		else if(INDDJNMPONH_Category == GCAHJLOGMCI.KNMMOMEHDON_GachaType.ANFKBNLLJFN_7 || _APHNELOFGAK_CurrencyId == 1001)
		{
	...
	// private bool MIJEHENMEOE(GCAHJLOGMCI.NFCAJPIJFAM _BJLONGBNPCI_SummonType) { }

	// // RVA: 0x10C5BF0 Offset: 0x10C5BF0 VA: 0x10C5BF0
	private bool DOMFHDPMCCO(List<KBPDNHOKEKD_ProductId> NNDGIAEFMOG, GCAHJLOGMCI.NFCAJPIJFAM_SummonType _BJLONGBNPCI_SummonType, bool DDNGPDGGJBN_LoadInfo_, bool AJOAFPDICDK_NoLabelCheck_/* = false*/, NPCHMKMAHMA COMIABFPIKA/* = 0*/)
	{
		//joined_r0x010c5c94
		for(int i = NNDGIAEFMOG.Count - 1; i >= 0; i--)
		{
			...
			if(summonType == _BJLONGBNPCI_SummonType || AJOAFPDICDK_NoLabelCheck_)
			{
				...
				if(ok)
				{
					if(MHKCPJDNJKI_Summons[(int)_BJLONGBNPCI_SummonType] != null)
					{
						int a = GCAHJLOGMCI.BDJDDKBCHKO(MHKCPJDNJKI_Summons[(int)_BJLONGBNPCI_SummonType].KAPMOPMDHJE_Label);
						int c = GCAHJLOGMCI.BDJDDKBCHKO(NNDGIAEFMOG[i].KAPMOPMDHJE_Label);
						if(c <= a)
							return true;
					}
					MHKCPJDNJKI_Summons[(int)_BJLONGBNPCI_SummonType] = NNDGIAEFMOG[i];
					...
					KACECFNECON = LNPCOGEJGLL[(int)_BJLONGBNPCI_SummonType];
					if(KACECFNECON == null)
						return true;
					...
					if(!string.IsNullOrEmpty(KACECFNECON.EABMLBFHJBH_CloseAt))
					{
						DateTime d;
						if(DateTime.TryParse(KACECFNECON.EABMLBFHJBH_CloseAt, out d))
						{
	...
	private bool HJLKMDHCGFP(KOPCFBCDBPC CCBEKGNDDBE, JBHCLFDBPKP BKGFCEIFMNF, GCAHJLOGMCI.NFCAJPIJFAM_SummonType _BJLONGBNPCI_SummonType)
	{
		...
		LNPCOGEJGLL[(int)_BJLONGBNPCI_SummonType] = KACECFNECON;
		...
		KJBGCLPMLCG_OpenedAt = CCBEKGNDDBE.KBFOIECIADN_OpenAt;
		INDDJNMPONH_Category = GCAHJLOGMCI.KNMMOMEHDON_GachaType.BCBJMKDAAKA_8_StepUp;
		if(!string.IsNullOrEmpty(KACECFNECON.OPFGFINHFCE_Name))
		{
			OPFGFINHFCE_Name = KACECFNECON.OPFGFINHFCE_Name;
			OriginalName = KACECFNECON.OriginalName;
		...
		HNKHCIDOKFF_PlateBgId = KACECFNECON.HNKHCIDOKFF_PlateBgId;
		int v;
		int.TryParse(CCBEKGNDDBE.FJGCDPLCIAK_UniqueKey.Substring(9, 4), out v);
		MGBDCFIKBPM = v;
		FDEBLMKEMLF_TypeAndSeriesId = (int)INDDJNMPONH_Category * 10000 + v;
		IJADMGDHEIE = FDEBLMKEMLF_TypeAndSeriesId.ToString("D5");
		int stepIdx = CCBEKGNDDBE.LKHAAGIJEPG_PlayerStatus.DBNAGGGJDAB_CurrentStepIndex;
		long viewOrder = 0;
		if(KACECFNECON != null)
			viewOrder = KACECFNECON.EEFLOOBOAGF_ViewOrder;
		EEFLOOBOAGF_ViewOrder = GCAHJLOGMCI.PMBGPACNPIN(INDDJNMPONH_Category, OriginalName, KJBGCLPMLCG_OpenedAt, viewOrder);
		NECDFDNBHFK_StepData = CCBEKGNDDBE;
		NJLONELPNCD = BKGFCEIFMNF;
		if(CCBEKGNDDBE.BMFEGOMNECF_Steps[stepIdx - 1].FHBJOLPCAPN_MaxCount < 1)
			return true;
		return CCBEKGNDDBE.LKHAAGIJEPG_PlayerStatus.LPIKIBKFOJE_CurrentStepCount < CCBEKGNDDBE.BMFEGOMNECF_Steps[stepIdx - 1].FHBJOLPCAPN_MaxCount;
	}
	...
	public bool KHEKNNFCAOI_Init(List<KBPDNHOKEKD_ProductId> BBKDLIPKADG, int MGBDCFIKBPM, GCAHJLOGMCI.KNMMOMEHDON_GachaType INDDJNMPONH, bool IAHLNPMFJMH_IsTutorial_, int APHNELOFGAK_CurrencyId_)
	{
		...
		if(b1)
		{
			...
			if(KACECFNECON == null)
			{
				EEFLOOBOAGF_ViewOrder = GCAHJLOGMCI.PMBGPACNPIN(INDDJNMPONH, OriginalName, KJBGCLPMLCG_OpenedAt, 0);
			}
			else
			{
				if(KACECFNECON.OPKCNBFBBKP_BannerId == 0)
				{
					EEFLOOBOAGF_ViewOrder = GCAHJLOGMCI.PMBGPACNPIN(INDDJNMPONH, OriginalName, KJBGCLPMLCG_OpenedAt, KACECFNECON.EEFLOOBOAGF_ViewOrder);
				}
				else
				{
	...
	public bool KHEKNNFCAOI_Init(KOPCFBCDBPC CCBEKGNDDBE, JBHCLFDBPKP BKGFCEIFMNF, int MGBDCFIKBPM, GCAHJLOGMCI.KNMMOMEHDON_GachaType INDDJNMPONH, int _APHNELOFGAK_CurrencyId)
	{
		MHKCPJDNJKI_Summons.Clear();
		LNPCOGEJGLL.Clear();
		DHIACJMOEBH.Clear();
		...
		if(INDDJNMPONH == GCAHJLOGMCI.KNMMOMEHDON_GachaType.BCBJMKDAAKA_8_StepUp)
		{
	...
	public bool EJGLNKNKLFC(int PHIGDFMDJBO, GCAHJLOGMCI.KNMMOMEHDON_GachaType INDDJNMPONH)
	{
		...
		if(b && b2)
		{
			...
			EEFLOOBOAGF_ViewOrder = GCAHJLOGMCI.PMBGPACNPIN(INDDJNMPONH, JpStringLiterals.StringLiteral_11117_Jp, 0, 0);
		}
		return b && b2;
	}

	// // RVA: 0x10C7DCC Offset: 0x10C7DCC VA: 0x10C7DCC
	public bool KIAIFPFBGJC(GCAHJLOGMCI.NFCAJPIJFAM_SummonType _BJLONGBNPCI_SummonType, CACGCMBKHDI_Request.HDHIKGLMOGF _BHFHGFKBOHH_OnSuccess, DJBHIFLHJLK HDFGHFOCHKE, DJBHIFLHJLK JGKOLBLPMPG)
	{
		...
		if(_BJLONGBNPCI_SummonType == GCAHJLOGMCI.NFCAJPIJFAM_SummonType.OBLEFFEJGIJ_8)
		{
			...
			{
				//0x10CB834
				CIOECGOMILE.HHCJCDFCLOB.DJICHKCLMCD_UpdateCurrencies(HFPHHNDMLKE.NFEAMMJIMPG.BBEPLKNMICJ_Balances);
				...
				req.MMEBLOIJBKE_Key = NECDFDNBHFK_StepData.FJGCDPLCIAK_UniqueKey;
				req.NNEGBDKOAHN_Hash = NECDFDNBHFK_StepData.LKHAAGIJEPG_PlayerStatus.ENJCKADOFBD_NextPurchaseHash;
				req.BHFHGFKBOHH_OnSuccess = (CACGCMBKHDI_Request NBGOAPNONNA) =>
				{
					...
					NECDFDNBHFK_StepData.LKHAAGIJEPG_PlayerStatus.ODDIHGPONFL(r.NFEAMMJIMPG.LKHAAGIJEPG_PlayerStatus);
		...
		else
		{
			...
			{
				//0x10CBB58
				CIOECGOMILE.HHCJCDFCLOB.DJICHKCLMCD_UpdateCurrencies(PDHHLMOCEDK.NFEAMMJIMPG.BBEPLKNMICJ_Balances);
				...
				{
					if (IMMAOANGPNK.HHCJCDFCLOB.NKEBMCIMJND_Database.GKMAHADAAFI_GachaTicket.DHIACJMOEBH.Contains(OMNAPCHLBHF(_BJLONGBNPCI_SummonType)))
					{
						req.BPNPBJALGHM_Quantity = CIOECGOMILE.HHCJCDFCLOB.NBJOCMAJLPK_GetTotalCurrency(OMNAPCHLBHF(_BJLONGBNPCI_SummonType));
				...
				{
					req.APHNELOFGAK_CurrencyId = OMNAPCHLBHF(_BJLONGBNPCI_SummonType);
				...
				req.BHFHGFKBOHH_OnSuccess = (CACGCMBKHDI_Request GCAKIEEGMDD) =>
				{
					...
					if(INDDJNMPONH_Category == GCAHJLOGMCI.KNMMOMEHDON_GachaType.GENEIBGNMPH_3)
					{
						...
						{
							CIOECGOMILE.HHCJCDFCLOB.AHEFHIMGIBI_PlayerData.KCCLEHLLOFG_Common.BKCJPIPJCCM_StaminaLotDone = IMMAOANGPNK.HHCJCDFCLOB.NKEBMCIMJND_Database.GDEKCOOBLMA_System.NCEMAEDMJLO_GetBeginnerGachaVersion(NKGJPJPHLIF.HHCJCDFCLOB.IBLPICFDGOF_ServerRequester.FJDBNGEPKHL_Time.KMEFBNBFJHI_GetServerTime());
						}
					}
					if(FJAOAGNFABN_HasOneDay)
					{
	...
	public bool ICBNPNKJCBK(GCAHJLOGMCI.NFCAJPIJFAM_SummonType _BJLONGBNPCI_SummonType, CDOPFBOHDEF _BHFHGFKBOHH_OnSuccess, DJBHIFLHJLK JGKOLBLPMPG, int _BPNPBJALGHM_Quantity/* = 1*/)
	{
		KBPDNHOKEKD_ProductId k = DBHIEABGKII_GetSummon(_BJLONGBNPCI_SummonType);
		if (_BJLONGBNPCI_SummonType != GCAHJLOGMCI.NFCAJPIJFAM_SummonType.OBLEFFEJGIJ_8 && k != null)
		{
			...
			req.BHFHGFKBOHH_OnSuccess = (CACGCMBKHDI_Request NHECPMNKEFK) =>
			{
				...
				_BHFHGFKBOHH_OnSuccess(r.NFEAMMJIMPG.HBHMAKNGKFK_Items);
			};
	...
	public void AHOOLEAGACO_GetItemSetRecord(IMCBBOAFION _BHFHGFKBOHH_OnSuccess, DJBHIFLHJLK JGKOLBLPMPG)
	{
		N.a.StartCoroutineWatched(NINJDNIAGEN_Coroutine_GetItemSetRecord(_BHFHGFKBOHH_OnSuccess, JGKOLBLPMPG));
	}

	// [IteratorStateMachineAttribute] // RVA: 0x6B6010 Offset: 0x6B6010 VA: 0x6B6010
	// // RVA: 0x10C87B4 Offset: 0x10C87B4 VA: 0x10C87B4
	private IEnumerator NINJDNIAGEN_Coroutine_GetItemSetRecord(IMCBBOAFION _BHFHGFKBOHH_OnSuccess, DJBHIFLHJLK MOBEEPPKFLG)
	{
	...
	public void OALDDFAJFDL_GetStepUpItemSetRecord(IMCBBOAFION _BHFHGFKBOHH_OnSuccess, DJBHIFLHJLK JGKOLBLPMPG)
	{
		N.a.StartCoroutineWatched(BAKEDPGFOLF_Coroutine_GetStepUpItemSetRecord(_BHFHGFKBOHH_OnSuccess, JGKOLBLPMPG));
	}

	// [IteratorStateMachineAttribute] // RVA: 0x6B6088 Offset: 0x6B6088 VA: 0x6B6088
	// // RVA: 0x10C88EC Offset: 0x10C88EC VA: 0x10C88EC
	private IEnumerator BAKEDPGFOLF_Coroutine_GetStepUpItemSetRecord(IMCBBOAFION _BHFHGFKBOHH_OnSuccess, DJBHIFLHJLK MOBEEPPKFLG)
	{
		...
		PNLGHFCFPPK = NKGJPJPHLIF.HHCJCDFCLOB.IBLPICFDGOF_ServerRequester.IFFNCAFNEAG_AddRequest(new EDOGNHECOMI_GetStepUpLotDetail(true));
		PNLGHFCFPPK.MMEBLOIJBKE = NECDFDNBHFK_StepData.FJGCDPLCIAK_UniqueKey;
	...
	private bool GHLJANAEKFJ(int ADANONKNBKE)
	{
		...
		{
			if (EJHPIMANJFP.HHCJCDFCLOB.MHKCPJDNJKI[i].LHENLPLKGLP_StuffId == ADANONKNBKE && EJHPIMANJFP.HHCJCDFCLOB.MHKCPJDNJKI[i].AJIFADGGAAJ_BoughtQuantity < EJHPIMANJFP.HHCJCDFCLOB.MHKCPJDNJKI[i].GCJMGMBNBCB_BuyLimit)
				return true;
		}
		return false;
	}
	...
	public bool ALPOJNBHNDK(int _APHNELOFGAK_CurrencyId, long JHNMKKNEENE)
	{
		for(int i = 0; i < IMMAOANGPNK.HHCJCDFCLOB.NKEBMCIMJND_Database.NBKNAAPBFFL_BonusVc.CDENCMNHNGA_table.Count; i++)
		{
			HHJHIFJIKAC_BonusVc.MNGJPJBCMBH dbVC = IMMAOANGPNK.HHCJCDFCLOB.NKEBMCIMJND_Database.NBKNAAPBFFL_BonusVc.CDENCMNHNGA_table[i];
	...
	public static bool GPKPIGPDFNL(List<LOBDIAABMKG> NNDGIAEFMOG, int HHIBBHFHENH, int _GPDIDIJDKAG_LinkCount)
	{
		int a1 = 0;
		int a2 = 0;
		int a3 = 0;
