...
public class KPJHLACKGJF_EventMission : IKDICBBFBMI_EventBase
{
		...
		if(ev != null)
		{
			...
			for(int i = 0; i < ev.IJJHFGOIDOK_Songs.Count; i++)
			{
				return ev.IJJHFGOIDOK_Songs[i].PDBPFJJCADD_OpenAt != 0 && 
					ev.IJJHFGOIDOK_Songs[i].FDBNFFNFOND_CloseAt != 0 && 
					ev.IJJHFGOIDOK_Songs[i].FDBNFFNFOND_CloseAt < DPJCPDKALGI_End1 && 
					t >= ev.IJJHFGOIDOK_Songs[i].PDBPFJJCADD_OpenAt && 
					ev.IJJHFGOIDOK_Songs[i].FDBNFFNFOND_CloseAt >= t && 
					ev.IJJHFGOIDOK_Songs[i].MPLGPBNJDJB_FMusicId == GHBPLHBNMBK;
			}
	...
	protected override void KKFLDCBHFJA(long JHNMKKNEENE)
	{
		...
		if(ev != null)
		{
			...
			DOLJEDAAKNN = ev.NGHKJOEDLIP_Settings.HEDAGCNPHGD;
			GLIMIGNNGGB_Start = ev.NGHKJOEDLIP_Settings.BONDDBOFBND_Start;
			DPJCPDKALGI_End1 = ev.NGHKJOEDLIP_Settings.HPNOGLIFJOP_End1;
			LOLAANGCGDO = ev.NGHKJOEDLIP_Settings.LNFKGHNHJKE;
			JDDFILGNGFH = ev.NGHKJOEDLIP_Settings.JGMDAOACOJF;
			LJOHLEGGGMC = ev.NGHKJOEDLIP_Settings.IDDBFFBPNGI;
			EMEKFFHCHMH_End = ev.NGHKJOEDLIP_Settings.KNLGKBBIBOH_End;
	...
	protected override void ODPJGHOJIOH(int LHJCOPMMIGO)
	{
		if(KBACNOCOANM != null)
		{
			...
			if(ev != null)
			{
				...
				{
					KKLGENJKEBN.HHCJCDFCLOB.LDOBHAAIDEJ_UpdateRankingScore(ev.NGHKJOEDLIP_Settings.OCGFKMHNEOF_EventKey, LHJCOPMMIGO, BOAGCEOHJEO.GOAOBNBGDBJ(GLIMIGNNGGB_Start, LOLAANGCGDO, save.NFIOKIBPJCJ_Uptime, save.DNBFMLBNAEE_Point), () =>
					{
						//0xD8B794
						CDINKAANIAA_Rank[LHJCOPMMIGO] = (int)KKLGENJKEBN.HHCJCDFCLOB.LPPCNCMEDFA_Rank;
						PLOOEECNHFB = true;
	...
	private void EBNMMCKCPKN(JGEOBNENMAH.HAJIFNABIFF OMNOFMEBLAD, int ACMMDAHKCIF, int FCLGIPFPIPH, int LCCGDFGGIHI, int ANOPDAGJIKG, int MHADLGMJKGK, bool IKGLAFOHGDO)
	{
		...
		if(ev != null)
		{
			...
			FHPEAPEANAI.LIPIAPOGHIP_EpisodeNum = LEPNPBIMHGM(ANOPDAGJIKG).Count;
			FHPEAPEANAI.ODCLHPGHDHA_EpisodeBonus = CEICDKGEONG(ANOPDAGJIKG, MHADLGMJKGK);
