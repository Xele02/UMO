...
public class MNDAMOGGJBJ
{
	public struct JFJJNPJNBPI
	{
		...
		public void MBFFGGFGPEN(int _BBPHAPFBFHK__id, int _HFJLPAOGLEH__count)
		{
			PPFNGGCBJKC_id = _BBPHAPFBFHK__id;
			HMFFHLPNMPH_count = _HFJLPAOGLEH__count;
		}
	}
	...
	public List<JFJJNPJNBPI> MHGAOHAHFDN { get; set; } // 0x10 DJBDDFLFPLE_bgs PGKLOONAKPD_bgs GLBMFLPHJLB_bgs
	public int PGGKBIIJOND { get; set; } // 0x14 PKFNBLMOBCL_bgs MKBBJIKHFJD_bgs KMOMHPMBMNL_bgs
	public List<JFJJNPJNBPI> INLBMFMOHCI_CostItems { get; set; } // 0x18 PIDPINOOAPN_bgs KLLCKKLHEHH_bgs EBBKKLLCDMB_bgs
	public int CMBGGPOFBOO_UcCost { get { return NEGMNPCHENF ^ FBGGEFFJJHB_xor; } set { NEGMNPCHENF = value ^ FBGGEFFJJHB_xor; } } //0x17B00B0 PPKIKKIKGMK_bgs 0x17B00C0 HOGAFMDJKPK_bgs
	...
	public int JPLMJPGDPAN(int _PPFNGGCBJKC_id)
	{
		...
		{
				return EKLNMHFCAOI_ItemManager.DLNFNHMPGLI_GetNumClamped(IMMAOANGPNK_NetMasterDataManager.HHCJCDFCLOB_Instance.NKEBMCIMJND_Database, CIOECGOMILE_NetPlayerDataManager.HHCJCDFCLOB_Instance.AHEFHIMGIBI_PlayerData, EKLNMHFCAOI_ItemManager.BKHFLDMOGBD_GetItemCategory(INLBMFMOHCI_CostItems[_PPFNGGCBJKC_id - 1].PPFNGGCBJKC_id), EKLNMHFCAOI_ItemManager.DEACAHNLMNI_getItemId(INLBMFMOHCI_CostItems[_PPFNGGCBJKC_id - 1].PPFNGGCBJKC_id), null);
		}
		return 0;
	}
	...
	private int JBMLCIPKFDF_GetNumItemInInventory(int _PPFNGGCBJKC_id)
	{
		...
		if(CIOECGOMILE_NetPlayerDataManager.HHCJCDFCLOB_Instance.AHEFHIMGIBI_PlayerData != null && IMMAOANGPNK_NetMasterDataManager.HHCJCDFCLOB_Instance.NKEBMCIMJND_Database != null)
		{
			return EKLNMHFCAOI_ItemManager.ALHCGDMEMID_GetNumItems(IMMAOANGPNK_NetMasterDataManager.HHCJCDFCLOB_Instance.NKEBMCIMJND_Database,
				CIOECGOMILE_NetPlayerDataManager.HHCJCDFCLOB_Instance.AHEFHIMGIBI_PlayerData, EKLNMHFCAOI_ItemManager.BKHFLDMOGBD_GetItemCategory(INLBMFMOHCI_CostItems[_PPFNGGCBJKC_id - 1].PPFNGGCBJKC_id), EKLNMHFCAOI_ItemManager.DEACAHNLMNI_getItemId(INLBMFMOHCI_CostItems[_PPFNGGCBJKC_id - 1].PPFNGGCBJKC_id), null);
		}
		return 0;
	}
	...
	public int KMIFDLLCBEL()
	{
		...
		if(res > 0)
		{
			...
			if(CIOECGOMILE_NetPlayerDataManager.HHCJCDFCLOB_Instance.AHEFHIMGIBI_PlayerData != null && IMMAOANGPNK_NetMasterDataManager.HHCJCDFCLOB_Instance.NKEBMCIMJND_Database != null)
			{
				for(int i = 0; i < INLBMFMOHCI_CostItems.Count; i++)
				{
					EKLNMHFCAOI_ItemManager.DPHGFMEPOCA_SetNumItems(IMMAOANGPNK_NetMasterDataManager.HHCJCDFCLOB_Instance.NKEBMCIMJND_Database, CIOECGOMILE_NetPlayerDataManager.HHCJCDFCLOB_Instance.AHEFHIMGIBI_PlayerData, EKLNMHFCAOI_ItemManager.BKHFLDMOGBD_GetItemCategory(INLBMFMOHCI_CostItems[i].PPFNGGCBJKC_id), EKLNMHFCAOI_ItemManager.DEACAHNLMNI_getItemId(INLBMFMOHCI_CostItems[i].PPFNGGCBJKC_id), CPBDJHKHEFN_GetNumItemLeftAfterUse(i + 1), null);
				}
				if(CMBGGPOFBOO_UcCost > 0)
				{
