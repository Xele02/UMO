...
public class NCPPAHHCCAO
{
	...
	public int IDELKEKDIFD { get { return HONLHOODCKN ^ FBGGEFFJJHB; } set { HONLHOODCKN = FBGGEFFJJHB ^ value; } } //0x1ADA120 HIMAGNHENDH 0x1ADA130 IOPEENMGODH
	public int BEHMEDMNJMC { get { return PDNDHGFKNOI ^ FBGGEFFJJHB; } set { PDNDHGFKNOI = FBGGEFFJJHB ^ value; } } //0x1ADA140 FFDHAICGIIF 0x1ADA150 IAIEKNACIJC
				...
				if (saveStamp.PPFNGGCBJKC_Id > 0)
				{
					if(saveStamp.PPFNGGCBJKC_Id <= IMMAOANGPNK.HHCJCDFCLOB.NKEBMCIMJND_Database.GAPONCJOKAC_DecoStamp.FHBIIONKIDI_Stamps.Count)
					{
						...
						if(dbStamp.PLALNIIBLOF_Enabled == 2)
						{
							...
							stamp.IDELKEKDIFD = dbStamp.JBFLEDKDFCO;
							stamp.BEHMEDMNJMC = dbStamp.ALAEHBKAEPB;
							...
							if(stamp.IDELKEKDIFD > 0)
							{
								if(stamp.IDELKEKDIFD <= IMMAOANGPNK.HHCJCDFCLOB.NKEBMCIMJND_Database.OMGFKMANMAB_Sns.KHCACDIKJLG_Characters.Count)
								{
									BOKMNHAFJHF_Sns.JFMDDEBLCAA snsChara = IMMAOANGPNK.HHCJCDFCLOB.NKEBMCIMJND_Database.OMGFKMANMAB_Sns.KHCACDIKJLG_Characters[stamp.IDELKEKDIFD - 1];
									if(snsChara.PPEGAKEIEGM_Enabled == 2)
									{
										stamp.CPKMLLNADLJ = snsChara.CPKMLLNADLJ;
									}
								}
							}
		...
		if(PPFNGGCBJKC > 0)
		{
			if(PPFNGGCBJKC <= IMMAOANGPNK.HHCJCDFCLOB.NKEBMCIMJND_Database.GAPONCJOKAC_DecoStamp.DMKMNGELNAE_Serif.Count)
			{
				...
				if(serif.JBFLEDKDFCO > 0)
				{
					...
					{
						BOKMNHAFJHF_Sns.JFMDDEBLCAA sns = IMMAOANGPNK.HHCJCDFCLOB.NKEBMCIMJND_Database.OMGFKMANMAB_Sns.KHCACDIKJLG_Characters[serif.JBFLEDKDFCO - 1];
