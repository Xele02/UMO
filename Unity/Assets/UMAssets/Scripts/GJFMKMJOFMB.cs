...
public class GJFMKMJOFMB
{
	...
	public void KHEKNNFCAOI_Init(bool JHOJAGMJABJ, bool IEABEILIGPO/* = false*/)
	{
		if(IEABEILIGPO)
		{
			...
			{
				if(NHPDPKHMFEP.HHCJCDFCLOB.LKNKNKAALJO[i].JJBGOIMEIPF_ItemId == BBDNDNHJEKJ)
				{
					CFNFKIIOOHC += NHPDPKHMFEP.HHCJCDFCLOB.LKNKNKAALJO[i].MBJIFDBEDAC_item_count;
				}
				else
				{
					BBDNDNHJEKJ = NHPDPKHMFEP.HHCJCDFCLOB.LKNKNKAALJO[i].JJBGOIMEIPF_ItemId;
					CFNFKIIOOHC = NHPDPKHMFEP.HHCJCDFCLOB.LKNKNKAALJO[i].MBJIFDBEDAC_item_count;
					ENHGKPMEICN = true;
				}
			}
		...
		NNBGGBOHLHI.Clear();
		OOHJDAPDCMH = 0;
		int a = CIOECGOMILE.HHCJCDFCLOB.AHEFHIMGIBI_PlayerData.HMMNDKHKEBC_MonthlyPass.FLBJKACKNOI_CurStamp;
		if (a == 0)
			a = 1;
		OOHJDAPDCMH = ELCNECGFGAO(CKPIHCGOEDP, a, HNNAJBJCNEJ) - 1;
		a = CIOECGOMILE.HHCJCDFCLOB.AHEFHIMGIBI_PlayerData.HMMNDKHKEBC_MonthlyPass.FLBJKACKNOI_CurStamp;
		if (a != 0)
			a--;
		if(a == 0)
		{
			a = CIOECGOMILE.HHCJCDFCLOB.AHEFHIMGIBI_PlayerData.HMMNDKHKEBC_MonthlyPass.EOHPPNNLBNH_Stamp.Count;
		}
		...
		if(JHOJAGMJABJ)
		{
			...
			{
				//LAB_00aaca34
				if(CIOECGOMILE.HHCJCDFCLOB.AHEFHIMGIBI_PlayerData.HMMNDKHKEBC_MonthlyPass.HKABHJKHFKL_RareGetCnt >= 2 && CIOECGOMILE.HHCJCDFCLOB.AHEFHIMGIBI_PlayerData.HMMNDKHKEBC_MonthlyPass.FCPHDFKFDCK_LoginCnt == 0)
				{
					KLMNKBCDGPI = true;
					PBBLOJPOLJG = 1;
					PCLCKJGCPEE = CIOECGOMILE.HHCJCDFCLOB.AHEFHIMGIBI_PlayerData.HMMNDKHKEBC_MonthlyPass.HKABHJKHFKL_RareGetCnt;
				}
			}
			else
			{
		...
		{
			HONLONNHJBP = CIOECGOMILE.HHCJCDFCLOB.AHEFHIMGIBI_PlayerData.HMMNDKHKEBC_MonthlyPass.FCPHDFKFDCK_LoginCnt;
		}
	}
