...
public class BEPHBEGDFFK
{
	...
	public class DMBKENKBIJD
	{
		...
		{
			if(DPBDFPPMIPH.INDDJNMPONH_Category != GCAHJLOGMCI.KNMMOMEHDON.OOABDNHIEFK_9)
			{
				if(DPBDFPPMIPH.INDDJNMPONH_Category != GCAHJLOGMCI.KNMMOMEHDON.DLOPEFGOAPD_10)
				{
					...
					if(tkt == null)
					{
						if(MEANCEOIMGE.FJICMLBOJCH() <= KBPDNHOKEKD_ProductId.KNEKLJHNHAK.AAPLMEGMNJA_4 && ((1 << (int)MEANCEOIMGE.FJICMLBOJCH()) & 0x16U) != 0) // 0001 0110
						{
							return 0;
						}
						return MEANCEOIMGE.NPPGKNGIFGK_Price;
					}
				}
			}
		...
		{
			return BEPHBEGDFFK.CMHHHCAKPCD(DPBDFPPMIPH, BJLONGBNPCI);
		}
	}
	...
	public void DOMFHDPMCCO(LOBDIAABMKG NDNHHGJKJGM, int IMKLJJIBHCN, long EOLFJGMAJAB)
	{
		...
		for(int i = 10; i >= 0; i--)
		{
			KBPDNHOKEKD_ProductId p = DPBDFPPMIPH.DBHIEABGKII((GCAHJLOGMCI.NFCAJPIJFAM)(i));
			if(p != null)
			{
				...
				data.KPIHBPMOGKL_LotCount = p.JHAIOJELFHI;
				data.MDEIKCBEHHC_Kakutei = DPBDFPPMIPH.KKODAOIJHMC(data.BJLONGBNPCI);
				if(DPBDFPPMIPH.INDDJNMPONH_Category == GCAHJLOGMCI.KNMMOMEHDON.DLOPEFGOAPD_10)
				{
					data.KPIHBPMOGKL_LotCount = Mathf.Clamp(CMHHHCAKPCD(DPBDFPPMIPH, data.BJLONGBNPCI), 1, 10);
				}
				if(data.BJLONGBNPCI <= GCAHJLOGMCI.NFCAJPIJFAM.AEFCOHJBLPO_11)
				{
					if(data.BJLONGBNPCI == GCAHJLOGMCI.NFCAJPIJFAM.NGAHKKOBGPA_9)
					{
						data.APHNELOFGAK_CurrencyId = DPBDFPPMIPH.OMNAPCHLBHF(data.BJLONGBNPCI);
					...
					else
					{
						if(((1 << (int)data.BJLONGBNPCI) & 0x454U) == 0) // 0100 0101 0100
						{
							...
							{
								data.APHNELOFGAK_CurrencyId = DPBDFPPMIPH.OMNAPCHLBHF(data.BJLONGBNPCI);
								data.JNMMPPILMBC = DPBDFPPMIPH.BANFOFKNKED(data.APHNELOFGAK_CurrencyId);
	...
	public DMBKENKBIJD EIPFDJBIOKN(bool HCFGINLICJN)
	{
		...
		{
			if(FEMAEEHOCIJ[i].BJLONGBNPCI != GCAHJLOGMCI.NFCAJPIJFAM.NGAHKKOBGPA_9 && !(FEMAEEHOCIJ[i].BJLONGBNPCI == GCAHJLOGMCI.NFCAJPIJFAM.GOAHICNDICO_5 && HCFGINLICJN))
			{
	...
	public DMBKENKBIJD IIPOPGHKHBA(bool HCFGINLICJN)
	{
		...
		{
			if (COMCFOFCGJM[i].BJLONGBNPCI != GCAHJLOGMCI.NFCAJPIJFAM.BPPLDIBMPKH_10 && !(COMCFOFCGJM[i].BJLONGBNPCI == GCAHJLOGMCI.NFCAJPIJFAM.LMHDFEKIDKG_6 && HCFGINLICJN))
			{
	...
	public DMBKENKBIJD PANFEKFCCOA()
	{
		for (int i = 0; i < FEMAEEHOCIJ.Count; i++)
		{
			if (FEMAEEHOCIJ[i].DPBDFPPMIPH.INDDJNMPONH_Category == GCAHJLOGMCI.KNMMOMEHDON.DLOPEFGOAPD_10)
				return FEMAEEHOCIJ[i];
			if(FEMAEEHOCIJ[i].BJLONGBNPCI >= GCAHJLOGMCI.NFCAJPIJFAM.GOAHICNDICO_5 && FEMAEEHOCIJ[i].BJLONGBNPCI <= GCAHJLOGMCI.NFCAJPIJFAM.LMHDFEKIDKG_6)
				return FEMAEEHOCIJ[i];
		}
		return null;
	}
	...
	public DMBKENKBIJD LBEHCJMJBGC()
	{
		for (int i = 0; i < COMCFOFCGJM.Count; i++)
		{
			if (COMCFOFCGJM[i].BJLONGBNPCI >= GCAHJLOGMCI.NFCAJPIJFAM.GOAHICNDICO_5 && COMCFOFCGJM[i].BJLONGBNPCI <= GCAHJLOGMCI.NFCAJPIJFAM.LMHDFEKIDKG_6)
				return COMCFOFCGJM[i];
		}
		return null;
	}
	...
	public DMBKENKBIJD CLPPBCBBNIB()
	{
		for (int i = 0; i < FEMAEEHOCIJ.Count; i++)
		{
			if (FEMAEEHOCIJ[i].BJLONGBNPCI >= GCAHJLOGMCI.NFCAJPIJFAM.NGAHKKOBGPA_9 && FEMAEEHOCIJ[i].BJLONGBNPCI <= GCAHJLOGMCI.NFCAJPIJFAM.BPPLDIBMPKH_10)
				return FEMAEEHOCIJ[i];
		}
		return null;
	}
	...
	public DMBKENKBIJD NILCJCEOBME()
	{
		for (int i = 0; i < COMCFOFCGJM.Count; i++)
		{
			if (COMCFOFCGJM[i].BJLONGBNPCI >= GCAHJLOGMCI.NFCAJPIJFAM.NGAHKKOBGPA_9 && COMCFOFCGJM[i].BJLONGBNPCI <= GCAHJLOGMCI.NFCAJPIJFAM.BPPLDIBMPKH_10)
				return COMCFOFCGJM[i];
		}
		return null;
	}
	...
	{
		return ENDIPBKOIOL(DPBDFPPMIPH, JHNMKKNEENE);
	}
	...
	public static List<NKFJNAANPNP.MOJLCADLMKH> BNGLMLIMFDM(LOBDIAABMKG DPBDFPPMIPH, long JHNMKKNEENE)
	{
		...
		if(DPBDFPPMIPH.INDDJNMPONH_Category == GCAHJLOGMCI.KNMMOMEHDON.OOABDNHIEFK_9)
		{
			int a = DPBDFPPMIPH.OMNAPCHLBHF(GCAHJLOGMCI.NFCAJPIJFAM.GOAHICNDICO_5);
			if (a == 0)
				a = DPBDFPPMIPH.OMNAPCHLBHF(GCAHJLOGMCI.NFCAJPIJFAM.LMHDFEKIDKG_6);
		...
		else if(DPBDFPPMIPH.INDDJNMPONH_Category != GCAHJLOGMCI.KNMMOMEHDON.DLOPEFGOAPD_10)
		{
			int a = DPBDFPPMIPH.OMNAPCHLBHF(GCAHJLOGMCI.NFCAJPIJFAM.GOAHICNDICO_5);
			if (a == 0)
				a = DPBDFPPMIPH.OMNAPCHLBHF(GCAHJLOGMCI.NFCAJPIJFAM.LMHDFEKIDKG_6);
		...
		EKLNMHFCAOI.FKGCBLHOOCL_Category cat = EKLNMHFCAOI.BKHFLDMOGBD_GetItemCategory(DPBDFPPMIPH.MJNOAMAFNHA);
		int itemId = EKLNMHFCAOI.DEACAHNLMNI_getItemId(DPBDFPPMIPH.MJNOAMAFNHA);
	...
	public bool GPKAMGNBGAB()
	{
		...
		return DPBDFPPMIPH.GPKAMGNBGAB(a);
	}
	...
	public bool ALPOJNBHNDK()
	{
		...
		return DPBDFPPMIPH.ALPOJNBHNDK(a, JHNMKKNEENE);
	}

	//// RVA: 0xC78FAC Offset: 0xC78FAC VA: 0xC78FAC
	public List<HGBOODNMNFM> BMJOENJMFEL(int BCCHOBPJJKE)
	{
		return DPBDFPPMIPH.KACECFNECON.NNDMIOEKKMM_NewEpisode.FindAll((HGBOODNMNFM FELIFAHDLON) =>
		{
			//0xC799FC
	...
	{
		if(DPBDFPPMIPH.KACECFNECON != null)
		{
			if(DPBDFPPMIPH.KACECFNECON.PGKIHFOKEHL_Feature != null)
			{
				for(int i = 0; i < DPBDFPPMIPH.KACECFNECON.PGKIHFOKEHL_Feature.Count; i++)
				{
					MLIBEPGADJH_Scene.KKLDOOJBJMN scene = IMMAOANGPNK.HHCJCDFCLOB.NKEBMCIMJND_Database.ECNHDEHADGL_Scene.CDENCMNHNGA_SceneList[DPBDFPPMIPH.KACECFNECON.PGKIHFOKEHL_Feature[i].DNJEJEANJGL_Value - 1];
