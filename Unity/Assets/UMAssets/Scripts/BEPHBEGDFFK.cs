...
public class BEPHBEGDFFK
{
	...
	public class DMBKENKBIJD
	{
		...
		public int ILFAHJEJCMH_GetPrice()
		{
			...
			{
				if(DPBDFPPMIPH_Gacha.INDDJNMPONH_Category != GCAHJLOGMCI.KNMMOMEHDON_GachaType.DLOPEFGOAPD_10_PassGacha)
				{
	...
	public void DOMFHDPMCCO(LOBDIAABMKG NDNHHGJKJGM, int IMKLJJIBHCN, long _EOLFJGMAJAB_CurrentTime)
	{
	...
	public DMBKENKBIJD PANFEKFCCOA()
	{
		for (int i = 0; i < FEMAEEHOCIJ.Count; i++)
		{
			if (FEMAEEHOCIJ[i].DPBDFPPMIPH_Gacha.INDDJNMPONH_Category == GCAHJLOGMCI.KNMMOMEHDON_GachaType.DLOPEFGOAPD_10_PassGacha)
				return FEMAEEHOCIJ[i];
	...
	{
		if(DPBDFPPMIPH.INDDJNMPONH_Category != GCAHJLOGMCI.KNMMOMEHDON_GachaType.DLOPEFGOAPD_10_PassGacha)
		{
	...
	public static List<NKFJNAANPNP.MOJLCADLMKH> BNGLMLIMFDM(LOBDIAABMKG DPBDFPPMIPH, long JHNMKKNEENE)
	{
		...
		else if(DPBDFPPMIPH.INDDJNMPONH_Category != GCAHJLOGMCI.KNMMOMEHDON_GachaType.DLOPEFGOAPD_10_PassGacha)
		{
			int a = DPBDFPPMIPH.OMNAPCHLBHF(GCAHJLOGMCI.NFCAJPIJFAM_SummonType.GOAHICNDICO_5);
			if (a == 0)
				a = DPBDFPPMIPH.OMNAPCHLBHF(GCAHJLOGMCI.NFCAJPIJFAM_SummonType.LMHDFEKIDKG_6);
		...
		{
			res.AddRange(CIOECGOMILE.HHCJCDFCLOB.AHEFHIMGIBI_PlayerData.GJCOJBDOOJG_LimitedCompoItem.BNGLMLIMFDM(itemId, JHNMKKNEENE));
		}
		else if(cat == EKLNMHFCAOI.FKGCBLHOOCL_Category.DLOPEFGOAPD_LimitedItem)
		{
			res.AddRange(CIOECGOMILE.HHCJCDFCLOB.AHEFHIMGIBI_PlayerData.AFHFIPLOKMN_LimitedItem.BNGLMLIMFDM(itemId, JHNMKKNEENE));
		}
		else if(cat == EKLNMHFCAOI.FKGCBLHOOCL_Category.CIOGEKJNMBB_RareUpItem)
		{
			res.AddRange(CIOECGOMILE.HHCJCDFCLOB.AHEFHIMGIBI_PlayerData.DPNKPPBEAGJ_RareUpItem.MCPIBDPKBBD(JHNMKKNEENE));
		}
		else
		{
			NKFJNAANPNP.MOJLCADLMKH data = new NKFJNAANPNP.MOJLCADLMKH();
			data.LHPDDGIJKNB((int)(JHNMKKNEENE * 0x227));
			data.HMFFHLPNMPH_Remaining = EKLNMHFCAOI.ALHCGDMEMID_GetNumItems(IMMAOANGPNK.HHCJCDFCLOB.NKEBMCIMJND_Database, CIOECGOMILE.HHCJCDFCLOB.AHEFHIMGIBI_PlayerData, cat, itemId, null);
	...
	{
		return DPBDFPPMIPH_Gacha.KACECFNECON.NNDMIOEKKMM_NewEpisode.FindAll((HGBOODNMNFM _FELIFAHDLON_ep) =>
		{
			//0xC799FC
			return _FELIFAHDLON_ep.ADDCEJNOJLG_Scenes.Find((CEBFFLDKAEC_SecureInt PPFNGGCBJKC) =>
			{
				//0xC79AE8
				return PPFNGGCBJKC.DNJEJEANJGL_Value == _BCCHOBPJJKE_SceneId;
			}) != null;
		});
	}
	...
	public bool NLFPCMJMAEM()
	{
		if(DPBDFPPMIPH_Gacha.KACECFNECON != null)
		{
			if(DPBDFPPMIPH_Gacha.KACECFNECON.PGKIHFOKEHL_Feature != null)
			{
				...
				{
					MLIBEPGADJH_Scene.KKLDOOJBJMN scene = IMMAOANGPNK.HHCJCDFCLOB.NKEBMCIMJND_Database.ECNHDEHADGL_Scene.CDENCMNHNGA_table[DPBDFPPMIPH_Gacha.KACECFNECON.PGKIHFOKEHL_Feature[i].DNJEJEANJGL_Value - 1];
