...
public class CBBJHPBGBAJ_Archive
{
	...
	public bool KHEKNNFCAOI_Init(byte[] GANPCOAGBBK_db)
	{
	...
	private bool DLBCLCKFDJP_GetFlag(byte[] _IAKPCFDLMKP_db, int POMLAOPLMNA_offset)
	{
		int A = POMLAOPLMNA_offset + 156; // ou 148 ??
		byte b = _IAKPCFDLMKP_db[A];
		return b == '0'; // Flag is standard file
	}
	
	// RVA: 0x18F8100 Offset: 0x18F8100 VA: 0x18F8100
	private string GIFPJBKOFAB_GetFilename(byte[] _IAKPCFDLMKP_db, int POMLAOPLMNA_offset)
	{
		int i = 0;
		for(; i < 99; i++)
		{
			if(_IAKPCFDLMKP_db[POMLAOPLMNA_offset + i] == 0)
				break;
		}
		Encoding enc = Encoding.UTF8;
		// +2 to remove the "./" prefix
		string str = enc.GetString(_IAKPCFDLMKP_db, POMLAOPLMNA_offset + 2, i - 2);
		return str;
	}
	
	// RVA: 0x18F81E8 Offset: 0x18F81E8 VA: 0x18F81E8
	private int CBOIHFIPHGN_GetSize(byte[] _IAKPCFDLMKP_db, int POMLAOPLMNA_offset)
	{
		Encoding enc = Encoding.UTF8;
		string str = enc.GetString(_IAKPCFDLMKP_db, POMLAOPLMNA_offset + 124, 12);
		int d = 1;
		int b = 0;
	...
	// private int JNJODIPMKFJ(byte[] _IAKPCFDLMKP_db, int _POMLAOPLMNA_offset); // RVA: 0x18F82E8 Offset: 0x18F82E8
