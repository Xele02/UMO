...
public class EBNNKJCEEFN
{
	...
	public static OKLFOAPMJAA_BbsData LIFAAGMJFEB(int _MLPEHNBNOGD_PlayerId)
	{
		OKLFOAPMJAA_BbsData res = new OKLFOAPMJAA_BbsData();
		StringBuilder str = new StringBuilder(32);
		AHAENNIFOAF.BCJOMHAIGGJ(str, _MLPEHNBNOGD_PlayerId);
		res.OBKGEDCKHHE_Init(str.ToString(), 60, true, null);
		res.EEBMKLOIEMB_SetAutoUpdateInterval(IMMAOANGPNK_NetMasterDataManager.HHCJCDFCLOB_Instance.NKEBMCIMJND_Database.HNMMJINNHII_Game.LPJLEHAJADA_GetIntParam("bbs_auto_update_interval_deco", 10));
		if (_MLPEHNBNOGD_PlayerId == NKGJPJPHLIF_SakashoManager.HHCJCDFCLOB_Instance.MDAMJIGBOLD_PlayerId)
