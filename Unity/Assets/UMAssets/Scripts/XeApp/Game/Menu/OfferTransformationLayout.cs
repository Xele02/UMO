...
namespace XeApp.Game.Menu
{
	public class OfferTransformationLayout : LayoutUGUIScriptBase
	{
		...
		public void initialize(KDHGBOOECKC.JNHGHDKLDEM _data, HEFCLPGPMLK.AAOPGOGGMID info, int platoonId)
		{
			...
			SettingText((KDHGBOOECKC.HHCJCDFCLOB.KGLLKKCFDEL(FKGMGBHBNOC.HPJOCKGKNCC_Form.MABDGNNOPCB_Fighter, a2, BOPFPIHGJMD.HBJMIJIOCAM.FMHLGHDKJBC_0_Atk) * KDHGBOOECKC.HHCJCDFCLOB.LBDENPEGONA(platoonId, BOPFPIHGJMD.HBJMIJIOCAM.FMHLGHDKJBC_0_Atk)) / 100, 
						(KDHGBOOECKC.HHCJCDFCLOB.LBDENPEGONA(platoonId, BOPFPIHGJMD.HBJMIJIOCAM.JIOPJDJBLFK_1_Hit) * KDHGBOOECKC.HHCJCDFCLOB.KGLLKKCFDEL(FKGMGBHBNOC.HPJOCKGKNCC_Form.MABDGNNOPCB_Fighter, a2, BOPFPIHGJMD.HBJMIJIOCAM.JIOPJDJBLFK_1_Hit)) / 100);
		...
		private bool SetRateState(BOPFPIHGJMD.MGPIJGMDLOM[] state, AbsoluteLayout[] list)
		{
			...
			for(int i = 0; i < 2; i++)
			{
				...
				if(state[i] == BOPFPIHGJMD.MGPIJGMDLOM.INIMBLOHIEF_0_set_Item)
				{
					list[i].StartChildrenAnimGoStop("01");
				}
				else if(state[i] == BOPFPIHGJMD.MGPIJGMDLOM.JIOPJDJBLFK_2_Hit)
				{
					list[i].StartChildrenAnimGoStop("03");
				}
				else if (state[i] == BOPFPIHGJMD.MGPIJGMDLOM.FMHLGHDKJBC_1_Atk)
				{
		...
		private void SetItemTexture(RawImageEx[] images, KDHGBOOECKC.BOBLBPGKIOH state, Action<int> loaded)
		{
			for(int i = 0; i < 2; i++)
			{
				...
				if(state.LHMDABPNDDH_state[i] == BOPFPIHGJMD.MGPIJGMDLOM.INIMBLOHIEF_0_set_Item)
				{
