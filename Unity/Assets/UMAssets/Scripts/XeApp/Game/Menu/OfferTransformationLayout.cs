...
namespace XeApp.Game.Menu
{
	public class OfferTransformationLayout : LayoutUGUIScriptBase
	{
		...
		public void initialize(KDHGBOOECKC_NetOfferManager.JNHGHDKLDEM _data, HEFCLPGPMLK.AAOPGOGGMID info, int platoonId)
		{
			...
			{
				//0x1713F80
				ChangeIsLoading(index, FKGMGBHBNOC.HPJOCKGKNCC_Form.MABDGNNOPCB_0_Fighter);
			});
			SetItemTexture(GarwalkItemIcon, m_data.NNDGIAEFMOG[1], (int index) =>
			{
				//0x1713F88
				ChangeIsLoading(index, FKGMGBHBNOC.HPJOCKGKNCC_Form.MOGAKDMDCJB_1_Gerwalk);
			});
			SetItemTexture(BatroidItemIcon, m_data.NNDGIAEFMOG[2], (int index) =>
			{
				//0x1713F90
				ChangeIsLoading(index, FKGMGBHBNOC.HPJOCKGKNCC_Form.GBLPNIGCPAP_2_Battroid);
			});
		}
