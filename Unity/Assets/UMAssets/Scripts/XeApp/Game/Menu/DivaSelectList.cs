...
namespace XeApp.Game.Menu
{
	public class DivaSelectList : LayoutUGUIScriptBase
	{
		...
		public void UpdateContent(FFHPBEPOMAK_DivaInfo selectedDiva, List<int> sortDivaIndexList, int slotNo, EEDKAACNBBG_MusicData musicData)
		{
			...
			{
				MenuScene.Instance.DivaIconCache.Load(selectedDiva.AHHJLDLAPAN_DivaId, selectedDiva.FFKMJNHFFFL_Costume.DAJGPBLEEOB_ModelId, selectedDiva.EKFONBFDAAP_ColorId, (IiconTexture texture) => {
					//0x17EC448
					texture.Set(m_selectDivaIconImage);
				});
		...
		private void UpdateScrollContent(int index, SwapScrollListContent content)
		{
			...
			FFHPBEPOMAK_DivaInfo f = GameManager.Instance.ViewPlayerData.NBIGLBMHEDC_DivaList[m_sortDivaList[index]];
			data.SetDivaIcon(f.AHHJLDLAPAN_DivaId, f.FFKMJNHFFFL_Costume.DAJGPBLEEOB_ModelId, f.EKFONBFDAAP_ColorId, IsCenter(GameManager.Instance.ViewPlayerData.NPFCMHCCDDH, f.AHHJLDLAPAN_DivaId), IsUnitMember(GameManager.Instance.ViewPlayerData.NPFCMHCCDDH, f.AHHJLDLAPAN_DivaId));
			for(int i = 0; i < 3; i++)
			{
				if (i == 0)
				{
		...
		public StayButton GetNavigationDivaListButton()
		{
			...
			for(int i = 0; i < 2; i++)
			{
				FFHPBEPOMAK_DivaInfo d2 = GameManager.Instance.ViewPlayerData.NBIGLBMHEDC_DivaList[m_sortDivaList[i]];
