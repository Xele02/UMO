...
namespace XeApp.Game.Menu
{
	public class RaidActSelect : TransitionRoot
	{
		...
		private PKNOKJNLPOE_NetEventRaidController raidController; // 0x9C
		...
		private IEnumerator Co_MusicRelatedInit()
		{
			...
			while(KDLPEDBKMID_NetDelayInstaller.HHCJCDFCLOB_Instance.LNHFLJBGGJB_IsRunning)
				yield return null;
			bool b = GameManager.Instance.localSave.EPJOACOONAC_GetSave().MCNEIJAOLNO_Select.JGAFBCMOGLP_Raid.FKEJBAHCMGC_CheckEvent(raidController.PGIIDPEGGPI_EventId);
		...
		public void OnClickPlayButton(JLOGEHCIBEJ_EventRaid.JJAFLOEBLDH_AttackType attackType)
		{
			...
			CheckBossAlive(() =>
			{
				...
				if(m_selectMusicData.IFNPBIJEPBO_IsDlded)
				{
					...
					{
						if(!CIOECGOMILE_NetPlayerDataManager.HHCJCDFCLOB_Instance.AHEFHIMGIBI_PlayerData.KCCLEHLLOFG_Common.ADKJDHPEAJH(GPFlagConstant.ID.IsShowRaidAssistHelp))
						{
							...
							{
								//0x1445E98
								CIOECGOMILE_NetPlayerDataManager.HHCJCDFCLOB_Instance.AHEFHIMGIBI_PlayerData.KCCLEHLLOFG_Common.BCLKCMDGDLD(GPFlagConstant.ID.IsShowRaidAssistHelp, true);
								ClickPlayButton();
		...
		{
			if(raidController != null && raidController.NGOFCFJHOMI_Status < KGCNCBOKCBA.GNENJEHKMHD_EventStatus.EMAMLLFAOJI_6_Counting)
			{
                List<IBJAKJJICBC> l = m_eventMusicData.GetList(line6Mode, false);
				for(int i = 0; i < l.Count; i++)
				{
					if(l[i].JAPLKHPLOOF(OHCAABOMEOF.KGOGMKMBCPP_EventType.CADKONMJEDA_11_EventRaid))
					{
						return true;
					}
				}
		...
		private void ApplyMusicInfoNormal()
		{
			...
			for(int i = m_selectMusicData.MGJKEJHEBPO_Blocks.Count - 1; i >= 0; i--)
			{
			...
			for(int i = 0; i < m_selectMusicData.MGJKEJHEBPO_Blocks.Count; i++)
			{
				m_musicInfo.SetDiffLock((Difficulty.Type)i, m_selectMusicData.MGJKEJHEBPO_Blocks[i].POOMOBGPCNE_IsLocked, true);
				m_musicInfo.SetDiffStatus((Difficulty.Type)i, m_selectMusicData.MGJKEJHEBPO_Blocks[i].CADENLBDAEB_IsNew, 
					m_selectMusicData.MGJKEJHEBPO_Blocks[i].BCGLDMKODLC_IsClear, 
					(RhythmGameConsts.ResultComboType)m_selectMusicData.MGJKEJHEBPO_Blocks[i].LCOHGOIDMDF_ComboRank);
			}
			if(m_selectMusicData.MGJKEJHEBPO_Blocks[(int)diff].BCGLDMKODLC_IsClear && 
				m_selectMusicData.MGJKEJHEBPO_Blocks[(int)diff].KNIFCANOHOC_score > 0)
			{
				m_musicInfo.SetHighscore(m_selectMusicData.MGJKEJHEBPO_Blocks[(int)diff].KNIFCANOHOC_score);
				m_musicInfo.SetMusicScoreRank((ResultScoreRank.Type)m_selectMusicData.MGJKEJHEBPO_Blocks[(int)diff].BAKLKJLPLOJ_MusicLevel.DLPBHJALHCK_GetScoreRank(m_selectMusicData.MGJKEJHEBPO_Blocks[(int)diff].KNIFCANOHOC_score));
			}
			else
			{
