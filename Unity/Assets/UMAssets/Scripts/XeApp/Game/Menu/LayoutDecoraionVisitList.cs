...
namespace XeApp.Game.Menu
{
	internal class LayoutDecoraionVisitList : SwapScrollListContent
	{
		...
		public void SetSetting(EAJCBFGKKFA_FriendInfo friend, bool isFan/* = false*/, int fanNum/* = 0*/)
		{
			...
			{
				m_mapNameText.text = IMMAOANGPNK_NetMasterDataManager.HHCJCDFCLOB_Instance.NKEBMCIMJND_Database.EPAHOAKPAJJ_DecoItem.EFEGBHACJAL_GetStringParam("deco_init_room_name", JpStringLiterals.StringLiteral_9829);
			}
			else
			{
			...
			m_MusicRateRankText.text = OEGIPPCADNA_NetEventUtarateManager.GEEFFAEGHAH(OEGIPPCADNA_NetEventUtarateManager.BFKAHKBKBJE(friend.PCEGKKLKFNO_FriendData.AHEFHIMGIBI_PlayerData.MHEAEGMIKIE_PublicStatus.AILEOFKIELL_UtaRateRank, friend.PCEGKKLKFNO_FriendData.AJECHDLMKOE_LastLogin), true);
		...
		{
			m_divaDeco.Change(friend.JIGONEMPPNP_Diva, friend, CIOECGOMILE_NetPlayerDataManager.HHCJCDFCLOB_Instance.AHEFHIMGIBI_PlayerData.GAAOPEGIPKA_FavoritePlayer.FFKIDMKHIOE(friend.MLPEHNBNOGD_PlayerId), DisplayType.Level);
