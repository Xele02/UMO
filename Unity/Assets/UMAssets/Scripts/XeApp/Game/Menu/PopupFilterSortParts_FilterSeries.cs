...
namespace XeApp.Game.Menu
{
	public class PopupFilterSortParts_FilterSeries : PopupFilterSortPartsBase
	{
		...
		public static uint CreateBit(PIGBBNDPPJC a_view)
		{
			...
			{
				a = (int)IMMAOANGPNK.HHCJCDFCLOB.NKEBMCIMJND_Database.PFEKKPABPKL_HomeBg.GCINIJEMHFK(id).AIHCEGFANAM_SerieAttr;
			}
			else if(cat == EKLNMHFCAOI.FKGCBLHOOCL_Category.PFIOMNHDHCO_Valkyrie)
			{
				a = IMMAOANGPNK.HHCJCDFCLOB.NKEBMCIMJND_Database.PEOALFEGNDH_Valkyrie.GCINIJEMHFK(id).AIHCEGFANAM_SerieAttr;
			}
			else if(cat == EKLNMHFCAOI.FKGCBLHOOCL_Category.KBHGPMNGALJ_Costume)
			{
				a = IMMAOANGPNK.HHCJCDFCLOB.NKEBMCIMJND_Database.MGFMPKLLGHE_Diva.GCINIJEMHFK_GetInfo(IMMAOANGPNK.HHCJCDFCLOB.NKEBMCIMJND_Database.MFPNGNMFEAL_Costume.EEOADCECNOM_GetCostumeInfo(id).AHHJLDLAPAN_DivaId).AIHCEGFANAM_SerieAttr;
			}
			uint res = 16;
			if (a > 0 && a < 5)
				res = (uint)(1 << (a - 1));
