...
namespace XeApp.Game.Menu
{
	public class VerticalMusicSelectScene : VerticalMusicSelectSceneBase
	{
		...
		protected override IEnumerator Co_OnPreSetCanvas()
		{
			...
			if(args == null)
			{
				m_isLine6Mode = GameManager.Instance.localSave.EPJOACOONAC_GetSave().MCNEIJAOLNO_Select.ADHMDONLHLJ_NewLive.LMPCJJKHHPA_IsLine6();
			}
			else
			{
				...
				if(GameManager.Instance.localSave.EPJOACOONAC_GetSave().MCNEIJAOLNO_Select.ADHMDONLHLJ_NewLive.LMPCJJKHHPA_IsLine6())
				{
					GameManager.Instance.localSave.EPJOACOONAC_GetSave().MCNEIJAOLNO_Select.ADHMDONLHLJ_NewLive.HPDBEKAGKOD_SetIsLine6(isLine6Mode);
			...
			m_eventCtrl = JEPBIIJDGEF_NetEventManager.HHCJCDFCLOB_Instance.MKBJOOAILBB_GetEventByStatus(KGCNCBOKCBA.GNENJEHKMHD_EventStatus.EMAMLLFAOJI_6_Counting, false);
			...
			GameManager.Instance.localSave.EPJOACOONAC_GetSave().MCNEIJAOLNO_Select.ADHMDONLHLJ_NewLive.FKJBADIPKHK_GetFreeMusicId(isEvent, out songId, out eventType);
			...
			openSimulationLive = IMMAOANGPNK_NetMasterDataManager.HHCJCDFCLOB_Instance.NKEBMCIMJND_Database.GDEKCOOBLMA_System.LPJLEHAJADA_GetIntParam("mv_player_level", 5) <= 
				CIOECGOMILE_NetPlayerDataManager.HHCJCDFCLOB_Instance.AHEFHIMGIBI_PlayerData.KCCLEHLLOFG_Common.KIECDDFNCAN_Level;

			m_simulationButton.SetTicketNum(CIOECGOMILE_NetPlayerDataManager.HHCJCDFCLOB_Instance.AHEFHIMGIBI_PlayerData.KCCLEHLLOFG_Common.GKKDNOFMJJN_NumTicket);
			Database.Instance.bonusData.ClearEpisodeBonus();
			if(m_pickupFreeMusicId > 0)
				songId = m_pickupFreeMusicId;
			if(IsCanDoUnitHelp())
			{
				GameManager.Instance.localSave.EPJOACOONAC_GetSave().PPCGEFGJJIC_SortProprty.JHCKHAMFHMG_VerticalMusicSelect.LHPDDGIJKNB_Reset();
				GameManager.Instance.localSave.HJMKBCFJOOH_Save();
			}
		...
		protected override IEnumerator Co_OnPostSetCanvas()
		{
			...
			JEPBIIJDGEF_NetEventManager.HHCJCDFCLOB_Instance.LFOBIPKFOEF(OHCAABOMEOF.KGOGMKMBCPP_EventType.KEILBOLBDHN_4_Score, 0, () =>
			{
				//0xBF0888
			...
			else
			{
				if (!MenuScene.Instance.DirtyChangeScene)
				{
					if (KDHGBOOECKC_NetOfferManager.HHCJCDFCLOB_Instance.LOCAIBNPKDL_IsPlayerLevelOk() &&
						!CIOECGOMILE_NetPlayerDataManager.HHCJCDFCLOB_Instance.AHEFHIMGIBI_PlayerData.DAEJHMCMFJD_Offer.MLBBKNLPBBD_IsTutoDone(BOPFPIHGJMD.PDLKAKEABDP_Tuto.EILIAPKFCEO_0_OfferRelease/*0*/))
					{
		...
		public void SetTicketDropIcon(int ticketId)
		{
			if(ticketId < 1)
				return;
			if(EKLNMHFCAOI_ItemManager.BKHFLDMOGBD_GetItemCategory(ticketId) == EKLNMHFCAOI_ItemManager.FKGCBLHOOCL_Category.CFLFPPDMFAE_RaidItem || 
				EKLNMHFCAOI_ItemManager.BKHFLDMOGBD_GetItemCategory(ticketId) == EKLNMHFCAOI_ItemManager.FKGCBLHOOCL_Category.CLMIJKACELE_EventTicket)
			{
		...
		{
			long date = NKGJPJPHLIF_SakashoManager.HHCJCDFCLOB_Instance.IBLPICFDGOF_ServerRequester.FJDBNGEPKHL_Time.KMEFBNBFJHI_GetServerTime();
			int lastMusicId = GetLastStoryFreeMusicId();
			int song_Thresold = IMMAOANGPNK_NetMasterDataManager.HHCJCDFCLOB_Instance.NKEBMCIMJND_Database.GDEKCOOBLMA_System.LPJLEHAJADA_GetIntParam("verticalmusicselect_music_type_threshold", 96000);
			m_originalMusicDataList.Clear();
			m_originalEventMusicDataList.Clear();
			for(int i = 0; i < eventCategoryId; i++)
			{
				List<IBJAKJJICBC> l = IBJAKJJICBC.FKDIMODKKJD_GetList((int)MUSIC_LIST_TYPE[i], date, true, false, false, false);
				List<IBJAKJJICBC> viewMusicDataList = IBJAKJJICBC.FKDIMODKKJD_GetList((int)MUSIC_LIST_TYPE[i], date, true, false, false, true);
			...
			List<IBJAKJJICBC> l2 = IBJAKJJICBC.FKDIMODKKJD_GetList(5, date, true, false, false, false);
			List<IBJAKJJICBC> l3 = IBJAKJJICBC.FKDIMODKKJD_GetList(5, date, true, false, false, true);
		...
		public void SelectAprilFoolFocus()
		{
			...
			IKDICBBFBMI_NetEventBaseController ev = JEPBIIJDGEF_NetEventManager.HHCJCDFCLOB_Instance.OEGDCBLNNFF(OHCAABOMEOF.KGOGMKMBCPP_EventType.DAMDPLEBNCB_AprilFool, KGCNCBOKCBA.GNENJEHKMHD_EventStatus.BCKENOKGLIJ_9_ResultRewardreceived/*9*/);
			if(ev != null)
			{
				if(ev is AMLGMLNGMFB_NetEventAprilFoolController)
				{
		...
		private void SelectArgsFocus(MusicSelectArgs args)
		{
			...
			m_musicSelectUISapporter.isLine6Mode = save.LMPCJJKHHPA_IsLine6();
			OHCAABOMEOF.KGOGMKMBCPP_EventType gameEventType = 0;
			int songId = 0;
			save.FKJBADIPKHK_GetFreeMusicId(save.OAALFANHEHL_IsEventTab(), out songId, out gameEventType);
			if(m_pickupFreeMusicId > 0)
			{
				songId = m_pickupFreeMusicId;
			}
			...
			{
				if(args.selection.eventCategory != OHCAABOMEOF.KGOGMKMBCPP_EventType.HJNNKCMLGFL_0_None)
				{
					m_musicTab = VerticalMusicSelecChoiceMusicListTab.MusicTab.Event;
					SetMusicTab(m_musicTab);
					list_no = -1;
		...
		private void ApplyEventInfo()
		{
			...
			NKOBMDPHNGP_NetEventRaidLobbyController eventInfo = JEPBIIJDGEF_NetEventManager.HHCJCDFCLOB_Instance.OEGDCBLNNFF(OHCAABOMEOF.KGOGMKMBCPP_EventType.MCGPGMGEPHG_EventRaidLobby, KGCNCBOKCBA.GNENJEHKMHD_EventStatus.BCKENOKGLIJ_9_ResultRewardreceived) as NKOBMDPHNGP_NetEventRaidLobbyController;
			if(eventInfo != null && eventInfo.AKNOOLKMEGJ())
			{
				m_eventTicketId = NKOBMDPHNGP_NetEventRaidLobbyController.ADPMLOEOAFD_GetTicketId();
			...
			if(m_eventCtrl != null && 
				(m_eventCtrl.HIDHLFCBIDE_EventType == OHCAABOMEOF.KGOGMKMBCPP_EventType.PFKOKHODEGL_EventBattle || 
					...
					m_eventCtrl.HIDHLFCBIDE_EventType == OHCAABOMEOF.KGOGMKMBCPP_EventType.NKDOEBONGNI_EventMission ||
					m_eventCtrl.HIDHLFCBIDE_EventType == OHCAABOMEOF.KGOGMKMBCPP_EventType.CADKONMJEDA_11_EventRaid || 
			...
			)
			{
				...
				if(m_eventBanner != null)
				{
					...
					else
					{
						...
						if(m_feverLimit != null)
						{
							if(m_eventCtrl is MANPIONIGNO_NetEventGoDivaController)
							{
								int f = (m_eventCtrl as MANPIONIGNO_NetEventGoDivaController).GNDOGPBIGIL_GetCurrentBonusRate(time);
								if(f > 0)
								{
			...
			m_scoreEventCtrl = JEPBIIJDGEF_NetEventManager.HHCJCDFCLOB_Instance.AJLEDCKMFLP_GetEventScore(KGCNCBOKCBA.GNENJEHKMHD_EventStatus.EMAMLLFAOJI_6_Counting);
			if(m_scoreEventCtrl != null)
			{
		...
		private void OnClickEventBanner()
		{
			if(m_eventCtrl != null)
			{
				...
				if(m_eventCtrl.DPJCPDKALGI_RankingEnd >= t || m_isEventTimeLimit)
				{
					...
					else if(m_eventCtrl.HIDHLFCBIDE_EventType == OHCAABOMEOF.KGOGMKMBCPP_EventType.NKDOEBONGNI_EventMission)
					{
		...
		protected override void DelayedApplyMusicInfo()
		{
			...
			if (!listIsEmpty)
			{
				if(selectMusicData.BNIAJAKIAJC_IsEventMinigame)
				{
					...
					eventType = selectMusicData.NOKBLCDMLPP_MinigameEventInfo.HIDHLFCBIDE_EventType;
				}
				else
				{
					songId = freeMusicId;
		...
		private void ApplyMusicInfoEventEntrance()
		{
			...
			if(selectMusicData.BNIAJAKIAJC_IsEventMinigame)
			{
				...
				endTime = selectMusicData.NOKBLCDMLPP_MinigameEventInfo.PCCFAKEOBIC_EndTime;
			}
			else
			{
				m_musicSelectUISapporter.SetMusicInfoStyle(VerticalMusicSelectUISapporter.MusicInfoStyle.OtherEventEntrance);
				endTime = selectMusicData.AFCMIOIGAJN_EventInfo.PCCFAKEOBIC_EndTime;
			}
		...
		private void ApplyMusicInfoNormal()
		{
			...
			if (selectMusicData == null)
				return;
			if (selectMusicData.MGJKEJHEBPO_Blocks.Count == 0)
				return;
			...
			m_musicSelectUISapporter.SetBookMark(m_musicTab == 0, ViewBookMarkMusicData.KNKGEALPDGF_GetBookmark(selectMusicData.DLAEJOBELBH_MusicId));
			m_musicSelectUISapporter.SetLineTypeToggle(m_musicSelectUISapporter.isLine6Mode);
		...
		private void OnClickLine6Button()
		{
			...
			if(selectMusicData != null)
			{
				if(selectMusicData.BNIAJAKIAJC_IsEventMinigame)
				{
					...
					selectEventCategory = selectMusicData.NOKBLCDMLPP_MinigameEventInfo.HIDHLFCBIDE_EventType;
				}
				else
				{
			...
			GameManager.Instance.localSave.EPJOACOONAC_GetSave().MCNEIJAOLNO_Select.ADHMDONLHLJ_NewLive.HPDBEKAGKOD_SetIsLine6(m_musicSelectUISapporter.isLine6Mode);
			GameManager.Instance.localSave.HJMKBCFJOOH_Save();
		...
		private void OnClickMusicTabButton(bool isNormal)
		{
			...
			int songId;
			OHCAABOMEOF.KGOGMKMBCPP_EventType eventType;
			GameManager.Instance.localSave.EPJOACOONAC_GetSave().MCNEIJAOLNO_Select.ADHMDONLHLJ_NewLive.FKJBADIPKHK_GetFreeMusicId(m_musicTab == VerticalMusicSelecChoiceMusicListTab.MusicTab.Event, out songId, out eventType);
		...
		private int MusicDataListSortCb(VerticalMusicDataList.MusicListData left, VerticalMusicDataList.MusicListData right)
		{
			...
			if(!left.ViewMusic.KDAJEGNBOFJ)
			{
				...
				{
					IBJAKJJICBC.ANJLFFPBAEF_DifficultyInfo lDiff = left.ViewMusic.MGJKEJHEBPO_Blocks[(int)diff];
					IBJAKJJICBC.ANJLFFPBAEF_DifficultyInfo rDiff = right.ViewMusic.MGJKEJHEBPO_Blocks[(int)diff];
					switch (GameManager.Instance.localSave.EPJOACOONAC_GetSave().PPCGEFGJJIC_SortProprty.JHCKHAMFHMG_VerticalMusicSelect.LHPDCGNKPHD_SortItem)
					{
						...
						case 44:
							...
							break;
						case 45:
							res = lDiff.HHMLMKAEJBJ_Score.NLKEBAOBJCM_combo.CompareTo(rDiff.HHMLMKAEJBJ_Score.NLKEBAOBJCM_combo);
							break;
						case 46:
		...
		private bool IsFilter()
		{
			...
			if ((data.ALGFGPCPGFK_filterRange | data.AONOGHPAENH_filterMusicUnLock | data.DPDBMECAIIO_FilterUnit | data.JJNLEPEKNDO_FilterCombo | data.EOCPIGDIFNB_FilterMusicAttr | data.PGMJCBIHNHK_FilterReward) != 0)
				return true;
			...
			if(!listIsEmpty)
			{
				if(selectMusicData != null)
				{
					if(selectMusicData.BNIAJAKIAJC_IsEventMinigame)
					{
						...
						gameEventType = selectMusicData.NOKBLCDMLPP_MinigameEventInfo.HIDHLFCBIDE_EventType;
					}
					else
					{
			...
			if(m_musicTab != VerticalMusicSelecChoiceMusicListTab.MusicTab.Event)
			{
				if(!musicListData.ViewMusic.KDAJEGNBOFJ)
				{
					...
					if(CheckMusicFilter_Series(series, musicListData.ViewMusic) && CheckMusicFilter_MusicAttr(data.EOCPIGDIFNB_FilterMusicAttr, musicListData.ViewMusic) &&
						CheckMusicFilter_Combo(data.JJNLEPEKNDO_FilterCombo, musicListData.ViewMusic, diff) && CheckMusicFilter_Reward(data.PGMJCBIHNHK_FilterReward, musicListData.ViewMusic, diff, m_musicSelectUISapporter.isLine6Mode) &&
		...
		private bool CheckMusicFilter_MusicBookMark(int index, IBJAKJJICBC musicData)
		{
			if (index == 0)
				return true;
			return ViewBookMarkMusicData.KNKGEALPDGF_GetBookmark(musicData.DLAEJOBELBH_MusicId, index - 1);
		}
		...
		private bool IsCanDoUnitHelp()
		{
			...
			if(multi_dance_player_level <= CIOECGOMILE_NetPlayerDataManager.HHCJCDFCLOB_Instance.AHEFHIMGIBI_PlayerData.KCCLEHLLOFG_Common.KIECDDFNCAN_Level)
			{
