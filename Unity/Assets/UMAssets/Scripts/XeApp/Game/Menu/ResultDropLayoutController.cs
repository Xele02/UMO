...
namespace XeApp.Game.Menu
{
	public class ResultDropLayoutController : MonoBehaviour
	{
		...
		public void Setup(InitParam initParam)
		{
			...
			{
				initParam.layoutItemList[i].Setup(initParam.viewDropResultData.HBHMAKNGKFK_items[i], initParam.viewDropResultData.DCBDCHPKLCN_Rank != ResultScoreRank.Type.C);
			}
		...
		private bool CheckTutorialCodition(TutorialConditionId conditionId)
		{
			if (conditionId < TutorialConditionId.Condition30)
			{
				if (conditionId == TutorialConditionId.Condition8)
				{
					...
					{
						if (JGEOBNENMAH.HHCJCDFCLOB.NHPGGBCKLHC_FriendData.PCEGKKLKFNO_FriendData.LHMDABPNDDH_state != IBIGBMDANNM.LJJOIIAEICI.HEEJBCDDOJJ_Friend)
						{
