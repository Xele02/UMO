...
namespace XeApp.Game.Menu
{
    public class SelectPosterTextureLoader : DecorationSelectItemTextureLoaderBase
    {
        ...
        {
            EKLNMHFCAOI_ItemManager.FKGCBLHOOCL_Category cat = 0;
            if(subId == 1)
            {
                cat = EKLNMHFCAOI_ItemManager.FKGCBLHOOCL_Category.AEFGOANHNMG_DecoItemPosterSceneBef;
            }
            else if(subId == 2)
            {
                cat = EKLNMHFCAOI_ItemManager.FKGCBLHOOCL_Category.KKGHNKKGLCO_DecoItemPosterSceneAft;
            }
            else
            {
                cat = EKLNMHFCAOI_ItemManager.FKGCBLHOOCL_Category.OOMMOOIIPJE_DecoItemPoster;
            }
            MenuScene.Instance.ItemTextureCache.Load(EKLNMHFCAOI_ItemManager.GJEEGMCBGGM_GetItemFullId(cat, id), (IiconTexture texture) =>
            {
                //0xA6706C
