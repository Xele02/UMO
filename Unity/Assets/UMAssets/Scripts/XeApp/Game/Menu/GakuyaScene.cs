...
namespace XeApp.Game.Menu
{
	public class GakuyaScene : TransitionRoot
	{
		...
		private IEnumerator Co_ApplyDivaInfos(int divaId)
		{
			...
			m_presentListWindow.SetItems(OJEGDIBEBHP.FKDIMODKKJD_GetList());
		...
		private void OnClickCostumeItem(int index)
		{
			...
			if(item.m_isNew)
			{
				...
				if (item.m_cosColor == 0)
					item.m_viewDiva.LEHDLBJJBNC_SetNotNew();
				else
		...
		private void InitDivaCostumeInfos()
		{
			...
			foreach(var d in GameManager.Instance.ViewPlayerData.NBIGLBMHEDC_DivaList)
			{
				...
				{
					m_divaCostumeInfos.Add(new DivaCostumeInfo(d.AHHJLDLAPAN_DivaId, d.EGAFMGDFFCH_HomeDivaCostume.DAJGPBLEEOB_ModelId, d.JFFLFIMIMOI_HomeColorId));
				}
				else
				{
