...
namespace XeApp.Game.Menu
{
	public class PopupAchieveRewardContent : UIBehaviour, IPopupContent
	{
		...
		public bool Unused() { return m_viewInitialized; }
		...
		private void GetAchieveNowRewardList(PopupAchieveRewardDetailSetting setting)
		{
			...
			{
				if(m_viewFreeReward.IOCLNNCJFKA_ClearReward[i].CMCKNKKCNDK_status == FPGEMAIAMBF_RewardData.LOIJICNJMKA.KPGOMKPPJEE_Status.JMAFBDCPBJD_2_NewAchieved)
				{
			...
			{
				if (m_viewFreeReward.PDONJHCHBAE_ScoreReward[i].CMCKNKKCNDK_status == FPGEMAIAMBF_RewardData.LOIJICNJMKA.KPGOMKPPJEE_Status.JMAFBDCPBJD_2_NewAchieved)
				{
			...
			{
				if (m_viewFreeReward.HFPMKBAANFO_ComboReward[i].CMCKNKKCNDK_status == FPGEMAIAMBF_RewardData.LOIJICNJMKA.KPGOMKPPJEE_Status.JMAFBDCPBJD_2_NewAchieved)
				{
		...
		public static bool CheckAchieve(FPGEMAIAMBF_RewardData viewFreeRewardData)
		{
			...
			{
				if (viewFreeRewardData.IOCLNNCJFKA_ClearReward[i].CMCKNKKCNDK_status == FPGEMAIAMBF_RewardData.LOIJICNJMKA.KPGOMKPPJEE_Status.JMAFBDCPBJD_2_NewAchieved)
					return true;
			}
			for (int i = 0; i < viewFreeRewardData.PDONJHCHBAE_ScoreReward.Count; i++)
			{
				if (viewFreeRewardData.PDONJHCHBAE_ScoreReward[i].CMCKNKKCNDK_status == FPGEMAIAMBF_RewardData.LOIJICNJMKA.KPGOMKPPJEE_Status.JMAFBDCPBJD_2_NewAchieved)
					return true;
			}
			for (int i = 0; i < viewFreeRewardData.HFPMKBAANFO_ComboReward.Count; i++)
			{
				if (viewFreeRewardData.HFPMKBAANFO_ComboReward[i].CMCKNKKCNDK_status == FPGEMAIAMBF_RewardData.LOIJICNJMKA.KPGOMKPPJEE_Status.JMAFBDCPBJD_2_NewAchieved)
					return true;
			}
			return false;
		}
