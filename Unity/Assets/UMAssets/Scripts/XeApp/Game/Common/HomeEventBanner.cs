...
namespace XeApp.Game.Common
{
	public class HomeEventBanner : MonoBehaviour
	{
		...
		public void Setup(List<IKDICBBFBMI_EventBase> list, long currentTime)
		{
			...
			{
				if(!(b && OHCAABOMEOF.BPJMGICFPBJ(list[i].PGIIDPEGGPI_EventId) == OHCAABOMEOF.KGOGMKMBCPP_EventType.CADKONMJEDA_11_EventRaid))
				{
					MessageBank bk = MessageManager.Instance.GetBank("menu");
					long l1, l2;
					string str;
					if(list[i].NGOFCFJHOMI_Status <= KGCNCBOKCBA.GNENJEHKMHD_EventStatus.DOAENCHBAEO_11_Ended)
					{
