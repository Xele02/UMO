...
[System.Obsolete("Use CHHECNJBMLA_NetEventBoxGachaController", true)]
public class CHHECNJBMLA { }
public class CHHECNJBMLA_NetEventBoxGachaController : IKDICBBFBMI_NetEventBaseController
{
	...
	public CHHECNJBMLA_NetEventBoxGachaController(string _OPFGFINHFCE_name) : base(_OPFGFINHFCE_name)
    {
        return;
    }
	...
	{
		IMDBGDNPLJA_EventBoxGacha evDb = IMMAOANGPNK_NetMasterDataManager.HHCJCDFCLOB_Instance.NKEBMCIMJND_Database.LBDOLHGDIEB_Find(JOPOPMLFINI_QuestName) as IMDBGDNPLJA_EventBoxGacha;
		if(evDb != null)
		{
	...
	protected override bool JIHMLILFOPG_IsEventActive(long _JHNMKKNEENE_Time)
	{
		...
		if(evDb != null)
		{
			GDIPLANPCEI g = IMMAOANGPNK_NetMasterDataManager.HHCJCDFCLOB_Instance.ACEFBFLFKFD_GetScheduleEventData(evDb.JIKKNHIAEKG_BlockName, _JHNMKKNEENE_Time);
			if(g != null)
			{
	...
	protected override bool IMCMNOPNGHO(long _JHNMKKNEENE_Time)
	{
		...
		if(evDb != null)
		{
			ALIPBIMCAPN_EventBoxGacha.GLNFOMDKJJH save = CIOECGOMILE_NetPlayerDataManager.HHCJCDFCLOB_Instance.AHEFHIMGIBI_PlayerData.MMAIJOCPJHP_EventBoxGacha.FBCJICEPLED[evDb.NGHKJOEDLIP_Settings.MOEKELIIDEO_SaveIdx];
	...
	{
		IMDBGDNPLJA_EventBoxGacha NENJCIMMEBC = IMMAOANGPNK_NetMasterDataManager.HHCJCDFCLOB_Instance.NKEBMCIMJND_Database.LBDOLHGDIEB_Find(JOPOPMLFINI_QuestName) as IMDBGDNPLJA_EventBoxGacha;
		if(NENJCIMMEBC != null)
		{
			...
			PIMFJALCIGK(EKLNMHFCAOI_ItemManager.FKGCBLHOOCL_Category.MHKFDBLMOGF_Scene);
			GPHEKCNDDIK = true;
		}
	}

	// // RVA: 0x12C07B4 Offset: 0x12C07B4 VA: 0x12C07B4
	private void PIMFJALCIGK(EKLNMHFCAOI_ItemManager.FKGCBLHOOCL_Category _INDDJNMPONH_type/* = EKLNMHFCAOI_ItemManager.FKGCBLHOOCL_Category.MHKFDBLMOGF_Scene*/)
	{
		IMDBGDNPLJA_EventBoxGacha evDb = IMMAOANGPNK_NetMasterDataManager.HHCJCDFCLOB_Instance.NKEBMCIMJND_Database.LBDOLHGDIEB_Find(JOPOPMLFINI_QuestName) as IMDBGDNPLJA_EventBoxGacha;
		if(evDb != null)
		{
			...
			for(int i = 0; i < evDb.PKPLOGBIDIG_Prizes.Count; i++)
			{
				...
				{
					if(EKLNMHFCAOI_ItemManager.BKHFLDMOGBD_GetItemCategory(evDb.PKPLOGBIDIG_Prizes[i].GLCLFMGPMAN_ItemId) == _INDDJNMPONH_type)
					{
						int rId = evDb.PKPLOGBIDIG_Prizes[i].IMMDGJAOPCD_BoxId * 10000 + EKLNMHFCAOI_ItemManager.DEACAHNLMNI_getItemId(evDb.PKPLOGBIDIG_Prizes[i].GLCLFMGPMAN_ItemId);
	...
	private IEnumerator NJIEIJJMAHK_Corotuine_PreSetupEventHome(IMCBBOAFION _BHFHGFKBOHH_OnSuccess, DJBHIFLHJLK _AOCANKOMKFG_OnError)
	{
		...
		KHGBJCFJGMA = IMMAOANGPNK_NetMasterDataManager.HHCJCDFCLOB_Instance.NKEBMCIMJND_Database.LBDOLHGDIEB_Find(JOPOPMLFINI_QuestName) as IMDBGDNPLJA_EventBoxGacha;
		if(KHGBJCFJGMA != null)
		{
			...
			{
				ALIPBIMCAPN_EventBoxGacha.GLNFOMDKJJH save = CIOECGOMILE_NetPlayerDataManager.HHCJCDFCLOB_Instance.AHEFHIMGIBI_PlayerData.MMAIJOCPJHP_EventBoxGacha.FBCJICEPLED[KHGBJCFJGMA.NGHKJOEDLIP_Settings.MOEKELIIDEO_SaveIdx];
	...
	private bool KOCPLGOAKKG()
	{
		...
		if(evDb != null)
		{
			ALIPBIMCAPN_EventBoxGacha.GLNFOMDKJJH save /*NHLBKJCPLBL*/ = CIOECGOMILE_NetPlayerDataManager.HHCJCDFCLOB_Instance.AHEFHIMGIBI_PlayerData.MMAIJOCPJHP_EventBoxGacha.FBCJICEPLED[evDb.NGHKJOEDLIP_Settings.MOEKELIIDEO_SaveIdx];
			...
			int num = EKLNMHFCAOI_ItemManager.ALHCGDMEMID_GetNumItems(IMMAOANGPNK_NetMasterDataManager.HHCJCDFCLOB_Instance.NKEBMCIMJND_Database, CIOECGOMILE_NetPlayerDataManager.HHCJCDFCLOB_Instance.AHEFHIMGIBI_PlayerData, EKLNMHFCAOI_ItemManager.BKHFLDMOGBD_GetItemCategory(ticketId), EKLNMHFCAOI_ItemManager.DEACAHNLMNI_getItemId(ticketId), null);
			num -= evDb.KGDBEMPMAIJ_Boxes[save.IMMDGJAOPCD_BoxId - 1].NDNHKMJPBCM_Cost;
			if(num < 0)
				num = 0;
			EKLNMHFCAOI_ItemManager.DPHGFMEPOCA_SetNumItems(IMMAOANGPNK_NetMasterDataManager.HHCJCDFCLOB_Instance.NKEBMCIMJND_Database, CIOECGOMILE_NetPlayerDataManager.HHCJCDFCLOB_Instance.AHEFHIMGIBI_PlayerData, EKLNMHFCAOI_ItemManager.BKHFLDMOGBD_GetItemCategory(ticketId), EKLNMHFCAOI_ItemManager.DEACAHNLMNI_getItemId(ticketId), num, null);
			...
			if(savePrize != null)
			{
				...
				JANMJPOKLFL_InventoryUtil.FEGDNPIEKJC(OAGBCBBHMPF.COIIJOEKBDH_InventoryAddType.GMOLAHOIJHE_16_EventGacha, OGNKHBFKHIP.ToString());
				JANMJPOKLFL_InventoryUtil.CPIICACGNBH_AddItem(CIOECGOMILE_NetPlayerDataManager.HHCJCDFCLOB_Instance.AHEFHIMGIBI_PlayerData, dbPrize.GLCLFMGPMAN_ItemId, dbPrize.JBGEEPFKIGG_val, null, 0);
	...
	public bool KOCPLGOAKKG(int _LAJNCHHNLBI_n)
	{
		...
		if(evDb != null)
		{
			...
			if(box != null)
			{
				...
				if(box.NDNHKMJPBCM_Cost <= EKLNMHFCAOI_ItemManager.ALHCGDMEMID_GetNumItems(IMMAOANGPNK_NetMasterDataManager.HHCJCDFCLOB_Instance.NKEBMCIMJND_Database, CIOECGOMILE_NetPlayerDataManager.HHCJCDFCLOB_Instance.AHEFHIMGIBI_PlayerData, EKLNMHFCAOI_ItemManager.BKHFLDMOGBD_GetItemCategory(box.GLCLFMGPMAN_ItemId), EKLNMHFCAOI_ItemManager.DEACAHNLMNI_getItemId(box.GLCLFMGPMAN_ItemId), null))
				{
					...
					OGNKHBFKHIP = null;
					ILCCJNDFFOB.HHCJCDFCLOB_Instance.LIPHGGFEJFJ_EventBoxGacha(box.PPFNGGCBJKC_id, _LAJNCHHNLBI_n, IPOENPPBFNK(), MPJNMBFIJMB(), box.GLCLFMGPMAN_ItemId, box.NDNHKMJPBCM_Cost * _LAJNCHHNLBI_n, EKLNMHFCAOI_ItemManager.ALHCGDMEMID_GetNumItems(IMMAOANGPNK_NetMasterDataManager.HHCJCDFCLOB_Instance.NKEBMCIMJND_Database, CIOECGOMILE_NetPlayerDataManager.HHCJCDFCLOB_Instance.AHEFHIMGIBI_PlayerData, EKLNMHFCAOI_ItemManager.BKHFLDMOGBD_GetItemCategory(box.GLCLFMGPMAN_ItemId), EKLNMHFCAOI_ItemManager.DEACAHNLMNI_getItemId(box.GLCLFMGPMAN_ItemId), null), this);
					return true;
				}
			}
		}
