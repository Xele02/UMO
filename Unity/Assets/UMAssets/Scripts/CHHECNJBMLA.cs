...
public class CHHECNJBMLA_EventBoxGacha : IKDICBBFBMI_EventBase
{
	...
	public override string IFKKBHPMALH()
	{
		...
		if(evDb != null)
		{
			return evDb.NGHKJOEDLIP_Settings.OCDMGOGMHGE;
		}
		return null;
	}
	...
	protected override bool IMCMNOPNGHO(long _JHNMKKNEENE_Time)
	{
		...
		if(evDb != null)
		{
			...
			if((!RuntimeSettings.CurrentSettings.UnlimitedEvent && save.EGBOHDFBAPB_CloseAt < t) || save.EGBOHDFBAPB_CloseAt == 0)
			{
				...
				save.EGBOHDFBAPB_CloseAt = evDb.NGHKJOEDLIP_Settings.IDDBFFBPNGI_RewardEnd;
				IDLMBIEAPIL(1);
			}
			return true;
		}
	...
	{
		IMDBGDNPLJA_EventBoxGacha NENJCIMMEBC = IMMAOANGPNK.HHCJCDFCLOB.NKEBMCIMJND_Database.LBDOLHGDIEB_GetDbSection(JOPOPMLFINI_QuestName) as IMDBGDNPLJA_EventBoxGacha;
		if(NENJCIMMEBC != null)
		{
			...
			for(int i = 0; i < evDb.PKPLOGBIDIG_Prizes.Count; i++)
			{
				if(evDb.PKPLOGBIDIG_Prizes[i].PLALNIIBLOF_en == 2)
				{
					if(EKLNMHFCAOI.BKHFLDMOGBD_GetItemCategory(evDb.PKPLOGBIDIG_Prizes[i].GLCLFMGPMAN_ItemId) == _INDDJNMPONH_Type)
					{
						...
						d[rId] += evDb.PKPLOGBIDIG_Prizes[i].ADPPAIPFHML_Num * evDb.PKPLOGBIDIG_Prizes[i].JBGEEPFKIGG_Value;
					}
				}
			}
			foreach(var it in d)
	...
	private IEnumerator NJIEIJJMAHK_Corotuine_PreSetupEventHome(IMCBBOAFION _BHFHGFKBOHH_OnSuccess, DJBHIFLHJLK _AOCANKOMKFG_OnError)
	{
		...
		KHGBJCFJGMA = IMMAOANGPNK.HHCJCDFCLOB.NKEBMCIMJND_Database.LBDOLHGDIEB_GetDbSection(JOPOPMLFINI_QuestName) as IMDBGDNPLJA_EventBoxGacha;
		if(KHGBJCFJGMA != null)
		{
				...
				for(int i = 0; i < save.PKPLOGBIDIG_Prizes.Count; i++)
				{
					if(save.PKPLOGBIDIG_Prizes[i].JBDBPCMMBIH_Id > 0 && save.PKPLOGBIDIG_Prizes[i].JBDBPCMMBIH_Id < evDb.PKPLOGBIDIG_Prizes.Count + 1)
					{
						...
						if(_IMMDGJAOPCD_BoxId == evDb.PKPLOGBIDIG_Prizes[idx].IMMDGJAOPCD_BoxId && evDb.PKPLOGBIDIG_Prizes[idx].PLALNIIBLOF_en == 2)
						{
							...
							d.HMFFHLPNMPH_Count = evDb.PKPLOGBIDIG_Prizes[idx].JBGEEPFKIGG_Value;
			...
			if(box != null)
			{
				...
				for(int i = 0; i < evDb.PKPLOGBIDIG_Prizes.Count; i++)
				{
					if(_IMMDGJAOPCD_BoxId == evDb.PKPLOGBIDIG_Prizes[i].IMMDGJAOPCD_BoxId && evDb.PKPLOGBIDIG_Prizes[i].PLALNIIBLOF_en == 2)
					{
						...
						d.HMFFHLPNMPH_Count = evDb.PKPLOGBIDIG_Prizes[i].JBGEEPFKIGG_Value;
	...
	private bool KOCPLGOAKKG()
	{
		...
		if(evDb != null)
		{
			...
			if(savePrize != null)
			{
				...
				JANMJPOKLFL_InventoryUtil.CPIICACGNBH_AddItem(CIOECGOMILE.HHCJCDFCLOB.AHEFHIMGIBI_PlayerData, dbPrize.GLCLFMGPMAN_ItemId, dbPrize.JBGEEPFKIGG_Value, null, 0);
				...
				f.HMFFHLPNMPH_Count = dbPrize.JBGEEPFKIGG_Value;
			...
			for(int i = 0; i < l.Count; i++)
			{
				if(l[i].PLALNIIBLOF_en == 2)
				{
					if(res == null)
					{
						...
						res.HMFFHLPNMPH_Count = l[i].JBGEEPFKIGG_Value;
