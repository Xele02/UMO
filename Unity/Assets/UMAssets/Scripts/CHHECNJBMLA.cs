...
public class CHHECNJBMLA_EventBoxGacha : IKDICBBFBMI_EventBase
{
	...
	{
		public int IHLEPGEJNPM; // 0x8
		public int JBDBPCMMBIH; // 0xC
		public int GLCLFMGPMAN_ItemId; // 0x10
		public int HMFFHLPNMPH; // 0x14
		public int AGKANHNHECE; // 0x18
		public int GCEANEOEGMD; // 0x1C
		public bool AEDMJLGNDHN; // 0x20
	}
	...
	protected override void KKFLDCBHFJA(long JHNMKKNEENE)
	{
		...
		if(NENJCIMMEBC != null)
		{
			...
			DOLJEDAAKNN = NENJCIMMEBC.NGHKJOEDLIP.HEDAGCNPHGD;
			GLIMIGNNGGB_Start = NENJCIMMEBC.NGHKJOEDLIP.BONDDBOFBND_Start;
			DPJCPDKALGI_End1 = NENJCIMMEBC.NGHKJOEDLIP.HPNOGLIFJOP_End1;
			LOLAANGCGDO = NENJCIMMEBC.NGHKJOEDLIP.LNFKGHNHJKE;
			JDDFILGNGFH = NENJCIMMEBC.NGHKJOEDLIP.JGMDAOACOJF;
			LJOHLEGGGMC = NENJCIMMEBC.NGHKJOEDLIP.IDDBFFBPNGI;
			EMEKFFHCHMH_End = NENJCIMMEBC.NGHKJOEDLIP.KNLGKBBIBOH_End;
				...
				for(int i = 0; i < save.PKPLOGBIDIG_Prizes.Count; i++)
				{
					if(save.PKPLOGBIDIG_Prizes[i].JBDBPCMMBIH_Id > 0 && save.PKPLOGBIDIG_Prizes[i].JBDBPCMMBIH_Id < evDb.PKPLOGBIDIG_Prizes.Count + 1)
					{
						...
						if(IMMDGJAOPCD == evDb.PKPLOGBIDIG_Prizes[idx].IMMDGJAOPCD_BoxId && evDb.PKPLOGBIDIG_Prizes[idx].PLALNIIBLOF_Enabled == 2)
						{
							...
							d.HMFFHLPNMPH = evDb.PKPLOGBIDIG_Prizes[idx].JBGEEPFKIGG_Val;
							d.GCEANEOEGMD = evDb.PKPLOGBIDIG_Prizes[idx].ADPPAIPFHML_Num;
							d.AGKANHNHECE = save.PKPLOGBIDIG_Prizes[i].AGKANHNHECE_Remain;
							d.AEDMJLGNDHN = evDb.PKPLOGBIDIG_Prizes[idx].AEDMJLGNDHN_IsSp != 0;
							d.IHLEPGEJNPM = i;
							HFNLAEHDODI.Add(d);
						}
					}
			...
			if(box != null)
			{
				...
				for(int i = 0; i < evDb.PKPLOGBIDIG_Prizes.Count; i++)
				{
					if(IMMDGJAOPCD == evDb.PKPLOGBIDIG_Prizes[i].IMMDGJAOPCD_BoxId && evDb.PKPLOGBIDIG_Prizes[i].PLALNIIBLOF_Enabled == 2)
					{
						...
						d.HMFFHLPNMPH = evDb.PKPLOGBIDIG_Prizes[i].JBGEEPFKIGG_Val;
						d.GCEANEOEGMD = evDb.PKPLOGBIDIG_Prizes[i].ADPPAIPFHML_Num;
						d.AGKANHNHECE = evDb.PKPLOGBIDIG_Prizes[i].ADPPAIPFHML_Num;
						d.AEDMJLGNDHN = evDb.PKPLOGBIDIG_Prizes[i].AEDMJLGNDHN_IsSp != 0;
						d.IHLEPGEJNPM = i;
						HFNLAEHDODI.Add(d);
					}
				}
	...
	private bool KOCPLGOAKKG()
	{
		...
		if(evDb != null)
		{
			...
			if(savePrize != null)
			{
				...
				JANMJPOKLFL.FEGDNPIEKJC(OAGBCBBHMPF.COIIJOEKBDH.GMOLAHOIJHE_16, OGNKHBFKHIP.ToString());
				JANMJPOKLFL.CPIICACGNBH_AddItem(CIOECGOMILE.HHCJCDFCLOB.AHEFHIMGIBI_ServerSave, dbPrize.GLCLFMGPMAN_ItemId, dbPrize.JBGEEPFKIGG_Val, null, 0);
	...
	public int IPOENPPBFNK()
	{
		...
		if(evDb != null)
		{
			...
			for(int i = 0; i < JMLKAGOACAE.Count; i++)
			{
				res += save.PKPLOGBIDIG_Prizes[JMLKAGOACAE[i].IHLEPGEJNPM].AGKANHNHECE_Remain;
			}
			return res;
		}
		return 0;
			...
			for(int i = 0; i < l.Count; i++)
			{
				if(l[i].PLALNIIBLOF_Enabled == 2)
				{
					if(res == null)
					{
						...
						res.HMFFHLPNMPH = l[i].JBGEEPFKIGG_Val;
						res.GCEANEOEGMD = l[i].ADPPAIPFHML_Num;
						res.AGKANHNHECE = l[i].ADPPAIPFHML_Num - save.FEFCFGNGPGC_Pickup;
						res.AEDMJLGNDHN = true;
					}
					else if(res.GLCLFMGPMAN_ItemId == l[i].GLCLFMGPMAN_ItemId)
					{
						res.GCEANEOEGMD += l[i].ADPPAIPFHML_Num;
						res.AGKANHNHECE = res.GCEANEOEGMD - save.FEFCFGNGPGC_Pickup;
					}
				}
			}
			return res;
