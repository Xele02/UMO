...
public class CGFNKMNBNBN
{
	...
	public static void HHGLKFFKFAB(int HILIKOLBGEJ = -1)
	{
		...
		for (int i = 0; i < listDiva.Count; i++)
		{
			...
			{
				BJPLLEBHAGO dbDiva = IMMAOANGPNK.HHCJCDFCLOB.NKEBMCIMJND_Database.MGFMPKLLGHE_Diva.CDENCMNHNGA[listDiva[i].DIPKCALNIII_DivaId - 1];
				if (CIOECGOMILE.HHCJCDFCLOB.AHEFHIMGIBI_ServerSave.KCCLEHLLOFG_Common.POCPLFJCHDD_HomeBg[dbDiva.CMBCBNEODPD - 1].BEBJKJKBOGH_Date == 0)
				{
