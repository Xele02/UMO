
// Namespace: 
public class CBBJHPBGBAJ // TypeDefIndex: 6357
{

// Namespace: 
    public class JBCFNCNGLPM // TypeDefIndex: 6358
    {
        // // Fields
        // public string OPFGFINHFCE; // 0x8
        // public byte[] DBBGALAPFGC; // 0xC

        // // Methods

        // // RVA: 0x18F80F8 Offset: 0x18F80F8 VA: 0x18F80F8
        // public void .ctor() { }
    }
	// Fields
	// private const int HMJJCPMKDEI = 512;
	// public List<CBBJHPBGBAJ.JBCFNCNGLPM> KGHAJGGMPKL; // 0x8

	// // Methods

	// // RVA: 0x18F7E78 Offset: 0x18F7E78 VA: 0x18F7E78
	public bool KHEKNNFCAOI(byte[] GANPCOAGBBK)
    {
        // GANPCOAGBBK = decypted DB
        //!!!
        return true;
    }

	// // RVA: 0x18F80A0 Offset: 0x18F80A0 VA: 0x18F80A0
	// private bool DLBCLCKFDJP(byte[] IAKPCFDLMKP, int POMLAOPLMNA) { }

	// // RVA: 0x18F8100 Offset: 0x18F8100 VA: 0x18F8100
	// private string GIFPJBKOFAB(byte[] IAKPCFDLMKP, int POMLAOPLMNA) { }

	// // RVA: 0x18F81E8 Offset: 0x18F81E8 VA: 0x18F81E8
	// private int CBOIHFIPHGN(byte[] IAKPCFDLMKP, int POMLAOPLMNA) { }

	// // RVA: 0x18F82E8 Offset: 0x18F82E8 VA: 0x18F82E8
	// private int JNJODIPMKFJ(byte[] IAKPCFDLMKP, int POMLAOPLMNA) { }

	// // RVA: 0x18F830C Offset: 0x18F830C VA: 0x18F830C
	// public string MNNKOKNBJNC() { }

	// // RVA: 0x18F8728 Offset: 0x18F8728 VA: 0x18F8728
	// public void .ctor() { }
}