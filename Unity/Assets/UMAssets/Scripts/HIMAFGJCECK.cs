...
public class HIMAFGJCECK
{
	...
	public void KHEKNNFCAOI(EDOHBJAPLPF_JsonData IDLHJIOMJBK, IKMBBPDBECA KPCLJMBJEOI)
	{
		...
		for(int i = 0; i < d.HNBFOAJIIAL_Count; i++)
		{
			...
			if(KACECFNECON != null)
			{
				int idx = KACECFNECON.FLADABCFDFA_Pickup.FindIndex((CEBFFLDKAEC_SecureInt _GHPLINIACBB_x) =>
				{
					//0x1838588
		...
		for(int i = 0; i < d.HNBFOAJIIAL_Count; i++)
		{
			...
			int idx = OJGPPOKENLG_ProbabilityPerGroupKey.FindIndex((BPADANIKFHP _GHPLINIACBB_x) =>
			{
				//0x1838650
			...
			if(idx > -1)
			{
				OJGPPOKENLG_ProbabilityPerGroupKey[idx].HBHMAKNGKFK_Items.Add(data);
			}
			idx = DDGPEFEEKFP_Items.FindIndex((IJPECOFPOCH _GHPLINIACBB_x) =>
			{
				//0x18386B4
