
using System.Collections.Generic;

public class CPHJGFLEFNF
{
	public int GOIKCKHMBDL; // 0x8
	public int LKNCHJCLLFN; // 0xC
	public int AKNELONELJK; // 0x10
	public int LIDKENJKLGA; // 0x14
	public int HBODCMLFDOB; // 0x18
	public int HGACHHHCHHM; // 0x1C
	public int GEIONHDKGEB; // 0x20
	public int KNIFCANOHOC; // 0x24
	public int IAGMJKNPIFD; // 0x28
	public int ILJODCEHHGH; // 0x2C
	public int JKAMFMNGEBB; // 0x30
	public long ICJEDACBMMF_ServerTime; // 0x38
	public int LMOBPKIDIHF_AverageFps; // 0x40
	public int IPAAOFCGEAB_MinFps; // 0x44
	public int CNDDKMJAIBG; // 0x48
	public int HJILBFGFFEM; // 0x4C
	public int PICAFKPEJCJ; // 0x50
	public int FILFPNDEINH; // 0x54
	public int POGINDBNBAJ; // 0x58
	public int MPHFGEPJOGL; // 0x5C
	public int HNCDHJDENJO; // 0x60
	public int IMIEPNOECFD_HasValkyrieMode; // 0x64
	public int GFODFMFGLJG_HadDivaMode; // 0x68
	public List<int> LAMMILPNINO = new List<int>(); // 0x6C
	public List<int> MNDPPLILCPJ = new List<int>(); // 0x70
	public List<int> JIHECDPAOKB = new List<int>(); // 0x74
	public int GAHLMIOHCAI; // 0x78
	public int[] FMMKKCANOKG; // 0x7C
	//public JBECFDNKPFD MFCGAKOAGFE; // 0x80
	public int KBINAMHOODM; // 0x84
	public int LMJNFABLJEO; // 0x88
	public int NMDHKLPAKHB; // 0x8C
	public int BNJBFKAIICK; // 0x90
	public string OEEOILKOKFM; // 0x94
	public int HPEHMCGPMJM; // 0x98
	public int MOJEOLJMNKP; // 0x9C
	public int NOEAGOJKEAJ; // 0xA0
	public int FCFICFMIHLI; // 0xA4
	public int DHMCFAMCPFK; // 0xA8
	public int GFCBHIAOCMC; // 0xAC
	public int CAOHBKEIGDM; // 0xB0
}
