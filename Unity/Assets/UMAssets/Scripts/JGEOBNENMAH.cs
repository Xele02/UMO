...
public class JGEOBNENMAH
{
				...
				if (KFIEMBCDEFO.HNNPBABEPBP(OMNOFMEBLAD, NHPGGBCKLHC_FriendPlayerData))
				{
					...
					if (OMNOFMEBLAD.JPJMALBLKDI_Tutorial == 0)
					{
						...
						for (int i = 0; i < c; i++)
						{
							...
							{
								GNGMCIAIKMA.HHCJCDFCLOB.HEFIKPAHCIA_HasBingoActive(OMNOFMEBLAD, -1);
							}
						}
						QuestUtility.UpdateQuestData(LayoutQuestTab.eTabType.Event);
					}
	...
	private void IBEIMGHLPPJ(FOCPLKMMCAL JNLKJCDFFMM, JGEOBNENMAH.HAJIFNABIFF OMNOFMEBLAD)
	{
		...
		data.FMMKKCANOKG = JNLKJCDFFMM.FFDDBFEIJKI;
	...
	private IEnumerator PLKLMBHENKB_Coroutine_NewSave(bool JCCBGECHIEI, IMCBBOAFION BHFHGFKBOHH, DJBHIFLHJLK MOBEEPPKFLG)
	{
		...
		MOMPDFMMICK HLGCPFMDJOA; // 0x34
		AHNHJFCAPCH KIIKDGOFFBM; // 0x38
		...
		if(!JCCBGECHIEI)
		{
			...
			if (LFDFPBEAAFA.PLMMGCFLFGC_Keys.Count < 1)
			{
				if (LFDFPBEAAFA.OKJNNGJJOFA_RaidBossId == 0)
				{
					...
					//LAB_00b132f8;
					while (!HINLOBICGDI.JFOKBBLFMLD)
						yield return null;
					//LAB_00b13324
					if (HINLOBICGDI.IAGEPLEBOKJ)
					{
						//LAB_00b14170
						if (HINLOBICGDI.LBJPGPOJOKP == 184)
						{
							//LAB_00b141c0
							LFDFPBEAAFA.OJCJPCHFPGO();
						}
				...
				{
					KIIKDGOFFBM = NKGJPJPHLIF.HHCJCDFCLOB.IBLPICFDGOF_ServerRequester.IFFNCAFNEAG_AddRequest(new AHNHJFCAPCH());
					KIIKDGOFFBM.BIHCCEHLAOD.KJPDHNJGEAH = LFDFPBEAAFA.OKJNNGJJOFA_RaidBossId;
					KIIKDGOFFBM.BIHCCEHLAOD.OHNJJIMGKGK = LFDFPBEAAFA.OHNJJIMGKGK_Names;
					IKPIMINCOPI_JsonMapper.EJCOJCGIBNG_ToJson(LFDFPBEAAFA.HJDBGMMPPEF_Player, LAFGAPBDKML);
					KIIKDGOFFBM.BIHCCEHLAOD.AHEFHIMGIBI_PlayerData = LAFGAPBDKML.ToString();
					KIIKDGOFFBM.BIHCCEHLAOD.CHDDDCCHJJH = LFDFPBEAAFA.CHDDDCCHJJH_Replace;
					//LAB_00b132b8;
					while (!KIIKDGOFFBM.JFOKBBLFMLD)
						yield return null;
					//LAB_00b13324
					if (KIIKDGOFFBM.IAGEPLEBOKJ)
					{
						//LAB_00b14170
						if (KIIKDGOFFBM.LBJPGPOJOKP == 184)
						{
							//LAB_00b141c0
							LFDFPBEAAFA.OJCJPCHFPGO();
						}
			...
			{
				HLGCPFMDJOA = NKGJPJPHLIF.HHCJCDFCLOB.IBLPICFDGOF_ServerRequester.IFFNCAFNEAG_AddRequest(new MOMPDFMMICK());
				HLGCPFMDJOA.EFDFLLPLDKD = LFDFPBEAAFA.PLMMGCFLFGC_Keys;
				HLGCPFMDJOA.HHIHCJKLJFF = LFDFPBEAAFA.OHNJJIMGKGK_Names;
				IKPIMINCOPI_JsonMapper.EJCOJCGIBNG_ToJson(LFDFPBEAAFA.HJDBGMMPPEF_Player, LAFGAPBDKML);
				HLGCPFMDJOA.AHEFHIMGIBI_PlayerData = LAFGAPBDKML.ToString();
				HLGCPFMDJOA.BLOCFLFHCFJ = !LFDFPBEAAFA.CHDDDCCHJJH_Replace;
				...
				while (!HLGCPFMDJOA.JFOKBBLFMLD)
					yield return null;
				if (HLGCPFMDJOA.IAGEPLEBOKJ)
				{
					if (HLGCPFMDJOA.LBJPGPOJOKP != 132)
					{
						//LAB_00b14170
						if (HLGCPFMDJOA.LBJPGPOJOKP == 184)
						{
							//LAB_00b141c0
							LFDFPBEAAFA.OJCJPCHFPGO();
						}
			...
			//LAB_00b13dd0
			NDABOOOOENC.HHCJCDFCLOB.NKFNJMEELMP(LFDFPBEAAFA.BIOBLLHBKBC_Gpgs);
			if(LFDFPBEAAFA.CFLLEDGILPK_CoopQuest.Count > 0)
			{
				...
				PBJPACKDIIB.Instance.KNPBADBCOLO(() =>
				{
					//0xB0AB80
					BEKAMBBOLBO = true;
				}, () =>
