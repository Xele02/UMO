...
public class JGEOBNENMAH
{
	...
	private IEnumerator KMOBPDBOCHJ_Co_GameStartSave(bool _DKKJPLALNFD_Skip, IMCBBOAFION _BHFHGFKBOHH_OnSuccess, DJBHIFLHJLK _AOCANKOMKFG_OnError)
	{
		...
		{
			PKNOKJNLPOE_EventRaid ev = JEPBIIJDGEF_EventInfo.HHCJCDFCLOB.MKBJOOAILBB_GetEventByStatus(KGCNCBOKCBA.GNENJEHKMHD_EventStatus.EMAMLLFAOJI_Counting_6, false) as PKNOKJNLPOE_EventRaid;
		...
		if(!AGDEBBENNCK.OBOPMHBPCFE_MvMode)
		{
			...
			else
			{
				...
				{
					NKGFGDGFGFM_SessionId = JDDGPJDKHNE.GPLMOKEIOLE();
					ILCCJNDFFOB.HHCJCDFCLOB.JMAJBHENDPF(AGDEBBENNCK, NKGFGDGFGFM_SessionId, ECHONOJEPHP_Stamina, FPPNIKFLAFM_UseStamina, JEPBIIJDGEF_EventInfo.HHCJCDFCLOB.MKBJOOAILBB_GetEventByStatus(KGCNCBOKCBA.GNENJEHKMHD_EventStatus.EMAMLLFAOJI_Counting_6, false));
				}
				else
				{
					if(AGDEBBENNCK.MNNHHJBBICA_GameEventType == 4)
					...
					else
					{
						if(AGDEBBENNCK.MNNHHJBBICA_GameEventType == 11)
						{
							PKNOKJNLPOE_EventRaid ev = JEPBIIJDGEF_EventInfo.HHCJCDFCLOB.OEGDCBLNNFF(OHCAABOMEOF.KGOGMKMBCPP_EventType.CADKONMJEDA_EventRaid, KGCNCBOKCBA.GNENJEHKMHD_EventStatus.EMAMLLFAOJI_Counting_6) as PKNOKJNLPOE_EventRaid;
							NKGFGDGFGFM_SessionId = JDDGPJDKHNE.GPLMOKEIOLE();
			...
			if(OMNOFMEBLAD.MNNHHJBBICA_GameEventType != 2)
			{
				...
				else
				{
					if (OMNOFMEBLAD.MNNHHJBBICA_GameEventType == 4)
					{
						...
						if(ev != null)
						{
                            FEHINJKHDAP_EventScore.ALGDNCMJHGN d = ev.JIPPHOKGLIH(false);
	...
	private IEnumerator IILJJMAEPCI_GameClear(HAJIFNABIFF OMNOFMEBLAD, IMCBBOAFION _BHFHGFKBOHH_OnSuccess, DJBHIFLHJLK _MOBEEPPKFLG_OnFail)
	{
		...
		else
		{
			...
			else
			{
				...
				{
					//L585
					PKNOKJNLPOE_EventRaid JGFEPLJIMEN = JEPBIIJDGEF_EventInfo.HHCJCDFCLOB.OEGDCBLNNFF(OHCAABOMEOF.KGOGMKMBCPP_EventType.CADKONMJEDA_EventRaid, KGCNCBOKCBA.GNENJEHKMHD_EventStatus.EMAMLLFAOJI_Counting_6) as PKNOKJNLPOE_EventRaid;
					int a = 0;
				...
				if (KFIEMBCDEFO.HNNPBABEPBP(OMNOFMEBLAD, NHPGGBCKLHC_FriendData))
				{
					...
					{
                        //1023
                        IKDICBBFBMI_EventBase PPPDHBBNEBO = JEPBIIJDGEF_EventInfo.HHCJCDFCLOB.MKBJOOAILBB_GetEventByStatus(KGCNCBOKCBA.GNENJEHKMHD_EventStatus.EMAMLLFAOJI_Counting_6, false);
					...
					if (OMNOFMEBLAD.JPJMALBLKDI_Tutorial == 0)
					{
						NKOBMDPHNGP_EventRaidLobby evRL = JEPBIIJDGEF_EventInfo.HHCJCDFCLOB.OEGDCBLNNFF(OHCAABOMEOF.KGOGMKMBCPP_EventType.MCGPGMGEPHG_EventRaidLobby, KGCNCBOKCBA.GNENJEHKMHD_EventStatus.EMAMLLFAOJI_Counting_6) as NKOBMDPHNGP_EventRaidLobby;
						if (evRL != null)
						{
	...
	private void IBEIMGHLPPJ(FOCPLKMMCAL _JNLKJCDFFMM_clear, JGEOBNENMAH.HAJIFNABIFF OMNOFMEBLAD)
	{
		...
		if (OMNOFMEBLAD.GHBPLHBNMBK_FreeMusicId > 0)
		{
			...
			{
				int prevScore = CIOECGOMILE.HHCJCDFCLOB.AHEFHIMGIBI_PlayerData.LCKMBHDMPIP_RecordMusic.FAMANJGJANN_FreeMusicInfo[OMNOFMEBLAD.GHBPLHBNMBK_FreeMusicId - 1].BDCAICINCKK_GetScoreForDiff(OMNOFMEBLAD.AKNELONELJK_difficulty);
				cappedScore = score;
				if (score < prevScore)
		...
		else
		{
			...
			{
				case 1:
					ILCCJNDFFOB.HHCJCDFCLOB.NJKBAICBOIN(data, OMNOFMEBLAD, NKGFGDGFGFM_SessionId, JEPBIIJDGEF_EventInfo.HHCJCDFCLOB.MKBJOOAILBB_GetEventByStatus(KGCNCBOKCBA.GNENJEHKMHD_EventStatus.EMAMLLFAOJI_Counting_6, false));
					break;
				case 2:
					{
						MHAPMOLCPKM_EventQuest ev = JEPBIIJDGEF_EventInfo.HHCJCDFCLOB.MKBJOOAILBB_GetEventByStatus(KGCNCBOKCBA.GNENJEHKMHD_EventStatus.EMAMLLFAOJI_Counting_6, false) as MHAPMOLCPKM_EventQuest;
	...
	public void EFHMAKNEGEA(HAJIFNABIFF OMNOFMEBLAD)
	{
		...
		if (OMNOFMEBLAD.GHBPLHBNMBK_FreeMusicId > 0)
		{
			...
			{
				scoreMax = CIOECGOMILE.HHCJCDFCLOB.AHEFHIMGIBI_PlayerData.LCKMBHDMPIP_RecordMusic.FAMANJGJANN_FreeMusicInfo[OMNOFMEBLAD.GHBPLHBNMBK_FreeMusicId - 1].BDCAICINCKK_GetScoreForDiff(OMNOFMEBLAD.AKNELONELJK_difficulty);
			}
		}
		...
		else
		{
			...
			{
				case 1:
					ILCCJNDFFOB.HHCJCDFCLOB.NJKBAICBOIN(c, OMNOFMEBLAD, NKGFGDGFGFM_SessionId, JEPBIIJDGEF_EventInfo.HHCJCDFCLOB.MKBJOOAILBB_GetEventByStatus(KGCNCBOKCBA.GNENJEHKMHD_EventStatus.EMAMLLFAOJI_Counting_6, false));
					break;
				case 2:
					{
						MHAPMOLCPKM_EventQuest ev = JEPBIIJDGEF_EventInfo.HHCJCDFCLOB.MKBJOOAILBB_GetEventByStatus(KGCNCBOKCBA.GNENJEHKMHD_EventStatus.EMAMLLFAOJI_Counting_6, false) as MHAPMOLCPKM_EventQuest;
	...
	public static KLBKPANJCPL_Score KFFCMNELJJB_GetMusicScore(EONOEHOKBEB_Music EPMMNEFADAP, int __AKNELONELJK_difficulty, bool _GIKLNODJKFK_IsLine6/* = false*/)
	{
		return IMMAOANGPNK.HHCJCDFCLOB.NKEBMCIMJND_Database.IBPAFKKEKNK_Music.ALJFMLEJEHH_GetMusicScore(EPMMNEFADAP.KKPAHLMJKIH_WavId, EPMMNEFADAP.BKJGCEOEPFB_VariationId, __AKNELONELJK_difficulty, _GIKLNODJKFK_IsLine6, true);
	}
		...
		if(IMMAOANGPNK.HHCJCDFCLOB.NKEBMCIMJND_Database != null)
		{
			...
			if(weekday_event_play_count_heal < 0)
			{
					...
					CIOECGOMILE.HHCJCDFCLOB.AHEFHIMGIBI_PlayerData.LCKMBHDMPIP_RecordMusic != null)
				{
					...
					if(record != null)
					{
						if(IMMAOANGPNK.HHCJCDFCLOB.NKEBMCIMJND_Database.CLLPBOPLICM_EventWeekDay.FLPDCNBLOKL(record.CAPAIICHDMH_WDat, _GHBPLHBNMBK_FreeMusicId))
						{
	...
	public bool HCFONACBEBD(OKGLGHCBCJP_Database _LKMHPJKIFDN_md, BBHNACPENDM_ServerSaveData _LDEGEHAEALK_ServerData, int _GHBPLHBNMBK_FreeMusicId, int MNLNFGKJANA)
	{
		...
		if(_LKMHPJKIFDN_md.CLLPBOPLICM_EventWeekDay != null && _LDEGEHAEALK_ServerData.LCKMBHDMPIP_RecordMusic != null)
		{
			...
			if(record != null)
			{
				if(_LKMHPJKIFDN_md.CLLPBOPLICM_EventWeekDay.FLPDCNBLOKL(record.CAPAIICHDMH_WDat, _GHBPLHBNMBK_FreeMusicId))
				{
					int cnt = 0;
					if(MNLNFGKJANA > -1)
					{
	...
	// public void CADNBFCHAKM(IMCBBOAFION _BHFHGFKBOHH_OnSuccess, DJBHIFLHJLK _MOBEEPPKFLG_OnFail) { }
