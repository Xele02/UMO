...
public class KDLPEDBKMID
{
	...
	private string FAKFFDIFAAH_GetFullServerNameForLocalFile(string CJEKGLGBIHF) // Download Path
	{
		...
		uint val = BEEINMBNKNM_Encryption.DIKDKNIKPNJ((uint)IMMAOANGPNK.HHCJCDFCLOB.NKEBMCIMJND_Database.GDEKCOOBLMA_System.NGHKJOEDLIP.KBNGOBEAHIC, CJEKGLGBIHF);
		JBBHNIACMFJ.Clear();
			...
			PMDNNKAPIKJ_FileDownloader.LBGNKOJFOFC = (JEHIAIPJNJF_FileDownloader.HCJPJKCIBDL_DldFileInfo JGBPLIGAILE) => {
				...
				if(fileinfo != null)
				{
					...
					if(a != null)
					{
						...
						{
							FECDBKKBAHO.FHOPNIJCFKA b = KEHOJEJMGLJ.HHCJCDFCLOB.KLIJFOBEKBE.ANIJHEBLMGB(JGBPLIGAILE.AJPIGKBIDDL_LocalFileName, CPHFEPHDJIB.FJDBNGEPKHL.KMEFBNBFJHI(), a.KKPAHLMJKIH_WavId);
							b.FNALNKKMKDC_ExpireTime = CPHFEPHDJIB.FJDBNGEPKHL.KMEFBNBFJHI() + a.NKGJOAEDCPH.PAAPNEMBHGN_Day * 24 * 60 * 60;
