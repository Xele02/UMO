...
public class CIOECGOMILE
{
	...
	public BBHNACPENDM_SaveData AHEFHIMGIBI_Save { get; private set; }  // 0x8 LCHHPGMCKJD // GNMGJMDJEGI LGLGNHAMNIK
	private BBHNACPENDM_SaveData CCNKAKCBBDJ { get; set; } // 0xC MOMIOKHCMKH GHMKOMEKBFO KKOICOJPENH
	public BBHNACPENDM_SaveData FMFKHDPKLOC { get; set; } // 0x10 FDDDKGAMLLB AAMOPGKKGEI OBEALILDFEM
	public BBHNACPENDM_SaveData MNJHBCIIHED { get; private set; } // 0x14 JHPFMKPOIOC KBOFNPLNBOA ICKBCAJFDOM
	...
	{
		HHCJCDFCLOB = this;
		AHEFHIMGIBI_Save = new BBHNACPENDM_SaveData();
		AHEFHIMGIBI_Save.HFCOIIHIENB = BBHNACPENDM_SaveData.BDADJONBIBO.FKNGHCNOEHO/*1*/;
		CCNKAKCBBDJ = new BBHNACPENDM_SaveData();
		CCNKAKCBBDJ.HFCOIIHIENB = BBHNACPENDM_SaveData.BDADJONBIBO.AFGALHECDIJ/*3*/;
		FMFKHDPKLOC = new BBHNACPENDM_SaveData();
		CCNKAKCBBDJ.HFCOIIHIENB = BBHNACPENDM_SaveData.BDADJONBIBO.GGEELFGJAMP/*2*/;
		MNJHBCIIHED = new BBHNACPENDM_SaveData();
		CCNKAKCBBDJ.HFCOIIHIENB = BBHNACPENDM_SaveData.BDADJONBIBO.LPKPFMHEKEM/*4*/;
	...
	private IEnumerator ODDEPBIJHOE_Coroutine_Load(IMCBBOAFION BHFHGFKBOHH, IMCBBOAFION FLENFOEFHPL, DJBHIFLHJLK MOBEEPPKFLG, bool JKJEFPNIPFO)
	{
		...
		AHEFHIMGIBI_Save.KHEKNNFCAOI_Init(DLMOKNDEMMB);
		...
		BPOJOBICBAC.HHIHCJKLJFF = AHEFHIMGIBI_Save.KPIDBPEKMFD();
		BPOJOBICBAC.IJMPLDBGMHC = AHEFHIMGIBI_Save.IIEMACPEEBJ;
		yield return BPOJOBICBAC.GDPDELLNOBO(N.a);

		StringBuilder str = new StringBuilder();
		bool firstDone = false;
		bool b = false;
		for(int i = 0; i < AHEFHIMGIBI_Save.MGJKEJHEBPO.Count; i++)
		{
			if(firstDone)
			{
				str.Append(",");
			}
			str.Append(AHEFHIMGIBI_Save.MGJKEJHEBPO[i].JIKKNHIAEKG);
			str.Append(":");
			str.Append(AHEFHIMGIBI_Save.MGJKEJHEBPO[i].LLBJFFFJEPJ);
			str.Append(":");
			str.Append(AHEFHIMGIBI_Save.MGJKEJHEBPO[i].FJMOAAPNCJI);
			firstDone = true;
			b = b | !AHEFHIMGIBI_Save.MGJKEJHEBPO[i].LLBJFFFJEPJ;
		}
		...
		BPOJOBICBAC = null;
		AHEFHIMGIBI_Save.NEBDDPDPAKJ(JKJEFPNIPFO);
		FMFKHDPKLOC.ODDIHGPONFL_Copy(AHEFHIMGIBI_Save);
		CCNKAKCBBDJ.ODDIHGPONFL_Copy(AHEFHIMGIBI_Save);
		PJKLMCGEJMK.DALFMJFKCGJ = AHEFHIMGIBI_Save.MCKEOKFMLAH;
		...
		while(true)
		{
			if(KPFKKDDOHCN.PLOOEECNHFB)
			{
				...
				while(true)
				{
					...
					{
						int a = DLMOKNDEMMB.FMPEMFPLPDA_Exp.PCJACJANGCA(AHEFHIMGIBI_Save.KCCLEHLLOFG_Common.KIECDDFNCAN_Level);
						...
						while(true)
						{
							...
							if(DICMPMEEIJL)
							{
								if(!JNDDNFEINEH)
								{
									...
									KPHFDEOFKLM();
									LBLNLMGHLAG();
									AHEFHIMGIBI_Save.KCCLEHLLOFG_Common.LPFFDGNDLKG(true, 2160);
									if(IPOHDKPGNFO())
									{
										FAFAKNJLLIC();
										CDIADEIOIHP();
									...
									NPAPJMLJBKM(false);
									AHEFHIMGIBI_Save.FHLMCCPCEAI_SaveHash.BEBJKJKBOGH = NKGJPJPHLIF.HHCJCDFCLOB.IBLPICFDGOF.FJDBNGEPKHL.KMEFBNBFJHI_GetServerTime();
									MIPMDKBKKJD();
									PCMPDIMCBJM();
									HLACGFNICAI();

									HHCJCDFCLOB.AHEFHIMGIBI_Save.BEKHNNCGIEL_Costume.AGEAPKNODHO();
									GameManager.Instance.localSave.FBCDKFENOEM();
									CHNJPFCKFOI.BCEAAAOLGEB();
									FGDMEFINCEE();
									if(AHEFHIMGIBI_Save.JHFIPCIHJNL_Base.IJHBIMNKOMC == 0)
									{
										if(FLENFOEFHPL != null)
										{
											FLENFOEFHPL();
	...
	{
		if(LNAHEIEIBOI && AHEFHIMGIBI_Save != null)
		{
			if(BLCDJDJJBHC)
				return;
			if(AHEFHIMGIBI_Save.PFAKPFKJJKA(false) != null)
			{
				BLCDJDJJBHC = true;
				DCLNIAMEGLA = AHEFHIMGIBI_Save.PFAKPFKJJKA(false);
			}
		}
	}
