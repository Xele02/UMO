...
public class CIOECGOMILE
{
	...
	public BBHNACPENDM AHEFHIMGIBI_Save { get; private set; }  // 0x8 LCHHPGMCKJD // GNMGJMDJEGI LGLGNHAMNIK
	...
	{
		HHCJCDFCLOB = this;
		AHEFHIMGIBI_Save = new BBHNACPENDM();
		AHEFHIMGIBI_Save.HFCOIIHIENB = BBHNACPENDM.BDADJONBIBO.FKNGHCNOEHO/*1*/;
		...
		while(true)
		{
			if(KPFKKDDOHCN.PLOOEECNHFB)
			{
				...
				while(true)
				{
					...
					{
						int a = DLMOKNDEMMB.FMPEMFPLPDA_Exp.PCJACJANGCA(AHEFHIMGIBI_Save.KCCLEHLLOFG.KIECDDFNCAN);
						...
						while(true)
						{
							...
							if(DICMPMEEIJL)
							{
								if(!JNDDNFEINEH)
								{
									...
									KPHFDEOFKLM();
									LBLNLMGHLAG();
									AHEFHIMGIBI_Save.KCCLEHLLOFG.LPFFDGNDLKG(true, 2160);
									if(IPOHDKPGNFO())
									{
										FAFAKNJLLIC();
										CDIADEIOIHP();
									...
									NPAPJMLJBKM(false);
									AHEFHIMGIBI_Save.FHLMCCPCEAI.BEBJKJKBOGH = NKGJPJPHLIF.HHCJCDFCLOB.IBLPICFDGOF.FJDBNGEPKHL.KMEFBNBFJHI();
									MIPMDKBKKJD();
									PCMPDIMCBJM();
									HLACGFNICAI();

									HHCJCDFCLOB.AHEFHIMGIBI_Save.BEKHNNCGIEL.AGEAPKNODHO();
									GameManager.Instance.localSave.FBCDKFENOEM();
									CHNJPFCKFOI.BCEAAAOLGEB();
									FGDMEFINCEE();
									if(AHEFHIMGIBI_Save.JHFIPCIHJNL.IJHBIMNKOMC == 0)
									{
										if(FLENFOEFHPL != null)
										{
											FLENFOEFHPL();
