...
public class CIOECGOMILE
{
	...
	public void DLFDPCDKHOB(LOBDIAABMKG EPNFGKBBJCE, GCAHJLOGMCI.NFCAJPIJFAM BJLONGBNPCI, CDOPFBOHDEF BHFHGFKBOHH, DJBHIFLHJLK HDFGHFOCHKE, DJBHIFLHJLK JGKOLBLPMPG)
	{
		...
		if(BJLONGBNPCI == GCAHJLOGMCI.NFCAJPIJFAM.OBLEFFEJGIJ_8)
		{
			if(EPNFGKBBJCE.NECDFDNBHFK != null)
			{
				...
				int DBNAGGGJDAB = EPNFGKBBJCE.NECDFDNBHFK.LKHAAGIJEPG.DBNAGGGJDAB_CurrentStepIndex;
				MMNNAPPLHFM m = EPNFGKBBJCE.NECDFDNBHFK.BMFEGOMNECF.Find((MMNNAPPLHFM GHPLINIACBB) =>
				{
					//0x100B934
					return GHPLINIACBB.AGBCJMMMLON == DBNAGGGJDAB;
				});
				//LAB_01004cec
				if (DBNAGGGJDAB >= m.LCJPKJMMIAP)
				{
					canRequest = true;
				}
				else
		...
		{
			KBPDNHOKEKD_ProductId k = EPNFGKBBJCE.DBHIEABGKII(BJLONGBNPCI);
			if(k != null)
			{
				if(EPNFGKBBJCE.MEONHEGJNEF())
				{
				...
				if(EPNFGKBBJCE.INDDJNMPONH_Category != GCAHJLOGMCI.KNMMOMEHDON.CCAPCGPIIPF_1)
				{
					if(BJLONGBNPCI < GCAHJLOGMCI.NFCAJPIJFAM.AEFCOHJBLPO_11 && ((1 << (int)BJLONGBNPCI) & 0x660U) != 0) // 0110 0110 0000
					{
						//LAB_01004cec
		...
		if (canRequest)
		{
			...
			if (EPNFGKBBJCE.INDDJNMPONH_Category == GCAHJLOGMCI.KNMMOMEHDON.BCBJMKDAAKA_8)
			{
				OMOEKOCNICP = new IAPIDHGIEED();
				OMOEKOCNICP.ODDIHGPONFL(EPNFGKBBJCE.NECDFDNBHFK.LKHAAGIJEPG);
			}
			KONHMOLMOCI = true;
	...
	private IEnumerator GPDPDNJMDNJ_Coroutine_DrawLotByPaidVC_AfterSave(LOBDIAABMKG EPNFGKBBJCE, GCAHJLOGMCI.NFCAJPIJFAM BJLONGBNPCI, List<ANGEDJODMKO> DMOHAHJDPAD, CDOPFBOHDEF BHFHGFKBOHH, DJBHIFLHJLK JGKOLBLPMPG, IAPIDHGIEED OMOEKOCNICP, IAPIDHGIEED ONNIHHLHLDP)
	{
		//0x100ECB0
		yield return null;
		EPNFGKBBJCE.DBHIEABGKII(BJLONGBNPCI);
		KBPDNHOKEKD_ProductId MEANCEOIMGE = EPNFGKBBJCE.DBHIEABGKII(BJLONGBNPCI);
		...
		if(EPNFGKBBJCE.INDDJNMPONH_Category == GCAHJLOGMCI.KNMMOMEHDON.BCBJMKDAAKA_8)
		{
			MMNNAPPLHFM m = EPNFGKBBJCE.NECDFDNBHFK.BMFEGOMNECF[OMOEKOCNICP.DBNAGGGJDAB_CurrentStepIndex];
			ILCCJNDFFOB.HHCJCDFCLOB.JNBKOAKJDAL(EPNFGKBBJCE.FDEBLMKEMLF_TypeAndSeriesId, l2.Count, 1, m.LCJPKJMMIAP, l2, AHEFHIMGIBI_ServerSave, OMOEKOCNICP, ONNIHHLHLDP, null);
		}
		...
		for(int i = 0; i < l2.Count; i++)
		{
			...
			if (!JANMJPOKLFL.HMDCGPGGOBA(AHEFHIMGIBI_ServerSave, EKLNMHFCAOI.BKHFLDMOGBD_GetItemCategory(l2[i].JJBGOIMEIPF_ItemFullId), EKLNMHFCAOI.DEACAHNLMNI_getItemId(l2[i].JJBGOIMEIPF_ItemFullId), l2[i].MBJIFDBEDAC_ItemCount))
			{
				l.Add(l2[i].MDPJFPHOPCH_Id);
				JANMJPOKLFL.CPIICACGNBH(AHEFHIMGIBI_ServerSave, l2[i].JJBGOIMEIPF_ItemFullId, l2[i].MBJIFDBEDAC_ItemCount, null, 0);
				if(cat == EKLNMHFCAOI.FKGCBLHOOCL_Category.MHKFDBLMOGF_Scene)
				{
					MMPBPOIFDAF_Scene.PMKOFEIONEG saveScene = AHEFHIMGIBI_ServerSave.PNLOINMCCKH_Scene.OPIBAPEGCLA[EKLNMHFCAOI.DEACAHNLMNI_getItemId(l2[i].JJBGOIMEIPF_ItemFullId)];
				...
				if(BLCDJDJJBHC)
				{
					...
					if(f == null)
					{
						...
						if(!IAHLNPMFJMH)
						{
							...
							else if(EPNFGKBBJCE.INDDJNMPONH_Category == GCAHJLOGMCI.KNMMOMEHDON.DLOPEFGOAPD_10)
							{
								...
								if (EKLNMHFCAOI.ALHCGDMEMID_GetNumItems(IMMAOANGPNK.HHCJCDFCLOB.NKEBMCIMJND_Database, CIOECGOMILE.HHCJCDFCLOB.AHEFHIMGIBI_ServerSave, EKLNMHFCAOI.BKHFLDMOGBD_GetItemCategory(EPNFGKBBJCE.MJNOAMAFNHA), EKLNMHFCAOI.DEACAHNLMNI_getItemId(EPNFGKBBJCE.MJNOAMAFNHA), null) < GachaUtility.GetMenuPrice(GachaUtility.selectCategory, GachaUtility.selectedCountType, GachaUtility.selectedLotType))
								{
									//LAB_01005aac
									HDFGHFOCHKE();
									return;
							...
							else if(EPNFGKBBJCE.INDDJNMPONH_Category == GCAHJLOGMCI.KNMMOMEHDON.GKDFKDLFNAJ_5)
							{
								//LAB_01005aac
								HDFGHFOCHKE();
								return;
							}
							else if(EPNFGKBBJCE.INDDJNMPONH_Category == GCAHJLOGMCI.KNMMOMEHDON.BKNHBNINDOC_6)
							{
								//LAB_01005aac
								HDFGHFOCHKE();
								return;
	...
	private IEnumerator GEOMKFMLDPL_Coroutine_DrawLotByFreeVC_AfterSave(LOBDIAABMKG EPNFGKBBJCE, GCAHJLOGMCI.NFCAJPIJFAM BJLONGBNPCI, List<MFDJIFIIPJD> BODJIDOHAHF, CDOPFBOHDEF BHFHGFKBOHH, DJBHIFLHJLK JGKOLBLPMPG, bool IAHLNPMFJMH)
	{
		//0x100D968
		yield return null;
		KBPDNHOKEKD_ProductId p = EPNFGKBBJCE.DBHIEABGKII(BJLONGBNPCI);
		long t = NKGJPJPHLIF.HHCJCDFCLOB.IBLPICFDGOF_ServerRequester.FJDBNGEPKHL.KMEFBNBFJHI_GetServerTime();
		if(EPNFGKBBJCE.INDDJNMPONH_Category == GCAHJLOGMCI.KNMMOMEHDON.DLOPEFGOAPD_10)
		{
			...
			EKLNMHFCAOI.DPHGFMEPOCA_SetNumItems(IMMAOANGPNK.HHCJCDFCLOB.NKEBMCIMJND_Database, CIOECGOMILE.HHCJCDFCLOB.AHEFHIMGIBI_ServerSave, EKLNMHFCAOI.BKHFLDMOGBD_GetItemCategory(EPNFGKBBJCE.MJNOAMAFNHA), EKLNMHFCAOI.DEACAHNLMNI_getItemId(EPNFGKBBJCE.MJNOAMAFNHA), EKLNMHFCAOI.ALHCGDMEMID_GetNumItems(IMMAOANGPNK.HHCJCDFCLOB.NKEBMCIMJND_Database, CIOECGOMILE.HHCJCDFCLOB.AHEFHIMGIBI_ServerSave, EKLNMHFCAOI.BKHFLDMOGBD_GetItemCategory(EPNFGKBBJCE.MJNOAMAFNHA), EKLNMHFCAOI.DEACAHNLMNI_getItemId(EPNFGKBBJCE.MJNOAMAFNHA), null) - GachaUtility.GetMenuPrice(GachaUtility.selectCategory, GachaUtility.selectedCountType, GachaUtility.selectedLotType), null);
		}
		else if(EPNFGKBBJCE.INDDJNMPONH_Category == GCAHJLOGMCI.KNMMOMEHDON.CCAPCGPIIPF_1)
		{
		...
		JANMJPOKLFL.FEGDNPIEKJC(OAGBCBBHMPF.COIIJOEKBDH.DOEHLCLBCNN_3, EPNFGKBBJCE.FDEBLMKEMLF_TypeAndSeriesId.ToString());
		ILCCJNDFFOB.HHCJCDFCLOB.JNBKOAKJDAL(EPNFGKBBJCE.FDEBLMKEMLF_TypeAndSeriesId, BODJIDOHAHF.Count, EPNFGKBBJCE.INDDJNMPONH_Category == GCAHJLOGMCI.KNMMOMEHDON.DLOPEFGOAPD_10 ? 4 : 3, BODJIDOHAHF.Count * p.NPPGKNGIFGK_Price, BODJIDOHAHF, AHEFHIMGIBI_ServerSave, null, null, null);
		JDDGPJDKHNE.HHCJCDFCLOB.NFNLGGHMEAM();
		JDDGPJDKHNE.HHCJCDFCLOB.FCMCNIMEAEA = true;
		bool b1 = false;
		for(int i = 0; i < BODJIDOHAHF.Count; i++)
		{
			if(EKLNMHFCAOI.BKHFLDMOGBD_GetItemCategory(BODJIDOHAHF[i].JJBGOIMEIPF_ItemFullId) == EKLNMHFCAOI.FKGCBLHOOCL_Category.MHKFDBLMOGF_Scene)
			{
			...
			JANMJPOKLFL.IKBLCEFCGDE = 0;
			JANMJPOKLFL.PJBJCBEMEEC = 0;
			if (!JANMJPOKLFL.HMDCGPGGOBA(AHEFHIMGIBI_ServerSave, EKLNMHFCAOI.BKHFLDMOGBD_GetItemCategory(BODJIDOHAHF[i].JJBGOIMEIPF_ItemFullId), EKLNMHFCAOI.DEACAHNLMNI_getItemId(BODJIDOHAHF[i].JJBGOIMEIPF_ItemFullId), BODJIDOHAHF[i].MBJIFDBEDAC_Cnt))
			{
				JANMJPOKLFL.CPIICACGNBH(AHEFHIMGIBI_ServerSave, BODJIDOHAHF[i].JJBGOIMEIPF_ItemFullId, BODJIDOHAHF[i].MBJIFDBEDAC_Cnt, null, 0);
				if(EKLNMHFCAOI.BKHFLDMOGBD_GetItemCategory(BODJIDOHAHF[i].JJBGOIMEIPF_ItemFullId) == EKLNMHFCAOI.FKGCBLHOOCL_Category.MHKFDBLMOGF_Scene)
				{
