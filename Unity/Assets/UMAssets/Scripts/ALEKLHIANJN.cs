...
public class ALEKLHIANJN
{
	...
	private int CEJCMBICONL(int KIJAPOFAGPN, int HMFFHLPNMPH)
	{
		...
		if(cat == EKLNMHFCAOI.FKGCBLHOOCL_Category.MABCLBNIOFA_ValkyrieItem)
		{
			...
			if(dbItem.PPFNGGCBJKC_Id != id)
				return -1;
			return HMFFHLPNMPH <= CIOECGOMILE.HHCJCDFCLOB.AHEFHIMGIBI_PlayerData.KCCLEHLLOFG_Common.MJAIFKFOPPI_ValItem[id - 1].BFINGCJHOHI_Cnt ? 1 : 0;
		}
		else if(cat == EKLNMHFCAOI.FKGCBLHOOCL_Category.HLCHKCJLEGK_GrowItem)
		{
			if(IMMAOANGPNK.HHCJCDFCLOB.NKEBMCIMJND_Database.NKDGLGCAPEI_GrowItem.CDENCMNHNGA_GrowItemList[id - 1].PPFNGGCBJKC != id)
				return -1;
			return HMFFHLPNMPH <= CIOECGOMILE.HHCJCDFCLOB.AHEFHIMGIBI_PlayerData.KCCLEHLLOFG_Common.KBMDMEEMGLK_GrowItem[id - 1].BFINGCJHOHI_Cnt ? 1 : 0;
		}
	...
	{
		return CIOECGOMILE.HHCJCDFCLOB.AHEFHIMGIBI_PlayerData.JJFFBDLIOCF_Valkyrie.OEMMJCLJMGB_GetLevel(GPPEFLKGGGJ_ValkyrieId) + 1 == CIEOBFIIPLD;
	}
