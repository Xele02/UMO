using System;
using System.Collections.Generic;
using System.Text.RegularExpressions;

[System.Obsolete("Use DFHCLGPLJCP_Rules", true)]
public class DFHCLGPLJCP { }
public class DFHCLGPLJCP_Rules
{
	public Regex FPEEJDLMOJB_FileRegex { get; private set; } // 0x8 KGCPPEAFJFC_bgs JNDAHPAODAB_bgs EAFPHJKJHPD_bgs
	public string IOMHINDAFOH_File { get; private set; } // 0xC HFKJBIDDJHL_bgs EIAEEOGCIMA_bgs MMFKGDDAIDG_bgs
	public int KPNKPGLPDHI_Op { get; private set; } // 0x10 OLDMILDBDME_bgs EDAJJEJELHN_bgs HLIEKAOJIGI_bgs
	public int PAAPNEMBHGN_Day { get; private set; } // 0x14 BFBNPHPDOCP_bgs LGLMKGLDFME_bgs BGNMBMCHOCF_bgs
	public bool EGELJDAGHPP { get { return (KPNKPGLPDHI_Op & 1) != 0; } } //0x197E46C ELPABIFMEMH_bgs
	public bool AKHBHEFFNJC { get { return ((KPNKPGLPDHI_Op >> 1) & 1) != 0; } } //0x197E478 BMDCOINKDMC_bgs
	public bool OHIOIDCLIEO { get { return ((KPNKPGLPDHI_Op >> 2) & 1) != 0; } } //0x197E484 BIGDBDKMGED_bgs
	public bool BMEHOMDBEIH { get { return ((KPNKPGLPDHI_Op >> 3) & 1) != 0; } } //0x197E490 IMICNGNNIME_bgs
	public bool OEACGDAKMMB { get { return ((KPNKPGLPDHI_Op >> 4) & 1) != 0; } } //0x197E49C NFPLGHKIAFI_bgs
	public bool BEIECJDELHK_WavFlag { get { return ((KPNKPGLPDHI_Op >> 5) & 1) != 0; } } //0x197E4A8 JNAPPKBMEHJ_bgs
	public bool PEOIMDCECDL { get { return ((KPNKPGLPDHI_Op >> 6) & 1) != 0; } } //0x197E4B4 ANNFICFGPIK_bgs
	public bool EELHAIGHFAC_EventFlag { get { return ((KPNKPGLPDHI_Op >> 7) & 1) != 0; } } //0x197E4C0 MBPJOOPLGBK_bgs
	public bool NGPLFEMNNPP_GachaFlag { get { return ((KPNKPGLPDHI_Op >> 8) & 1) != 0; } } //0x197E4CC NHIFINDFAJF_bgs
	public bool NIPNNJBPBFL_PickupFlag { get { return ((KPNKPGLPDHI_Op >> 9) & 1) != 0; } } //0x197E4D8 NNECCEFDGMB_bgs
	public bool ALKDIKCBKDC_ItemFlag { get { return ((KPNKPGLPDHI_Op >> 10) & 1) != 0; } } //0x197E4E4 IAHFHFALMDB_bgs
	public bool ABKMOFPEHEF { get { return ((KPNKPGLPDHI_Op >> 11) & 1) != 0; } } //0x197E4F0 DHMLEGIECEC_bgs
	public bool PNKPHBHMLNM { get { return ((KPNKPGLPDHI_Op >> 12) & 1) != 0; } } //0x197E4FC COJEDBKBBHK_bgs

	// // RVA: 0x197E508 Offset: 0x197E508 VA: 0x197E508
	// public uint CAOGDCBPBAN() { }

	// // RVA: 0x197E550 Offset: 0x197E550 VA: 0x197E550
	// public void KHEKNNFCAOI_Init(EDOHBJAPLPF_JsonData _IDLHJIOMJBK_data) { }
	//IOMHINDAFOH_File = MIMGPBHAJCO_file
	//PAAPNEMBHGN_Day = BAOFEFFADPD_day
	//KPNKPGLPDHI_Op = BEPIOAIFCEC_op

	// // RVA: 0x197E6FC Offset: 0x197E6FC VA: 0x197E6FC
	public void KHEKNNFCAOI_Init(BJALOLHCBKC _NKGJOAEDCPH_rule)
	{
		IOMHINDAFOH_File = _NKGJOAEDCPH_rule.MIMGPBHAJCO_file;
		PAAPNEMBHGN_Day = (int)_NKGJOAEDCPH_rule.BAOFEFFADPD_day;
		KPNKPGLPDHI_Op = (int)_NKGJOAEDCPH_rule.BEPIOAIFCEC_op;
		FPEEJDLMOJB_FileRegex = new Regex(IOMHINDAFOH_File);
	}
}

[System.Obsolete("Use NLEFIGMHGCO_MVerList", true)]
public class NLEFIGMHGCO { }
public class NLEFIGMHGCO_MVerList
{
	public string MIMGPBHAJCO_file { get; private set; } // 0x8 ICEMDJBNGJA_bgs JPMHJMDLFPI_get_file NLHKGLABKDI_set_file
	public int IJEKNCDIIAE_mver { get; private set; } // 0xC FAHNCMHNFCG_bgs KJIMMIBDCIL_get_mver DMEGNOKIKCD_set_mver

	// // RVA: 0xC2E2D8 Offset: 0xC2E2D8 VA: 0xC2E2D8
	// public uint CAOGDCBPBAN() { }

	// // RVA: 0xC2E318 Offset: 0xC2E318 VA: 0xC2E318
	// public void KHEKNNFCAOI_Init(EDOHBJAPLPF_JsonData _IDLHJIOMJBK_data) { }
	//MIMGPBHAJCO_file = MIMGPBHAJCO_file
	//IJEKNCDIIAE_mver = IJEKNCDIIAE_mver

	// // RVA: 0xC2E454 Offset: 0xC2E454 VA: 0xC2E454
	public void KHEKNNFCAOI_Init(PFCNGLGHLJA _IDLHJIOMJBK_data)
	{
		MIMGPBHAJCO_file = _IDLHJIOMJBK_data.MIMGPBHAJCO_file;
		IJEKNCDIIAE_mver = _IDLHJIOMJBK_data.IJEKNCDIIAE_mver;
	}
}

public class PKKHIEAEDPC
{
	public DFHCLGPLJCP_Rules NKGJOAEDCPH_rule; // 0x8
	public Match MABBBOEAPAA_p; // 0xC
	public int KKPAHLMJKIH_WavId; // 0x10
	public int EKANGPODCEP_EventId; // 0x14
	public int HHGMPEEGFMA_GachaId; // 0x18
	public int DIDOFBIHPMB_PickupId; // 0x1C
	public int INFIBMLIHLO_ItemId; // 0x20
	public string CJEKGLGBIHF_path; // 0x24
}

[System.Obsolete("Use LFPJCEMANCK_Asset", true)]
public class LFPJCEMANCK { }
public class LFPJCEMANCK_Asset : DIHHCBACKGG_DbSection
{
	public int[] OEEJIIIICCP; // 0x28
	public Dictionary<string, int> AJHAKFLPNHF = new Dictionary<string, int>(); // 0x2C
	public Dictionary<string, int> ALMGDPAGHMJ = new Dictionary<string, int>(); // 0x30

	public List<DFHCLGPLJCP_Rules> KCMFDGNNPIL_Rules { get; private set; } // 0x20 LOJMMLKKEEP_bgs JGCLPBFPPHI_bgs OBABGCHHDDH_bgs
	public List<NLEFIGMHGCO_MVerList> OAOOIDNBOGA_MVer_List { get; private set; } // 0x24 NPFKICOMNGC_bgs LKMOIDPNAPB_bgs JHMPDPDLAIN_bgs

	// // RVA: 0xD6D49C Offset: 0xD6D49C VA: 0xD6D49C
	public PKKHIEAEDPC NBHDIKJMLEN(string _CJEKGLGBIHF_path)
    {
		if(KCMFDGNNPIL_Rules != null && KCMFDGNNPIL_Rules.Count > 0)
		{
			for(int i = 0; i < KCMFDGNNPIL_Rules.Count; i++)
			{
				Match match = KCMFDGNNPIL_Rules[i].FPEEJDLMOJB_FileRegex.Match(_CJEKGLGBIHF_path);
				if (match.Success)
				{
					PKKHIEAEDPC data = new PKKHIEAEDPC();
					data.NKGJOAEDCPH_rule = KCMFDGNNPIL_Rules[i];
					data.CJEKGLGBIHF_path = _CJEKGLGBIHF_path;
					data.MABBBOEAPAA_p = match;
					if(match.Groups.Count > 0)
					{
						if(match.Groups[AFEHLCGHAEE_Strings.IKOFFIOMOME_wav/*wav*/].Value != "")
						{
							int wavId = 0;
							if(Int32.TryParse(match.Groups[AFEHLCGHAEE_Strings.IKOFFIOMOME_wav/*wav*/].Value, out wavId))
							{
								data.KKPAHLMJKIH_WavId = wavId;
							}
						}
						if(match.Groups[AFEHLCGHAEE_Strings.JJCJCOANHBL_event/*event*/].Value != "")
						{
							int eventId = 0;
							if(Int32.TryParse(match.Groups[AFEHLCGHAEE_Strings.JJCJCOANHBL_event/*event*/].Value, out eventId))
							{
								data.EKANGPODCEP_EventId = eventId;
							}
						}
						if(match.Groups["gacha"].Value != "")
						{
							int gachaId = 0;
							if(Int32.TryParse(match.Groups["gacha"].Value, out gachaId))
							{
								data.HHGMPEEGFMA_GachaId = gachaId;
							}
						}
						if(match.Groups["pickup"].Value != "")
						{
							int pickupId = 0;
							if (Int32.TryParse(match.Groups["pickup"].Value, out pickupId))
							{
								data.DIDOFBIHPMB_PickupId = pickupId;
							}
						}
						if (match.Groups["itemid"].Value != "")
						{
							int itemId = 0;
							if (match.Groups["itemid"].Value.Contains("40000_"))
								data.INFIBMLIHLO_ItemId = 40000;
							else if (Int32.TryParse(match.Groups["itemid"].Value, out itemId))
							{
								data.INFIBMLIHLO_ItemId = itemId;
							}
						}
					}
					if (!data.NKGJOAEDCPH_rule.PEOIMDCECDL)
						return data;
					if (OEEJIIIICCP == null)
						return data;
					for(int j = 0; j < OEEJIIIICCP.Length; j++)
					{
						if (OEEJIIIICCP[j] == data.KKPAHLMJKIH_WavId)
							return data;
					}
				}
			}
		}
        return null;
    }

	// // RVA: 0xD6E12C Offset: 0xD6E12C VA: 0xD6E12C
	public int MCHKDCGEAOB()
	{
		for(int i = 0; i < KCMFDGNNPIL_Rules.Count; i++)
		{
			if(KCMFDGNNPIL_Rules[i].PEOIMDCECDL)
			{
				return KCMFDGNNPIL_Rules[i].PAAPNEMBHGN_Day;
			}
		}
		return 1;
	}

	// RVA: 0xD6E270 Offset: 0xD6E270 VA: 0xD6E270
	public LFPJCEMANCK_Asset()
    {
        JIKKNHIAEKG_BlockName = "";
        LNIMEIMBCMF = false;
        KCMFDGNNPIL_Rules = new List<DFHCLGPLJCP_Rules>(100);
        LMHMIIKCGPE = 5;
        OAOOIDNBOGA_MVer_List = new List<NLEFIGMHGCO_MVerList>();
    }

	// // RVA: 0xD6E3E8 Offset: 0xD6E3E8 VA: 0xD6E3E8 Slot: 8
	protected override void KMBPACJNEOF_Reset()
    {
		KCMFDGNNPIL_Rules.Clear();
		OAOOIDNBOGA_MVer_List.Clear();
		AJHAKFLPNHF.Clear();
		ALMGDPAGHMJ.Clear();
	}

	// // RVA: 0xD6E4E4 Offset: 0xD6E4E4 VA: 0xD6E4E4 Slot: 9
	public override bool IIEMACPEEBJ_Deserialize(byte[] _DBBGALAPFGC_bytes)
    {
		CPHIOGHFPKN parser = CPHIOGHFPKN.HEGEKFMJNCC(_DBBGALAPFGC_bytes);
		NNDMLKDMALA(parser);
		MCIFBBDDPID(parser);
		OEEJIIIICCP = parser.KLDHBJPDAFG;
		{
			string[] array = parser.DNGKHGLIGKJ;
			for(int i = 0; i < array.Length; i++)
			{
				if(array[i][0] == '/')
				{
					AJHAKFLPNHF.Add("/android"+ array[i], i);
				}
				else
				{
					AJHAKFLPNHF.Add("/android" + "/" + array[i], i);
				}
			}
		}
		{
			string[] array = parser.AMBODKHPHFH;
			for (int i = 0; i < array.Length; i++)
			{
				if (array[i][0] == '/')
				{
					ALMGDPAGHMJ.Add("/android" + array[i], i);
				}
				else
				{
					ALMGDPAGHMJ.Add("/android" + "/" + array[i], i);
				}
			}
		}
        return true;
    }

	// // RVA: 0xD6EB28 Offset: 0xD6EB28 VA: 0xD6EB28 Slot: 10
	public override bool IIEMACPEEBJ_Deserialize(EDOHBJAPLPF_JsonData OILEIIEIBHP, int _KAPMOPMDHJE_label)
    {
        TodoLogger.LogError(TodoLogger.DbJson, "TODO");
        return true;
    }

	// // RVA: 0xD6EB58 Offset: 0xD6EB58 VA: 0xD6EB58
	// private bool NNDMLKDMALA(EDOHBJAPLPF_JsonData OILEIIEIBHP, int _KAPMOPMDHJE_label) { }
	//KCMFDGNNPIL_Rules = NKGJOAEDCPH_rule

	// // RVA: 0xD6E848 Offset: 0xD6E848 VA: 0xD6E848
	private bool NNDMLKDMALA(CPHIOGHFPKN COBJEGDKDJA)
	{
		{
			BJALOLHCBKC[] array = COBJEGDKDJA.IPHMDEOOFAI;
			for(int i = 0; i < array.Length; i++)
			{
				DFHCLGPLJCP_Rules data = new DFHCLGPLJCP_Rules();
				data.KHEKNNFCAOI_Init(array[i]);
				KCMFDGNNPIL_Rules.Add(data);
			}
		}
		return true;
	}

	// // RVA: 0xD6ED84 Offset: 0xD6ED84 VA: 0xD6ED84
	// private bool MCIFBBDDPID(EDOHBJAPLPF_JsonData OILEIIEIBHP, int _KAPMOPMDHJE_label) { }
	//OAOOIDNBOGA_MVer_List = ECHKEPABLIP_mver_list

	// // RVA: 0xD6E9B8 Offset: 0xD6E9B8 VA: 0xD6E9B8
	private bool MCIFBBDDPID(CPHIOGHFPKN COBJEGDKDJA)
	{
		PFCNGLGHLJA[] array = COBJEGDKDJA.DOGNLEPGCIK;
		for(int i = 0; i < array.Length; i++)
		{
			NLEFIGMHGCO_MVerList data = new NLEFIGMHGCO_MVerList();
			data.KHEKNNFCAOI_Init(array[i]);
			OAOOIDNBOGA_MVer_List.Add(data);
		}
		return true;
	}

	// // RVA: 0xD6EFB0 Offset: 0xD6EFB0 VA: 0xD6EFB0 Slot: 11
	public override uint CAOGDCBPBAN()
    {
        TodoLogger.LogError(TodoLogger.DbIntegrityCheck, "DFHCLGPLJCP_Rules.CAOGDCBPBAN");
        return 0;
    }
}
