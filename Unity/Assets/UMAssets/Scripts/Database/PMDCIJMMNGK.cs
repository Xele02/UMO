...
public class PMDCIJMMNGK_GachaTicket : DIHHCBACKGG_DbSection
{
	...
	[UMOMember()]
	public List<int> DHIACJMOEBH_vcIdsList = new List<int>(); // 0x24

	// RVA: 0xFEC644 Offset: 0xFEC644 VA: 0xFEC644
	public EJAKHFONNGN ACEHGKOLBCG(int _KIJAPOFAGPN_ItemId)
	{
		EKLNMHFCAOI_ItemManager.FKGCBLHOOCL_Category itemCat = EKLNMHFCAOI_ItemManager.BKHFLDMOGBD_GetItemCategory(_KIJAPOFAGPN_ItemId);
		...
		for(int i = 0; i < array.Length; i++)
		{
			...
			if(array[i].IJEKNCDIIAE_mver <= IEFOPDOOLOK_MasterVersion)
			{
				if(array[i].PLALNIIBLOF_en > 1)
				{
					...
					DHIACJMOEBH_vcIdsList.Add(data.GJDNBENICPF_vcid);
				}
			}
		}
		return true;
