...
public class DHOJHGODBAB_Quest : DIHHCBACKGG_DbSection
{
	...
	private bool EELEJILKBIM_LoadDailyQuest(GEDOHHLGKCG ENCJBKPHGAL)
	{
		...
		if (array.Length < 21)
		{
			for (int i = 0; i < array.Length; i++)
			{
				...
				data.INDDJNMPONH = (int)array[i].MAPNDFCJFLJ;
				data.CHOFDPDFPDC = (int)array[i].JBFLEDKDFCO;
				data.FCDKJAKLGMB = (int)array[i].PMDLBHLNIDP;
				data.AKBHPFBDDOL = (int)array[i].JIMJHIDEHNM;
				data.EIHOBHDKCDB = (int)array[i].JGOHPDKCJKB;
				data.HHIBBHFHENH = 0;
	...
	private bool GBNIHKOIMFO_LoadNormalQuest(GEDOHHLGKCG ENCJBKPHGAL)
	{
		...
		if (array.Length < 1701)
		{
			for (int i = 0; i < array.Length; i++)
			{
				...
				if(JKAECBCNHAN_IsEnabled((int)array[i].IJEKNCDIIAE, data.INDDJNMPONH != 0 ? 2 : data.INDDJNMPONH, 0) != 2)
				{
					data.INDDJNMPONH = 0;
				}
			}
	...
	private bool IHJNKFOJPKM(GEDOHHLGKCG ENCJBKPHGAL)
	{
		...
		for(int i = 0; i < array.Length; i++)
		{
			...
			data.PPFNGGCBJKC = (int)array[i].PPFNGGCBJKC;
			data.GLCLFMGPMAN = (int)array[i].CLDDAEMKHOG;
			data.HMFFHLPNMPH = (int)array[i].PIMBMBIICMK;
			data.BGFPPGPJONG = (int)array[i].LJNAKDMILMC;
			data.APNMKLJMPMD = (int)array[i].OILNNGPCLGD;
...
public class CNLPPCFJEID_QuestInfo
{
	...
	public int INDDJNMPONH { get { return MKENMKMJFKP ^ FBGGEFFJJHB; } set { MKENMKMJFKP = value ^ FBGGEFFJJHB; } } //0x175CDA8 GHAILOLPHPF 0x175CCE4 BACGOKIGMBC
	public int CHOFDPDFPDC { get { return ICLBAMBDKNI ^ FBGGEFFJJHB; } set { ICLBAMBDKNI = value ^ FBGGEFFJJHB; } } //0x175CDB8 IBCDKHDLOKG 0x175CCF4 ICHJGBKDCGM
	...
	public int HHIBBHFHENH { get { return IOAGHJGBNLC ^ FBGGEFFJJHB; } set { IOAGHJGBNLC = value ^ FBGGEFFJJHB; } } //0x175CDE8 MEHBABCCHOO 0x175CD14 FBCMDNCLBDC
	...
	public int AKBHPFBDDOL { get { return APJGMOJDPAK ^ FBGGEFFJJHB; } set { APJGMOJDPAK = value ^ FBGGEFFJJHB; } } //0x175CE08 ANBHKALCHDJ 0x175CD24 GDANPNDMDCC
	public int EIHOBHDKCDB { get { return FKKHMCINELD ^ FBGGEFFJJHB; } set { FKKHMCINELD = value ^ FBGGEFFJJHB; } } //0x175CE18 EALKEGPNHGK 0x175CD04 LNFEIPOKKNG
...
public class LCKMNLOLDPD
{
	...
	public int GLCLFMGPMAN { get { return AHGCGHAAHOO ^ FBGGEFFJJHB; } set { AHGCGHAAHOO = value ^ FBGGEFFJJHB; } } //0xD9A99C LNJAENEGDEL 0xD9A9AC JHIDBGCHOKL
	public int HMFFHLPNMPH { get { return HLMAFFLCCKD ^ FBGGEFFJJHB; } set { HLMAFFLCCKD = value ^ FBGGEFFJJHB; } } //0xD9A9BC NJOGDDPICKG 0xD9A9CC NBBGMMBICNA
	public int BGFPPGPJONG { get { return IPFBMBMNAGL ^ FBGGEFFJJHB; } set { IPFBMBMNAGL = value ^ FBGGEFFJJHB; } } //0xD9A9DC LBMNPGFFCJN 0xD9A9EC NDNCLLKIJHA
	public int APNMKLJMPMD { get { return OHDDANALELB ^ FBGGEFFJJHB; } set { OHDDANALELB = value ^ FBGGEFFJJHB; } } //0xD9A9FC ICCFOKKODOH 0xD9AA0C CBPDLNBEJAE
