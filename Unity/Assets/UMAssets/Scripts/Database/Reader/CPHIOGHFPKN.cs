...
{
	public string MIMGPBHAJCO_file { get; set; } // 0x8 ICEMDJBNGJA JPMHJMDLFPI NLHKGLABKDI
	public uint BAOFEFFADPD_day { get; set; } // 0xC MNBJKHKHMCL KHNONKOPIPO LBJPPBMDOGE
	public uint BEPIOAIFCEC_op { get; set; } // 0x10 OHIHJLAHFOF GDBDINJGOKM JJHKMPPEBFG
}
...
public class CPHIOGHFPKN
{
	...
	public static CPHIOGHFPKN HEGEKFMJNCC(byte[] NIODCJLINJN)// 0x176634C
	{
		...
		for(int IPHMDEOOFAI_idx = 0; IPHMDEOOFAI_idx < res_readData.CMNLDHJPMPHLength; IPHMDEOOFAI_idx++)
		{
			...
			IPHMDEOOFAI_data.MIMGPBHAJCO_file = IPHMDEOOFAI_readData.DCEDMFDNAKH;
			IPHMDEOOFAI_data.BAOFEFFADPD_day = IPHMDEOOFAI_readData.FKGLOPMFMCP;
			IPHMDEOOFAI_data.BEPIOAIFCEC_op = IPHMDEOOFAI_readData.IAPOHBCMPPB;
		...
		for(int DOGNLEPGCIK_idx = 0; DOGNLEPGCIK_idx < res_readData.GBHCJIPEFDFLength; DOGNLEPGCIK_idx++)
		{
			...
			DOGNLEPGCIK_data.MIMGPBHAJCO_file = DOGNLEPGCIK_readData.DCEDMFDNAKH;
			DOGNLEPGCIK_data.IJEKNCDIIAE_mver = DOGNLEPGCIK_readData.OFMGALJGDAO;
