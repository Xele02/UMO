
using System.Collections.Generic;
using XeSys;

[System.Obsolete("Use LMBBEGIAKAD_EventQuest", true)]
public class LMBBEGIAKAD { }
[UMOClass()]
public class LMBBEGIAKAD_EventQuest : DIHHCBACKGG_DbSection
{
	[UMOClass()]
	public class BILPFCFHJLE
	{
		[UMOMember()]
		public int OBGBAOLONDD_UniqueId; // 0x8
		[UMOMember()]
		public string OPFGFINHFCE_name; // 0xC
		[UMOMember()]
		public string HEDAGCNPHGD_RankingName; // 0x10
		[UMOMember()]
		public string OCGFKMHNEOF_name_for_api; // 0x14
		[UMOMember()]
		public string FEMMDNIELFC_Desc; // 0x18
		[UMOMember()]
		public long BONDDBOFBND_RankingStart; // 0x20
		[UMOMember()]
		public long HPNOGLIFJOP_RankingEnd; // 0x28
		[UMOMember()]
		public long EHHFFKAFOMC; // 0x30
		[UMOMember()]
		public long LNFKGHNHJKE_RankingEnd2; // 0x38
		[UMOMember()]
		public long JGMDAOACOJF_RewardStart; // 0x40
		[UMOMember()]
		public long IDDBFFBPNGI_RewardEnd; // 0x48
		[UMOMember()]
		public long KNLGKBBIBOH_RewardEnd2; // 0x50
		[UMOMember()]
		public int KHIKEGLBGAF_RankingRewardScene; // 0x58
		[UMOMember()]
		public int DPJCFLKFEPN_RankingRewardScene2; // 0x5C
		[UMOMember()]
		public int MJBKGOJBPAD_item_type; // 0x60
		[UMOMember()]
		public int JKFPADIAKCK_Songs; // 0x64
		[UMOMember()]
		public int EKJFNHHKBPL; // 0x68
		[UMOMember()]
		public sbyte POGEFBMBPCB_MonthOdd; // 0x6C
		[UMOMember()]
		public sbyte AHKNMANFILO_DayGroup; // 0x6D
		[UMOMember()]
		public sbyte MOEKELIIDEO_SaveIdx; // 0x6E
		[UMOMember()]
		public sbyte HKKNEAGCIEB_RankingSupport; // 0x6F
		[UMOMember()]
		public sbyte AHKPNPNOAMO_ExtreamOpen; // 0x70
		[UMOMember()]
		public string OMCAOJJGOGG_Lb; // 0x74
		[UMOMember()]
		private NNJFKLBPBNK_SecureString EBGIDCIIGDO_KeyPrefix = new NNJFKLBPBNK_SecureString(); // 0x78
		[UMOMember()]
		public List<int> JHPCPNJJHLI_RankingThreshold = new List<int>(); // 0x7C
		[UMOMember()]
		public List<int> EPHAKDAPAHE = new List<int>(); // 0x80
		[UMOMember()]
		public List<int> HIAMHLLHAON_PlayCost = new List<int>(); // 0x84
		[UMOMember()]
		public List<int> BKBEPOFJOJG = new List<int>(); // 0x88
		[UMOMember()]
		public int HIOOGLEJBKM_StartAdventureId; // 0x8C
		[UMOMember()]
		public int FJCADCDNPMP_EndAdventureId; // 0x90
		[UMOMember()]
		public int[] EJBGHLOOLBC_HelpIds; // 0x94
		[UMOMember()]
		public List<int> EEOGPJJCKHH_Drops = new List<int>(); // 0x98
		[UMOMember()]
		public List<int> HEABAENHIDE = new List<int>(); // 0x9C

		public string OCDMGOGMHGE_KeyPrefix { get { return EBGIDCIIGDO_KeyPrefix.DNJEJEANJGL_Value; } set { EBGIDCIIGDO_KeyPrefix.DNJEJEANJGL_Value = value; } } //0x10BB8BC HBAAAKFHDBB_get_KeyPrefix 0x10BA320 NHJLJOIPOFK_set_KeyPrefix

		//// RVA: 0x10B883C Offset: 0x10B883C VA: 0x10B883C
		public void LHPDDGIJKNB_Reset() 
		{
			OBGBAOLONDD_UniqueId = 0;
			BONDDBOFBND_RankingStart = 0;
			HPNOGLIFJOP_RankingEnd = 0;
			JGMDAOACOJF_RewardStart = 0;
			IDDBFFBPNGI_RewardEnd = 0;
			EHHFFKAFOMC = 0;
			KHIKEGLBGAF_RankingRewardScene = 0;
			DPJCFLKFEPN_RankingRewardScene2 = 0;
			MJBKGOJBPAD_item_type = 1;
			POGEFBMBPCB_MonthOdd = 0;
			AHKNMANFILO_DayGroup = 0;
			OPFGFINHFCE_name = null;
			HEDAGCNPHGD_RankingName = null;
			OCGFKMHNEOF_name_for_api = null;
			FEMMDNIELFC_Desc = null;
			OCDMGOGMHGE_KeyPrefix = "";
			EPHAKDAPAHE.Clear();
			HEABAENHIDE.Clear();
			JHPCPNJJHLI_RankingThreshold.Clear();
			HIAMHLLHAON_PlayCost.Clear();
			OMCAOJJGOGG_Lb = "";
			HIOOGLEJBKM_StartAdventureId = 0;
			FJCADCDNPMP_EndAdventureId = 0;
			EJBGHLOOLBC_HelpIds = null;
			AHKPNPNOAMO_ExtreamOpen = 0;
			JKFPADIAKCK_Songs = 0;
			EKJFNHHKBPL = 0;
			EEOGPJJCKHH_Drops.Clear();
		}

		//// RVA: 0x10BB49C Offset: 0x10BB49C VA: 0x10BB49C
		//public uint CAOGDCBPBAN() { }
	}

	[UMOClass()]
	public class HIIIHLEJEGI
	{
		[UMOMember()]
		public int PPFNGGCBJKC_id; // 0x8
		[UMOMember()]
		public int KFCIJBLDHOK_v1; // 0xC
		[UMOMember()]
		public int JLEIHOEGMOP_v2; // 0x10
	}

	[UMOClass()]
	public class PAPFNFFDMHG
	{
		public int FBGGEFFJJHB_xor; // 0x8
		private int EHOIENNDEDH_IdCrypted; // 0xC
		private int FMDDJNKEBBB_Crypted; // 0x10
		private int MDKGAAMBJDJ_Crypted; // 0x14
		private int GGEBNMPPEID_Crypted; // 0x18

		[UMOMember()]
		public int PPFNGGCBJKC_id { get { return EHOIENNDEDH_IdCrypted ^ FBGGEFFJJHB_xor; } set { EHOIENNDEDH_IdCrypted = value ^ FBGGEFFJJHB_xor; } } //0x10BBA20 DEMEPMAEJOO_get_id 0x10BBA30 HIGKAIDMOKN_set_id
		[UMOMember()]
		public int CNKFPJCGNFE_SceneId { get { return FMDDJNKEBBB_Crypted ^ FBGGEFFJJHB_xor; } set { FMDDJNKEBBB_Crypted = value ^ FBGGEFFJJHB_xor; } } //0x10BBA40 GJBOGOFHGNP_get_SceneId 0x10BBA50 GJDGIDCMJMH_set_SceneId
		[UMOMember()]
		public int GNFBMCGMCFO_BonusBefore { get { return MDKGAAMBJDJ_Crypted ^ FBGGEFFJJHB_xor; } set { MDKGAAMBJDJ_Crypted = value ^ FBGGEFFJJHB_xor; } } //0x10BBA60 NCIMMDJLPLJ_get_BonusBefore 0x10BBA70 HNAJAFBHOLM_set_BonusBefore
		[UMOMember()]
		public int BFFGFAMJAIG_BonusAfter { get { return GGEBNMPPEID_Crypted ^ FBGGEFFJJHB_xor; } set { GGEBNMPPEID_Crypted = value ^ FBGGEFFJJHB_xor; } } //0x10BBA80 PKMNOMELPMN_get_BonusAfter 0x10BBA90 IABBJBAHKCE_set_BonusAfter

		//// RVA: 0x10BB794 Offset: 0x10BB794 VA: 0x10BB794
		//public uint CAOGDCBPBAN() { }

		//// RVA: 0x10BA8E8 Offset: 0x10BA8E8 VA: 0x10BA8E8
		//public void BAFFAONJPCE(int JBGJDEELLOP, int KNEFBLHBDBG, HGHOHICGBEP JMHECKKKMLK) { }

		//// RVA: 0x10BBAA0 Offset: 0x10BBAA0 VA: 0x10BBAA0
		//public void LHPDDGIJKNB_Reset(int KNEFBLHBDBG) { }
	}

	[UMOClass()]
	public class OMHPMBCNAPC
	{
		public int FBGGEFFJJHB_xor; // 0x8
		private int EHOIENNDEDH_IdCrypted; // 0xC
		private int MDKGAAMBJDJ_Crypted; // 0x10
		private int GGEBNMPPEID_Crypted; // 0x14

		[UMOMember()]
		public int PPFNGGCBJKC_id { get { return EHOIENNDEDH_IdCrypted ^ FBGGEFFJJHB_xor; } set { EHOIENNDEDH_IdCrypted = value ^ FBGGEFFJJHB_xor; } } //0x10BB9B0 DEMEPMAEJOO_get_id 0x10BB9C0 HIGKAIDMOKN_set_id
		[UMOMember()]
		public int GNFBMCGMCFO_BonusBefore { get { return MDKGAAMBJDJ_Crypted ^ FBGGEFFJJHB_xor; } set { MDKGAAMBJDJ_Crypted = value ^ FBGGEFFJJHB_xor; } } //0x10BB9D0 NCIMMDJLPLJ_get_BonusBefore 0x10BB9E0 HNAJAFBHOLM_set_BonusBefore
		[UMOMember()]
		public int BFFGFAMJAIG_BonusAfter { get { return GGEBNMPPEID_Crypted ^ FBGGEFFJJHB_xor; } set { GGEBNMPPEID_Crypted = value ^ FBGGEFFJJHB_xor; } } //0x10BB9F0 PKMNOMELPMN_get_BonusAfter 0x10BBA00 IABBJBAHKCE_set_BonusAfter

		//// RVA: 0x10BB7C0 Offset: 0x10BB7C0 VA: 0x10BB7C0
		//public uint CAOGDCBPBAN() { }

		//// RVA: 0x10BABB0 Offset: 0x10BABB0 VA: 0x10BABB0
		//public void BAFFAONJPCE(int JBGJDEELLOP, int KNEFBLHBDBG, HGHOHICGBEP JMHECKKKMLK) { }

		//// RVA: 0x10BBA10 Offset: 0x10BBA10 VA: 0x10BBA10
		//public void LHPDDGIJKNB_Reset(int KNEFBLHBDBG) { }
	}

	[UMOClass()]
	public class JGMJOONMPJN
	{
		public int FBGGEFFJJHB_xor; // 0x8
		private int HHEEHHHLKHC_Crypted; // 0xC
		private int LGAIHLFAPDC_Crypted; // 0x10
		[UMOMember()]
		public List<int> KDNMBOBEGJM_GachaIds = new List<int>(); // 0x14

		[UMOMember()]
		public int KHPHAAMGMJP_EpId { get { return HHEEHHHLKHC_Crypted ^ FBGGEFFJJHB_xor; } set { HHEEHHHLKHC_Crypted = value ^ FBGGEFFJJHB_xor; } } //0x10BB8E8 ABFDDKBBPCH_get_EpId 0x10BB8F8 MHDOIIEMDEH_set_EpId
		[UMOMember()]
		public int OFIAENKCJME_BaseBonus { get { return LGAIHLFAPDC_Crypted ^ FBGGEFFJJHB_xor; } set { LGAIHLFAPDC_Crypted = value ^ FBGGEFFJJHB_xor; } } //0x10BB908 KADLAKFANGA_get_BaseBonus 0x10BB918 AIDAPNCEPOB_set_BaseBonus

		//// RVA: 0x10BB7E4 Offset: 0x10BB7E4 VA: 0x10BB7E4
		//public uint CAOGDCBPBAN() { }

		//// RVA: 0x10BAEB8 Offset: 0x10BAEB8 VA: 0x10BAEB8
		//public void BAFFAONJPCE(int JBGJDEELLOP, int KNEFBLHBDBG, HGHOHICGBEP JMHECKKKMLK) { }

		//// RVA: 0x10BB928 Offset: 0x10BB928 VA: 0x10BB928
		//public void LHPDDGIJKNB_Reset(int KNEFBLHBDBG) { }
	}

	[UMOClass()]
	public class FFJHPLIMOLK
	{
		[UMOMember()]
		public int GLCLFMGPMAN_ItemId; // 0x8
		[UMOMember()]
		public int HMFFHLPNMPH_count; // 0xC
		[UMOMember()]
		public int NMKEOMCMIPP_RewardId; // 0x10
	}

	[UMOClass()]
	public class LNGOGDOIJHI
	{
		[UMOMember()]
		public int PPFNGGCBJKC_id; // 0x8
		[UMOMember()]
		public int DNBFMLBNAEE_point; // 0xC
		[UMOMember()]
		public List<FFJHPLIMOLK> AHJNPEAMCCH_rewards = new List<FFJHPLIMOLK>(); // 0x10
		[UMOMember()]
		public bool JOPPFEHKNFO_Pickup; // 0x14

		//// RVA: 0x10BB64C Offset: 0x10BB64C VA: 0x10BB64C
		//public uint CAOGDCBPBAN() { }
	}

	[UMOMember()]
	public BILPFCFHJLE NGHKJOEDLIP_Settings = new BILPFCFHJLE(); // 0x20
	[UMOMember()]
	public List<LNGOGDOIJHI> FCIPEDFHFEM_TotalRewards = new List<LNGOGDOIJHI>(); // 0x24 totalreward?
	[UMOMember()]
	public List<AKIIJBEJOEP> NNMPGOAGEOL_quests = new List<AKIIJBEJOEP>(); // 0x28 // quests
	[UMOMember()]
	public List<PKADMPNDMGP> HKODBHPMLCP_cards = new List<PKADMPNDMGP>(); // 0x2C cards
	[UMOMember()]
	public List<PAPFNFFDMHG> GEGAEDDGNMA_Bonuses = new List<PAPFNFFDMHG>(); // 0x30 //EpBonusByScene
	[UMOMember()]
	public List<OMHPMBCNAPC> OGMHMAGDNAM_EpBonusBySceneRarity = new List<OMHPMBCNAPC>(); // 0x34
	[UMOMember()]
	public List<JGMJOONMPJN> LHAKGDAGEMM_EpBonusInfos = new List<JGMJOONMPJN>(); // 0x38
	[UMOMember()]
	public List<HIIIHLEJEGI> ADPFKHEMNBL = new List<HIIIHLEJEGI>(); // 0x44
	[UMOMember()]
	public List<AIGOEAPJGEB> MBHDIJJEOFL = new List<AIGOEAPJGEB>(); // 0x48

	[UMOMember()]
	public Dictionary<string, NNJFKLBPBNK_SecureString> FJOEBCMGDMI_m_stringParam { get; private set; } // 0x3C IHKPIFIBECO GAMGELHIHHI_get_m_stringParam DDDEJIJGGBJ_set_m_stringParam
	[UMOMember()]
	public Dictionary<string, CEBFFLDKAEC_SecureInt> OHJFBLFELNK_m_intParam { get; private set; } // 0x40 KLDCHOIPJGB AEMNOGNEBOJ_get_m_intParam DGKDBOAMNBB_set_m_intParam

	//// RVA: 0x10B7EB4 Offset: 0x10B7EB4 VA: 0x10B7EB4
	public string EFEGBHACJAL_GetStringParam(string _LJNAKDMILMC_key, string _KKMJBMKHGNH_noval)
	{
		if(FJOEBCMGDMI_m_stringParam.ContainsKey(_LJNAKDMILMC_key))
			return FJOEBCMGDMI_m_stringParam[_LJNAKDMILMC_key].DNJEJEANJGL_Value;
		return _KKMJBMKHGNH_noval;
	}

	//// RVA: 0x10B7F98 Offset: 0x10B7F98 VA: 0x10B7F98
	public int LPJLEHAJADA_GetIntParam(string _LJNAKDMILMC_key, int _KKMJBMKHGNH_noval)
	{
		if(OHJFBLFELNK_m_intParam.ContainsKey(_LJNAKDMILMC_key))
			return OHJFBLFELNK_m_intParam[_LJNAKDMILMC_key].DNJEJEANJGL_Value;
		return _KKMJBMKHGNH_noval;
	}

	//// RVA: 0x10B807C Offset: 0x10B807C VA: 0x10B807C
	public List<int> GEFEGEHCFNE(int _AKNELONELJK_difficulty)
	{
		List<int> res = new List<int>(HKODBHPMLCP_cards.Count);
		for(int i = 0; i < HKODBHPMLCP_cards.Count; i++)
		{
			if(HKODBHPMLCP_cards[i].PPEGAKEIEGM_Enabled == 2)
			{
				if(HKODBHPMLCP_cards[i].DGMIADAEGAI_TargetDifficultyType == _AKNELONELJK_difficulty + 1)
					res.Add(i);
			}
		}
		return res;
	}

	// RVA: 0x10B8264 Offset: 0x10B8264 VA: 0x10B8264
	public LMBBEGIAKAD_EventQuest()
	{
		JIKKNHIAEKG_BlockName = "";
		LNIMEIMBCMF = false;
		OHJFBLFELNK_m_intParam = new Dictionary<string, CEBFFLDKAEC_SecureInt>();
		FJOEBCMGDMI_m_stringParam = new Dictionary<string, NNJFKLBPBNK_SecureString>();
		LMHMIIKCGPE = 37;
	}

	// RVA: 0x10B8674 Offset: 0x10B8674 VA: 0x10B8674 Slot: 8
	protected override void KMBPACJNEOF_Reset()
	{
		NGHKJOEDLIP_Settings.LHPDDGIJKNB_Reset();
		FCIPEDFHFEM_TotalRewards.Clear();
		HKODBHPMLCP_cards.Clear();
		OHJFBLFELNK_m_intParam.Clear();
		FJOEBCMGDMI_m_stringParam.Clear();
		ADPFKHEMNBL.Clear();
		GEGAEDDGNMA_Bonuses.Clear();
		LHAKGDAGEMM_EpBonusInfos.Clear();
		OGMHMAGDNAM_EpBonusBySceneRarity.Clear();
	}

	// RVA: 0x10B89F0 Offset: 0x10B89F0 VA: 0x10B89F0 Slot: 9
	public override bool IIEMACPEEBJ_Deserialize(byte[] _DBBGALAPFGC_bytes)
	{
		HGHOHICGBEP data = HGHOHICGBEP.HEGEKFMJNCC(_DBBGALAPFGC_bytes);
		DGKKMKLCEDF_DeserializeSetting(data);
		GPEKKMGGBPF(data);
		CFOFJPLEDEA(data);
		FBPNBANCLDA(data);
		for(int i = 0; i < data.BHGDNGHDDAC.Length; i++)
		{
			CEBFFLDKAEC_SecureInt d = new CEBFFLDKAEC_SecureInt();
			d.DNJEJEANJGL_Value = data.BHGDNGHDDAC[i].JBGEEPFKIGG_val;
			OHJFBLFELNK_m_intParam.Add(data.BHGDNGHDDAC[i].LJNAKDMILMC_key, d);
		}
		for(int i = 0; i < data.MHGMDJNOLMI.Length; i++)
		{
			NNJFKLBPBNK_SecureString d = new NNJFKLBPBNK_SecureString();
			d.DNJEJEANJGL_Value = data.MHGMDJNOLMI[i].JBGEEPFKIGG_val;
			FJOEBCMGDMI_m_stringParam.Add(data.MHGMDJNOLMI[i].LJNAKDMILMC_key, d);
		}
		for(int i = 0; i < data.KIEKMCKCMGD.Length; i++)
		{
			HIIIHLEJEGI d = new HIIIHLEJEGI();
			d.PPFNGGCBJKC_id = data.KIEKMCKCMGD[i].PPFNGGCBJKC_id;
			d.KFCIJBLDHOK_v1 = data.KIEKMCKCMGD[i].KFCIJBLDHOK_v1;
			d.JLEIHOEGMOP_v2 = data.KIEKMCKCMGD[i].JLEIHOEGMOP_v2;
			ADPFKHEMNBL.Add(d);
		}

		UnityEngine.Debug.LogError(NGHKJOEDLIP_Settings.OPFGFINHFCE_name+" "+NGHKJOEDLIP_Settings.OBGBAOLONDD_UniqueId);

		if(!RuntimeSettings.CurrentSettings.LoadRawDatabase)
		{
			// Update dates
			UMOEventList.EventData CurrenEvent = UMOEventList.GetCurrentEvent();
			if (CurrenEvent != null && CurrenEvent.EnableBlock(JIKKNHIAEKG_BlockName))
			{
				System.DateTime date = Utility.GetLocalDateTime(Utility.GetCurrentUnixTime());
				System.DateTime date2 = Utility.GetLocalDateTime(NGHKJOEDLIP_Settings.BONDDBOFBND_RankingStart);
				date = date.AddDays(-1);
				long offset = Utility.GetTargetUnixTime(date.Year, date.Month, date.Day, date2.Hour, date2.Minute, date2.Second) - NGHKJOEDLIP_Settings.BONDDBOFBND_RankingStart;
				if (NGHKJOEDLIP_Settings.BONDDBOFBND_RankingStart != 0) NGHKJOEDLIP_Settings.BONDDBOFBND_RankingStart += offset;
				if (NGHKJOEDLIP_Settings.EHHFFKAFOMC != 0) NGHKJOEDLIP_Settings.EHHFFKAFOMC += offset;
				if (NGHKJOEDLIP_Settings.HPNOGLIFJOP_RankingEnd != 0) NGHKJOEDLIP_Settings.HPNOGLIFJOP_RankingEnd += offset;
				if (NGHKJOEDLIP_Settings.LNFKGHNHJKE_RankingEnd2 != 0) NGHKJOEDLIP_Settings.LNFKGHNHJKE_RankingEnd2 += offset;
				if (NGHKJOEDLIP_Settings.JGMDAOACOJF_RewardStart != 0) NGHKJOEDLIP_Settings.JGMDAOACOJF_RewardStart += offset;
				if (NGHKJOEDLIP_Settings.IDDBFFBPNGI_RewardEnd != 0) NGHKJOEDLIP_Settings.IDDBFFBPNGI_RewardEnd += offset;
				if (NGHKJOEDLIP_Settings.KNLGKBBIBOH_RewardEnd2 != 0) NGHKJOEDLIP_Settings.KNLGKBBIBOH_RewardEnd2 += offset;

				for(int i = 0; i < NNMPGOAGEOL_quests.Count; i++)
				{
					if (NNMPGOAGEOL_quests[i].KJBGCLPMLCG_OpenedAt != 0) NNMPGOAGEOL_quests[i].KJBGCLPMLCG_OpenedAt = NGHKJOEDLIP_Settings.BONDDBOFBND_RankingStart;
					if (NNMPGOAGEOL_quests[i].GJFPFFBAKGK_CloseAt != 0) NNMPGOAGEOL_quests[i].GJFPFFBAKGK_CloseAt = NGHKJOEDLIP_Settings.HPNOGLIFJOP_RankingEnd;
				}
			}
		}
		return true;
	}

	// RVA: 0x10BA1C0 Offset: 0x10BA1C0 VA: 0x10BA1C0 Slot: 10
	public override bool IIEMACPEEBJ_Deserialize(EDOHBJAPLPF_JsonData OILEIIEIBHP, int _KAPMOPMDHJE_label)
	{
		//NGHKJOEDLIP_Settings = setting
		//FCIPEDFHFEM_TotalRewards = total_rewards
		//NNMPGOAGEOL_quests = quests
		//HKODBHPMLCP_cards = cards
		TodoLogger.LogError(TodoLogger.DbJson, "LMBBEGIAKAD_EventQuest.IIEMACPEEBJ_Deserialize");
		return true;
	}

	//// RVA: 0x10B8E30 Offset: 0x10B8E30 VA: 0x10B8E30
	private bool DGKKMKLCEDF_DeserializeSetting(HGHOHICGBEP HDNNNIPDHEB)
	{
		NGHKJOEDLIP_Settings.OBGBAOLONDD_UniqueId = (int)HDNNNIPDHEB.HMBHNLCFDIH.OBGBAOLONDD_UniqueId;
		NGHKJOEDLIP_Settings.OPFGFINHFCE_name = HDNNNIPDHEB.HMBHNLCFDIH.OPFGFINHFCE_name;
		NGHKJOEDLIP_Settings.HEDAGCNPHGD_RankingName = HDNNNIPDHEB.HMBHNLCFDIH.HEDAGCNPHGD_RankingName;
		NGHKJOEDLIP_Settings.OCGFKMHNEOF_name_for_api = HDNNNIPDHEB.HMBHNLCFDIH.OCGFKMHNEOF_name_for_api;
		NGHKJOEDLIP_Settings.BONDDBOFBND_RankingStart = HDNNNIPDHEB.HMBHNLCFDIH.BONDDBOFBND_RankingStart;
		NGHKJOEDLIP_Settings.HPNOGLIFJOP_RankingEnd = HDNNNIPDHEB.HMBHNLCFDIH.HPNOGLIFJOP_RankingEnd;
		NGHKJOEDLIP_Settings.EHHFFKAFOMC = HDNNNIPDHEB.HMBHNLCFDIH.EHHFFKAFOMC;
		NGHKJOEDLIP_Settings.LNFKGHNHJKE_RankingEnd2 = HDNNNIPDHEB.HMBHNLCFDIH.LNFKGHNHJKE_RankingEnd2;
		NGHKJOEDLIP_Settings.JGMDAOACOJF_RewardStart = HDNNNIPDHEB.HMBHNLCFDIH.JGMDAOACOJF_RewardStart;
		NGHKJOEDLIP_Settings.IDDBFFBPNGI_RewardEnd = HDNNNIPDHEB.HMBHNLCFDIH.IDDBFFBPNGI_RewardEnd;
		NGHKJOEDLIP_Settings.KNLGKBBIBOH_RewardEnd2 = HDNNNIPDHEB.HMBHNLCFDIH.KNLGKBBIBOH_RewardEnd2;
		NGHKJOEDLIP_Settings.KHIKEGLBGAF_RankingRewardScene = (int)HDNNNIPDHEB.HMBHNLCFDIH.KHIKEGLBGAF_RankingRewardScene;
		NGHKJOEDLIP_Settings.DPJCFLKFEPN_RankingRewardScene2 = (int)HDNNNIPDHEB.HMBHNLCFDIH.DPJCFLKFEPN_RankingRewardScene2;
		NGHKJOEDLIP_Settings.POGEFBMBPCB_MonthOdd = (sbyte)HDNNNIPDHEB.HMBHNLCFDIH.JMJDLDEIFKE;
		NGHKJOEDLIP_Settings.AHKNMANFILO_DayGroup = (sbyte)HDNNNIPDHEB.HMBHNLCFDIH.AHKNMANFILO_DayGroup;
		NGHKJOEDLIP_Settings.MOEKELIIDEO_SaveIdx = (sbyte)HDNNNIPDHEB.HMBHNLCFDIH.MOEKELIIDEO_SaveIdx;
		NGHKJOEDLIP_Settings.OCDMGOGMHGE_KeyPrefix = HDNNNIPDHEB.HMBHNLCFDIH.OCDMGOGMHGE_KeyPrefix;
		NGHKJOEDLIP_Settings.MJBKGOJBPAD_item_type = HDNNNIPDHEB.HMBHNLCFDIH.MJBKGOJBPAD_item_type;
		NGHKJOEDLIP_Settings.FEMMDNIELFC_Desc = HDNNNIPDHEB.HMBHNLCFDIH.FEMMDNIELFC_Desc;
		NGHKJOEDLIP_Settings.HKKNEAGCIEB_RankingSupport = (sbyte)HDNNNIPDHEB.HMBHNLCFDIH.HKKNEAGCIEB_RankingSupport;
		NGHKJOEDLIP_Settings.HIOOGLEJBKM_StartAdventureId = HDNNNIPDHEB.HMBHNLCFDIH.HIOOGLEJBKM_StartAdventureId;
		NGHKJOEDLIP_Settings.FJCADCDNPMP_EndAdventureId = HDNNNIPDHEB.HMBHNLCFDIH.FJCADCDNPMP_EndAdventureId;
		NGHKJOEDLIP_Settings.EJBGHLOOLBC_HelpIds = HDNNNIPDHEB.HMBHNLCFDIH.EJBGHLOOLBC_HelpIds;
		NGHKJOEDLIP_Settings.AHKPNPNOAMO_ExtreamOpen = (sbyte)HDNNNIPDHEB.HMBHNLCFDIH.AHKPNPNOAMO_ExtreamOpen;
		NGHKJOEDLIP_Settings.JKFPADIAKCK_Songs = HDNNNIPDHEB.HMBHNLCFDIH.JKFPADIAKCK_Songs;
		NGHKJOEDLIP_Settings.OMCAOJJGOGG_Lb = HDNNNIPDHEB.HMBHNLCFDIH.OMCAOJJGOGG_Lb;
		NGHKJOEDLIP_Settings.EKJFNHHKBPL = HDNNNIPDHEB.HMBHNLCFDIH.EKJFNHHKBPL;
		for(int i = 0; i < HDNNNIPDHEB.HMBHNLCFDIH.JHPCPNJJHLI_RankingThreshold.Length; i++)
		{
			if(HDNNNIPDHEB.HMBHNLCFDIH.JHPCPNJJHLI_RankingThreshold[i] < 1001)
			{
				NGHKJOEDLIP_Settings.JHPCPNJJHLI_RankingThreshold.Add(HDNNNIPDHEB.HMBHNLCFDIH.JHPCPNJJHLI_RankingThreshold[i]);
			}
		}
		for(int i = 0; i < HDNNNIPDHEB.HMBHNLCFDIH.EPHAKDAPAHE.Length; i++)
		{
			NGHKJOEDLIP_Settings.EPHAKDAPAHE.Add(HDNNNIPDHEB.HMBHNLCFDIH.EPHAKDAPAHE[i]);
		}
		for(int i = 0; i < HDNNNIPDHEB.HMBHNLCFDIH.HEABAENHIDE.Length; i++)
		{
			NGHKJOEDLIP_Settings.HEABAENHIDE.Add(HDNNNIPDHEB.HMBHNLCFDIH.HEABAENHIDE[i]);
		}
		for(int i = 0; i < HDNNNIPDHEB.HMBHNLCFDIH.HIAMHLLHAON_PlayCost.Length; i++)
		{
			NGHKJOEDLIP_Settings.HIAMHLLHAON_PlayCost.Add((int)HDNNNIPDHEB.HMBHNLCFDIH.HIAMHLLHAON_PlayCost[i]);
		}
		for(int i = 0; i < HDNNNIPDHEB.HMBHNLCFDIH.BKBEPOFJOJG.Length; i++)
		{
			NGHKJOEDLIP_Settings.BKBEPOFJOJG.Add((int)HDNNNIPDHEB.HMBHNLCFDIH.BKBEPOFJOJG[i]);
		}
		NGHKJOEDLIP_Settings.EEOGPJJCKHH_Drops.Clear();
		for(int i = 0; i < HDNNNIPDHEB.HMBHNLCFDIH.EEOGPJJCKHH_Drops.Length; i++)
		{
			NGHKJOEDLIP_Settings.EEOGPJJCKHH_Drops.Add((int)HDNNNIPDHEB.HMBHNLCFDIH.EEOGPJJCKHH_Drops[i]);
		}
		AIGOEAPJGEB.KHEKNNFCAOI_Init(MBHDIJJEOFL, HDNNNIPDHEB);
		return true;
	}

	//// RVA: 0x10BA300 Offset: 0x10BA300 VA: 0x10BA300
	//private bool DGKKMKLCEDF_DeserializeSetting(EDOHBJAPLPF_JsonData _IDLHJIOMJBK_data, int _KAPMOPMDHJE_label) { }

	//// RVA: 0x10BA354 Offset: 0x10BA354 VA: 0x10BA354
	//private void EEEKPHDPJHG(List<int> NNDGIAEFMOG, EDOHBJAPLPF_JsonData OBHAFLMHAKG, string _OPFGFINHFCE_name, ref bool _NGJDHLGMHMH_IsInvalid) { }

	//// RVA: 0x10BA534 Offset: 0x10BA534 VA: 0x10BA534
	//private void EEEKPHDPJHG(List<int> NNDGIAEFMOG, string OAIBJJHGCNM) { }

	//// RVA: 0x10B9AAC Offset: 0x10B9AAC VA: 0x10B9AAC
	private bool GPEKKMGGBPF(HGHOHICGBEP HDNNNIPDHEB)
	{
		for(int i = 0; i < HDNNNIPDHEB.MCHFABAKPMH.Length; i++)
		{
			LNGOGDOIJHI d = new LNGOGDOIJHI();
			d.PPFNGGCBJKC_id = (int)HDNNNIPDHEB.MCHFABAKPMH[i].PPFNGGCBJKC_id;
			d.DNBFMLBNAEE_point = (int)HDNNNIPDHEB.MCHFABAKPMH[i].FOILNHKHHDF_pt;
			d.JOPPFEHKNFO_Pickup = HDNNNIPDHEB.MCHFABAKPMH[i].JOPPFEHKNFO_Pickup != 0;
			for(int j = 0; j < HDNNNIPDHEB.MCHFABAKPMH[i].AIHOJKFNEEN_itm.Length; j++)
			{
				FFJHPLIMOLK d2 = new FFJHPLIMOLK();
				d2.GLCLFMGPMAN_ItemId = HDNNNIPDHEB.MCHFABAKPMH[i].AIHOJKFNEEN_itm[j];
				d2.HMFFHLPNMPH_count = (int)HDNNNIPDHEB.MCHFABAKPMH[i].BFINGCJHOHI_cnt[j];
				d2.NMKEOMCMIPP_RewardId = (int)HDNNNIPDHEB.MCHFABAKPMH[i].JJHPDDPKBHF[j];
				d.AHJNPEAMCCH_rewards.Add(d2);
			}
			FCIPEDFHFEM_TotalRewards.Add(d);
		}
		return true;
	}

	//// RVA: 0x10BA308 Offset: 0x10BA308 VA: 0x10BA308
	//private bool GPEKKMGGBPF(EDOHBJAPLPF_JsonData OBHAFLMHAKG, int _KAPMOPMDHJE_label) { }

	//// RVA: 0x10B9E88 Offset: 0x10B9E88 VA: 0x10B9E88
	private bool CFOFJPLEDEA(HGHOHICGBEP HDNNNIPDHEB)
	{
		int xor = (int)Utility.GetCurrentUnixTime() * 11 + 1;
		for(int i = 0; i < HDNNNIPDHEB.MDDOGIAFDEI.Length; i++)
		{
			AKIIJBEJOEP d = new AKIIJBEJOEP();
			d.KHEKNNFCAOI_Init(JIKKNHIAEKG_BlockName, i + 1, xor, HDNNNIPDHEB);
			NNMPGOAGEOL_quests.Add(d);
			xor += 13;
		}
		return true;
	}

	//// RVA: 0x10BA310 Offset: 0x10BA310 VA: 0x10BA310
	//private bool CFOFJPLEDEA(EDOHBJAPLPF_JsonData OBHAFLMHAKG, int _KAPMOPMDHJE_label) { }

	//// RVA: 0x10BA020 Offset: 0x10BA020 VA: 0x10BA020
	private bool FBPNBANCLDA(HGHOHICGBEP HDNNNIPDHEB)
	{
		int xor = (int)Utility.GetCurrentUnixTime() * 11 + 1;
		for(int i = 0; i < HDNNNIPDHEB.DGPHHJCPNOO.Length; i++)
		{
			PKADMPNDMGP d = new PKADMPNDMGP();
			d.KHEKNNFCAOI_Init(JIKKNHIAEKG_BlockName, i + 1, xor, HDNNNIPDHEB);
			HKODBHPMLCP_cards.Add(d);
			xor += 13;
		}
		return true;
	}

	//// RVA: 0x10BA318 Offset: 0x10BA318 VA: 0x10BA318
	//private bool FBPNBANCLDA(EDOHBJAPLPF_JsonData OBHAFLMHAKG, int _KAPMOPMDHJE_label) { }

	//// RVA: 0x10BA764 Offset: 0x10BA764 VA: 0x10BA764
	//private bool JBNBGGDDEGG(HGHOHICGBEP JMHECKKKMLK) { }

	//// RVA: 0x10BAA2C Offset: 0x10BAA2C VA: 0x10BAA2C
	//private bool ABICOINNGEK(HGHOHICGBEP JMHECKKKMLK) { }

	//// RVA: 0x10BACB4 Offset: 0x10BACB4 VA: 0x10BACB4
	//private bool EGLHCMDNFOE(HGHOHICGBEP JMHECKKKMLK) { }

	// RVA: 0x10BB08C Offset: 0x10BB08C VA: 0x10BB08C Slot: 11
	public override uint CAOGDCBPBAN()
	{
		TodoLogger.LogError(TodoLogger.DbIntegrityCheck, "LMBBEGIAKAD_EventQuest.CAOGDCBPBAN");
		return 0;
	}
}
