...
public class JNKEEAOKNCI_Skill : DIHHCBACKGG_DbSection
{
	...
	public EHGAHMIBPIB HJGDBBPDHON(int _AOGDKBPNGCI_EffectIdx)
	{
		return CNKKCMLOAGN[_AOGDKBPNGCI_EffectIdx - 1];
	}

	//// RVA: 0x1B90934 Offset: 0x1B90934 VA: 0x1B90934
	public FOKHDKJJOFB_EffectByNumDiva EFJFIIPIMOO_GetEffectValue(int _AOGDKBPNGCI_EffectIdx)
	{
		return CHOMLDBLPLC_EffectValue[_AOGDKBPNGCI_EffectIdx - 1];
	}

	//// RVA: 0x1B909B4 Offset: 0x1B909B4 VA: 0x1B909B4
	//private List<PPGHMBNIAEC> DGKHKMNMMDN(JNKEEAOKNCI_Skill.MKGJHBAKMBD_SkillType _INDDJNMPONH_type) { }

	//// RVA: 0x1B909E0 Offset: 0x1B909E0 VA: 0x1B909E0
	//private List<HBDCPGLAPHH> LGONDBKGPMH(JNKEEAOKNCI_Skill.MKGJHBAKMBD_SkillType _INDDJNMPONH_type) { }

	//// RVA: 0x1B90A1C Offset: 0x1B90A1C VA: 0x1B90A1C
	//private List<KFCIIMBBNCD> BHHJHMEKCCB(JNKEEAOKNCI_Skill.MKGJHBAKMBD_SkillType _INDDJNMPONH_type) { }
	...
	public override bool IIEMACPEEBJ_Deserialize(EDOHBJAPLPF_JsonData OILEIIEIBHP, int _KAPMOPMDHJE_label)
	{
		...
		TodoLogger.LogError(TodoLogger.DbJson, "DB Skills IIEMACPEEBJ_Deserialize");
		return true;
	}
	...
	private bool BKIPCJEJGGF(FLHKDAGGLNK GGIHPBILDIG)
	{
		...
		for(int i = 0; i < array.Length; i++)
		{
			...
			{
				uint[] array2 = array[i].OCAMDLMPBGA_dv;
				for (int j = 0; j < array2.Length; j++)
				{
			...
			{
				uint[] array2 = array[i].JMAGFEENOED_dv2;
				for (int j = 0; j < array2.Length; j++)
				{
	...
	{
		if (_GJLFANGDGCL_Target == MKGJHBAKMBD_SkillType.KBHGPMNGALJ_2_Costume)
		{
			...
			for(int i = 0; i < array.Length; i++)
			{
				...
				data.HEKHODDJHAO_P1 = (int)array[i].POOHGENNICD_p1;
				data.AKGNPLBDKLN_P2 = (int)array[i].LLGBAPAACLL_p2;
				data.JGNHOGKKPDM = 0;
	...
	//private bool MMINKHFAOJJ(EDOHBJAPLPF_JsonData OBHAFLMHAKG, int _KAPMOPMDHJE_label, JNKEEAOKNCI_Skill.MKGJHBAKMBD_SkillType _GJLFANGDGCL_Target) { }
	...
	private bool ELDPNELNCOH(FLHKDAGGLNK GGIHPBILDIG)
	{
		...
		for(int i = 0; i < array.Length; i++)
		{
			...
			data.LGCPDKGMLJI = (int)array[i].MJMPANIBFED_pid;
			data.CHOFDPDFPDC_ConfigValue = (int)array[i].ODMJFHDIGLP_cnd;
	...
	private bool ANPNMONLBNA(FLHKDAGGLNK GGIHPBILDIG)
	{
		...
		for (int i = 0; i < array.Length; i++)
		{
			...
			{
				int[] array2 = array[i].NANNGLGOFKH_value;
	...
	private bool DBINMPHFJEA(FLHKDAGGLNK GGIHPBILDIG)
	{
		...
		for (int i = 0; i < array.Length; i++)
		{
			...
			{
				int[] array2 = array[i].KCOHMHFBDKF_Value1;
			...
			{
				int[] array2 = array[i].HLMMBNCIIAC_Value2;
	...
	private bool DFGFFILCCEI(FLHKDAGGLNK GGIHPBILDIG, MKGJHBAKMBD_SkillType _GJLFANGDGCL_Target)
	{
		if(_GJLFANGDGCL_Target == MKGJHBAKMBD_SkillType.MHKFDBLMOGF_0_Scene)
		{
			...
			for(int i = 0; i < array.Length; i++)
			{
				...
				data.MJMPANIBFED_pid = (int)array[i].MJMPANIBFED_pid;
				data.GJLFANGDGCL_Target = (byte)array[i].AGNHPHEJKMK_tgt;
				data.INDDJNMPONH_type = (byte)array[i].GBJFNGCDKPM_typ;
				data.OAFPGJLCNFM_cond = (int)array[i].ODMJFHDIGLP_cnd & 0xff;
				data.BBFKKANELFP_EffectType = (byte)array[i].BBFKKANELFP_EffectType;
				{
					uint[] array2 = array[i].KFCIJBLDHOK_v1;
					for(int j = 0; j < array2.Length; j++)
					{
		...
		else if (_GJLFANGDGCL_Target == MKGJHBAKMBD_SkillType.KBHGPMNGALJ_2_Costume)
		{
	...
	private bool LEKKJEDCPNK(FLHKDAGGLNK EJPJKHPLGMO)
	{
		...
		for(int i = 0; i < array.Length; i++)
		{
			...
			{
				int[] array2 = array[i].LGFLNOJDHHC_Targets;
	...
	//private bool DFGFFILCCEI(EDOHBJAPLPF_JsonData OBHAFLMHAKG, int _KAPMOPMDHJE_label, JNKEEAOKNCI_Skill.MKGJHBAKMBD_SkillType _GJLFANGDGCL_Target) { }
	//MJMPANIBFED_pid = MJMPANIBFED_pid
	//GJLFANGDGCL_Target = AGNHPHEJKMK_tgt
	...
	private bool INLJKKMEENJ(FLHKDAGGLNK GGIHPBILDIG, MKGJHBAKMBD_SkillType _GJLFANGDGCL_Target)
	{
		if(_GJLFANGDGCL_Target == MKGJHBAKMBD_SkillType.MHKFDBLMOGF_0_Scene)
		{
			...
			for(int i = 0; i < array.Length; i++)
			{
				...
				data.BEKFHEBCPEE_lid = (int)array[i].BEKFHEBCPEE_lid;
				...
				data.CPNAGMFCIJK_TriggerType = (byte)array[i].NNONCOCHGMD_tt;
				data.JGDJACOPHJP_LineTarget = (byte)array[i].PLMHOKJIPJK_lin;
				data.JGNHOGKKPDM = (byte)array[i].INANEEGAEEG;
				data.MKPJBDFDHOL_ThumbId = new int[2];
				data.MKPJBDFDHOL_ThumbId[0] = (int)array[i].HEOLEHDFLJO_ico;
				data.MKPJBDFDHOL_ThumbId[1] = (int)array[i].AOELPHMDDND;
				data.GIEFBAHPMMM = (int)array[i].KPECMLFDLOI_pri;
				data.CEFHDLLAPDH_MusicId = (int)array[i].FCBDGLEPGFJ_mid;
				...
				{
					uint[] array2 = array[i].MHHNOKGNJFN_tv;
					for(int j = 0; j < array2.Length; j++)
					{
		...
		else if (_GJLFANGDGCL_Target == MKGJHBAKMBD_SkillType.AIFGINAKBMA_1_EnemySkill)
		{
			...
			for (int i = 0; i < array.Length; i++)
			{
				...
				data.MKPJBDFDHOL_ThumbId = new int[0];
				{
					...
					{
						data.LFGFBMJNBKN_ConfigValue[j] = (short)array[i].MHHNOKGNJFN_tv;
					}
				}
				{
					uint[] array2 = array[i].CBDOEDKIOJK_ev;
	...
	private bool JAHNPCHIDIA(FLHKDAGGLNK GGIHPBILDIG, MKGJHBAKMBD_SkillType _GJLFANGDGCL_Target)
	{
		...
		for (int i = 0; i < array2.Count; i++)
		{
			for(int j = 0; j < array.Length; j++)
			{
				...
				{
					array2[i].AOPELJFAMCL_LiveSkillType = (int)array[j].AOPELJFAMCL_LiveSkillType;
					break;
				}
	...
	//private bool FOMPOPFNLME(EDOHBJAPLPF_JsonData OBHAFLMHAKG, int _KAPMOPMDHJE_label, JNKEEAOKNCI_Skill.MKGJHBAKMBD_SkillType _GJLFANGDGCL_Target) { }

	//// RVA: 0x1B96F7C Offset: 0x1B96F7C VA: 0x1B96F7C
	//private bool INLJKKMEENJ(EDOHBJAPLPF_JsonData OBHAFLMHAKG, int _KAPMOPMDHJE_label, JNKEEAOKNCI_Skill.MKGJHBAKMBD_SkillType _GJLFANGDGCL_Target) { }
	...
	private bool DGENOFKIBBE(FLHKDAGGLNK GGIHPBILDIG)
	{
		...
		for(int i = 0; i < array.Length; i++)
		{
			...
			{
				int[] array2 = array[i].FDBOPFEOENF_diva;
				for(int j = 0; j < array2.Length; j++)
				{
	...
	private bool JDCPBKNMHMM(FLHKDAGGLNK GGIHPBILDIG)
	{
		...
		for(int i = 0; i < array.Length; i++)
		{
			...
			data.CEFHDLLAPDH_MusicId = (int)array[i].FCBDGLEPGFJ_mid;
			data.NFIBKOACELP_Attr = (int)array[i].BPBLHHFKBBG;
			data.BHADMHLIFMM = (int)array[i].OCAMDLMPBGA_dv;
...
public class PPGHMBNIAEC
{
	...
	private void DKGNGLBCPKA(ref string IAPANOCEMBI, int GBMHFDKCFGB, int _AOGDKBPNGCI_EffectIdx)
	{
		string res = "";
		string a, b;
		...
		EGLDFPILJLG_SkillBuffEffect[_AOGDKBPNGCI_EffectIdx] == (int)SkillBuffEffect.Type.ScoreUpPercentage_Intimacy)
		{
			...
			a = string.Format("[v{0}_max]", _AOGDKBPNGCI_EffectIdx + 1);
			res = IAPANOCEMBI;
		...
		{
			res = IAPANOCEMBI;
			a = _AOGDKBPNGCI_EffectIdx == 0 ? "[v1]" : "[v2]";
			b = NKGHBKFMFCI_BuffValue[GBMHFDKCFGB - 1, _AOGDKBPNGCI_EffectIdx].ToString();
		}
	...
	public bool OEJNKFONOGJ(int _FJFCNGNGIBN_Attribute)
	{
		return NFIBKOACELP_Attr == _FJFCNGNGIBN_Attribute;
	}
}
