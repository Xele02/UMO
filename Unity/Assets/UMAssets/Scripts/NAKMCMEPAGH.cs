...
public class NAKMCMEPAGH
{
	...
	public int FILFPNDEINH_Combo; // 0x20
	...
	public int IMIEPNOECFD_ValkyrieMode; // 0x3C
	public int BENEAPDMALA_Mk; // 0x40
	public long PDGJIJOMAKO_Medl; // 0x48
	public int PFOMECFACLL_Shop; // 0x50
	...
	public int BJDKMJFCOOM_LCnt { get { return CCNDOFGDOGF ^ FBGGEFFJJHB_xor; } set { CCNDOFGDOGF = value ^ FBGGEFFJJHB_xor; } } //0x17C0578 JKLLJKHDJFM_bgs 0x17C0588 OHDMFLKNGDA_bgs
	public int BKEKKFPEPBG_LDt { get { return DAPDDMJNNGG ^ FBGGEFFJJHB_xor; } set { DAPDDMJNNGG = value ^ FBGGEFFJJHB_xor; } } //0x17C0598 POIFAKPKGPE_bgs 0x17C05A8 KPEDOCAGMJP_bgs
	...
	public void KGFIAHEEMDN_AddSerieClear(int _MGBDCFIKBPM_Serie)
	{
		int a = 0;
		if(_MGBDCFIKBPM_Serie >= 2 && _MGBDCFIKBPM_Serie < 6)
		{
			a = 1;
			switch(_MGBDCFIKBPM_Serie)
			{
				case 2:
					break;
				default:
					a = _MGBDCFIKBPM_Serie - 1;
					break;
			}
		}
	...
	public void DHECAFOELPF_SetHighScore(int _KNIFCANOHOC_score)
	{
		if (EDLBLCGHECJ_MaxScore < _KNIFCANOHOC_score)
			EDLBLCGHECJ_MaxScore = _KNIFCANOHOC_score;
	}

	// // RVA: 0x17C07E0 Offset: 0x17C07E0 VA: 0x17C07E0
	public void EJJAPFPJLHP_AddUc(int _ACGLMKEBMDL_uc)
	{
		HOHBKPPOLLA_Uc += _ACGLMKEBMDL_uc;
	...
	{
		if (IMIEPNOECFD_ValkyrieMode < 99999999)
			IMIEPNOECFD_ValkyrieMode++;
	}
	...
	public void KPNLMLPFKMJ_AddMedal(int _MHKJEBNOPIM_Medal)
	{
		if(PDGJIJOMAKO_Medl + _MHKJEBNOPIM_Medal < 999999999999)
			PDGJIJOMAKO_Medl += _MHKJEBNOPIM_Medal;
		else
	...
	{
		PFOMECFACLL_Shop += _HMFFHLPNMPH_count;
		if(PFOMECFACLL_Shop > 99999999)
			PFOMECFACLL_Shop = 99999999;
	}

	// // RVA: 0x17C0920 Offset: 0x17C0920 VA: 0x17C0920
	public void MFCKAGIINJL_AddDecoShop(int _HMFFHLPNMPH_count)
	{
		DHNOLFBEHKN_Dcshp += _HMFFHLPNMPH_count;
		if(DHNOLFBEHKN_Dcshp > 99999999)
			DHNOLFBEHKN_Dcshp = 99999999;
	}

	// // RVA: 0x17B5CFC Offset: 0x17B5CFC VA: 0x17B5CFC
	// public void CIKKGHDIDPJ(int _HMFFHLPNMPH_count) { }

	// // RVA: 0x17C0940 Offset: 0x17C0940 VA: 0x17C0940
	public void FFMDFKBLECC(int _HMFFHLPNMPH_count)
	{
		MJBCBJDMODC_Valu += _HMFFHLPNMPH_count;
		if(MJBCBJDMODC_Valu > 99999999)
			MJBCBJDMODC_Valu = 99999999;
	}

	// // RVA: 0x17C0960 Offset: 0x17C0960 VA: 0x17C0960
	public void KNGCKBNFEGM_AddVOp(int _FGHGMHPNEMG_Type, int _HMFFHLPNMPH_count)
	{
		if(_FGHGMHPNEMG_Type < GKOAPFJFKEJ_VOpC.Length)
		{
			GKOAPFJFKEJ_VOpC[_FGHGMHPNEMG_Type] += _HMFFHLPNMPH_count;
			if (GKOAPFJFKEJ_VOpC[_FGHGMHPNEMG_Type] < 100000000)
				return;
			GKOAPFJFKEJ_VOpC[_FGHGMHPNEMG_Type] = 99999999;
		}
	}

	// // RVA: 0x17C0A58 Offset: 0x17C0A58 VA: 0x17C0A58
	public void PMDMJKCANFF_AddDOp(int _AHHJLDLAPAN_DivaId, int _HMFFHLPNMPH_count)
	{
		if(_AHHJLDLAPAN_DivaId > 0)
		{
			if(_AHHJLDLAPAN_DivaId - 1 < LOOAKNLDONN_DOpC.Length)
			{
				LOOAKNLDONN_DOpC[_AHHJLDLAPAN_DivaId - 1] += _HMFFHLPNMPH_count;
				if (LOOAKNLDONN_DOpC[_AHHJLDLAPAN_DivaId - 1] < 100000000)
					return;
				LOOAKNLDONN_DOpC[_AHHJLDLAPAN_DivaId - 1] = 99999999;
			}
		}
	}
	...
	public bool AGBOGBEOFME(NAKMCMEPAGH OIKJFMGEICL)
	{
		if(MILCBLJDADN_MusicClear != OIKJFMGEICL.MILCBLJDADN_MusicClear ||
			...
			FILFPNDEINH_Combo != OIKJFMGEICL.FILFPNDEINH_Combo ||
			...
			IMIEPNOECFD_ValkyrieMode != OIKJFMGEICL.IMIEPNOECFD_ValkyrieMode ||
			BENEAPDMALA_Mk != OIKJFMGEICL.BENEAPDMALA_Mk ||
			PDGJIJOMAKO_Medl != OIKJFMGEICL.PDGJIJOMAKO_Medl ||
			PFOMECFACLL_Shop != OIKJFMGEICL.PFOMECFACLL_Shop ||
	...
	public void ODDIHGPONFL_Copy(NAKMCMEPAGH GPBJHKLFCEP)
	{
		...
		IMIEPNOECFD_ValkyrieMode = GPBJHKLFCEP.IMIEPNOECFD_ValkyrieMode;
		BENEAPDMALA_Mk = GPBJHKLFCEP.BENEAPDMALA_Mk;
		PDGJIJOMAKO_Medl = GPBJHKLFCEP.PDGJIJOMAKO_Medl;
		PFOMECFACLL_Shop = GPBJHKLFCEP.PFOMECFACLL_Shop;
	...
	// public void AGHKODFKOJI(BHBONAHFKHD JBBHNIACMFJ, string _JIKKNHIAEKG_BlockName, string MJBACHKCIHA, int _OIPCCBHIKIA_index, NAKMCMEPAGH _OHMCIEMIKCE_t, bool EFOEPDLNLJG) { }
}
