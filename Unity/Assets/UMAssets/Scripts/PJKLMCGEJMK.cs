using System.Collections.Generic;
using XeApp.Core.WorkerThread;
using UnityEngine;

// Namespace:
public class PJKLMCGEJMK // TypeDefIndex: 10326
{
        // // Fields
        // private int NLGJBBGAOLH; // 0x8
        // private float BLKIMNAILKK; // 0xC
        // private const double IACPOAJPMNO = 1;
        // private int MFNENCLNGKF; // 0x10
        // private int OANLLFOHEPJ; // 0x14
        // private long NNODMPKKCJH; // 0x18
        // private long JDIBBDGNFKH; // 0x20
        // public static long DALFMJFKCGJ; // 0x0
        // public static bool KCEDAACIBFG; // 0x8
        // private KPKEOIJHIMN OPFEFKOOMED; // 0x2C
        // private OBOKMHHMOIL IEBFCJACLPN; // 0x30
        // public bool IEFOIIAEBBJ; // 0x34
        // public bool NFECEPJEMHG; // 0x35
        // public bool LMBLIFCNKCJ; // 0x36
        // private bool NAJENHKNJLN; // 0x37
        // private IIDJLAEDMPI HFMOEKIBNKA; // 0x38
        // private LGNBLDHKLJK DNMDKBFINPG; // 0x3C
        // public bool BEBMDHKKNDA; // 0x40
        // public bool KAEMPHIPDFN; // 0x41
        // public bool BLFILNOBHMM; // 0x42
        // public DMADPGMDPHC LJPJJHGIEAO; // 0x44
        // public MFKJNKIKBOP IGAMFDCEGFC; // 0x48
        // public DIDEKBFMOBM LLFDDHIIPBK; // 0x4C
        // public DIDEKBFMOBM CCBAMLACFCF; // 0x50
        // public DIDEKBFMOBM NIJMLFNDAJP; // 0x54
        // public IHABJNEGMEA EGAOFAHEPME; // 0x58
        // public DLMJDDACNLA OMAGHCDMBBI; // 0x5C
        // public OEEHODIFBIG DMPHNPAFHEG; // 0x60
        // public bool OLBAIKLLIFE; // 0x64
        private List<CACGCMBKHDI_Request> FCICFIAOLAM_RequestList; // 0x68
        // private bool DMGPJMPINFJ; // 0x6C
        // [CompilerGeneratedAttribute] // RVA: 0x669224 Offset: 0x669224 VA: 0x669224
        // private PJKLMCGEJMK.AHADNLCOPOL <CLCJNFNMNBH>k__BackingField; // 0x70
        // private PJKLMCGEJMK.FPHHIOBKMFD LCIGLIDJILJ; // 0x74

        // // Properties
        // private bool IGIIAEPHNCC { get; set; }
        // private int JMEKPMDNKEO { get; set; }
        // private KPKEOIJHIMN.GIDACIOHFNN DLDIKFJIGBE { get; set; }
        public WorkerThreadQueue BNJPAKLNOPA_WorkerThreadQueue { get; set; } // EGCCKJEDANG // 0x28 IMDNDFIKMHN ODBGIMFJOHN
        // public KPKEOIJHIMN DAOHAKABAFG { get; }
        // public OBOKMHHMOIL OAKOMPBPGLD { get; }
        // public bool MBOIDKCMCDL { get; set; }
        // public IIDJLAEDMPI FJDBNGEPKHL { get; }
        // public LGNBLDHKLJK LOMEEJGIAHO { get; }
        // public PJKLMCGEJMK.AHADNLCOPOL CMCKNKKCNDK { get; set; }

        // // Methods

        // // RVA: 0x930B9C Offset: 0x930B9C VA: 0x930B9C
        // private bool BLJMGAEEDAC() { }

        // // RVA: 0x930C78 Offset: 0x930C78 VA: 0x930C78
        // private void NNFLLPLMHBG(bool NANNGLGOFKH) { }

        // // RVA: 0x930D48 Offset: 0x930D48 VA: 0x930D48
        // private int MMACOJFDCJC() { }

        // // RVA: 0x930E1C Offset: 0x930E1C VA: 0x930E1C
        // private void MLFFBGAHKJI(int NANNGLGOFKH) { }

        // // RVA: 0x930EEC Offset: 0x930EEC VA: 0x930EEC
        // private KPKEOIJHIMN.GIDACIOHFNN BEPAHJLMMOG() { }

        // // RVA: 0x930FC0 Offset: 0x930FC0 VA: 0x930FC0
        // private void PFABEFOBEMC(KPKEOIJHIMN.GIDACIOHFNN NANNGLGOFKH) { }

        // // RVA: 0x9310A0 Offset: 0x9310A0 VA: 0x9310A0
        public KPKEOIJHIMN JACGBOGNFKM()
		{
			UnityEngine.Debug.LogError("TODO");
            return null;
		}

        // // RVA: 0x9310A8 Offset: 0x9310A8 VA: 0x9310A8
        // public OBOKMHHMOIL MFLBDBHHKMP() { }

        // // RVA: 0x9310B0 Offset: 0x9310B0 VA: 0x9310B0
        // public bool NPCNCBJAJNO() { }

        // // RVA: 0x9310B8 Offset: 0x9310B8 VA: 0x9310B8
        // public void GOFGBIANJGF(bool NANNGLGOFKH) { }

        // // RVA: 0x925074 Offset: 0x925074 VA: 0x925074
        public IIDJLAEDMPI FIIMIGEKDCM()
		{
			UnityEngine.Debug.LogError("TODO");
            return null;
		}

        // // RVA: 0x9310C0 Offset: 0x9310C0 VA: 0x9310C0
        // public LGNBLDHKLJK LEFKJNJCAPO() { }

        // [CompilerGeneratedAttribute] // RVA: 0x6C3558 Offset: 0x6C3558 VA: 0x6C3558
        // // RVA: 0x9310C8 Offset: 0x9310C8 VA: 0x9310C8
        // public PJKLMCGEJMK.AHADNLCOPOL CNKGOPKANGF() { }

        // [CompilerGeneratedAttribute] // RVA: 0x6C3568 Offset: 0x6C3568 VA: 0x6C3568
        // // RVA: 0x9310D0 Offset: 0x9310D0 VA: 0x9310D0
        // private void CHJGGLFGALP(PJKLMCGEJMK.AHADNLCOPOL NANNGLGOFKH) { }

        // // RVA: 0x9310D8 Offset: 0x9310D8 VA: 0x9310D8
        public void IJBGPAENLJA(MonoBehaviour DANMJLOBLIE)
        {
            //CLCJNFNMNBH
            //LCIGLIDJILJ
            UnityEngine.Debug.LogWarning("TODO PJKLMCGEJMK.IJBGPAENLJA");
            BNJPAKLNOPA_WorkerThreadQueue = new XeApp.Core.WorkerThread.WorkerThreadQueue();
            BNJPAKLNOPA_WorkerThreadQueue.Start();
        }

        // // RVA: 0x931278 Offset: 0x931278 VA: 0x931278
        // public void BAGMHFKPFIF() { }

        // // RVA: 0x931734 Offset: 0x931734 VA: 0x931734
        // public void FFBCKMFKFME() { }

        // // RVA: 0x931760 Offset: 0x931760 VA: 0x931760
        // private void LFKLIOKFGLP() { }

        // // RVA: 0x931934 Offset: 0x931934 VA: 0x931934
        // private void JADLLIFCGLG() { }

        // // RVA: 0x931BD0 Offset: 0x931BD0 VA: 0x931BD0
        public CACGCMBKHDI_Request IFFNCAFNEAG_AddRequest(CACGCMBKHDI_Request ADKIDBJCAJA_Request)
        {
            ADKIDBJCAJA_Request.BNJPAKLNOPA_WorkerThreadQueue = BNJPAKLNOPA_WorkerThreadQueue;
            ADKIDBJCAJA_Request.EDBGNGILAKA(false);

            /*double time = UnityEngine.Time.get_realtimeSinceStartup();
            ADKIDBJCAJA.IOPPCLACPOF(time);
            ADKIDBJCAJA.FNIKLDHAPEG(time);
            ADKIDBJCAJA.BPAIAMDPKBJ(time);
            ADKIDBJCAJA.CFICLNJACCD = NLGJBBGAOLH;
            ADKIDBJCAJA.GJAEJFLLKGC = BLKIMNAILKK;
            ADKIDBJCAJA.KAEMPHIPDFN = KAEMPHIPDFN;
            ADKIDBJCAJA.CKOOCBJGHBI = NNODMPKKCJH;*/
            //NNODMPKKCJH = ; ?

            FCICFIAOLAM_RequestList.Add(ADKIDBJCAJA_Request);

            return ADKIDBJCAJA_Request;
        }

        // // RVA: -1 Offset: -1
        // public T IFFNCAFNEAG<T>(T ADKIDBJCAJA) { }
        
        // [IteratorStateMachineAttribute] // RVA: 0x6C3578 Offset: 0x6C3578 VA: 0x6C3578
        // // RVA: 0x93188C Offset: 0x93188C VA: 0x93188C
        // private IEnumerator NBCKHIAINIM(CACGCMBKHDI ADKIDBJCAJA) { }

        // // RVA: 0x931E18 Offset: 0x931E18 VA: 0x931E18
        // private bool EHLBPCGHLCL(CACGCMBKHDI ADKIDBJCAJA, SakashoErrorId KLCMLLLIANB, bool OHPAOJNLDJO) { }

        // [IteratorStateMachineAttribute] // RVA: 0x6C35F0 Offset: 0x6C35F0 VA: 0x6C35F0
        // // RVA: 0x931F34 Offset: 0x931F34 VA: 0x931F34
        // private IEnumerator MELKAFEGNMD(CACGCMBKHDI ADKIDBJCAJA, SakashoErrorId KLCMLLLIANB, int MJACIGCPNDA, string IIGAKOKGKIB, EHLCCMEDIOH IICBBDEPBAM) { }

        // [IteratorStateMachineAttribute] // RVA: 0x6C3668 Offset: 0x6C3668 VA: 0x6C3668
        // // RVA: 0x932064 Offset: 0x932064 VA: 0x932064
        // private IEnumerator AOMMJNJGFEL(CACGCMBKHDI ADKIDBJCAJA, KPKEOIJHIMN.GIDACIOHFNN CMCKNKKCNDK) { }

        // // RVA: 0x9316EC Offset: 0x9316EC VA: 0x9316EC
        // private void DDBHLHBJEGP(PJKLMCGEJMK.AHADNLCOPOL HOENAFAJMGI) { }

        // // RVA: 0x932144 Offset: 0x932144 VA: 0x932144
        // public void CEEAFKHANJB(int EAABKFGHKBG, int ABFKANCAMPN) { }

        // // RVA: 0x9321AC Offset: 0x9321AC VA: 0x9321AC
        // private void NELKBIEGFIF(CACGCMBKHDI ADKIDBJCAJA) { }

        // // RVA: 0x9324C8 Offset: 0x9324C8 VA: 0x9324C8
        // private void OHFKAECPJPO(CACGCMBKHDI ADKIDBJCAJA) { }

        // // RVA: 0x93299C Offset: 0x93299C VA: 0x93299C
        // private static int OMNAPCHLBHF(CACGCMBKHDI ADKIDBJCAJA) { }

        // // RVA: 0x932A64 Offset: 0x932A64 VA: 0x932A64
        // private static int GJNFKJOCKKA(CACGCMBKHDI ADKIDBJCAJA) { }

        // // RVA: 0x932B64 Offset: 0x932B64 VA: 0x932B64
        // private static int CBILGOPCOIG(CACGCMBKHDI ADKIDBJCAJA) { }

        // // RVA: 0x932C38 Offset: 0x932C38 VA: 0x932C38
        // private static bool EDAAKPJIKLE(CACGCMBKHDI ADKIDBJCAJA) { }

        // // RVA: 0x932CD4 Offset: 0x932CD4 VA: 0x932CD4
        // private bool BKGNCCKFMPF(CACGCMBKHDI ADKIDBJCAJA, IMCBBOAFION BHFHGFKBOHH, IMCBBOAFION LNGMPEGMLBN, DJBHIFLHJLK AOCANKOMKFG) { }

        // [IteratorStateMachineAttribute] // RVA: 0x6C36E0 Offset: 0x6C36E0 VA: 0x6C36E0
        // // RVA: 0x932EDC Offset: 0x932EDC VA: 0x932EDC
        // private IEnumerator NFMEJDJCOOK(CACGCMBKHDI ADKIDBJCAJA, IMCBBOAFION BHFHGFKBOHH, IMCBBOAFION LNGMPEGMLBN, DJBHIFLHJLK AOCANKOMKFG) { }

        // [IteratorStateMachineAttribute] // RVA: 0x6C3758 Offset: 0x6C3758 VA: 0x6C3758
        // // RVA: 0x932E1C Offset: 0x932E1C VA: 0x932E1C
        // private IEnumerator EPGFMHDOBAA(CACGCMBKHDI KEABHGFLDJF, IMCBBOAFION BHFHGFKBOHH, IMCBBOAFION LNGMPEGMLBN, DJBHIFLHJLK AOCANKOMKFG) { }

        // [IteratorStateMachineAttribute] // RVA: 0x6C37D0 Offset: 0x6C37D0 VA: 0x6C37D0
        // // RVA: 0x932FF4 Offset: 0x932FF4 VA: 0x932FF4
        // private IEnumerator GGOECCJJPFN(CACGCMBKHDI ADKIDBJCAJA) { }

        // RVA: 0x9330A0 Offset: 0x9330A0 VA: 0x9330A0
        public PJKLMCGEJMK()
        {
            // NLGJBBGAOL=3
            // BLKIMNAILKK = 3.0
            // NNODMPKKCJH = 1
            // JDIBBDGNFKH = 1
            // OPFEFKOOMED
            // IEBFCJACLPN
            // HFMOEKIBNKA
            // DNMDKBFINPG
            FCICFIAOLAM_RequestList = new List<CACGCMBKHDI_Request>();
        }
}
