...
public class KNKDBNFMAKF_EventSp : IKDICBBFBMI_EventBase
{
	...
	public int CKBANLLONPF(long JHNMKKNEENE)
	{
		...
		if(ev != null && GPHEKCNDDIK)
		{
			...
			{
				//0x112CCC0
				return JHNMKKNEENE >= _GHPLINIACBB_x.PDBPFJJCADD_OpenAt && _GHPLINIACBB_x.FDBNFFNFOND_CloseAt >= JHNMKKNEENE;
			});
			if(l.Count > 0)
			{
	...
	public override long FBGDBGKNKOD_GetCurrentPoint()
	{
		...
		if(ev != null && GPHEKCNDDIK)
		{
			NILOACEHJKJ_EventSP.MDCMFPENCEK save = CIOECGOMILE.HHCJCDFCLOB.AHEFHIMGIBI_PlayerData.DAKCBBBMIMD_EventSP.FBCJICEPLED[ev.NGHKJOEDLIP.MOEKELIIDEO_SlotIdx];
	...
	protected override bool IMCMNOPNGHO(long JHNMKKNEENE)
	{
		...
		if(ev != null)
		{
			...
			if(b2 || save.EGBOHDFBAPB_CloseAt == 0 || (!RuntimeSettings.CurrentSettings.UnlimitedEvent && save.EGBOHDFBAPB_CloseAt < ev.NGHKJOEDLIP.OHGDNJLFDFF_Start))
			{
				save.LHPDDGIJKNB();
				if(ev.NGHKJOEDLIP.HKKNEAGCIEB != 0)
	...
	private IEnumerator NJIEIJJMAHK_Corotuine_PreSetupEventHome(IMCBBOAFION _BHFHGFKBOHH_OnSuccess, DJBHIFLHJLK _AOCANKOMKFG_OnError)
	{
		...
		if(KEHCNBNPJHB != null)
		{
			...
			{
				NILOACEHJKJ_EventSP.MDCMFPENCEK save = CIOECGOMILE.HHCJCDFCLOB.AHEFHIMGIBI_PlayerData.DAKCBBBMIMD_EventSP.FBCJICEPLED[KEHCNBNPJHB.NGHKJOEDLIP.MOEKELIIDEO_SlotIdx];
	...
	private NILOACEHJKJ_EventSP.MDCMFPENCEK DHKFENOKOAN()
	{
		...
		if(ev != null)
		{
            return CIOECGOMILE.HHCJCDFCLOB.AHEFHIMGIBI_PlayerData.DAKCBBBMIMD_EventSP.FBCJICEPLED[ev.NGHKJOEDLIP.MOEKELIIDEO_SlotIdx];
        }
		return null;
	}
	...
	public bool NEMGJHBEJCP(int _BCOKKAALGHC_Group, long JHNMKKNEENE, out long BFBBPOPIEEJ, out long IKBGGDNIJJH, out long HIBAPCCPIFD, out long PCEIHGCEAID)
	{
