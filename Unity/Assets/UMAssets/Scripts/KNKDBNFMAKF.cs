...
//XeApp.Game.Net.Event

[System.Obsolete("Use KNKDBNFMAKF_NetEventSpController", true)]
public class KNKDBNFMAKF { }
public class KNKDBNFMAKF_NetEventSpController : IKDICBBFBMI_NetEventBaseController
{
	...
	public override OHCAABOMEOF.KGOGMKMBCPP_EventType HIDHLFCBIDE_EventType { get { return OHCAABOMEOF.KGOGMKMBCPP_EventType.ENPJADLIFAB_EventSp; } } //0x1123014 DKHCGLCNKCD_bgs  Slot: 4
	...
	public KNKDBNFMAKF_NetEventSpController(string _OPFGFINHFCE_name) : base(_OPFGFINHFCE_name)
    {
        return;
    }
	...
	{
		OEIJEFBBJBD_EventSp ev = IMMAOANGPNK_NetMasterDataManager.HHCJCDFCLOB_Instance.NKEBMCIMJND_Database.LBDOLHGDIEB_Find(JOPOPMLFINI_QuestName) as OEIJEFBBJBD_EventSp;
	...
	public List<int> COILOHPIAFL(long _JHNMKKNEENE_Time, long NCHKMPGONNO)
	{
		...
		if(ev != null && GPHEKCNDDIK)
		{
			for(int i = 0; i < ev.ODACAJBCJHO.Count; i++)
			{
				...
				{
					if(ev.ODACAJBCJHO[i].PDBPFJJCADD_open_at >= NCHKMPGONNO && _JHNMKKNEENE_Time >= ev.ODACAJBCJHO[i].PDBPFJJCADD_open_at && ev.ODACAJBCJHO[i].FDBNFFNFOND_close_at >= _JHNMKKNEENE_Time)
					{
	...
	public int CKBANLLONPF(long _JHNMKKNEENE_Time)
	{
		...
		if(ev != null && GPHEKCNDDIK)
		{
			...
			{
				//0x112CCC0
				return _JHNMKKNEENE_Time >= _GHPLINIACBB_x.PDBPFJJCADD_open_at && _GHPLINIACBB_x.FDBNFFNFOND_close_at >= _JHNMKKNEENE_Time;
			});
			if(l.Count > 0)
			{
				l.Sort((OEIJEFBBJBD_EventSp.NJJEAKMBGPN _HKICMNAACDA_a, OEIJEFBBJBD_EventSp.NJJEAKMBGPN _BNKHBCBJBKI_b) =>
				{
					//0x112CC80
					return _BNKHBCBJBKI_b.BDEOMEBFDFF_gacha_id - _HKICMNAACDA_a.BDEOMEBFDFF_gacha_id;
				});
				return l[0].BDEOMEBFDFF_gacha_id;
			}
		}
		return 0;
	}
	...
	public override long FBGDBGKNKOD_GetCurrentPoint()
	{
		...
		if(ev != null && GPHEKCNDDIK)
		{
			NILOACEHJKJ_EventSP.MDCMFPENCEK save = CIOECGOMILE_NetPlayerDataManager.HHCJCDFCLOB_Instance.AHEFHIMGIBI_PlayerData.DAKCBBBMIMD_EventSP.FBCJICEPLED[ev.NGHKJOEDLIP_Settings.MOEKELIIDEO_SaveIdx];
	...
	public override void MJFKJHJJLMN_GetRanks(int LHJCOPMMIGO/* = 0*/, bool _FBBNPFFEJBN_Force/* = false*/)
	{
		...
		OEIJEFBBJBD_EventSp ev/*OHGNMDOGLJM*/ = IMMAOANGPNK_NetMasterDataManager.HHCJCDFCLOB_Instance.NKEBMCIMJND_Database.LBDOLHGDIEB_Find(JOPOPMLFINI_QuestName) as OEIJEFBBJBD_EventSp;
		if(ev != null && GPHEKCNDDIK)
		{
			...
			if(_FBBNPFFEJBN_Force || (NKGJPJPHLIF_SakashoManager.HHCJCDFCLOB_Instance.IBLPICFDGOF_ServerRequester.FJDBNGEPKHL_Time.KMEFBNBFJHI_GetServerTime() - KPOMHFLKMKI_LastRankUpdateTime[LHJCOPMMIGO] >= get_rank_cooling_time))
			{
				//LAB_01124374
	...
	protected override bool IMCMNOPNGHO(long _JHNMKKNEENE_Time)
	{
		...
		if(ev != null)
		{
			...
			if(b2 || save.EGBOHDFBAPB_closed_at == 0 || (!RuntimeSettings.CurrentSettings.UnlimitedEvent && save.EGBOHDFBAPB_closed_at < ev.NGHKJOEDLIP_Settings.OHGDNJLFDFF_Start))
			{
	...
	protected override void PJDGDNJNCNM_UpdateStatusImpl(long _JHNMKKNEENE_Time)
	{
		...
		{
			NILOACEHJKJ_EventSP.MDCMFPENCEK save = CIOECGOMILE_NetPlayerDataManager.HHCJCDFCLOB_Instance.AHEFHIMGIBI_PlayerData.DAKCBBBMIMD_EventSP.FBCJICEPLED[ev.NGHKJOEDLIP_Settings.MOEKELIIDEO_SaveIdx];
			s = KGCNCBOKCBA.GNENJEHKMHD_EventStatus.FFLKPBPBPEP_1_NotStarted;
			if(_JHNMKKNEENE_Time >= ev.NGHKJOEDLIP_Settings.OHGDNJLFDFF_Start)
			{
				if(DPJCPDKALGI_RankingEnd >= _JHNMKKNEENE_Time)
				{
					...
					if(!save.IMFBCJOIJKJ_Entry)
						s = KGCNCBOKCBA.GNENJEHKMHD_EventStatus.NCJFJLPJMLI_2_CanEntry;
					else
					{
						s = KGCNCBOKCBA.GNENJEHKMHD_EventStatus.MEAJLPAHINL_5_ChallengePeriod;
						if(MBHDIJJEOFL != null)
						{
							for(int i = 0; i < MBHDIJJEOFL.Count; i++)
							{
				...
				{
					s = KGCNCBOKCBA.GNENJEHKMHD_EventStatus.EMAMLLFAOJI_6_Counting;
					if(_JHNMKKNEENE_Time >= ev.NGHKJOEDLIP_Settings.JGMDAOACOJF_RewardStart)
					{
						...
						{
							s = KGCNCBOKCBA.GNENJEHKMHD_EventStatus.DOAENCHBAEO_11_Ended;
							if(_JHNMKKNEENE_Time >= ev.NGHKJOEDLIP_Settings.KNLGKBBIBOH_RewardEnd2)
							{
								s = KGCNCBOKCBA.GNENJEHKMHD_EventStatus.HINPDNKNAHO_10_Reward2;
							}
						}
						else
						{
				...
				req.BHFHGFKBOHH_OnSuccess = (CACGCMBKHDI_NetBaseAction KCFBGMKDIMI) =>
				{
					...
					else
					{
                        ...
                        KKLGENJKEBN_NetEventRankingManager.HHCJCDFCLOB_Instance.DNJKPPCBINA(KBACNOCOANM_Ranking.OCGFKMHNEOF_name_for_api, (IMCBBOAFION)(() =>
						{
                            ...
                            NILOACEHJKJ_EventSP.MDCMFPENCEK save = CIOECGOMILE_NetPlayerDataManager.HHCJCDFCLOB_Instance.AHEFHIMGIBI_PlayerData.DAKCBBBMIMD_EventSP.FBCJICEPLED[(int)ev.NGHKJOEDLIP_Settings.MOEKELIIDEO_SaveIdx];
							save.HPLMECLKFID_RRcv = true;
                            PLOOEECNHFB_IsDone = true;
						}), (DJBHIFLHJLK)(() =>
						{
	...
	protected override bool JLPDECMHLIM_CanShowStartAdventureInternal()
	{
		...
		{
			NILOACEHJKJ_EventSP.MDCMFPENCEK save = CIOECGOMILE_NetPlayerDataManager.HHCJCDFCLOB_Instance.AHEFHIMGIBI_PlayerData.DAKCBBBMIMD_EventSP.FBCJICEPLED[ev.NGHKJOEDLIP_Settings.MOEKELIIDEO_SaveIdx];
			if(save.PIEBOKFJKKH_RTodayCount != NKGJPJPHLIF_SakashoManager.HHCJCDFCLOB_Instance.DHMLDAGGKCD)
			{
	...
	public override void FGDDBFHGCGP_SetStartAdventureShown(bool _JKDJCFEBDHC_Enabled, long _JHNMKKNEENE_Time/* = 0*/)
	{
		if(_JKDJCFEBDHC_Enabled)
		{
			...
			{
				NILOACEHJKJ_EventSP.MDCMFPENCEK save = CIOECGOMILE_NetPlayerDataManager.HHCJCDFCLOB_Instance.AHEFHIMGIBI_PlayerData.DAKCBBBMIMD_EventSP.FBCJICEPLED[ev.NGHKJOEDLIP_Settings.MOEKELIIDEO_SaveIdx];
				save.PIEBOKFJKKH_RTodayCount = NKGJPJPHLIF_SakashoManager.HHCJCDFCLOB_Instance.DHMLDAGGKCD;
			}
	...
	private IEnumerator NJIEIJJMAHK_Corotuine_PreSetupEventHome(IMCBBOAFION _BHFHGFKBOHH_OnSuccess, DJBHIFLHJLK _AOCANKOMKFG_OnError)
	{
		...
		KEHCNBNPJHB = IMMAOANGPNK_NetMasterDataManager.HHCJCDFCLOB_Instance.NKEBMCIMJND_Database.LBDOLHGDIEB_Find(JOPOPMLFINI_QuestName) as OEIJEFBBJBD_EventSp;
		if(KEHCNBNPJHB != null)
		{
			...
			{
				NILOACEHJKJ_EventSP.MDCMFPENCEK save = CIOECGOMILE_NetPlayerDataManager.HHCJCDFCLOB_Instance.AHEFHIMGIBI_PlayerData.DAKCBBBMIMD_EventSP.FBCJICEPLED[KEHCNBNPJHB.NGHKJOEDLIP_Settings.MOEKELIIDEO_SaveIdx];
				...
				else
				{
					...
					if(save.LGADCGFMLLD_step < 2 && NGOFCFJHOMI_Status < KGCNCBOKCBA.GNENJEHKMHD_EventStatus.EMAMLLFAOJI_6_Counting)
					{
					...
					else if(save.LGADCGFMLLD_step != 2 || NGOFCFJHOMI_Status < KGCNCBOKCBA.GNENJEHKMHD_EventStatus.EMAMLLFAOJI_6_Counting)
					{
						//LAB_0112f248
					}
					else
	...
	public void POHBAGJLOLI_IncDivaCount(JJOELIOGMKK_DivaIntimacyInfo.OPOEENHEJOC_CounterType _INDDJNMPONH_type)
	{
		OEIJEFBBJBD_EventSp ev = IMMAOANGPNK_NetMasterDataManager.HHCJCDFCLOB_Instance.NKEBMCIMJND_Database.LBDOLHGDIEB_Find(JOPOPMLFINI_QuestName) as OEIJEFBBJBD_EventSp;
		if(ev != null)
		{
            NILOACEHJKJ_EventSP.MDCMFPENCEK save = CIOECGOMILE_NetPlayerDataManager.HHCJCDFCLOB_Instance.AHEFHIMGIBI_PlayerData.DAKCBBBMIMD_EventSP.FBCJICEPLED[ev.NGHKJOEDLIP_Settings.MOEKELIIDEO_SaveIdx];
			if(_INDDJNMPONH_type == JJOELIOGMKK_DivaIntimacyInfo.OPOEENHEJOC_CounterType.EMIFDDHCOFB_1_DivaPresent)
			{
				if(save.ECABPJAIIOM_DivaPresentCount < 9999)
					save.ECABPJAIIOM_DivaPresentCount++;
			}
			else if(_INDDJNMPONH_type == JJOELIOGMKK_DivaIntimacyInfo.OPOEENHEJOC_CounterType.FNGFADPFKOD_0_DivaIntimacy)
			{
				...
				return;
			HEFIKPAHCIA_UpdateMission(GBNDFCEDNMG.CJDGJFINBFH_ClearType.POHBAGJLOLI_26_IncDivaCount);
	...
	public int ACEEBCPOEBF_GetDivaCount(JJOELIOGMKK_DivaIntimacyInfo.OPOEENHEJOC_CounterType _INDDJNMPONH_type)
	{
		OEIJEFBBJBD_EventSp ev = IMMAOANGPNK_NetMasterDataManager.HHCJCDFCLOB_Instance.NKEBMCIMJND_Database.LBDOLHGDIEB_Find(JOPOPMLFINI_QuestName) as OEIJEFBBJBD_EventSp;
		if(ev != null)
		{
	...
	public int NJIKJJNLAPL(long _JHNMKKNEENE_Time, out long MGPEOHKLOEP, out long LFAFFICDFMJ_Done)
	{
		...
		if(ev != null)
		{
			...
			if(d != null)
			{
				...
				LFAFFICDFMJ_Done = d.FDBNFFNFOND_close_at;
	...
	public int IIPGGJAKIEO(int MMAIKCLDLDI)
	{
		...
		if(ev != null)
		{
			...
			{
				//0x112EA34
				return JPAEDJJFFOI.BDEOMEBFDFF_gacha_id == MMAIKCLDLDI;
			});
			if(d != null)
			{
				...
				{
					//0x112EA6C
					return d.PDBPFJJCADD_open_at >= JPAEDJJFFOI.PDBPFJJCADD_open_at && JPAEDJJFFOI.FDBNFFNFOND_close_at >= d.PDBPFJJCADD_open_at;
				});
				if(d2 != null)
	...
	public int EDHFKGEIAHB_GetHelpId(int AHGHKCGLFBG)
	{
		...
		if(ev != null)
		{
			int replace_quest_help_id = ev.LPJLEHAJADA_GetIntParam("replace_quest_help_id", 0);
			if(AHGHKCGLFBG > 0)
			{
				KHDPDECJHLD.Clear();
				KHDPDECJHLD.AppendFormat("replace_quest_help_id_{0}", AHGHKCGLFBG);
				int v = ev.LPJLEHAJADA_GetIntParam(KHDPDECJHLD.ToString(), 0);
				if(v > 0)
					return v;
			}
	...
	private NILOACEHJKJ_EventSP.MDCMFPENCEK DHKFENOKOAN()
	{
		...
		if(ev != null)
		{
            return CIOECGOMILE_NetPlayerDataManager.HHCJCDFCLOB_Instance.AHEFHIMGIBI_PlayerData.DAKCBBBMIMD_EventSP.FBCJICEPLED[ev.NGHKJOEDLIP_Settings.MOEKELIIDEO_SaveIdx];
        }
		return null;
	}
	...
	public int MEDEJHKNAFG_GetCurrentMissionGroup(long LPEKHFOMCAH)
	{
		...
		if(ev != null)
		{
			...
			{
				//0x112EAE8
				return LPEKHFOMCAH >= JPAEDJJFFOI.PDBPFJJCADD_open_at && JPAEDJJFFOI.FDBNFFNFOND_close_at >= LPEKHFOMCAH && JPAEDJJFFOI.PLALNIIBLOF_en == 2;
			});
			if(d != null)
			{
	...
	{
		return MEDEJHKNAFG_GetCurrentMissionGroup(NKGJPJPHLIF_SakashoManager.HHCJCDFCLOB_Instance.IBLPICFDGOF_ServerRequester.FJDBNGEPKHL_Time.KMEFBNBFJHI_GetServerTime());
	}
	...
	public bool NEMGJHBEJCP(int _BCOKKAALGHC_Group, long _JHNMKKNEENE_Time, out long BFBBPOPIEEJ, out long IKBGGDNIJJH, out long HIBAPCCPIFD, out long PCEIHGCEAID)
	{
		...
		if(ev != null)
		{
			...
			{
				if(_JHNMKKNEENE_Time >= l[i].PDBPFJJCADD_open_at && l[i].FDBNFFNFOND_close_at >= _JHNMKKNEENE_Time)
					v = i;
			}
			if(l.Count > 0)
			{
				BFBBPOPIEEJ = l[v].PDBPFJJCADD_open_at;
				PCEIHGCEAID = l[v].FDBNFFNFOND_close_at;
				KHDPDECJHLD.Clear();
	...
	public int JLLGJIIHFJO_GetTitleTextureId(int AHGHKCGLFBG)
	{
		...
		if(ev != null)
		{
			...
			if(v < 1)
			{
				v = ev.LPJLEHAJADA_GetIntParam("quest_title_texture_id_0", 0);
				if(v < 1)
	...
	public int CBEACDJOIBO_GetIconTextureId(int AHGHKCGLFBG)
	{
		...
		if(ev != null)
		{
			...
			if(v < 1)
			{
				v = ev.LPJLEHAJADA_GetIntParam("quest_icon_texture_id_0", 0);
				if(v < 1)
					v = PGIIDPEGGPI_EventId;
			}
			return v;
