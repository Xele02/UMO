...
namespace CriWare.CriMana.Detail
{
	...
	{
        [RendererResourceFactoryPriority(0)]
        public class RendererResourceFactoryEditorVlcSofdecPrimeYuv : RendererResourceFactory
        {
            // Slot: 7
            public override RendererResource CreateRendererResource(int playerId, MovieInfo movieInfo, bool additive, Shader userShader)
            {
                if(movieInfo.codecType == CodecType.SofdecPrime && VLCManager.Instance != null)
                {
                    return new RendererResourceEditorVlcSofdecPrimeYuv(playerId, movieInfo, additive, userShader);
                }
                return null;
            }
    ...
    public class RendererResourceEditorVlcSofdecPrimeYuv : RendererResource
    {
        ...
        public RendererResourceEditorVlcSofdecPrimeYuv(int playerId, MovieInfo movieInfo, bool additive, Shader userShader)
        {
            ...
            if(userShader == null)
                userShader = Shader.Find("CriMana/SofdecEditorYuv");
            shader = userShader;
            /*if(hasAlpha)
            {
