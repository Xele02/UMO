...
public class OEGIPPCADNA
{
	...
	{
		KKLGENJKEBN.HHCJCDFCLOB.JPNACOLKHLB(CJHEHIMLGGL, NEFEFHBHFFF, (int JGNBPFCJLKI, List<IBIGBMDANNM> MAGKKPOFJIM) =>
		{
			//0x1B3E848
			PLOOEECNHFB_IsDone = true;
			BMKBAMFBAPJ.Clear();
	...
	private void EJHFJGKPHCM(int NEFEFHBHFFF, List<IBIGBMDANNM> NNDGIAEFMOG, int KKGKLNDOCFI)
	{
		if(NNDGIAEFMOG != null)
		{
			...
			{
				RankingListInfo r = HGGPIBNLALJ.Find((RankingListInfo GHPLINIACBB) =>
				{
					//0x1B3EE40
					return GHPLINIACBB.friend.MLPEHNBNOGD_Id == NNDGIAEFMOG[i].MLPEHNBNOGD_Id;
				});
				if(r == null)
				{
	...
	private void FBKCFKPLMAL(List<JCDOJGNNIIL> PFEGHILDOGF)
	{
		...
		for (int i = 0; i < PFEGHILDOGF.Count; i++)
		{
			...
			for(int k = 0; k < PFEGHILDOGF[i].LJNPPPOBHFK.Count; k++)
			{
				...
				j.NGEPPDAOIBN(EKLNMHFCAOI.BKHFLDMOGBD_GetItemCategory(id), EKLNMHFCAOI.DEACAHNLMNI_getItemId(id), PFEGHILDOGF[i].CCAPNEAOIBP[k], 0, EKLNMHFCAOI.FABCKNDLPDH_GetItemRarity(EKLNMHFCAOI.BKHFLDMOGBD_GetItemCategory(id), EKLNMHFCAOI.DEACAHNLMNI_getItemId(id)));
			}
		}
	}
}
