...
public class KEPNMGHABPI
{
    ...
    public class AELPHOILEBM
    {
        ...
        public void KHEKNNFCAOI(KEPNMGHABPI PAKIOKPJOBF)
        {
			...
			MLPLGFLKKLI = 30;
			IGNIIEBMFIN = 1;
			ADCMNODJBGJ = PAKIOKPJOBF.OJAGHKJAJEJ.Title;
			DFJDDIGFGKJ = 2;
			MLMHPBOKJCL = 1;
		}
    }
    ...
    public void OMEFFONNMBC(int KPNKPGLPDHI, Action<LNCLNAOPNKF> BHFHGFKBOHH, DJBHIFLHJLK MOBEEPPKFLG, SakashoBbsThreadCriteria IPKCADIAAPG)
    {
		...
		data.IGNIIEBMFIN = 1;
		data.MLMHPBOKJCL = 0;
		data.ADCMNODJBGJ = OJAGHKJAJEJ.Title;
		HJCEIODJLOJ(KPNKPGLPDHI, data, (List<LNCLNAOPNKF> NMHGLNGACMI) =>
		{
			...
			if(NMHGLNGACMI.IsEmpty())
			{
				...
				{
					//0x19F8808
					return OHMCIEMIKCE.ECAIHFNAAOM_Title == data.ADCMNODJBGJ;
				};
	...
	public void FNKJBKJIKPC(int KPNKPGLPDHI, bool PAEAHCEJLKG, Action<List<LNCLNAOPNKF>> BHFHGFKBOHH, DJBHIFLHJLK MOBEEPPKFLG, KEPNMGHABPI.AELPHOILEBM NGHKJOEDLIP, SakashoBbsThreadCriteria IPKCADIAAPG)
	{
		if(NGHKJOEDLIP == null)
		{
			...
			NGHKJOEDLIP.ADCMNODJBGJ = OJAGHKJAJEJ.Title;
		}
	...
	public void HMOPLMHMNNP(int KPNKPGLPDHI, int BOINIEAKFPL, EIAMFNIAFGN NGHKJOEDLIP, Action<NFIMGIABIOI_GetBbsThreadComments.CIIDLDOOKBB> BHFHGFKBOHH, DJBHIFLHJLK MOBEEPPKFLG)
	{
		if((KPNKPGLPDHI & 1) == 0)
		{
	...
	{
		if(!string.IsNullOrEmpty(NGHKJOEDLIP.ADCMNODJBGJ))
		{
			JCIBAFAILAG.RemoveAll((JKHBHIGMJIC OHMCIEMIKCE) =>
			{
				//0x19F90FC
				return OHMCIEMIKCE.ADCMNODJBGJ_Title != NGHKJOEDLIP.ADCMNODJBGJ;
			});
		}
		bool b = JCIBAFAILAG.IsEmpty() && !CBNPNMPLJPP;
		if (!JCIBAFAILAG.IsEmpty() && !CBNPNMPLJPP)
		{
			JCIBAFAILAG.Sort(new DGGNPDDJNOO());
			b = JCIBAFAILAG[JCIBAFAILAG.Count - 1].ADGNGHIJFFC_MaxComments <= JCIBAFAILAG[JCIBAFAILAG.Count - 1].OMMDEKMFBAF_CommentsCount;
		}
		int cnt = JCIBAFAILAG.Count;
		if (b)
			cnt++;
		List<LNCLNAOPNKF> l = new List<LNCLNAOPNKF>();
	...
	private void OCKOIMLGLCF(int KPNKPGLPDHI, List<List<CAIPMAMHNJP>> GHIHLNKPKGH, int POMLAOPLMNA, int HMFFHLPNMPH, List<KEPNMGHABPI.LNCLNAOPNKF> NMHGLNGACMI, KEPNMGHABPI.EIAMFNIAFGN NGHKJOEDLIP, Action<List<KEPNMGHABPI.CAIPMAMHNJP>> BHFHGFKBOHH, DJBHIFLHJLK MOBEEPPKFLG)
	{
		...
		HMOPLMHMNNP(KPNKPGLPDHI, GNAOMJEPCLO.BOINIEAKFPL_Id, NGHKJOEDLIP, (NFIMGIABIOI_GetBbsThreadComments.CIIDLDOOKBB CHCEIJOJDHL) =>
		{
			//0x19FAD18
