...
public class LAMCONGFONF
{
	...
	public void FAMFKPBPIAA_GetRankingPlayerList(int LHJCOPMMIGO, bool PFFJNEFNAMI, int CJHEHIMLGGL, IMCBBOAFION KLMFJJCNBIP, DJBHIFLHJLK IDAEHNGOKAE, DJBHIFLHJLK JGKOLBLPMPG)
	{
		DPPIBCENJJJ = false;
		...
		else
		{
			...
			KKLGENJKEBN.HHCJCDFCLOB.FAMFKPBPIAA_GetRankingPlayerList(e.OCGFKMHNEOF_NameForApi, PFFJNEFNAMI, CJHEHIMLGGL, LHJCOPMMIGO, (int NEFEFHBHFFF, List<IBIGBMDANNM> MAGKKPOFJIM) =>
			{
				...
				if(MAGKKPOFJIM != null)
				{
					...
					{
						//0xD93100
						return _GHPLINIACBB_x.MLPEHNBNOGD_Id == ANKHNOGKHDD;
					});
					if(d != null)
					{
	...
	public void JPNACOLKHLB(int CJHEHIMLGGL, int NEFEFHBHFFF, int KKGKLNDOCFI, IMCBBOAFION KLMFJJCNBIP, DJBHIFLHJLK IDAEHNGOKAE, DJBHIFLHJLK JGKOLBLPMPG)
	{
		...
		KKLGENJKEBN.HHCJCDFCLOB.JPNACOLKHLB_AddRankingPlayerListSecond(CJHEHIMLGGL, NEFEFHBHFFF, (int JGNBPFCJLKI, List<IBIGBMDANNM> MAGKKPOFJIM) =>
		{
			...
			else
			{
				...
				CBDFJFCGKNK(JGNBPFCJLKI, MAGKKPOFJIM, 0);
			}
			if(KLMFJJCNBIP != null)
				KLMFJJCNBIP();
		}, () =>
	...
	public void AMKJFGLEJGE_RequestUpdateEventPoint(int LHJCOPMMIGO, int _ABLHIAEDJAI_CurrentValue, long JHNMKKNEENE)
	{
		EECOJKDJIFG e = DAKMIKNKHMF(LHJCOPMMIGO);
		if(e != null)
		{
			...
			{
				EECOJKDJIFG ee = KKLGENJKEBN.HHCJCDFCLOB.JPDPFGFMKHK.Find((EECOJKDJIFG _GHPLINIACBB_x) =>
				{
					//0xD938B8
				...
				if (ee != null)
				{
					...
					data.OBGBAOLONDD_Unq = IMMAOANGPNK.HHCJCDFCLOB.NKEBMCIMJND_Database.EFHMLHKODOD_EventGoDivaRanking.NGHKJOEDLIP.OBGBAOLONDD;
	...
	public void FGMOMBKGCNF(int LHJCOPMMIGO, int _ABLHIAEDJAI_CurrentValue/* = 0*/, IMCBBOAFION KLMFJJCNBIP/* = null*/, DJBHIFLHJLK NIMPEHIECJH/* = null*/, DJBHIFLHJLK JGKOLBLPMPG/* = null*/)
	{
		PLOOEECNHFB_IsDone = false;
		NPNNPNAIONN_IsError = false;
		EECOJKDJIFG e = DAKMIKNKHMF(LHJCOPMMIGO);
		...
		KKLGENJKEBN.HHCJCDFCLOB.LDOBHAAIDEJ_UpdateRankingScore(e.OCGFKMHNEOF_NameForApi, LHJCOPMMIGO, a, () =>
		{
			//0xD93904
	...
	private void EJHFJGKPHCM(int NEFEFHBHFFF, List<IBIGBMDANNM> NNDGIAEFMOG, int KKGKLNDOCFI)
	{
		if(NNDGIAEFMOG != null)
		{
			for(int i = 0; i < NNDGIAEFMOG.Count; i++)
			{
				...
				{
					//0xD93BB4
					return NNDGIAEFMOG[i].MLPEHNBNOGD_Id == _GHPLINIACBB_x.friend.MLPEHNBNOGD_Id;
				});
				if(info == null)
				{
			...
			if(NNDGIAEFMOG.Count > 0)
			{
				...
				for(int i = 0; i < NNDGIAEFMOG.Count; i++)
				{
					...
					if(info == null)
					{
						...
						finfo.KHEKNNFCAOI(NNDGIAEFMOG[i]);
