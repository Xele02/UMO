...
public class LAMCONGFONF
{
	...
	public static LAMCONGFONF HHCJCDFCLOB_Instance { get; private set; } // 0x0 NKACBOEHELJ_bgs OKPMHKNCNAL_bgs
	...
	{
		JPJGOECJFEE_EventGoDivaRanking dbGo = IMMAOANGPNK_NetMasterDataManager.HHCJCDFCLOB_Instance.NKEBMCIMJND_Database.EFHMLHKODOD_EventGoDivaRanking;
		if(dbGo != null)
		{
			for(int i = 0; i < dbGo.NGHKJOEDLIP_Settings.MPCAGEPEJJJ_Key.Count; i++)
			{
				...
				AEJCLHOIDDD[i] = KKLGENJKEBN_NetEventRankingManager.HHCJCDFCLOB_Instance.JPDPFGFMKHK_rankings.Find((EECOJKDJIFG PKLPKMLGFGK) => {
					//0xD92A98
	...
	public List<int> IFHPGJGLPPF_GetRankingThreshold()
	{
		...
		if(IMMAOANGPNK_NetMasterDataManager.HHCJCDFCLOB_Instance.NKEBMCIMJND_Database.EFHMLHKODOD_EventGoDivaRanking != null)
		{
			res.AddRange(IMMAOANGPNK_NetMasterDataManager.HHCJCDFCLOB_Instance.NKEBMCIMJND_Database.EFHMLHKODOD_EventGoDivaRanking.NGHKJOEDLIP_Settings.JHPCPNJJHLI_RankingThreshold);
	...
	public void FAMFKPBPIAA_GetRankingPlayerList(int LHJCOPMMIGO, bool PFFJNEFNAMI, int _CJHEHIMLGGL_Position, IMCBBOAFION _KLMFJJCNBIP_OnSuccess, DJBHIFLHJLK _IDAEHNGOKAE_OnRankingError, DJBHIFLHJLK _JGKOLBLPMPG_OnFail)
	{
		...
		else
		{
			...
			KKLGENJKEBN_NetEventRankingManager.HHCJCDFCLOB_Instance.FAMFKPBPIAA_GetRankingPlayerList(e.OCGFKMHNEOF_name_for_api, PFFJNEFNAMI, _CJHEHIMLGGL_Position, LHJCOPMMIGO, (int NEFEFHBHFFF, List<IBIGBMDANNM> MAGKKPOFJIM) =>
			{
				...
				if(MAGKKPOFJIM != null)
				{
					int ANKHNOGKHDD = NKGJPJPHLIF_SakashoManager.HHCJCDFCLOB_Instance.CAFHLEFMMGD_GetPlayerId();
	...
	public void HEOKADCEAGL_GetRanks(int LHJCOPMMIGO, bool PFFJNEFNAMI, IMCBBOAFION _KLMFJJCNBIP_OnSuccess, DJBHIFLHJLK _IDAEHNGOKAE_OnRankingError, DJBHIFLHJLK _JGKOLBLPMPG_OnFail)
	{
		PLOOEECNHFB_IsDone = false;
		NPNNPNAIONN_IsError = false;
		EECOJKDJIFG e = DAKMIKNKHMF_GetRankingInfoForIndex(LHJCOPMMIGO);
		...
		else
		{
			...
			{
				KKLGENJKEBN_NetEventRankingManager.HHCJCDFCLOB_Instance.HEOKADCEAGL_GetRanks(e.OCGFKMHNEOF_name_for_api, () =>
				{
					//0xD93350
					ONJBNDCMMNL[LHJCOPMMIGO].FJOLNJLLJEJ_rank = (int)KKLGENJKEBN_NetEventRankingManager.HHCJCDFCLOB_Instance.LPPCNCMEDFA_Rank;
					ONJBNDCMMNL[LHJCOPMMIGO].DNBFMLBNAEE_point = (int)KKLGENJKEBN_NetEventRankingManager.HHCJCDFCLOB_Instance.IPNEJCOFBIB_ScoreTrunc;
					ONJBNDCMMNL[LHJCOPMMIGO].BEKAMBBOLBO_Done = true;
					PLOOEECNHFB_IsDone = true;
					if (_KLMFJJCNBIP_OnSuccess != null)
						_KLMFJJCNBIP_OnSuccess();
				}, () =>
				{
					...
					ONJBNDCMMNL[LHJCOPMMIGO].FJOLNJLLJEJ_rank = 0;
					ONJBNDCMMNL[LHJCOPMMIGO].DNBFMLBNAEE_point = 0;
					ONJBNDCMMNL[LHJCOPMMIGO].BEKAMBBOLBO_Done = KKLGENJKEBN_NetEventRankingManager.HHCJCDFCLOB_Instance.CJMFJOMECKI_ErrorId == SakashoErrorId.RANKING_PLAYER_NOT_FOUND;
					PLOOEECNHFB_IsDone = true;
					if (_IDAEHNGOKAE_OnRankingError != null)
						_IDAEHNGOKAE_OnRankingError();
				}, () =>
	...
	public void AMKJFGLEJGE_RequestUpdateEventPoint(int LHJCOPMMIGO, int _ABLHIAEDJAI_CurrentValue, long _JHNMKKNEENE_Time)
	{
		...
		if(e != null)
		{
			if(IMMAOANGPNK_NetMasterDataManager.HHCJCDFCLOB_Instance.NKEBMCIMJND_Database.EFHMLHKODOD_EventGoDivaRanking != null && IMMAOANGPNK_NetMasterDataManager.HHCJCDFCLOB_Instance.NKEBMCIMJND_Database.EFHMLHKODOD_EventGoDivaRanking.NGHKJOEDLIP_Settings != null)
			{
				EECOJKDJIFG ee = KKLGENJKEBN_NetEventRankingManager.HHCJCDFCLOB_Instance.JPDPFGFMKHK_rankings.Find((EECOJKDJIFG _GHPLINIACBB_x) =>
				{
					//0xD938B8
				...
				if (ee != null)
				{
					...
					data.OBGBAOLONDD_UniqueId = IMMAOANGPNK_NetMasterDataManager.HHCJCDFCLOB_Instance.NKEBMCIMJND_Database.EFHMLHKODOD_EventGoDivaRanking.NGHKJOEDLIP_Settings.OBGBAOLONDD_UniqueId;
					data.NOBCHBHEPNC_Idx = LHJCOPMMIGO;
					JGEOBNENMAH_NetGameManager.HHCJCDFCLOB_Instance.CBGMOGIBALL.Add(data);
					KIAFBCEKMEA[LHJCOPMMIGO].BEKAMBBOLBO_Done = false;
					CEPDOODGDEM[LHJCOPMMIGO].BEKAMBBOLBO_Done = false;
				}
			}
	...
	public void FGMOMBKGCNF_UpdateRankingValue(int LHJCOPMMIGO, int _ABLHIAEDJAI_CurrentValue/* = 0*/, IMCBBOAFION _KLMFJJCNBIP_OnSuccess/* = null*/, DJBHIFLHJLK NIMPEHIECJH/* = null*/, DJBHIFLHJLK _JGKOLBLPMPG_OnFail/* = null*/)
	{
		PLOOEECNHFB_IsDone = false;
		NPNNPNAIONN_IsError = false;
		EECOJKDJIFG e = DAKMIKNKHMF_GetRankingInfoForIndex(LHJCOPMMIGO);
		...
		KKLGENJKEBN_NetEventRankingManager.HHCJCDFCLOB_Instance.LDOBHAAIDEJ_UpdateRankingScore(e.OCGFKMHNEOF_name_for_api, LHJCOPMMIGO, a, () =>
		{
			//0xD93904
			KIAFBCEKMEA[LHJCOPMMIGO].FJOLNJLLJEJ_rank = (int)KKLGENJKEBN_NetEventRankingManager.HHCJCDFCLOB_Instance.LPPCNCMEDFA_Rank;
			KIAFBCEKMEA[LHJCOPMMIGO].DNBFMLBNAEE_point = (int)KKLGENJKEBN_NetEventRankingManager.HHCJCDFCLOB_Instance.IPNEJCOFBIB_ScoreTrunc;
	...
	//public static int BFKAHKBKBJE(int FJOLNJLLJEJ_rank, long EAKFLINAPOG) { }
}
