...
public class FFHPBEPOMAK
{
	public class CLKBDNBMJCO
	{
		...
		{
			DEKKMGAFJCG.MNNLOBDPCCH a = CIOECGOMILE.HHCJCDFCLOB.AHEFHIMGIBI_Save.DGCJCAHIAPP.NBIGLBMHEDC[AHHJLDLAPAN - 1];
			IJNIDKLFABD = a.MMCEMJILMJI ^ FBGGEFFJJHB;
			JHKNDAGGOMG = a.HDPANGMKKCP ^ FBGGEFFJJHB;
			KLDFJGOKIKC = a.FFMLBEEBHDD ^ FBGGEFFJJHB;
		}
	...
	private BBHNACPENDM LDEGEHAEALK; // 0x78
	...
	public void KHEKNNFCAOI(int AHHJLDLAPAN_DivaId, BBHNACPENDM NIMOGBDCMLJ, bool OJEBNBLHPNP = false)
	{
		BNFDBPPOAOE = true;
		EEBBAGGJOGH = false;
		if(NIMOGBDCMLJ == null)
		...
		PKLPGBKKFOL = LDEGEHAEALK.DGCJCAHIAPP.NBIGLBMHEDC[AHHJLDLAPAN_DivaId - 1].ANAJIAENLNB;
		HMBECPGHPOE = LDEGEHAEALK.DGCJCAHIAPP.NBIGLBMHEDC[AHHJLDLAPAN_DivaId - 1].LKIFDCEKDCK;
		FOHEIOLPAOG = LDEGEHAEALK.DGCJCAHIAPP.NBIGLBMHEDC[AHHJLDLAPAN_DivaId - 1].KBOLNIBLIND;
		FJODMPGPDDD = LDEGEHAEALK.DGCJCAHIAPP.NBIGLBMHEDC[AHHJLDLAPAN_DivaId - 1].CPGFPEDMDEH != 0;
		IPJMPBANBPP = IMMAOANGPNK.HHCJCDFCLOB.NKEBMCIMJND_Database.MGFMPKLLGHE_Diva.CDENCMNHNGA[AHHJLDLAPAN_DivaId - 1].PPEGAKEIEGM == 2;
		CIEOBFIIPLD = LDEGEHAEALK.DGCJCAHIAPP.NBIGLBMHEDC[AHHJLDLAPAN_DivaId - 1].OKMELNIIMMO();
		...
		for(int i = 0; i < LDEGEHAEALK.DGCJCAHIAPP.NBIGLBMHEDC[AHHJLDLAPAN_DivaId - 1].JCHHIPOPNIN; i++)
		{
			int val = LDEGEHAEALK.DGCJCAHIAPP.NBIGLBMHEDC[AHHJLDLAPAN_DivaId - 1].EBDNICPAFLB[i];
			DJICAKGOGFO.Add(val);
			if(val == 0)
			{
			...
			{
				FGBKIPNGGJM.Add(LDEGEHAEALK.PNLOINMCCKH.OPIBAPEGCLA[val].PDNIFBEGMHC);
				GDJPNDKMKPG.Add(LDEGEHAEALK.PNLOINMCCKH.OPIBAPEGCLA[val].EMOJHJGHJLN);
				HHKFOOFPOJL.Add(LDEGEHAEALK.PNLOINMCCKH.OPIBAPEGCLA[val].JPIPENJGGDD);
			}
		}
		FGFIBOBAPIA = LDEGEHAEALK.DGCJCAHIAPP.NBIGLBMHEDC[AHHJLDLAPAN_DivaId - 1].PIGLAEFPNEK;
		if(FGFIBOBAPIA != 0)
		{
			FFKMEFKOBHO = LDEGEHAEALK.PNLOINMCCKH.OPIBAPEGCLA[FGFIBOBAPIA].PDNIFBEGMHC;
			PFCJIKGENEH = LDEGEHAEALK.PNLOINMCCKH.OPIBAPEGCLA[FGFIBOBAPIA].EMOJHJGHJLN;
			AICGALGPFMO = LDEGEHAEALK.PNLOINMCCKH.OPIBAPEGCLA[FGFIBOBAPIA].JPIPENJGGDD;
		}
		JPIDIENBGKH = LDEGEHAEALK.DGCJCAHIAPP.NBIGLBMHEDC[AHHJLDLAPAN_DivaId - 1].BEEAIAAJOHD;
		EKFONBFDAAP_ColorId = LDEGEHAEALK.DGCJCAHIAPP.NBIGLBMHEDC[AHHJLDLAPAN_DivaId - 1].AFNIOJHODAG;
		FFKMJNHFFFL.KHEKNNFCAOI(AHHJLDLAPAN_DivaId, JPIDIENBGKH, LDEGEHAEALK, OJEBNBLHPNP);
		KIIMFCFMMDN = LDEGEHAEALK.DGCJCAHIAPP.NBIGLBMHEDC[AHHJLDLAPAN_DivaId - 1].HPJMPINPKEP;
		JFFLFIMIMOI = LDEGEHAEALK.DGCJCAHIAPP.NBIGLBMHEDC[AHHJLDLAPAN_DivaId - 1].KKEPMONFGEI;
		EGAFMGDFFCH.KHEKNNFCAOI(AHHJLDLAPAN_DivaId, LDEGEHAEALK.DGCJCAHIAPP.NBIGLBMHEDC[AHHJLDLAPAN_DivaId - 1].HPJMPINPKEP, 
	...
	public void KHEKNNFCAOI(int AHHJLDLAPAN_DivaId, int PAOGPLDOMMI, int PDEEMMEHDPK, int KOHAAFNNBOE, GCIJNCFDNON AFBMEMCHJCL, List<GCIJNCFDNON> HDJOHAJPGBA, bool LGNOCBFCAAK = false)
	{
		...
		{
			FFKMJNHFFFL.KHEKNNFCAOI(AHHJLDLAPAN_DivaId,PDEEMMEHDPK,CIOECGOMILE.HHCJCDFCLOB.AHEFHIMGIBI_Save,false);
		}
		FJODMPGPDDD = false;
	...
	public void HCDGELDHFHB()
	{
		...
		{
			JLJGCBOHJID_Status.Add(CIOECGOMILE.HHCJCDFCLOB.AHEFHIMGIBI_Save.BEKHNNCGIEL.NNIKNCGNDHK(AHHJLDLAPAN_DivaId));
		}
