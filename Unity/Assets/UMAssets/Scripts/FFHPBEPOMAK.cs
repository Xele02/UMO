...
public class FFHPBEPOMAK_DivaInfo
{
	...
	public void KHEKNNFCAOI(int AHHJLDLAPAN_DivaId, BBHNACPENDM_ServerSaveData NIMOGBDCMLJ, bool OJEBNBLHPNP = false)
	{
		...
		FJODMPGPDDD = LDEGEHAEALK.DGCJCAHIAPP_Diva.NBIGLBMHEDC_DivaList[AHHJLDLAPAN_DivaId - 1].CPGFPEDMDEH_Have != 0/* || RuntimeSettings.CurrentSettings.ForceDivaUnlock*/;
		IPJMPBANBPP = IMMAOANGPNK.HHCJCDFCLOB.NKEBMCIMJND_Database.MGFMPKLLGHE_Diva.CDENCMNHNGA_Divas[AHHJLDLAPAN_DivaId - 1].PPEGAKEIEGM_Enabled == 2;
		...
		for(int i = 0; i < LDEGEHAEALK.DGCJCAHIAPP_Diva.NBIGLBMHEDC_DivaList[AHHJLDLAPAN_DivaId - 1].JCHHIPOPNIN; i++)
		{
			...
			else
			{
				...
				GDJPNDKMKPG.Add(LDEGEHAEALK.PNLOINMCCKH_Scene.OPIBAPEGCLA[val].EMOJHJGHJLN_Sb);
		...
		if(FGFIBOBAPIA_SceneId != 0)
		{
			...
			PFCJIKGENEH = LDEGEHAEALK.PNLOINMCCKH_Scene.OPIBAPEGCLA[FGFIBOBAPIA_SceneId].EMOJHJGHJLN_Sb;
	...
	public void KHEKNNFCAOI(int AHHJLDLAPAN_DivaId, int PAOGPLDOMMI_Level, int PDEEMMEHDPK_CostumeId, int KOHAAFNNBOE_ColorId, GCIJNCFDNON_SceneInfo AFBMEMCHJCL_MainScene, List<GCIJNCFDNON_SceneInfo> HDJOHAJPGBA_SubScene, bool LGNOCBFCAAK = false)
	{
		...
		FJODMPGPDDD = false;
		IPJMPBANBPP = d.PPEGAKEIEGM_Enabled == 2;
		OPFGFINHFCE_DivaName = EJOJNFDHDHN_GetName(AHHJLDLAPAN_DivaId);
			...
			for(int i = 0; i < HDJOHAJPGBA_SubScene.Count; i++)
			{
				...
				else
				{
					...
					else
					{
						...
						HHKFOOFPOJL.Add(HDJOHAJPGBA_SubScene[i].JPIPENJGGDD_NumBoard);
					}
				}
			}
		}
		...
		{
			AICGALGPFMO = AFBMEMCHJCL_MainScene.JPIPENJGGDD_NumBoard;
		...
		else
		{
			...
			if(!ILBBPPMLLFM)
			{
				...
				else
				{
					...
					PFCJIKGENEH = LDEGEHAEALK.PNLOINMCCKH_Scene.OPIBAPEGCLA[BCCHOBPJJKE - 1].EMOJHJGHJLN_Sb;
	...
	public bool IFFMDJHENHB(int IMJIADPJJMM, int BCCHOBPJJKE, bool ILBBPPMLLFM, KDMCFCBMAOI BBEBHGEHMMI, BLHOMPPGBMI NGMPDMCBNJB)
	{
		...
		else
		{
			...
			else
			{
				...
				GDJPNDKMKPG[IMJIADPJJMM] = LDEGEHAEALK.PNLOINMCCKH_Scene.OPIBAPEGCLA[BCCHOBPJJKE - 1].EMOJHJGHJLN_Sb;
	...
	public void ODFBCANBLIJ(GCIJNCFDNON_SceneInfo PNLOINMCCKH)
	{
		...
		else
		{
			...
			FFKMEFKOBHO = PNLOINMCCKH.KBOLNIBLIND;
			PFCJIKGENEH = PNLOINMCCKH.ODKMKEHJOCK;
			AICGALGPFMO = PNLOINMCCKH.JPIPENJGGDD_NumBoard;
		}
		ELHBGKLLOIO();
	}
	...
	public void GOEDGNGDMCN(GCIJNCFDNON_SceneInfo PNLOINMCCKH, int IMJIADPJJMM)
	{
		...
		else
		{
			...
			FGBKIPNGGJM[IMJIADPJJMM] = PNLOINMCCKH.KBOLNIBLIND;
			GDJPNDKMKPG[IMJIADPJJMM] = PNLOINMCCKH.ODKMKEHJOCK;
			HHKFOOFPOJL[IMJIADPJJMM] = PNLOINMCCKH.JPIPENJGGDD_NumBoard;
		}
		ELHBGKLLOIO();
	}
