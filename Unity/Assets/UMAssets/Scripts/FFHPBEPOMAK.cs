	...
	public void KHEKNNFCAOI(int AHHJLDLAPAN_DivaId, int PAOGPLDOMMI, int PDEEMMEHDPK, int KOHAAFNNBOE, GCIJNCFDNON AFBMEMCHJCL, List<GCIJNCFDNON> HDJOHAJPGBA, bool LGNOCBFCAAK = false)
	{
		...
		if(HDJOHAJPGBA != null)
		{
			...
			for(int i = 0; i < HDJOHAJPGBA.Count; i++)
			{
				...
				{
					if(HDJOHAJPGBA[i].BCCHOBPJJKE_SceneId == 0)
					{
						DJICAKGOGFO.Add(0);
						FGBKIPNGGJM.Add(null);
						GDJPNDKMKPG.Add(null);
					...
					{
						DJICAKGOGFO.Add(HDJOHAJPGBA[i].BCCHOBPJJKE_SceneId);
		...
		if(AFBMEMCHJCL == null || AFBMEMCHJCL.BCCHOBPJJKE_SceneId == 0)
		{
			AICGALGPFMO = 0;
			FFKMEFKOBHO = null;
			FGFIBOBAPIA = 0;
		...
		else
		{
			...
			FGFIBOBAPIA = AFBMEMCHJCL.BCCHOBPJJKE_SceneId;
