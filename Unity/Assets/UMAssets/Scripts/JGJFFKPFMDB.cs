...
public enum GOBDOEHKLHN
{
	...
	FJGMPDJFELN_4 = 4,
	KLPAIDGGGCN_5 = 5,
	GBMFNHOFGOP_6_Purchase = 6,
	IPGGCCPIMMI_7_Cancel = 7,
	EJHJJEBEHMG_8 = 8,
}
...
public class JGJFFKPFMDB
{
		...
		if(_PPFNGGCBJKC_id < SakashoErrorId.STORE_SERVER_ERROR)
		{
			...
			else
			{
				if(_PPFNGGCBJKC_id != SakashoErrorId.INTERNAL_SERVER_ERROR)
				{
					...
					{
						//LAB_00b1a1bc
						str = MessageManager.Instance.GetBank(ECLAOLBGCDD_tbl[0]).GetMessageByLabel("popup_internal_client_error_01");
					}
				}
				else
				{
					str = string.Format(MessageManager.Instance.GetBank(ECLAOLBGCDD_tbl[0]).GetMessageByLabel("popup_internal_server_error_retry_text"), Array.Empty<object>());
				}
			}
		}
		else
	...
	public static string HHAEOPHPCEF(SakashoErrorId _PPFNGGCBJKC_id)
	{
		if(_PPFNGGCBJKC_id > SakashoErrorId.FRIEND_LIMIT_EXCEEDED)
		{
			...
			return MessageManager.Instance.GetBank(ECLAOLBGCDD_tbl[0]).GetMessageByLabel("popup_inh_title_005");
		}
		else
		{
