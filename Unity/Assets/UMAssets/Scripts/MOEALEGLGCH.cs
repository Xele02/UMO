...
public class MOEALEGLGCH
{
	...
	public int GJFJBGNCBAP(LFAFJCNKLML _IDLHJIOMJBK_Data, int _ANAJIAENLNB_Level)
	{
		LFAFJCNKLML.FHLDDEKAJKI f = _IDLHJIOMJBK_Data.OCOOHBINGBG_LevelInfo[_ANAJIAENLNB_Level];
	...
	private int EBFFMJHDHIO(int _ANAJIAENLNB_Level)
	{
		int res = 0;
		for(int i = 0; i < MGJKEJHEBPO_Blocks.Count; i++)
		{
			if (MGJKEJHEBPO_Blocks[i].GKIKAABHAAD_Level >= _ANAJIAENLNB_Level)
				res++;
		}
		return res;
	}
	...
	public string EGFDDHPPFNE(LFAFJCNKLML _IDLHJIOMJBK_Data, int _ANAJIAENLNB_Level, int _OIPCCBHIKIA_index)
	{
		LFAFJCNKLML.FHLDDEKAJKI f = _IDLHJIOMJBK_Data.OCOOHBINGBG_LevelInfo[_ANAJIAENLNB_Level];
	...
	public bool HEOGHOIOHGI(LFAFJCNKLML _IDLHJIOMJBK_Data, int _ANAJIAENLNB_Level, int _OIPCCBHIKIA_index, out int KOGEGJOOMIG, out int _JDLJPNMLFID_ItemCount)
	{
	...
	public bool KFJHILDJCCB(LFAFJCNKLML _IDLHJIOMJBK_Data, int _ANAJIAENLNB_Level)
	{
		LFAFJCNKLML.FHLDDEKAJKI d = _IDLHJIOMJBK_Data.OCOOHBINGBG_LevelInfo[_ANAJIAENLNB_Level];
				...
				if (d.KBOLNIBLIND_unlock[i].INDDJNMPONH_Type < LCLCCHLDNHJ_Costume.LKLGPLFNJBA_UnlockCondition.AEFCOHJBLPO/*4*/)
				{
					switch(d.KBOLNIBLIND_unlock[i].INDDJNMPONH_Type)
					{
						...
						case LCLCCHLDNHJ_Costume.LKLGPLFNJBA_UnlockCondition.BCJHILDCONA_2_UseItems/*2*/:
							...
							b = d.KBOLNIBLIND_unlock[i].PIBLLGLCJEO_Param[0] <= CIOECGOMILE.HHCJCDFCLOB.AHEFHIMGIBI_PlayerData.KCCLEHLLOFG_Common.EFBKCNNFIPJ(dbCosItem.PPFNGGCBJKC_id).BFINGCJHOHI_Count;
							break;
	...
	//public bool EJOALPKJKIG(LFAFJCNKLML _IDLHJIOMJBK_Data, int _ANAJIAENLNB_Level) { }
	...
	public int DLJJACACBDI(int JBPCPCBLNDC, int _MHFBCINOJEE_Num, int _HMFFHLPNMPH_Count)
	{
		if(_HMFFHLPNMPH_Count > 0)
		{
            ...
            if (saveCos.BEEAIAAJOHD_CostumeId == JBPCPCBLNDC)
			{
				...
				{
					return _HMFFHLPNMPH_Count <= CIOECGOMILE.HHCJCDFCLOB.AHEFHIMGIBI_PlayerData.KCCLEHLLOFG_Common.EFBKCNNFIPJ(_MHFBCINOJEE_Num).BFINGCJHOHI_Count ? 1 : 0;
				}
            }
		}
		return -1;
	...
	public int JDLAFDBFLOM(LFAFJCNKLML _IDLHJIOMJBK_Data, IMCBBOAFION _BHFHGFKBOHH_OnSuccess)
	{
		if(CIOECGOMILE.HHCJCDFCLOB.AHEFHIMGIBI_PlayerData.PFAKPFKJJKA(true) == null)
		{
			...
			for(int i = 0; i < d.KBOLNIBLIND_unlock.Length; i++)
			{
				if(d.KBOLNIBLIND_unlock[i].INDDJNMPONH_Type == LCLCCHLDNHJ_Costume.LKLGPLFNJBA_UnlockCondition.BCJHILDCONA_2_UseItems)
				{
					...
					if(d.KBOLNIBLIND_unlock[i].PIBLLGLCJEO_Param[0] > 0)
					{
						...
						saveCosItem.BFINGCJHOHI_Count -= d.KBOLNIBLIND_unlock[i].PIBLLGLCJEO_Param[0];
						if(saveCosItem.BFINGCJHOHI_Count < 0)
							saveCosItem.BFINGCJHOHI_Count = 0;
						b = true;
                    }
                }
			}
			...
			if(!b)
			{
				CIOECGOMILE.HHCJCDFCLOB.AHEFHIMGIBI_PlayerData.BEKHNNCGIEL_Costume.BEEJCDHCONE(_IDLHJIOMJBK_Data.JPIDIENBGKH_CostumeId, cos.ANAJIAENLNB_Level);
			}
        }
	...
	public int CALNLFGDMEE(int JBPCPCBLNDC, int KOGEGJOOMIG, int _HMFFHLPNMPH_Count, IMCBBOAFION _BHFHGFKBOHH_OnSuccess)
	{
		...
		if(a1 > 0)
		{
			...
			bool b1 = saveCos.EAMPKNCDCFM(IMMAOANGPNK.HHCJCDFCLOB.NKEBMCIMJND_Database.MFPNGNMFEAL_Costume, dbCosItem.JBGEEPFKIGG_Value * _HMFFHLPNMPH_Count, out d);
			...
			if(d.ACGLMKEBMDL_Uc <= a2)
			{
				...
				if(f == null)
				{
					...
					if(saveItem.BFINGCJHOHI_Count < 0)
						saveItem.BFINGCJHOHI_Count = 0;
					...
					if(diff > 0)
					{
						...
						{
							CIOECGOMILE.HHCJCDFCLOB.AHEFHIMGIBI_PlayerData.OEKEIGFAIGN_Counter.BDLNMOIOMHK_Total.KNCLIEBAPJD_Cosu += saveCos.ANAJIAENLNB_Level - prevLevel;
					...
					ILCCJNDFFOB.HHCJCDFCLOB.BABLKKLEPGD(JBPCPCBLNDC, prevLevel, saveCos.ANAJIAENLNB_Level, prevPoint, saveCos.DNBFMLBNAEE_Point, EKLNMHFCAOI.GJEEGMCBGGM_GetItemFullId(EKLNMHFCAOI.FKGCBLHOOCL_Category.NEIIGCODGBA_CostumeItem, KOGEGJOOMIG), _HMFFHLPNMPH_Count, d.ACGLMKEBMDL_Uc);
				}
