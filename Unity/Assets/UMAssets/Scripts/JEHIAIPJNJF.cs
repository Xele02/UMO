...
public class JEHIAIPJNJF_FileDownloader : IDisposable
{
    ...
    {
        PBIMGBKLDPP_0_Reset = 0,
        JGHMJIGGJHI_1_Running = 1,
	...
	public class HCJPJKCIBDL_DldFileInfo
    {
        ...
        public int OIPCCBHIKIA_index; // 0x14
    ...
    public delegate void FMOECHMCHPE(JEHIAIPJNJF_FileDownloader.HCJPJKCIBDL_DldFileInfo _KOGBMDOONFA_Info);
	...
	{
		double res = 0.0f;
		if(CMCKNKKCNDK_status != JEHIAIPJNJF_FileDownloader.NKLKJEOKIFO_Status.FEJIMBDPMKI_2_Success && JOJMBFBGMGN_DldQueue != null)
		{
			for(int i = 0; i < JOJMBFBGMGN_DldQueue.Count; i++)
			{
	...
	public JEHIAIPJNJF_FileDownloader(int JAGOLJBNFMP_NumSimultDld/* = 3*/)
	{
		...
		CMCKNKKCNDK_status = JEHIAIPJNJF_FileDownloader.NKLKJEOKIFO_Status.PBIMGBKLDPP_0_Reset;
	...
	public void DOMFHDPMCCO_Init(List<GCGNICILKLD_AssetFileInfo> IDJBKGBMDAJ, string JCILKDKNDLE_BaseUrl, string _OGCDNCDMLCA_LocalPath)
	{
		...
		for(int i = 0; i < IDJBKGBMDAJ.Count; i++)
		{
			...
			str.Set(_OGCDNCDMLCA_LocalPath);
			str.Append(IDJBKGBMDAJ[i].OIEAICNAMNB_LocalFileName);
	...
	public void DOMFHDPMCCO_Init(List<KDLPEDBKMID.EMEKAOMPFNC_LocalFileInfo> CEKHMLAEKIK)
	{
		...
		CMCKNKKCNDK_status = NKLKJEOKIFO_Status.PBIMGBKLDPP_0_Reset;
	}
	...
	public void LAOEGNLOJHC_Start()
	{
		...
		if(CMCKNKKCNDK_status != JEHIAIPJNJF_FileDownloader.NKLKJEOKIFO_Status.PBIMGBKLDPP_0_Reset/*0*/)
			return;
		Debug.Log(JpStringLiterals.StringLiteral_11856);
		CMCKNKKCNDK_status = JEHIAIPJNJF_FileDownloader.NKLKJEOKIFO_Status.JGHMJIGGJHI_1_Running/*1*/;
	}

	// // RVA: 0x1C3578C Offset: 0x1C3578C VA: 0x1C3578C
	public void PBIMGBKLDPP_Reset()
	{
		UnityEngine.Debug.Log(string.Format(JpStringLiterals.StringLiteral_11857, CMCKNKKCNDK_status.ToString()));
		if(CMCKNKKCNDK_status < JEHIAIPJNJF_FileDownloader.NKLKJEOKIFO_Status.LLGCBKEOHNP_3_Error/*3*/ || CMCKNKKCNDK_status > JEHIAIPJNJF_FileDownloader.NKLKJEOKIFO_Status.LPLEIJIFOKN_4_AllError/*4*/)
		{
			if(CMCKNKKCNDK_status == JEHIAIPJNJF_FileDownloader.NKLKJEOKIFO_Status.FEJIMBDPMKI_2_Success/*2*/)
			{
				CMCKNKKCNDK_status = JEHIAIPJNJF_FileDownloader.NKLKJEOKIFO_Status.PBIMGBKLDPP_0_Reset/*0*/;
				return;
			}
	...
	public void FBANBDCOEJL_Update()
	{
		...
		if(CMCKNKKCNDK_status != JEHIAIPJNJF_FileDownloader.NKLKJEOKIFO_Status.JGHMJIGGJHI_1_Running/*1*/)
		{
			...
			if(CMCKNKKCNDK_status != JEHIAIPJNJF_FileDownloader.NKLKJEOKIFO_Status.LLGCBKEOHNP_3_Error/*3*/)
				return;
		}
		CriFsWebInstaller.ExecuteMain();
		int numFailed = PKDGOJNCLBK_Update();
		if(numFailed < 1)
		{
			if(FCPPMGNCGNO() != 0)
			{
			...
			if(KJIGCCPJBFK_List.Count != 0)
				return;
			CMCKNKKCNDK_status = JEHIAIPJNJF_FileDownloader.NKLKJEOKIFO_Status.FEJIMBDPMKI_2_Success/*2*/;
		}
		else
		{
			if(KJIGCCPJBFK_List.Count == numFailed)
			{
				CMCKNKKCNDK_status = JEHIAIPJNJF_FileDownloader.NKLKJEOKIFO_Status.LPLEIJIFOKN_4_AllError;
			}
			else
			{
				CMCKNKKCNDK_status = JEHIAIPJNJF_FileDownloader.NKLKJEOKIFO_Status.LLGCBKEOHNP_3_Error/*3*/;
			}
		}
	}
	...
	private int PKDGOJNCLBK_Update()
	{
		int index = 0;
		int res = 0;
		...
		while(true)
		{
			while(true)
			{
				...
				if(status == CriFsWebInstaller.Status.Error)
				{
					DLHJNILCAGE(info);
					index = index + 1;
					CMCKNKKCNDK_status = JEHIAIPJNJF_FileDownloader.NKLKJEOKIFO_Status.LLGCBKEOHNP_3_Error;
					res = res + 1;
				}
				else
				{
	...
	public bool MNFGKBAEFFL_IsTimeout()
	{
		for(int i = 0; i < KJIGCCPJBFK_List.Count; i++)
		{
	...
	private void DLHJNILCAGE(JEHIAIPJNJF_FileDownloader.AFGDFAJEBFA_DldInfo _PHKJOMLDNOB_context)
	{
		if(_PHKJOMLDNOB_context.GGFGJEDLKNC)
			return;
		bool a = false;
		string str = "";
		switch(_PHKJOMLDNOB_context.IOKJFDPOEFP_InstallerStatusInfo.error)
		{
		...
		UnityEngine.Debug.LogError(string.Concat(new string[5]
		{
			...
			str,
			" [",
			_PHKJOMLDNOB_context.ICKGJODOCBB.NFCMNIEHJML_ServerPath,
			"]"
		}));
		if(!(a && !BIBKLAMCKGN))
		{
			_PHKJOMLDNOB_context.GGFGJEDLKNC = true;
			return;
		}
		BIBKLAMCKGN = true;
