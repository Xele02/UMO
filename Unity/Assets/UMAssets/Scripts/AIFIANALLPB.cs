...
public class AIFIANALLPB
{
	...
	private void OOGDPKGCMED(CACGCMBKHDI_Request KFBCOGJKEJP)
	{
		...
		else
		{
			...
			request = NKGJPJPHLIF.HHCJCDFCLOB.IBLPICFDGOF.IFFNCAFNEAG_AddRequest(request);
			request.IPKCADIAAPG_Criteria = JCHEPJKLGCF;
	...
	private IEnumerator LLONPPGBFFN_Corutine_RemoveItems(List<GJDFHLBONOL> NCHOFJOBAOE, List<GJDFHLBONOL> CLDOJOHGBJA)
	{
		...
		//0xCCFE5C
		OKDOIAEGADK = NKGJPJPHLIF.HHCJCDFCLOB.IBLPICFDGOF;
		...
		if (BHFHGFKBOHH != null)
			BHFHGFKBOHH();
		ECFNAOCFKKN_Date = NKGJPJPHLIF.HHCJCDFCLOB.IBLPICFDGOF.FJDBNGEPKHL.KMEFBNBFJHI_GetServerTime();
		PLOOEECNHFB = true;
	}
