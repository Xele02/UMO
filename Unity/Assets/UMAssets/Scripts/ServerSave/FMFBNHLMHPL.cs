...
public class FMFBNHLMHPL_EventAprilFool : KLFDBFMNLBL_ServerSaveBlock
{
	public class LCFOEDLCCON
	{
		...
		public string MDADLCOCEBN_session_id = ""; // 0x1C
		...
		public List<IKCGAJKCPFN> NNMPGOAGEOL_quests = new List<IKCGAJKCPFN>(50); // 0x40
		...
		public int MHKICPIMFEI_PlayCount; // 0x58
		public List<CEBFFLDKAEC_SecureInt> AAJOOBMPOEP_Nsxq = new List<CEBFFLDKAEC_SecureInt>(1); // 0x5C

		public int LGADCGFMLLD_step { get { return AFGHJEJKDHN_StepCrypted ^ ENOBDCFHELD; } set { AFGHJEJKDHN_StepCrypted = ENOBDCFHELD ^ value; FBDJBMDEENG_StepCrypted2 = FCEJCHGLFGN ^ value; } } //0x1199604 MAFBDLKFHGJ_get_step 0x119A410 EPEFBOIALDI_set_step
		public bool PPKAKKGCAOJ_IsPickup { get { return JDNEMJLJJLC_PickupCrypted == 87; } set { JDNEMJLJJLC_PickupCrypted = value ? (sbyte)87 : (sbyte)19; } } //0x11995F0 KBHPMOPAPBL_bgs 0x119A3E0 FJGACEGCMGM_bgs
		public int CFKFIODHFNC_Act { get { return ENOBDCFHELD ^ IACIALIHPAJ_ActCrypted; } set { IACIALIHPAJ_ActCrypted = ENOBDCFHELD ^ value; MJGLINBDCKG = FCEJCHGLFGN ^ value; } } //0x1199614 OHHANDMGKHM_bgs 0x119A424 PLHGJGHHEOP_bgs
		public bool KBAHNBKMFDL_is_minigame_clear { get { return (PPHCFBMEDND_IsMinigameClearCrypted ^ ENOBDCFHELD) == 87; } set { PPHCFBMEDND_IsMinigameClearCrypted = ENOBDCFHELD ^ (value ? 87 : 19); MILIPFCLCNC_IsMinigameClearCrypted2 = FCEJCHGLFGN ^ (value ? 87 : 19); } } //0x1199624 AJJDGCJIILD_bgs 0x119A438 KJJGGIBOMHP_bgs
		public int OMCAOJJGOGG_Lb { get { return HFIHMDILNFD_LbCrypted ^ ENOBDCFHELD; } set { HFIHMDILNFD_LbCrypted = ENOBDCFHELD ^ value; IGMFFIKLEHP_LbCrypted2 = FCEJCHGLFGN ^ value; } } //0x1199640 PBEMPHPDDDB_get_Lb 0x119A49C MCADMIEOCCF_set_Lb
		...
		public void LHPDDGIJKNB_Reset()
		{
			...
			BEBJKJKBOGH_date = 0;
			CFKFIODHFNC_Act = 0;
			KBAHNBKMFDL_is_minigame_clear = false;
			AAJOOBMPOEP_Nsxq.Clear();
			AAJOOBMPOEP_Nsxq.Add(new CEBFFLDKAEC_SecureInt());
		}

		//// RVA: 0x119A680 Offset: 0x119A680 VA: 0x119A680
		public void ODDIHGPONFL_Copy(LCFOEDLCCON GPBJHKLFCEP)
		{
			...
			KBAHNBKMFDL_is_minigame_clear = GPBJHKLFCEP.KBAHNBKMFDL_is_minigame_clear;
			...
			AAJOOBMPOEP_Nsxq[0].DNJEJEANJGL_Value = GPBJHKLFCEP.AAJOOBMPOEP_Nsxq[0].DNJEJEANJGL_Value;
			MDADLCOCEBN_session_id = string.Copy(GPBJHKLFCEP.MDADLCOCEBN_session_id);
		...
		public bool AGBOGBEOFME(LCFOEDLCCON OIKJFMGEICL)
		{
			if (IMFBCJOIJKJ_Entry != OIKJFMGEICL.IMFBCJOIJKJ_Entry ||
				...
				KBAHNBKMFDL_is_minigame_clear != OIKJFMGEICL.KBAHNBKMFDL_is_minigame_clear ||
				INLNJOGHLJE_Show != OIKJFMGEICL.INLNJOGHLJE_Show ||
				OMCAOJJGOGG_Lb != OIKJFMGEICL.OMCAOJJGOGG_Lb ||
				MHKICPIMFEI_PlayCount != OIKJFMGEICL.MHKICPIMFEI_PlayCount ||
				MPCAGEPEJJJ_Key != OIKJFMGEICL.MPCAGEPEJJJ_Key ||
				MDADLCOCEBN_session_id != OIKJFMGEICL.MDADLCOCEBN_session_id
				)
				return false;
			for(int i = 0; i < 50; i++)
			{
			...
			if (AAJOOBMPOEP_Nsxq[0].DNJEJEANJGL_Value != OIKJFMGEICL.AAJOOBMPOEP_Nsxq[0].DNJEJEANJGL_Value)
				return false;
			return true;
		}

		//// RVA: 0x119B534 Offset: 0x119B534 VA: 0x119B534
		//public void AGHKODFKOJI(BHBONAHFKHD JBBHNIACMFJ, string _JIKKNHIAEKG_BlockName, string MJBACHKCIHA, int _OIPCCBHIKIA_index, FMFBNHLMHPL_EventAprilFool.LCFOEDLCCON _OHMCIEMIKCE_t, bool EFOEPDLNLJG) { }
	}
	...
	public override bool DMICHEJIAJL { get { return false; } } // 0x119DF54 NFKFOODCJJB_bgs
	...
	public override void OKJPIBHMKMJ(EDOHBJAPLPF_JsonData OILEIIEIBHP, long _MCKEOKFMLAH_SaveId)
	{
		...
		for(int i = 0; i < 9; i++)
		{
			...
			for (int j = 0; j < 50; j++)
			{
				quest.Add(FBCJICEPLED[i].NNMPGOAGEOL_quests[j].NOJCMGAFAAC_ToJsonData());
			}
			EDOHBJAPLPF_JsonData nxsq = new EDOHBJAPLPF_JsonData();
			nxsq.LAJDIPCJCPO_SetJsonType(JFBMDLGBPEN_JsonType.BDHGEFMCJDF_Array);
			nxsq.Add(FBCJICEPLED[i].AAJOOBMPOEP_Nsxq[0].DNJEJEANJGL_Value);

			EDOHBJAPLPF_JsonData data2 = new EDOHBJAPLPF_JsonData();
			data2[AFEHLCGHAEE_Strings.LJNAKDMILMC_key] = FBCJICEPLED[i].MPCAGEPEJJJ_Key;
			data2[AFEHLCGHAEE_Strings.KOMKKBDABJP_end] = FBCJICEPLED[i].EGBOHDFBAPB_closed_at;
			...
			data2[AFEHLCGHAEE_Strings.KBAHNBKMFDL_is_minigame_clear] = FBCJICEPLED[i].KBAHNBKMFDL_is_minigame_clear ? 1 : 0;
			data2["show"] = FBCJICEPLED[i].INLNJOGHLJE_Show;
			data2[AFEHLCGHAEE_Strings.MDADLCOCEBN_session_id] = FBCJICEPLED[i].MDADLCOCEBN_session_id;
			data2["lb"] = FBCJICEPLED[i].OMCAOJJGOGG_Lb;
			data2["play_cnt"] = FBCJICEPLED[i].MHKICPIMFEI_PlayCount;
			data.Add(data2);
		}
		if(!EMBGIDLFKGM)
		{
		...
		if (!blockMissing)
		{
			...
			else
			{
				...
				for(int i = 0; i < c; i++)
				{
					...
					FGCNMLBACGO_GetString(data, AFEHLCGHAEE_Strings.LJNAKDMILMC_key, "", ref isInvalid);
					LCFOEDLCCON CCBEKGNDDBE = FBCJICEPLED[i];
					CCBEKGNDDBE.MPCAGEPEJJJ_Key = FGCNMLBACGO_GetString(data, AFEHLCGHAEE_Strings.LJNAKDMILMC_key, "", ref isInvalid);
					CCBEKGNDDBE.EGBOHDFBAPB_closed_at = DKMPHAPBDLH_GetLong(data, AFEHLCGHAEE_Strings.KOMKKBDABJP_end, 0, ref isInvalid);
					CCBEKGNDDBE.IMFBCJOIJKJ_Entry = CJAENOMGPDA_GetInt(data, AFEHLCGHAEE_Strings.CDMGDFLPPHN_entry, 0, ref isInvalid) != 0;
					CCBEKGNDDBE.ABBJBPLHFHA_Spurt = CJAENOMGPDA_GetInt(data, AFEHLCGHAEE_Strings.MAFAIIHJAFG_spurt, 0, ref isInvalid) != 0;
					CCBEKGNDDBE.PPKAKKGCAOJ_IsPickup = CJAENOMGPDA_GetInt(data, "pickup", 1, ref isInvalid) != 0;
					CCBEKGNDDBE.LGADCGFMLLD_step = CJAENOMGPDA_GetInt(data, AFEHLCGHAEE_Strings.CDMGDFLPPHN_entry, 0, ref isInvalid);
					CCBEKGNDDBE.BEBJKJKBOGH_date = CJAENOMGPDA_GetInt(data, AFEHLCGHAEE_Strings.BEBJKJKBOGH_date, 0, ref isInvalid);
					CCBEKGNDDBE.CFKFIODHFNC_Act = CJAENOMGPDA_GetInt(data, "act", 0, ref isInvalid);
					CCBEKGNDDBE.KBAHNBKMFDL_is_minigame_clear = CJAENOMGPDA_GetInt(data, AFEHLCGHAEE_Strings.KBAHNBKMFDL_is_minigame_clear, 0, ref isInvalid) != 0;
					CCBEKGNDDBE.INLNJOGHLJE_Show = CJAENOMGPDA_GetInt(data, "show", 0, ref isInvalid);
					CCBEKGNDDBE.MDADLCOCEBN_session_id = FGCNMLBACGO_GetString(data, AFEHLCGHAEE_Strings.MDADLCOCEBN_session_id, "", ref isInvalid);
					CCBEKGNDDBE.OMCAOJJGOGG_Lb = CJAENOMGPDA_GetInt(data, "lb", 0, ref isInvalid);
					CCBEKGNDDBE.MHKICPIMFEI_PlayCount = CJAENOMGPDA_GetInt(data, "play_cnt", 0, ref isInvalid);
					if(data.BBAJPINMOEP_Contains(AFEHLCGHAEE_Strings.NNMPGOAGEOL_quests))
					{
						...
						for(int j = 0; j < c2; j++)
						{
							...
							IKCGAJKCPFN data2 = CCBEKGNDDBE.NNMPGOAGEOL_quests[j];
							data2.PPFNGGCBJKC_id = CJAENOMGPDA_GetInt(quest2, AFEHLCGHAEE_Strings.PPFNGGCBJKC_id, j + 1, ref isInvalid);
							data2.EALOBDHOCHP_stat = CJAENOMGPDA_GetInt(quest2, AFEHLCGHAEE_Strings.EALOBDHOCHP_stat, 0, ref isInvalid);
							data2.HMFFHLPNMPH_count = CJAENOMGPDA_GetInt(quest2, AFEHLCGHAEE_Strings.HMFFHLPNMPH_count, 0, ref isInvalid);
							data2.BEBJKJKBOGH_date = DKMPHAPBDLH_GetLong(quest2, AFEHLCGHAEE_Strings.BEBJKJKBOGH_date, 0, ref isInvalid);
							data2.CADENLBDAEB_IsNew = CJAENOMGPDA_GetInt(quest2, AFEHLCGHAEE_Strings.KLJGEHBKMMG_new, 0, ref isInvalid) != 0;
						}
					}
					if(data.BBAJPINMOEP_Contains("nxsq"))
					{
						...
						{
							//0x119DF5C
							CCBEKGNDDBE.AAJOOBMPOEP_Nsxq[_OIPCCBHIKIA_index].DNJEJEANJGL_Value = _JBGEEPFKIGG_val;
						}, ref isInvalid);
					}
				}
			}
	...
	public override void BMGGKONLFIC_Copy(KLFDBFMNLBL_ServerSaveBlock GPBJHKLFCEP)
	{
		...
		{
			FBCJICEPLED[i].ODDIHGPONFL_Copy(o.FBCJICEPLED[i]);
		}
	}
