...
public class FEHINJKHDAP_EventScore : KLFDBFMNLBL_ServerSaveBlock
{
	public class ALGDNCMJHGN
	{
		...
		public long DNBFMLBNAEE_Point { get { return AADPAJOLEEF_PointCrypted ^ ENOBDCFHELD; } set { value = value >= 100000000 ? 99999999: value; AADPAJOLEEF_PointCrypted = value ^ ENOBDCFHELD; IOJOBGHPLIE_Crypted = value ^ FCEJCHGLFGN; } } //0x14CCBBC JKHIIAEMMDE 0x14CCBD4 PFFKLBLEPKB
			...
			else
			{
				if(block.HNBFOAJIIAL_Count > 0)
				{
					...
					for(int i = 0; i < cnt; i++)
					{
						...
						{
							//0x14CC4C0
							FBCJICEPLED[i].AGGFHNMMGMN_RewardCombo[OIPCCBHIKIA] = (byte)JBGEEPFKIGG;
						}, ref isInvalid);
						...
						{
							//0x14CC984
							FBCJICEPLED[i].DKIIINIEKHP_RewardScoreL6[OIPCCBHIKIA] = (byte)JBGEEPFKIGG;
						}, ref isInvalid);
						...
						{
							//0x14CCABC
							FBCJICEPLED[i].EEECMKPLPNL_ComboRankL6[OIPCCBHIKIA] = (sbyte)JBGEEPFKIGG;
						}, ref isInvalid);
					}
				}
			}
