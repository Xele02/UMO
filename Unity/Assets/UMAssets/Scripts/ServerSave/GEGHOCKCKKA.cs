...
public class GEGHOCKCKKA_LimitedCompoItem : KLFDBFMNLBL_ServerSaveBlock
{
	public class AIAPDPDOIKG
	{
		public class AGGJCJMIIMJ
		{
			...
			public int PPFNGGCBJKC_id { get { return EHOIENNDEDH_IdCrypted ^ FBGGEFFJJHB_xor; } set { EHOIENNDEDH_IdCrypted = value ^ FBGGEFFJJHB_xor; } } //0x16BB5F0 DEMEPMAEJOO_get_id 0x16BBAEC HIGKAIDMOKN_set_id
			public int PGPGPJNBIOH_uid { get { return CNGMGFCGLOK ^ FBGGEFFJJHB_xor; } set { CNGMGFCGLOK = value ^ FBGGEFFJJHB_xor; } } //0x16B98C4 CJEDGLPDJBN_get_uid 0x16BBADC EKJKPIKJLJJ_set_uid
			public int DOJDOLDDBPP_Hav { get { return DJJOKAOHAML ^ FBGGEFFJJHB_xor; } set { DJJOKAOHAML = value ^ FBGGEFFJJHB_xor; } } //0x16BB610 FHJFILKDGGO_bgs 0x16BBB1C DEEAELHLIEC_bgs
			public int DIAPHCJBPFD_get_date { get { return JECIEFEOBDO ^ FBGGEFFJJHB_xor; } set { JECIEFEOBDO = value ^ FBGGEFFJJHB_xor; } } //0x16B98B4 EIEMCCANNJE_bgs 0x16BBAFC JLGBEBCNDJN_bgs
			public int IOLJPHAOBOH_Use { get { return DADJGIPLGBK ^ FBGGEFFJJHB_xor; } set { DADJGIPLGBK = value ^ FBGGEFFJJHB_xor; } } //0x16BB600 EHKEMJJNKKI_bgs 0x16BBB0C GHMCNDFEFIF_bgs
			public int HNKFMAJIFJD_ExpireAt { get {
					DateTime date = Utility.GetLocalDateTime(EDLLKJAKIGO(PPFNGGCBJKC_id, DIAPHCJBPFD_get_date) + DIAPHCJBPFD_get_date);
					return (int)Utility.GetTargetUnixTime(date.Year, date.Month, date.Day, 23, 59, 59);
				} } //0x16B91A4 CEOIBKAGPAG_bgs
			private long OLNKAECIGJH { get { return Utility.GetTargetUnixTime(2030, 1, 1, 0, 0, 0); } } //0x16BD4AC GBAOINLDCEM_bgs
			...
			public bool NIENPFFLMCH(long _JHNMKKNEENE_Time)
			{
				...
				{
					if (_JHNMKKNEENE_Time >= DIAPHCJBPFD_get_date)
						return HNKFMAJIFJD_ExpireAt >= _JHNMKKNEENE_Time;
				}
				return false;
			}
			...
			{
				return HNKFMAJIFJD_ExpireAt >= OLNKAECIGJH;
			}
			...
			//public void AGHKODFKOJI(BHBONAHFKHD JBBHNIACMFJ, string _JIKKNHIAEKG_BlockName, string MJBACHKCIHA, int _OIPCCBHIKIA_index, GEGHOCKCKKA_LimitedCompoItem.AIAPDPDOIKG.AGGJCJMIIMJ _OHMCIEMIKCE_t, bool KFCGIKHEEMB) { }
		}
		...
		//public void PKAINJJBMMH_ChangeKeyAndSetTime(int _PPFNGGCBJKC_id, long KNCKIJBOODM) { }
	}
	...
	public override bool DMICHEJIAJL { get { return false; } } // 0x16BC740 NFKFOODCJJB_bgs
	...
	public int BLKPKBICPKK(int _PPFNGGCBJKC_id, long _JHNMKKNEENE_Time)
	{
		...
		for(int i = 0; i < a.HFAHJPBLCHK; i++)
		{
			...
			{
				if(a.PJADHDHKOEJ[i].HNKFMAJIFJD_ExpireAt < c)
				{
					c = a.PJADHDHKOEJ[i].HNKFMAJIFJD_ExpireAt;
				}
			}
		}
		if (c == int.MaxValue)
	...
	public int CPIICACGNBH_AddItem(int _PPFNGGCBJKC_id, long _JHNMKKNEENE_Time, bool _DDGFCOPPBBN_test/* = false*/)
	{
		if(_PPFNGGCBJKC_id > 0)
		{
			...
			for(int i = 0; i < data.HFAHJPBLCHK; i++)
			{
				...
				{
					if(data.PJADHDHKOEJ[i].DIAPHCJBPFD_get_date <= MinGet)
					{
						MinGet = data.PJADHDHKOEJ[i].DIAPHCJBPFD_get_date;
						MinGetIdx = i;
					}
				}
			...
			if(MinGetIdx > -1)
			{
				...
				data2.DIAPHCJBPFD_get_date = (int)_JHNMKKNEENE_Time;
				...
				return data.PJADHDHKOEJ[MinGetIdx].HNKFMAJIFJD_ExpireAt;
			}
		}
		return 0;
	}
				...
				for (int i = 0; i < ODHBHOGFNAA.Count; i++)
				{
					...
					if(block.BBAJPINMOEP_Contains(str))
					{
						...
						for(int j = 0; j < cnt; j++)
						{
							...
							data.PJADHDHKOEJ[j].PPFNGGCBJKC_id = CJAENOMGPDA_GetInt(b2, "id", 0, ref isInvalid);
