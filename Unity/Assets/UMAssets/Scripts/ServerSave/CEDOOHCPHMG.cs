...
public class CEDOOHCPHMG_UnitGoDiva : KLFDBFMNLBL_ServerSaveBlock
{
					...
					for (int j = 0; j < array.HNBFOAJIIAL_Count; j++)
					{
						...
						if(MEFHDDNABLM != null)
						{
							...
							if(array[j].BBAJPINMOEP_Contains(AFEHLCGHAEE_Strings.FDBOPFEOENF_diva))
							{
								...
								{
									AMCGONHBGGF a = MEFHDDNABLM.FDBOPFEOENF[k];
	...
	// // RVA: 0x12B4DD8 Offset: 0x12B4DD8 VA: 0x12B4DD8 Slot: 9
...
public class MPBEHHIAGOI
{
	...
	public void FBFHJCNNPAK(int AHHJLDLAPAN)
	{
		...
		for(int i = 0; i < data.FDBOPFEOENF.Count; i++)
		{
			...
			data.FDBOPFEOENF[i].BEEAIAAJOHD_CId = 0;
			data.FDBOPFEOENF[i].AFNIOJHODAG_ColId = 0;
			for(int j = 0; j < data.FDBOPFEOENF[i].EBDNICPAFLB_SSlot.Length; j++)
			{
				data.FDBOPFEOENF[i].EBDNICPAFLB_SSlot[j] = 0;
			}
		}
	}
