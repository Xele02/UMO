...
public class LGIDLHLBFFJ_MonthlyPass : KLFDBFMNLBL_ServerSaveBlock
{
	public class LCDJCBAPAML
	{
		...
		public int PPFNGGCBJKC_id { get { return EHOIENNDEDH_IdCrypted ^ FBGGEFFJJHB_xor; } set { EHOIENNDEDH_IdCrypted = value ^ FBGGEFFJJHB_xor; INCHFKJOIEK = value; } } //0x17F12D0 DEMEPMAEJOO_get_id 0x17F1368 HIGKAIDMOKN_set_id
		public long FDFGEMODIIF_StartedAt { get { return OILIPGICBIK ^ BBEGLBMOBOF_xorl; } set { OILIPGICBIK = value ^ BBEGLBMOBOF_xorl; LFCBBOPEGDP = value; } } //0x17F1408 CBDHPDMLJKB_bgs 0x17F14A4 OPBOAMBLLDF_bgs
		public long NKMNFPMMJND_expired_at { get { return NJKMDELFJGE ^ BBEGLBMOBOF_xorl; } set { NJKMDELFJGE = value ^ BBEGLBMOBOF_xorl; GDKODEGAAPA = value; } } //0x17F154C JCDIJBHKGMA_bgs 0x17F15E8 FDMBGEAJNPK_bgs
		public long EMOHDABPCHD_CheckAt { get { return AOMKDIEDDHH ^ BBEGLBMOBOF_xorl; } set { AOMKDIEDDHH = value ^ BBEGLBMOBOF_xorl; CBGEGLCNOLB = value; } } //0x17F1690 KNPJCIBDBJC_bgs 0x17F172C BDFIIDDKKCE_bgs
		...
		public bool AGBOGBEOFME(LCDJCBAPAML OIKJFMGEICL)
		{
			if(FDFGEMODIIF_StartedAt != OIKJFMGEICL.FDFGEMODIIF_StartedAt ||
				NKMNFPMMJND_expired_at != OIKJFMGEICL.NKMNFPMMJND_expired_at ||
		...
		public void ODDIHGPONFL_Copy(LCDJCBAPAML GPBJHKLFCEP)
		{
			FDFGEMODIIF_StartedAt = GPBJHKLFCEP.FDFGEMODIIF_StartedAt;
			NKMNFPMMJND_expired_at = GPBJHKLFCEP.NKMNFPMMJND_expired_at;
		...
		{
			if (FDFGEMODIIF_StartedAt == 0 && NKMNFPMMJND_expired_at == 0 && EMOHDABPCHD_CheckAt == 0)
				return false;
			return true;
		}

		//// RVA: 0x17F1A64 Offset: 0x17F1A64 VA: 0x17F1A64
		//public void AGHKODFKOJI(BHBONAHFKHD JBBHNIACMFJ, string _JIKKNHIAEKG_BlockName, string MJBACHKCIHA, int _OIPCCBHIKIA_index, LGIDLHLBFFJ_MonthlyPass.LCDJCBAPAML _OHMCIEMIKCE_t, bool EFOEPDLNLJG) { }
	}
	...
	public class ODHIHCNALDL
	{
		...
		public long MMPCPKODGKI_reset_at; // 0x10
		...
		public void LHPDDGIJKNB_Reset()
		{
			...
			for(int i = 0; i < 20; i++)
			{
				GIJFDNOIHCL_Dates.Add(BBEGLBMOBOF_xorl);
			}
			JJPDPNJFBHN_TableId = 0;
			MMPCPKODGKI_reset_at = 0;
		}

		// RVA: 0x17F2D10 Offset: 0x17F2D10 VA: 0x17F2D10
		public long LPGIECKPBDK(int _OIPCCBHIKIA_index)
		{
			return GIJFDNOIHCL_Dates[_OIPCCBHIKIA_index] ^ BBEGLBMOBOF_xorl;
		}

		// RVA: 0x17F11F0 Offset: 0x17F11F0 VA: 0x17F11F0
		public void ACCIBAABEPN(int _OIPCCBHIKIA_index, long _JGNBPFCJLKI_d)
		{
			GIJFDNOIHCL_Dates[_OIPCCBHIKIA_index] = _JGNBPFCJLKI_d ^ BBEGLBMOBOF_xorl;
		}
		...
		public void ODDIHGPONFL_Copy(ODHIHCNALDL GPBJHKLFCEP)
		{
			for(int i = 0; i < 20; i++)
			{
			...
			MMPCPKODGKI_reset_at = GPBJHKLFCEP.MMPCPKODGKI_reset_at;
		}

		// RVA: 0x17F2EE8 Offset: 0x17F2EE8 VA: 0x17F2EE8
		public bool CHFOOMPEABN()
		{
			if(MMPCPKODGKI_reset_at == 0)
			{
		...
		public void DOMFHDPMCCO_Init(int BIHELPHMCCA, long _EOLFJGMAJAB_CurrentTime)
		{
			JJPDPNJFBHN_TableId = BIHELPHMCCA;
			MMPCPKODGKI_reset_at = _EOLFJGMAJAB_CurrentTime;
			for(int i = 0; i < GIJFDNOIHCL_Dates.Count; i++)
			{
				ACCIBAABEPN(i, 0);
			}
		...
		//JJPDPNJFBHN_TableId = table_id
		//MMPCPKODGKI_reset_at = reset_at
		//GIJFDNOIHCL_Dates = dates

		// RVA: 0x17F33A0 Offset: 0x17F33A0 VA: 0x17F33A0
		//public void AGHKODFKOJI(BHBONAHFKHD JBBHNIACMFJ, string _JIKKNHIAEKG_BlockName, string MJBACHKCIHA, int _OIPCCBHIKIA_index, LGIDLHLBFFJ_MonthlyPass.ODHIHCNALDL _OHMCIEMIKCE_t, bool EFOEPDLNLJG) { }
	}
	...
	public static int FBGGEFFJJHB_xor = 0x1741f74; // 0x4
	public static long BBEGLBMOBOF_xorl = 0x725731f; // 0x8
	...
	public int PCBLFLDMCJA_StampDay { get { return HCPMMDELCDC ^ FBGGEFFJJHB_xor; } set { HCPMMDELCDC = value ^ FBGGEFFJJHB_xor; } } // CJPCEIPEIOB_bgs 0xD8403C GIJHJLEGKPG_bgs 0xD840D4
	public int FADAJDBCBJK_TopPlanDay { get { return CDKBOHDFJKG ^ FBGGEFFJJHB_xor; } set { CDKBOHDFJKG = value ^ FBGGEFFJJHB_xor; } } // FHNELMGCLLA_bgs 0xD84170 AKFPDEMMBKM_bgs 0xD84208
	public int FLBJKACKNOI_CurStamp { get { return AFMHNALKGNC ^ FBGGEFFJJHB_xor; } set { AFMHNALKGNC = value ^ FBGGEFFJJHB_xor; } } //CPKDPLPJIMI_bgs 0xD842A4 NBMJIIHOMHH_bgs 0xD8433C
	public int FCPHDFKFDCK_LoginCnt { get { return PLPIEGNDIGG ^ FBGGEFFJJHB_xor; } set { PLPIEGNDIGG = value ^ FBGGEFFJJHB_xor; KJDIBKHGMKO = value; } } // EKJKOALHDAH_bgs 0xD843D8  BLKCKIEOFOH_bgs 0xD84470
	public int HKABHJKHFKL_RareGetCnt { get { return POAGOKLJCNF ^ FBGGEFFJJHB_xor; } set { POAGOKLJCNF = value ^ FBGGEFFJJHB_xor; PHJODLGIEJA = value; } } //IDKLGKKOHII_bgs 0xD84510 LHCMKDALNEE_bgs 0xD845A8
	public int JAMCDEDFHHK_HotenCnt { get { return JJCMEHFOCED ^ FBGGEFFJJHB_xor; } set { JJCMEHFOCED = value ^ FBGGEFFJJHB_xor; CEFBCMKNPKB = value; } } //CHGOJFDDCEG_bgs 0xD84648 ABJLDAFNEOJ_bgs 0xD846E0
	public long BKONHFNBHKL_Aextm { get { return FLMJPPIPMGP ^ BBEGLBMOBOF_xorl; } set { FLMJPPIPMGP = value ^ BBEGLBMOBOF_xorl; FDGICFBLMKO = value; } } //ADJIEHKCCCN_bgs 0xD84780 CPDDIJHNBCJ_bgs 0xD8481C
	public override bool DMICHEJIAJL { get { return true; } } // 0xD8AC0C NFKFOODCJJB_bgs

	// // RVA: 0xD848C8 Offset: 0xD848C8 VA: 0xD848C8
	// public KBCCGHLCFNO_MonthlyPass.JKGFAIPDNDL LOGJJALCFPF() { }
	...
	// public KBCCGHLCFNO_MonthlyPass.JKGFAIPDNDL ECBMDDGPKGN() { }
	...
	public override void OKJPIBHMKMJ(EDOHBJAPLPF_JsonData OILEIIEIBHP, long _MCKEOKFMLAH_SaveId)
	{
		...
		data["stamp_day"] = PCBLFLDMCJA_StampDay;
		data["topplan_day"] = FADAJDBCBJK_TopPlanDay;
		data["cur_stamp"] = FLBJKACKNOI_CurStamp;
		data["login_cnt"] = FCPHDFKFDCK_LoginCnt;
		data["rare_get_cnt"] = HKABHJKHFKL_RareGetCnt;
		data["hoten_cnt"] = JAMCDEDFHHK_HotenCnt;
		data["platform"] = KJBOIGIDKIF_Platform;
		data["status"] = PMCKOEGANFB_Status;
		data["aextm"] = BKONHFNBHKL_Aextm;
		data["lguk"] = CPENFPEPDFC_Lguk.DNJEJEANJGL_Value;
		data["lauk"] = BPMLNLPDCAJ_Lauk.DNJEJEANJGL_Value;
		{
			EDOHBJAPLPF_JsonData data2 = new EDOHBJAPLPF_JsonData();
			data2[POFDDFCGEGP_Underscore] = "";
			for(int i = 0; i < DNKJAIHCDFN_First.Count; i++)
			{
				if(DNKJAIHCDFN_First[i].CHFOOMPEABN())
				{
					EDOHBJAPLPF_JsonData data3 = new EDOHBJAPLPF_JsonData();
					data3["id"] = DNKJAIHCDFN_First[i].PPFNGGCBJKC_id;
					data3["started_at"] = DNKJAIHCDFN_First[i].FDFGEMODIIF_StartedAt;
					data3["expired_at"] = DNKJAIHCDFN_First[i].NKMNFPMMJND_expired_at;
					data3["checked_at"] = DNKJAIHCDFN_First[i].EMOHDABPCHD_CheckAt;
		...
		}
		{
			...
			{
				if (FMPLMFLMJNE_Last[i].CHFOOMPEABN())
				{
					EDOHBJAPLPF_JsonData data3 = new EDOHBJAPLPF_JsonData();
					data3["id"] = FMPLMFLMJNE_Last[i].PPFNGGCBJKC_id;
					data3["started_at"] = FMPLMFLMJNE_Last[i].FDFGEMODIIF_StartedAt;
					data3["expired_at"] = FMPLMFLMJNE_Last[i].NKMNFPMMJND_expired_at;
					data3["checked_at"] = FMPLMFLMJNE_Last[i].EMOHDABPCHD_CheckAt;
					data2[POFDDFCGEGP_Underscore + (i + 1)] = data3;
				}
			}
			data["last"] = data2;
		}
		{
			...
			{
				if (AOHBAOAPGDM_Raw[i].CHFOOMPEABN())
				{
					EDOHBJAPLPF_JsonData data3 = new EDOHBJAPLPF_JsonData();
					data3["id"] = AOHBAOAPGDM_Raw[i].PPFNGGCBJKC_id;
					data3["started_at"] = AOHBAOAPGDM_Raw[i].FDFGEMODIIF_StartedAt;
					data3["expired_at"] = AOHBAOAPGDM_Raw[i].NKMNFPMMJND_expired_at;
					data3["checked_at"] = AOHBAOAPGDM_Raw[i].EMOHDABPCHD_CheckAt;
					data2[POFDDFCGEGP_Underscore + (i + 1)] = data3;
				}
			}
			data["raw"] = data2;
		}
		{
			...
			for (int i = 0; i < 2; i++)
			{
				if (EOHPPNNLBNH_Stamp[i].CHFOOMPEABN())
				{
					EDOHBJAPLPF_JsonData data3 = new EDOHBJAPLPF_JsonData();
					data3["table_id"] = EOHPPNNLBNH_Stamp[i].JJPDPNJFBHN_TableId;
					data3["reset_at"] = EOHPPNNLBNH_Stamp[i].MMPCPKODGKI_reset_at;
					{
						EDOHBJAPLPF_JsonData data4 = new EDOHBJAPLPF_JsonData();
						data4.LAJDIPCJCPO_SetJsonType(JFBMDLGBPEN_JsonType.BDHGEFMCJDF_Array);
						for(int j = 0; j < 20; j++)
						{
							data4.Add(EOHPPNNLBNH_Stamp[i].LPGIECKPBDK(j));
						}
						data3["dates"] = data4;
					}
					data2[POFDDFCGEGP_Underscore + (i + 1)] = data3;
			...
			data["stamp"] = data2;
			data["stamp_hosei"] = MPLPHLAHJOC_StampHosei;
	...
	public override bool IIEMACPEEBJ_Deserialize(EDOHBJAPLPF_JsonData OILEIIEIBHP)
	{
		...
		else
		{
			...
			PCBLFLDMCJA_StampDay = CJAENOMGPDA_GetInt(block, "stamp_day", 0, ref isInvalid);
			FADAJDBCBJK_TopPlanDay = CJAENOMGPDA_GetInt(block, "topplan_day", 0, ref isInvalid);
			FLBJKACKNOI_CurStamp = CJAENOMGPDA_GetInt(block, "cur_stamp", 0, ref isInvalid);
			FCPHDFKFDCK_LoginCnt = CJAENOMGPDA_GetInt(block, "login_cnt", 0, ref isInvalid);
			HKABHJKHFKL_RareGetCnt = CJAENOMGPDA_GetInt(block, "rare_get_cnt", 0, ref isInvalid);
			JAMCDEDFHHK_HotenCnt = CJAENOMGPDA_GetInt(block, "hoten_cnt", 0, ref isInvalid);
			KJBOIGIDKIF_Platform = CJAENOMGPDA_GetInt(block, "platform", 0, ref isInvalid);
			PMCKOEGANFB_Status = CJAENOMGPDA_GetInt(block, "status", 0, ref isInvalid);
			BKONHFNBHKL_Aextm = DKMPHAPBDLH_GetLong(block, "aextm", 0, ref isInvalid);
			CPENFPEPDFC_Lguk.DNJEJEANJGL_Value = FGCNMLBACGO_GetString(block, "lguk", "", ref isInvalid);
			BPMLNLPDCAJ_Lauk.DNJEJEANJGL_Value = FGCNMLBACGO_GetString(block, "lauk", "", ref isInvalid);
			if (block.BBAJPINMOEP_Contains("first"))
			{
				...
				for (int i = 0; i < DNKJAIHCDFN_First.Count; i++)
				{
					...
					if (b.BBAJPINMOEP_Contains(str))
					{
						...
						data.PPFNGGCBJKC_id = CJAENOMGPDA_GetInt(block, AFEHLCGHAEE_Strings.PPFNGGCBJKC_id, i + 1, ref isInvalid);
						data.FDFGEMODIIF_StartedAt = DKMPHAPBDLH_GetLong(block, "started_at", 0, ref isInvalid);
						data.NKMNFPMMJND_expired_at = DKMPHAPBDLH_GetLong(block, "expired_at", 0, ref isInvalid);
						data.EMOHDABPCHD_CheckAt = DKMPHAPBDLH_GetLong(block, "checked_at", 0, ref isInvalid);
					}
				}
			}
			else
			...
			if (block.BBAJPINMOEP_Contains("stamp"))
			{
				...
				for (int i = 0; i < EOHPPNNLBNH_Stamp.Count; i++)
				{
					...
					if (b.BBAJPINMOEP_Contains(str))
					{
						...
						data.JJPDPNJFBHN_TableId = CJAENOMGPDA_GetInt(b2, "table_id", 0, ref isInvalid);
						data.MMPCPKODGKI_reset_at = DKMPHAPBDLH_GetLong(b2, "reset_at", 0, ref isInvalid);
			...
			MPLPHLAHJOC_StampHosei = CJAENOMGPDA_GetInt(block, "stamp_hosei", 0, ref isInvalid);
		}
	...
	public override void BMGGKONLFIC_Copy(KLFDBFMNLBL_ServerSaveBlock GPBJHKLFCEP)
	{
		...
		for(int i = 0; i < DNKJAIHCDFN_First.Count; i++)
		{
			DNKJAIHCDFN_First[i].ODDIHGPONFL_Copy(m.DNKJAIHCDFN_First[i]);
		}
		for(int i = 0; i < FMPLMFLMJNE_Last.Count; i++)
		{
			FMPLMFLMJNE_Last[i].ODDIHGPONFL_Copy(m.FMPLMFLMJNE_Last[i]);
		}
		for(int i = 0; i < AOHBAOAPGDM_Raw.Count; i++)
		{
			AOHBAOAPGDM_Raw[i].ODDIHGPONFL_Copy(m.AOHBAOAPGDM_Raw[i]);
		}
		for(int i = 0; i < EOHPPNNLBNH_Stamp.Count; i++)
		{
			EOHPPNNLBNH_Stamp[i].ODDIHGPONFL_Copy(m.EOHPPNNLBNH_Stamp[i]);
		}
	}
