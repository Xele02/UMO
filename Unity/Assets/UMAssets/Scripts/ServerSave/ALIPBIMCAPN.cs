...
public class ALIPBIMCAPN_EventBoxGacha : KLFDBFMNLBL_ServerSaveBlock
{
	public class EHDCAEHLLKF
	{
		...
		public void ODDIHGPONFL_Copy(EHDCAEHLLKF GPBJHKLFCEP)
		{
	...
	public class GLNFOMDKJJH
	{
		...
		public void ODDIHGPONFL_Copy(GLNFOMDKJJH GPBJHKLFCEP)
		{
			...
			NNCCGILOOIE_Num = GPBJHKLFCEP.NNCCGILOOIE_Num;
			...
			for(int i = 0; i < 100; i++)
			{
				PKPLOGBIDIG_Prizes[i].ODDIHGPONFL_Copy(GPBJHKLFCEP.PKPLOGBIDIG_Prizes[i]);
			}
		}
		...
		public bool AGBOGBEOFME(GLNFOMDKJJH OIKJFMGEICL)
		{
			...
			if(NNCCGILOOIE_Num != OIKJFMGEICL.NNCCGILOOIE_Num) return false;
			...
			if(EGBOHDFBAPB_closed_at != OIKJFMGEICL.EGBOHDFBAPB_closed_at) return false;
		...
		// public void AGHKODFKOJI(BHBONAHFKHD JBBHNIACMFJ, string JIKKNHIAEKG, string MJBACHKCIHA, int _OIPCCBHIKIA_index, ALIPBIMCAPN.GLNFOMDKJJH _OHMCIEMIKCE_t, bool EFOEPDLNLJG) { }
	}
	...
	public override void OKJPIBHMKMJ(EDOHBJAPLPF_JsonData OILEIIEIBHP, long _MCKEOKFMLAH_SaveId)
	{
		...
		for(int i = 0; i < 6; i++)
		{
			...
			data2["box_cnt"] = FBCJICEPLED[i].NNCCGILOOIE_Num;
			...
			for(int j = 0; j < 100; j++)
			{
				data3.Add(FBCJICEPLED[i].PKPLOGBIDIG_Prizes[j].NOJCMGAFAAC_ToJsonData());
			}
			data2["prize"] = data3;
			data.Add(data2);
		}
