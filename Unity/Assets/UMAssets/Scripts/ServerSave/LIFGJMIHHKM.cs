...
public class LIFGJMIHHKM_LimitedItem : KLFDBFMNLBL_ServerSaveBlock
{
	public class DOPNDKHMMGI
	{
		...
		public DOPNDKHMMGI(int _DOOGFEGEKLG_Max, int JKPPKAHPPKH)
		{
	...
	public int CPIICACGNBH(int PPFNGGCBJKC, long JHNMKKNEENE, bool _DDGFCOPPBBN_test/* = false*/)
	{
		if(PPFNGGCBJKC > 0)
		{
	...
	public List<int> BHNDPHCBCKN(int PPFNGGCBJKC, long JHNMKKNEENE)
	{
		...
		if (PPFNGGCBJKC > 0)
		{
			DOPNDKHMMGI COCEIPAKJKF_Item = ODHBHOGFNAA[PPFNGGCBJKC - 1];
			for(int i = 0; i < COCEIPAKJKF_Item.MAHLPMJFLLJ; i++)
			{
				if(COCEIPAKJKF_Item.PJADHDHKOEJ[i].NIENPFFLMCH(JHNMKKNEENE))
				{
					res.Add(i);
				}
			}
