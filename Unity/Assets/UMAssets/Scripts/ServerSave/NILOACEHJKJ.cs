...
public class NILOACEHJKJ_EventSP : KLFDBFMNLBL_ServerSaveBlock
{
	...
	public class MDCMFPENCEK
	{
		...
		public List<IKCGAJKCPFN> NNMPGOAGEOL = new List<IKCGAJKCPFN>(180); // 0xA4
	...
	public override void OKJPIBHMKMJ(EDOHBJAPLPF_JsonData OILEIIEIBHP, long MCKEOKFMLAH)
	{
		...
		for(int i = 0; i < 180; i++)
		{
			data5.Add(FBCJICEPLED[0].NNMPGOAGEOL[i].NOJCMGAFAAC());
		}
				...
				if(block.HNBFOAJIIAL_Count > 0)
				{
					...
					for(int i = 0; i < 1; i++)
					{
						...
						if(d.BBAJPINMOEP_Contains(AFEHLCGHAEE_Strings.NNMPGOAGEOL_quests))
						{
							...
							for(int j = 0; j < cnt; j++)
							{
								IKCGAJKCPFN ik = CCBEKGNDDBE.NNMPGOAGEOL[j];
