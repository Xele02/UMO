	...
	private void HLKDMJHBPON()
	{
		...
		for(int i = 0; i < DGCJCAHIAPP_Diva.NBIGLBMHEDC_DivaList.Count; i++)
		{
			if(DGCJCAHIAPP_Diva.NBIGLBMHEDC_DivaList[i].BEEAIAAJOHD_CostumeId != 0)
			{
				...
				{
					if(!BEKHNNCGIEL_Costume.FABAGMLEKIB[DGCJCAHIAPP_Diva.NBIGLBMHEDC_DivaList[i].BEEAIAAJOHD_CostumeId - 1].CGKAEMGLHNK_Possessed())
					{
		...
		for(int i = 0; i < 16; i++)
		{
			...
			{
				if(MLAFAACKKBG_Unit.GCINIJEMHFK(i).FDBOPFEOENF[j].DIPKCALNIII_Id != 0)
				{
					if(!IMMAOANGPNK.HHCJCDFCLOB.NKEBMCIMJND_Database.MFPNGNMFEAL_Costume.OEMKAFGPOCE_IsCostumeAvaiable(MLAFAACKKBG_Unit.GCINIJEMHFK(i).FDBOPFEOENF[j].BEEAIAAJOHD_CId, MLAFAACKKBG_Unit.GCINIJEMHFK(i).FDBOPFEOENF[j].DIPKCALNIII_Id))
					{
						MLAFAACKKBG_Unit.GCINIJEMHFK(i).FDBOPFEOENF[j].BEEAIAAJOHD_CId = 0;
					}
					else
					{
						if(!BEKHNNCGIEL_Costume.FABAGMLEKIB[MLAFAACKKBG_Unit.GCINIJEMHFK(i).FDBOPFEOENF[j].BEEAIAAJOHD_CId - 1].CGKAEMGLHNK_Possessed())
						{
							MLAFAACKKBG_Unit.GCINIJEMHFK(i).FDBOPFEOENF[j].BEEAIAAJOHD_CId = 0;
							MLAFAACKKBG_Unit.GCINIJEMHFK(i).FDBOPFEOENF[j].AFNIOJHODAG_ColId = 0;
						}
					}
				}
			}
