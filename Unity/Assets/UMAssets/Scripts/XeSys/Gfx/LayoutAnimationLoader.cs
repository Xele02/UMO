...
namespace XeSys.Gfx
{
	public class LayoutAnimationLoader
	{
		...
		{
			TodoLogger.LogError(TodoLogger.Layout, "Anim loaded : "+fro.path+" error : "+fro.error);
			string list;
			if(fro.unityObject != null)
			{
				list = (fro.unityObject as TextAsset).text;
				TodoLogger.LogError(TodoLogger.Layout, "Loading anim "+fro.path+" as text asset "+list);
			}
			else
			{
				list = Encoding.ASCII.GetString(fro.bytes);
				TodoLogger.LogError(TodoLogger.Layout, "Loading anim "+fro.path+" as bytes asset "+list);
			}
