...
public class LGMEPLIJLNB
{
	...
	public static List<LGMEPLIJLNB> FKDIMODKKJD_GetEpisodeRewards(int KELFCMEOPPM)
	{
        ...
        OCLHKHAMDHF_Episode.JEHNEEBBDBO_EpisodeInfo saveEp = CIOECGOMILE.HHCJCDFCLOB.AHEFHIMGIBI_PlayerData.NGHJPEIKLJL_Episode.BBAJKJPKOHD_EpisodeList[KELFCMEOPPM - 1];
		FMLIFJBPFNA_Step f = IMMAOANGPNK.HHCJCDFCLOB.NKEBMCIMJND_Database.MOLEPBNJAGE_Episode.KODIKHBMBBJ_Steps.Find((FMLIFJBPFNA_Step _GHPLINIACBB_x) => {
			//0x17F58C0
			return _GHPLINIACBB_x.IOFHEGJPHKG_SId == NDFIEMPPMLF.IOFHEGJPHKG_StepId;
		});
		int startPoint = 0;
		for(int i = 0; i < NDFIEMPPMLF.FGOGPCMHPIN_Count; i++)
		{
            short FCEJJEPFIPH_rwid = NDFIEMPPMLF.HHJGBJCIFON_Rewards[i];
	...
	public static LGMEPLIJLNB BMFKMFNPGPC(int KELFCMEOPPM, bool MPCFDBHLOGM)
	{
		...
		{
			HMGPODKEFBA_EpisodeInfo NDFIEMPPMLF = IMMAOANGPNK.HHCJCDFCLOB.NKEBMCIMJND_Database.MOLEPBNJAGE_Episode.BBAJKJPKOHD_EpisodeList[KELFCMEOPPM - 1];
			OCLHKHAMDHF_Episode.JEHNEEBBDBO_EpisodeInfo saveInfo = CIOECGOMILE.HHCJCDFCLOB.AHEFHIMGIBI_PlayerData.NGHJPEIKLJL_Episode.BBAJKJPKOHD_EpisodeList[KELFCMEOPPM - 1];
...
public class PIGBBNDPPJC
{
	...
	{
		CIOECGOMILE.HHCJCDFCLOB.AHEFHIMGIBI_PlayerData.NGHJPEIKLJL_Episode.BBAJKJPKOHD_EpisodeList[KELFCMEOPPM_EpId - 1].LHMOAJAIJCO_IsNew = false;
	}
	...
	public void KHEKNNFCAOI(int KELFCMEOPPM)
	{
		...
		else
		{
			...
			OCLHKHAMDHF_Episode.JEHNEEBBDBO_EpisodeInfo serverEpInfo = CIOECGOMILE.HHCJCDFCLOB.AHEFHIMGIBI_PlayerData.NGHJPEIKLJL_Episode.BBAJKJPKOHD_EpisodeList[KELFCMEOPPM - 1];
			...
			FKMAEKNOLJB_EpisodeNo = NDFIEMPPMLF_epInfo.EILKGEADKGH_Order;
			...
			{
				short EIHOBHDKCDB_RewardId = NDFIEMPPMLF_epInfo.HHJGBJCIFON_Rewards[JJJNKGBCFMI_Step];
				...
				{
					//0x16D369C
					return NDFIEMPPMLF_epInfo.IOFHEGJPHKG_StepId == _GHPLINIACBB_x.IOFHEGJPHKG_SId;
				});
				...
				if(st != null && rw != null)
				{
					...
					JBFLCHFEIGL.DNBFMLBNAEE_Point = st.JENFHJDFFAD_Pt[JJJNKGBCFMI_Step];
	...
	public void FBANBDCOEJL()
	{
		if(KELFCMEOPPM_EpId > 0)
		{
			...
			OCLHKHAMDHF_Episode.JEHNEEBBDBO_EpisodeInfo saveEpInfo = CIOECGOMILE.HHCJCDFCLOB.AHEFHIMGIBI_PlayerData.NGHJPEIKLJL_Episode.BBAJKJPKOHD_EpisodeList[KELFCMEOPPM_EpId - 1];
			...
			if(JJJNKGBCFMI_Step < FGOGPCMHPIN_Count)
			{
				int EIHOBHDKCDB_RewardId = epInfo.HHJGBJCIFON_Rewards[JJJNKGBCFMI_Step];
				FMLIFJBPFNA_Step step = IMMAOANGPNK.HHCJCDFCLOB.NKEBMCIMJND_Database.MOLEPBNJAGE_Episode.KODIKHBMBBJ_Steps.Find((FMLIFJBPFNA_Step _GHPLINIACBB_x) =>
				{
					//0x16D37A4
					return _GHPLINIACBB_x.IOFHEGJPHKG_SId == epInfo.IOFHEGJPHKG_StepId;
				});
				JNIKPOIKFAC_Reward reward = IMMAOANGPNK.HHCJCDFCLOB.NKEBMCIMJND_Database.MOLEPBNJAGE_Episode.LFAAEPAAEMB_Rewards.Find((JNIKPOIKFAC_Reward _GHPLINIACBB_x) =>
				{
					//0x16D383C
				...
				if(step != null && reward != null)
				{
					...
					data.DNBFMLBNAEE_Point = step.JENFHJDFFAD_Pt[JJJNKGBCFMI_Step];
	...
	{
		OCLHKHAMDHF_Episode.JEHNEEBBDBO_EpisodeInfo info = CIOECGOMILE.HHCJCDFCLOB.AHEFHIMGIBI_PlayerData.NGHJPEIKLJL_Episode.BBAJKJPKOHD_EpisodeList[KELFCMEOPPM_EpId - 1];
		if(info.HPLMMKHBKIG_Id == KELFCMEOPPM_EpId)
		{
			...
			EGOLBAPFHHD_Common.AMCANGCIBEG info3 = CIOECGOMILE.HHCJCDFCLOB.AHEFHIMGIBI_PlayerData.KCCLEHLLOFG_Common.GJODJNIHKKF_EpiItem[BNGHLLCONJM - 1];
			if(HMFFHLPNMPH != 0 && BNGHLLCONJM == info2.PPFNGGCBJKC_Id)
			{
				return HMFFHLPNMPH <= info3.BFINGCJHOHI_Count ? 1 : 0;
			}
		}
		return -1;
	}
	...
	public int PFMLAOPEAMJ(int BNGHLLCONJM, int HMFFHLPNMPH, IMCBBOAFION _BHFHGFKBOHH_OnSuccess)
	{
		...
		if (res > 0)
		{
			...
			if(f == null)
			{
				...
				OCLHKHAMDHF_Episode.JEHNEEBBDBO_EpisodeInfo ep = CIOECGOMILE.HHCJCDFCLOB.AHEFHIMGIBI_PlayerData.NGHJPEIKLJL_Episode.BBAJKJPKOHD_EpisodeList[KELFCMEOPPM_EpId - 1];
				KIICLPJJBNL_EpiItem.NKGPGMOHAFM info2 = IMMAOANGPNK.HHCJCDFCLOB.NKEBMCIMJND_Database.NEGGMILDEEF_EpiItem.CDENCMNHNGA_table[BNGHLLCONJM - 1];
				EGOLBAPFHHD_Common.AMCANGCIBEG info3 = CIOECGOMILE.HHCJCDFCLOB.AHEFHIMGIBI_PlayerData.KCCLEHLLOFG_Common.GJODJNIHKKF_EpiItem[BNGHLLCONJM - 1];
				info3.BFINGCJHOHI_Count -= HMFFHLPNMPH;
				if(info3.BFINGCJHOHI_Count < 0)
					info3.BFINGCJHOHI_Count = 0;
	...
	{
		return GMDHJBGLBEI(KELFCMEOPPM, CIOECGOMILE.HHCJCDFCLOB.AHEFHIMGIBI_PlayerData);
	}

	//// RVA: 0x16D11D0 Offset: 0x16D11D0 VA: 0x16D11D0
	private bool FIBLGPNEKEM(int KELFCMEOPPM)
	{
		for(int i = 0; i < CIOECGOMILE.HHCJCDFCLOB.AHEFHIMGIBI_PlayerData.PNLOINMCCKH_Scene.OPIBAPEGCLA.Count; i++)
		{
			MMPBPOIFDAF_Scene.PMKOFEIONEG serverScene = CIOECGOMILE.HHCJCDFCLOB.AHEFHIMGIBI_PlayerData.PNLOINMCCKH_Scene.OPIBAPEGCLA[i];
	...
	public static int GMDHJBGLBEI(int KELFCMEOPPM, BBHNACPENDM_ServerSaveData _AHEFHIMGIBI_PlayerData)
	{
		OCLHKHAMDHF_Episode.JEHNEEBBDBO_EpisodeInfo saveEp = CIOECGOMILE.HHCJCDFCLOB.AHEFHIMGIBI_PlayerData.NGHJPEIKLJL_Episode.BBAJKJPKOHD_EpisodeList[KELFCMEOPPM - 1];
		MLIBEPGADJH_Scene sceneDb = IMMAOANGPNK.HHCJCDFCLOB.NKEBMCIMJND_Database.ECNHDEHADGL_Scene;
		List<MMPBPOIFDAF_Scene.PMKOFEIONEG> sceneList = _AHEFHIMGIBI_PlayerData.PNLOINMCCKH_Scene.OPIBAPEGCLA;
		int l30 = 0;
		int l2c = 0;
		for(int i = 0; i < sceneList.Count; i++)
		{
			...
			{
				if(sceneDb.CDENCMNHNGA_table[saveScene.PPFNGGCBJKC_Id - 1].KELFCMEOPPM_Ep == KELFCMEOPPM)
				{
