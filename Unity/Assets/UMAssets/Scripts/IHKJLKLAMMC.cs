...
public class IHKJLKLAMMC
{
	...
	public void KHEKNNFCAOI(List<LOBDIAABMKG> MKHCLKMFBLJ)
	{
		...
		for(int i = 0; i < l.Count; i++)
		{
			if(l[i].PLALNIIBLOF_Enabled == 2)
			{
				...
				if(l1 == 0)
					l1 = dbEv.NGHKJOEDLIP.BONDDBOFBND_RankingStart;
					...
					case 5:
						{
							...
							{
								//0x120236C
								return _GHPLINIACBB_x.PPFNGGCBJKC_Id == KECECJPFEPM.BOJCOPAALNC_EventId && _GHPLINIACBB_x.PLALNIIBLOF_Enabled == 2;
							});
							if(d != null)
							{
							...
							if(d != null)
							{
								...
								if(s != null)
								{
									...
									if(t >= s.NGHKJOEDLIP.JGMDAOACOJF)
									{
										...
										KECECJPFEPM.FDBNFFNFOND_CloseAt = s.NGHKJOEDLIP.IDDBFFBPNGI;
										//LAB_011ffc68
										KECECJPFEPM.MMJHAMFEHCH = OHKECKAPJJL.NBADGBJMBMM.IIBKMHIDNPM_1;
									}
									//LAB_011ffca4
	...
	private void NBKMNMGEELE(OHKECKAPJJL IOCGPICMPOG, OEIJEFBBJBD_EventSp NDFIEMPPMLF, List<LOBDIAABMKG> MKHCLKMFBLJ, long JHNMKKNEENE)
	{
		...
		{
			NDNHHGJKJGM = NDFIEMPPMLF.AIJIJLFKNOM.Find((OEIJEFBBJBD_EventSp.NJJEAKMBGPN _GHPLINIACBB_x) =>
			{
				//0x12026FC
				return _GHPLINIACBB_x.PPFNGGCBJKC_Id == IOCGPICMPOG.BOJCOPAALNC_EventId && _GHPLINIACBB_x.PLALNIIBLOF_Enabled == 2;
			});
		}
		else if(IOCGPICMPOG.PNDEAHGLJIC_BtnType == OHKECKAPJJL.GPNHNIGPGCL.ILBPPODCPPP_9_Gacha2)
		{
			NDNHHGJKJGM = NDFIEMPPMLF.OAIHCOFBEIE.Find((OEIJEFBBJBD_EventSp.NJJEAKMBGPN _GHPLINIACBB_x) =>
			{
				//0x1202694
				return _GHPLINIACBB_x.PPFNGGCBJKC_Id == IOCGPICMPOG.BOJCOPAALNC_EventId && _GHPLINIACBB_x.PLALNIIBLOF_Enabled == 2;
			});
		...
		{
			NDNHHGJKJGM = NDFIEMPPMLF.MODIJHPMHGJ.Find((OEIJEFBBJBD_EventSp.NJJEAKMBGPN _GHPLINIACBB_x) =>
			{
				//0x120262C
				return _GHPLINIACBB_x.PPFNGGCBJKC_Id == IOCGPICMPOG.BOJCOPAALNC_EventId && _GHPLINIACBB_x.PLALNIIBLOF_Enabled == 2;
			});
		...
		if(NDNHHGJKJGM != null)
		{
			...
			IOCGPICMPOG.FDBNFFNFOND_CloseAt = NDNHHGJKJGM.FDBNFFNFOND_CloseAt;
			IOCGPICMPOG.BDEOMEBFDFF_GachaId = NDNHHGJKJGM.BDEOMEBFDFF_GachaId;
			if(JHNMKKNEENE >= IOCGPICMPOG.PDBPFJJCADD_Start && IOCGPICMPOG.FDBNFFNFOND_CloseAt >= JHNMKKNEENE)
			{
				IOCGPICMPOG.OAAKAAFFFLE = OHKECKAPJJL.ONKLMFNGCHJ.LAOEGNLOJHC_2;
				LOBDIAABMKG d = MKHCLKMFBLJ.Find((LOBDIAABMKG _GHPLINIACBB_x) =>
				{
					//0x1202764
					return NDNHHGJKJGM.BDEOMEBFDFF_GachaId == _GHPLINIACBB_x.FDEBLMKEMLF_TypeAndSeriesId;
				});
				if(d != null)
				{
					if(IOCGPICMPOG.PNDEAHGLJIC_BtnType == OHKECKAPJJL.GPNHNIGPGCL.LAJAJEMJBFC_12_Gacha3)
	...
	{
		int idx = NDFIEMPPMLF.LMLMPLLNEDG.FindIndex((OEIJEFBBJBD_EventSp.BPNLIPDNKOH _GHPLINIACBB_x) =>
		{
			//0x12027B0
			return _GHPLINIACBB_x.PPFNGGCBJKC == IOCGPICMPOG.BOJCOPAALNC_EventId;
		});
		...
		IOCGPICMPOG.FDBNFFNFOND_CloseAt = NDFIEMPPMLF.LMLMPLLNEDG[idx].FDBNFFNFOND_CloseAt;
	}
