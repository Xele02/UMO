...
public class IHKJLKLAMMC
{
	...
	public string AFMHNPCDAFI_GetBgId(long _JHNMKKNEENE_Time)
	{
		if(KEFGPJBKAOD_BgId == "")
		{
            IKDICBBFBMI_NetEventBaseController ev = JEPBIIJDGEF_NetEventManager.HHCJCDFCLOB_Instance.OEGDCBLNNFF(OHCAABOMEOF.KGOGMKMBCPP_EventType.ENPJADLIFAB_EventSp, KGCNCBOKCBA.GNENJEHKMHD_EventStatus.BCKENOKGLIJ_9_ResultRewardreceived);
			OEIJEFBBJBD_EventSp dbEv = IMMAOANGPNK_NetMasterDataManager.HHCJCDFCLOB_Instance.NKEBMCIMJND_Database.LBDOLHGDIEB_Find(ev.JOPOPMLFINI_QuestName) as OEIJEFBBJBD_EventSp;
	...
	public void KHEKNNFCAOI_Init(List<LOBDIAABMKG_GachaProductData> MKHCLKMFBLJ)
	{
        KNKDBNFMAKF_NetEventSpController ev = JEPBIIJDGEF_NetEventManager.HHCJCDFCLOB_Instance.OEGDCBLNNFF(OHCAABOMEOF.KGOGMKMBCPP_EventType.ENPJADLIFAB_EventSp, KGCNCBOKCBA.GNENJEHKMHD_EventStatus.BCKENOKGLIJ_9_ResultRewardreceived) as KNKDBNFMAKF_NetEventSpController;
			...
			if(l[i].PLALNIIBLOF_en == 2)
			{
				...
				switch(l[i].PNDEAHGLJIC_BtnType)
				{
					...
					case 5:
						{
							...
							if(d != null)
							{
                                CHHECNJBMLA_NetEventBoxGachaController ev2 = JEPBIIJDGEF_NetEventManager.HHCJCDFCLOB_Instance.OIKOHACJPCB_GetEventById(d.BDEOMEBFDFF_gacha_id) as CHHECNJBMLA_NetEventBoxGachaController;
								if(ev2 != null)
								{
									IMDBGDNPLJA_EventBoxGacha dbEv2 = IMMAOANGPNK_NetMasterDataManager.HHCJCDFCLOB_Instance.NKEBMCIMJND_Database.LBDOLHGDIEB_Find(ev2.JOPOPMLFINI_QuestName) as IMDBGDNPLJA_EventBoxGacha;
					...
					case 7:
						{
							...
							if(d != null)
							{
                                ...
                                IKDICBBFBMI_NetEventBaseController ev2 = JEPBIIJDGEF_NetEventManager.HHCJCDFCLOB_Instance.OIKOHACJPCB_GetEventById(d.DNJLJMKKDNA_EventId);
								if(ev2 != null)
								{
					...
					case 18:
						...
						{
							JKICPBIIHNE_Bingo.HNOGDJFJGPM b = GNGMCIAIKMA.HHCJCDFCLOB_Instance.EBEDAPJFHCE_GetBingo(KECECJPFEPM.BOJCOPAALNC_EventId);
							if(b != null)
							{
								KECECJPFEPM.OAAKAAFFFLE = OHKECKAPJJL.ONKLMFNGCHJ_SpStep.NHICGIPPMBD_1_NotStarted;
								...
								if(GNGMCIAIKMA.HHCJCDFCLOB_Instance.EAJLHMIMAPE(KECECJPFEPM.BOJCOPAALNC_EventId))
								{
									//LAB_011ff2a8
									KECECJPFEPM.OAAKAAFFFLE = OHKECKAPJJL.ONKLMFNGCHJ_SpStep.OLCLJKOKJCD_5_End;
									KECECJPFEPM.FICHDKOOOOB_Enabled = false;
	...
	{
        IKDICBBFBMI_NetEventBaseController ev = JEPBIIJDGEF_NetEventManager.HHCJCDFCLOB_Instance.OIKOHACJPCB_GetEventById(_EKANGPODCEP_EventId);
		if(ev == null)
			return null;
		return IMMAOANGPNK_NetMasterDataManager.HHCJCDFCLOB_Instance.NKEBMCIMJND_Database.LBDOLHGDIEB_Find(ev.JOPOPMLFINI_QuestName);
    }

	// // RVA: 0x1200230 Offset: 0x1200230 VA: 0x1200230
	private void NBKMNMGEELE(OHKECKAPJJL IOCGPICMPOG, OEIJEFBBJBD_EventSp _NDFIEMPPMLF_master, List<LOBDIAABMKG_GachaProductData> MKHCLKMFBLJ, long _JHNMKKNEENE_Time)
	{
		...
		if(NDNHHGJKJGM_Gach != null)
		{
			...
			if(_JHNMKKNEENE_Time >= IOCGPICMPOG.PDBPFJJCADD_open_at && IOCGPICMPOG.FDBNFFNFOND_close_at >= _JHNMKKNEENE_Time)
			{
				...
				LOBDIAABMKG_GachaProductData d = MKHCLKMFBLJ.Find((LOBDIAABMKG_GachaProductData _GHPLINIACBB_x) =>
				{
					//0x1202764
				...
				if(d != null)
				{
					if(IOCGPICMPOG.PNDEAHGLJIC_BtnType == OHKECKAPJJL.GPNHNIGPGCL_SpBtnType.LAJAJEMJBFC_12_Gacha3)
					{
						if(!d.FJAOAGNFABN_HasOneDay)
							return;
						EGOLBAPFHHD_Common.PCHECKGDJDK d2 = CIOECGOMILE_NetPlayerDataManager.HHCJCDFCLOB_Instance.AHEFHIMGIBI_PlayerData.KCCLEHLLOFG_Common.BGDMJGDEKFJ_GetGachaDraw(d.FDEBLMKEMLF_TypeAndSeriesId);
						if(d2 == null)
							return;
						IOCGPICMPOG.OAAKAAFFFLE = OHKECKAPJJL.ONKLMFNGCHJ_SpStep.LAOEGNLOJHC_2_Start;
					...
					{
						if(LOBDIAABMKG_GachaProductData.GPKPIGPDFNL(MKHCLKMFBLJ, d.HHIBBHFHENH_LinkQuestId, d.GPDIDIJDKAG_LinkCount))
							return;
					}
					else
						return;
	...
	private void JLAMIHICOLK(OHKECKAPJJL IOCGPICMPOG, OEIJEFBBJBD_EventSp _NDFIEMPPMLF_master, long _JHNMKKNEENE_Time)
	{
		...
		for(int i = 0; i < EJHPIMANJFP_NetPaidVCProdcutManager.HHCJCDFCLOB_Instance.MHKCPJDNJKI_products.Count; i++)
		{
			...
			{
				if(EJHPIMANJFP_NetPaidVCProdcutManager.HHCJCDFCLOB_Instance.MHKCPJDNJKI_products[i].LHENLPLKGLP_StuffId >= 0 && EJHPIMANJFP_NetPaidVCProdcutManager.HHCJCDFCLOB_Instance.MHKCPJDNJKI_products[i].LHENLPLKGLP_StuffId == _NDFIEMPPMLF_master.LMLMPLLNEDG[idx].ALMEBFDEGBG[j])
				{
					d = _NDFIEMPPMLF_master.LMLMPLLNEDG[idx];
					if(EJHPIMANJFP_NetPaidVCProdcutManager.HHCJCDFCLOB_Instance.MHKCPJDNJKI_products[i].AJIFADGGAAJ_BoughtQuantity < EJHPIMANJFP_NetPaidVCProdcutManager.HHCJCDFCLOB_Instance.MHKCPJDNJKI_products[i].GCJMGMBNBCB_BuyLimit)
						break;
				}
			}
		}
	...
	private void IPPIDIDKAKO(OHKECKAPJJL BABOFCLOBGG)
	{
		...
		long t = NKGJPJPHLIF_SakashoManager.HHCJCDFCLOB_Instance.IBLPICFDGOF_ServerRequester.FJDBNGEPKHL_Time.KMEFBNBFJHI_GetServerTime();
		if(GNGMCIAIKMA.HHCJCDFCLOB_Instance.GBCPDBJEDHL(t))
		{
			...
			GNGMCIAIKMA.HHCJCDFCLOB_Instance.DJGFICMNGGP_SetBingoId(args.viewData.PGIIDPEGGPI_EventId);
			GNGMCIAIKMA.HHCJCDFCLOB_Instance.BHFGBNNEMLI(args.viewData.PGIIDPEGGPI_EventId);
	...
	public void GEOGJOBNKHG(OHKECKAPJJL BABOFCLOBGG)
	{
		switch(BABOFCLOBGG.LPDLBACJKIB_TransId)
		{
			...
			case 2:
				{
                    IKDICBBFBMI_NetEventBaseController ev = JEPBIIJDGEF_NetEventManager.HHCJCDFCLOB_Instance.OIKOHACJPCB_GetEventById(BABOFCLOBGG.GGHDEDJFFOM);
					if(ev != null)
					{
