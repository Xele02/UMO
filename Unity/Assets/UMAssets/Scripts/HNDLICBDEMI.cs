...
public class HNDLICBDEMI
{
	...
	public static void FMLGCFKNKIA_GetDecoPlayerLevelAndEnabled(out int OCCIIDFGNEH, out bool HGNKCCAMHAM)
	{
		...
		{
			OCCIIDFGNEH = IMMAOANGPNK_NetMasterDataManager.HHCJCDFCLOB_Instance.NKEBMCIMJND_Database.GDEKCOOBLMA_System.LPJLEHAJADA_GetIntParam("deco_player_level", OCCIIDFGNEH);
		}
		if(CIOECGOMILE_NetPlayerDataManager.HHCJCDFCLOB_Instance.AHEFHIMGIBI_PlayerData == null)
			return;
		if(OCCIIDFGNEH > 0)
		{
			if(OCCIIDFGNEH <= CIOECGOMILE_NetPlayerDataManager.HHCJCDFCLOB_Instance.AHEFHIMGIBI_PlayerData.KCCLEHLLOFG_Common.KIECDDFNCAN_Level)
				HGNKCCAMHAM = true;
		}
	}
