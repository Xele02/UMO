...
public class FOCPLKMMCAL
{
	...
	public bool HNNPBABEPBP(JGEOBNENMAH.HAJIFNABIFF OMNOFMEBLAD, EAJCBFGKKFA_FriendInfo IPGFALGDLHN_FriendInfo)
	{
		...
		if(OMNOFMEBLAD.GHBPLHBNMBK_FreeMusicId < 1)
		{
			...
			int prevUc = CIOECGOMILE.HHCJCDFCLOB.AHEFHIMGIBI_PlayerData.KCCLEHLLOFG_Common.NFHLDFJIBKI_HaveUc;
			if (!BEEAKFJAPOB(OMNOFMEBLAD, mData, false))
				return false;
			MJKFDDKLLFP_DropUnionCredit = CIOECGOMILE.HHCJCDFCLOB.AHEFHIMGIBI_PlayerData.KCCLEHLLOFG_Common.NFHLDFJIBKI_HaveUc - prevUc;
			return true;
		}
		if (OMNOFMEBLAD.JPJMALBLKDI_Tutorial == 0)
		{
						...
						if(!b && !GNAMCOJKEFE)
						{
							...
							if(eventScore == null)
							{
								if(!OMNOFMEBLAD.PMCGHPOGLGM_IsSkip)
								{
									...
									if(b2)
									{
										CIOECGOMILE.HHCJCDFCLOB.AHEFHIMGIBI_PlayerData.FLBPFBFKBFC_FreeScoreMax.POIKGADFLHF_SetPreciseScoreForMusic(OMNOFMEBLAD.GHBPLHBNMBK_FreeMusicId, scoreMusic, NKGJPJPHLIF.HHCJCDFCLOB.IBLPICFDGOF_ServerRequester.FJDBNGEPKHL.KMEFBNBFJHI_GetServerTime());
									}
								}
								rmusic.IFGGJEPHIAN();
								if (l.Count > 0)
			...
			if(cat < EKLNMHFCAOI.FKGCBLHOOCL_Category.EMOLGEDEEJP_EventItem)
			{
				if(cat != EKLNMHFCAOI.FKGCBLHOOCL_Category.HJNNKCMLGFL_None)
				{
					if(cat == EKLNMHFCAOI.FKGCBLHOOCL_Category.MHKFDBLMOGF_Scene)
					{
						...
						MMPBPOIFDAF_Scene.PMKOFEIONEG saveScene = CIOECGOMILE.HHCJCDFCLOB.AHEFHIMGIBI_PlayerData.PNLOINMCCKH_Scene.OPIBAPEGCLA[itemId - 1];
	...
	private void BGGHEILFEIN(EKLNMHFCAOI.FKGCBLHOOCL_Category INDDJNMPONH, int PPFNGGCBJKC, int HMFFHLPNMPH)
	{
		...
		JANMJPOKLFL.CPIICACGNBH_AddItem(CIOECGOMILE.HHCJCDFCLOB.AHEFHIMGIBI_PlayerData, INDDJNMPONH, PPFNGGCBJKC, HMFFHLPNMPH, null, 0);
	}
	...
	private void KJAFCPAFDBK(JGEOBNENMAH.HAJIFNABIFF OMNOFMEBLAD, EONOEHOKBEB_Music EPMMNEFADAP)
	{
		if(OMNOFMEBLAD.KLCIIHKFPPO_StoryMusicId > 0)
		{
			...
			if(dbStory != null)
			{
				NEKDCJKANAH_StoryRecord.HKDNILFKCFC saveStory = CIOECGOMILE.HHCJCDFCLOB.AHEFHIMGIBI_PlayerData.LNOOKHJBENO_StoryRecord.MMKAJBFBKNH[dbStory.LFLLLOPAKCO_Id - 1];
	...
	private void LKDINNAPACD(JGEOBNENMAH.HAJIFNABIFF OMNOFMEBLAD, EONOEHOKBEB_Music EPMMNEFADAP)
	{
		if(OMNOFMEBLAD.JPJMALBLKDI_Tutorial < 1)
		{
			...
			{
				CIOECGOMILE.HHCJCDFCLOB.AHEFHIMGIBI_PlayerData.OEKEIGFAIGN_Counter.EJFAEKPGKNJ_Day.FBKAPLHEACL_AddMClr();
				CIOECGOMILE.HHCJCDFCLOB.AHEFHIMGIBI_PlayerData.OEKEIGFAIGN_Counter.EJFAEKPGKNJ_Day.DPKJNIPGGMJ_AddSRnk(PENICOGGNLF_ScoreRank);
				if(OMNOFMEBLAD.HGEKDNNJAAC_HadAwakenDivaMode)
				{
					CIOECGOMILE.HHCJCDFCLOB.AHEFHIMGIBI_PlayerData.OEKEIGFAIGN_Counter.EJFAEKPGKNJ_Day.MAFAKCMFHEE_AddSdv();
				}
				CIOECGOMILE.HHCJCDFCLOB.AHEFHIMGIBI_PlayerData.OEKEIGFAIGN_Counter.EJFAEKPGKNJ_Day.CIGILPOKMAN_AddAS(OMNOFMEBLAD.BCGBODDEFKP_NumSkillActive);
				...
				{
					CIOECGOMILE.HHCJCDFCLOB.AHEFHIMGIBI_PlayerData.OEKEIGFAIGN_Counter.BEJONIOEGCI(EPMMNEFADAP.DLAEJOBELBH_Id, OMNOFMEBLAD.AKNELONELJK_Difficulty, true, OMNOFMEBLAD.LCOHGOIDMDF_ComboRank != 0, OMNOFMEBLAD.OJFNDOIFOOE_NoteResultCount, OMNOFMEBLAD.PMCGHPOGLGM_IsSkip, OMNOFMEBLAD.LFGNLKKFOCD_IsLine6);
				}
				if(!OMNOFMEBLAD.LFGNLKKFOCD_IsLine6)
				{
					for(int j = 0; j < 5; j++)
			...
			if(!OMNOFMEBLAD.PMCGHPOGLGM_IsSkip)
			{
				if(CIOECGOMILE.HHCJCDFCLOB.AHEFHIMGIBI_PlayerData.MHEAEGMIKIE_PublicStatus.AEBENOJEGOJ_MaxScore < OMNOFMEBLAD.KNIFCANOHOC_Score)
				{
					...
					CIOECGOMILE.HHCJCDFCLOB.AHEFHIMGIBI_PlayerData.MHEAEGMIKIE_PublicStatus.JHOIMONJKLG_MaxId = IMMAOANGPNK.HHCJCDFCLOB.NKEBMCIMJND_Database.IBPAFKKEKNK_Music.CIKALPJDGMF_ResolveMusicId(OMNOFMEBLAD.GHBPLHBNMBK_FreeMusicId, EPMMNEFADAP.DLAEJOBELBH_Id);
