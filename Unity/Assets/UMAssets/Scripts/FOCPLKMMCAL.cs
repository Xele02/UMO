...
public class FOCPLKMMCAL
{
		...
		if(music != null)
		{
			...
			if(k != null)
			{
				...
				else
				{
					...
					{
						// L237
						HAEDCCLHEMN_EventBattle evBattle = JEPBIIJDGEF_EventInfo.HHCJCDFCLOB.MKBJOOAILBB_GetEventByStatus(KGCNCBOKCBA.GNENJEHKMHD_EventStatus.EMAMLLFAOJI_6, false) as HAEDCCLHEMN_EventBattle;
						if(evBattle != null)
						{
	...
	public bool HNNPBABEPBP(JGEOBNENMAH.HAJIFNABIFF OMNOFMEBLAD, EAJCBFGKKFA_FriendInfo IPGFALGDLHN_FriendInfo)
	{
		...
		if(OMNOFMEBLAD.JPJMALBLKDI_Tutorial == 0)
		{
			BDKANJGDMPK_GetBaseUnionCredit(score.ANAJIAENLNB_F_pt, false, OMNOFMEBLAD.LFGNLKKFOCD_IsLine6);
		}
		...
		switch(OMNOFMEBLAD.MNNHHJBBICA_GameEventType)
		{
			...
			case 6:
				...
				break;
			case 14:
				TodoLogger.LogError(TodoLogger.EventGoDiva_14, "HNNPBABEPBP event");
				break;
			default:
				break;
		}
	...
	private bool BEEAKFJAPOB(JGEOBNENMAH.HAJIFNABIFF OMNOFMEBLAD, EONOEHOKBEB_Music EPMMNEFADAP_MusicInfo, bool JCCBGECHIEI)
	{
		...
		if (OMNOFMEBLAD.JPJMALBLKDI_Tutorial < 1)
		{
			...
			else
			{
				...
				if(OMNOFMEBLAD.MFJKNCACBDG_OpenEventType == 0 || JEPBIIJDGEF_EventInfo.HHCJCDFCLOB.MKBJOOAILBB_GetEventByStatus(KGCNCBOKCBA.GNENJEHKMHD_EventStatus.EMAMLLFAOJI_6, false) == null)
				{
					d = false;
				}
				else
				{
					event_ = JEPBIIJDGEF_EventInfo.HHCJCDFCLOB.MKBJOOAILBB_GetEventByStatus(KGCNCBOKCBA.GNENJEHKMHD_EventStatus.EMAMLLFAOJI_6, false);
	...
	private bool ILDLMKGBKIL(JGEOBNENMAH.HAJIFNABIFF OMNOFMEBLAD, EONOEHOKBEB_Music EPMMNEFADAP_MusicInfo, bool JCCBGECHIEI)
	{
		if(OMNOFMEBLAD.JPJMALBLKDI_Tutorial < 1)
		{
			...
			{
				TodoLogger.LogError(TodoLogger.EventGoDiva_14, "ILDLMKGBKIL");
			}
