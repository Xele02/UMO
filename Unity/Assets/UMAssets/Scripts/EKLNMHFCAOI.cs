...
public class EKLNMHFCAOI
{
	...
	{
		return INCKKODFJAP((FKGCBLHOOCL_Category)(KIJAPOFAGPN / 10000), KIJAPOFAGPN % 10000);
	}

	// // RVA: 0x12F6114 Offset: 0x12F6114 VA: 0x12F6114
	public static string INCKKODFJAP(FKGCBLHOOCL_Category INDDJNMPONH, int JBGEEPFKIGG)
	{
		int id = JBGEEPFKIGG;
	...
	public static string NDBLEADIDLA(FKGCBLHOOCL_Category INDDJNMPONH, int JBGEEPFKIGG = 0)
	{
		if(JBGEEPFKIGG > 0)
		{
			...
			if(str != "")
			{
				str = MessageManager.Instance.GetBank(AFEHLCGHAEE_Strings.NDFIEMPPMLF_master).GetMessageByLabel(str);
				if(str != "")
					return str;
			}
		}
		return KBHKKMMINOD[(int)INDDJNMPONH];
	}
	...
	public static int ALHCGDMEMID_GetNumItems(OKGLGHCBCJP_Database LKMHPJKIFDN, BBHNACPENDM_ServerSaveData LDEGEHAEALK, FKGCBLHOOCL_Category INDDJNMPONH, int NANNGLGOFKH, IKDICBBFBMI_EventBase JMFICAICKBC)
	{
		switch(INDDJNMPONH)
		{
			...
			case FKGCBLHOOCL_Category.JHGPNDLNPFA_DivaItem:
				return LDEGEHAEALK.DGCJCAHIAPP_Diva.NBIGLBMHEDC_DivaList[NANNGLGOFKH - 1].CPGFPEDMDEH;
			case FKGCBLHOOCL_Category.DMMIIBCMCFG_EnergyItem:
