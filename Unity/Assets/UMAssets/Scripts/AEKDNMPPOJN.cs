...
public class AEKDNMPPOJN
{
	...
	{
		MMPBPOIFDAF_Scene.PMKOFEIONEG IBDJFHFIIHN_s = CIOECGOMILE.HHCJCDFCLOB.AHEFHIMGIBI_PlayerData.PNLOINMCCKH_Scene.OPIBAPEGCLA_Scenes[_BCCHOBPJJKE_SceneId - 1];
		...
		else
		{
			...
			int MJBODMOLOBC_luck = MABBBOEAPAA_p.AGOEDLNOHND(IMMAOANGPNK.HHCJCDFCLOB.NKEBMCIMJND_Database.JEMMMJEJLNL_Board, IBDJFHFIIHN_s.EMOJHJGHJLN_Sb, IBDJFHFIIHN_s.JPIPENJGGDD_NumBoard, mltMax);
				...
				if(!PLOEDFHCGFA.JMHIDPKHELB)
				{
					...
					else
					{
						if(EKLNMHFCAOI.BKHFLDMOGBD_GetItemCategory(PLOEDFHCGFA.MJNOAMAFNHA_CostItemId) == EKLNMHFCAOI.FKGCBLHOOCL_Category.HLCHKCJLEGK_GrowItem)
						{
							...
							{
								//0x15BE164
								ILCCJNDFFOB.HHCJCDFCLOB.EEOLHMGNJGO_UnlockLimitOver(_BCCHOBPJJKE_SceneId, MABBBOEAPAA_p.EKLIPGELKCL_Rarity, MJBODMOLOBC_luck, PLOEDFHCGFA.MJNOAMAFNHA_CostItemId, PLOEDFHCGFA.IJEOIMGILCK, PLOEDFHCGFA.GNKGDDMMJPF, ICPDDMIBEIL_Bf, IBDJFHFIIHN_s.DMNIMMGGJJJ_Leaf);
