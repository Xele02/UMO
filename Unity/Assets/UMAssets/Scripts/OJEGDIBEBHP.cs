...
public class OJEGDIBEBHP
{
	...
	public void KHEKNNFCAOI_Init(int _PPFNGGCBJKC_id)
	{
		...
		{
			GJALOMELEHD_Intimacy.ELAIMNHBCFB db = IMMAOANGPNK_NetMasterDataManager.HHCJCDFCLOB_Instance.NKEBMCIMJND_Database.KDIALKDKBGE_Intimacy.CJAEGOMDICD[_PPFNGGCBJKC_id - 1];
			EGOLBAPFHHD_Common.DEEGPPKGGLK save = CIOECGOMILE_NetPlayerDataManager.HHCJCDFCLOB_Instance.AHEFHIMGIBI_PlayerData.KCCLEHLLOFG_Common.DHJFHILPKLB_IntimacyPresent[_PPFNGGCBJKC_id - 1];
			ADJBIEOILPJ_ItemPresentId = _PPFNGGCBJKC_id;
			PNJICDLDCAE = db.JBGEEPFKIGG_val;
			OPFGFINHFCE_name = EKLNMHFCAOI_ItemManager.INCKKODFJAP_GetItemName(EKLNMHFCAOI_ItemManager.FKGCBLHOOCL_Category.DLBHNNOHLMM_PresentItem, _PPFNGGCBJKC_id);
			JONPKLHMOBL_Category = db.JONPKLHMOBL_Category;
			KIJAPOFAGPN_ItemId = EKLNMHFCAOI_ItemManager.GJEEGMCBGGM_GetItemFullId(EKLNMHFCAOI_ItemManager.FKGCBLHOOCL_Category.DLBHNNOHLMM_PresentItem, _PPFNGGCBJKC_id);
	...
	public static List<OJEGDIBEBHP> FKDIMODKKJD_GetList()
	{
		...
		if(IMMAOANGPNK_NetMasterDataManager.HHCJCDFCLOB_Instance.NKEBMCIMJND_Database != null)
		{
			...
			{
				int num = IMMAOANGPNK_NetMasterDataManager.HHCJCDFCLOB_Instance.NKEBMCIMJND_Database.KDIALKDKBGE_Intimacy.CJAEGOMDICD.Count;
				int num2 = CIOECGOMILE_NetPlayerDataManager.HHCJCDFCLOB_Instance.AHEFHIMGIBI_PlayerData.KCCLEHLLOFG_Common.DHJFHILPKLB_IntimacyPresent.Count;
				if (num2 < num)
					num = num2;
				for(int i = 0; i < num; i++)
				{
