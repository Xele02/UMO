...
namespace XeApp.Game.Menu
{
	public class LayoutPopupUnlockDiva : LayoutUGUIScriptBase
	{
		...
		public void SetStatus(PopupUnlock.UnlockInfo info)
		{
			if(info.param.id < 0)
				return;
			FFHPBEPOMAK_DivaInfo f = GameManager.Instance.ViewPlayerData.NBIGLBMHEDC_Divas[info.param.id - 1];
		...
		public void PlayVoice(PopupUnlock.UnlockInfo info)
		{
			if(info.param.id < 0)
				return;
			SoundManager.Instance.voGreeting.Play(GameManager.Instance.ViewPlayerData.NBIGLBMHEDC_Divas[info.param.id - 1].AHHJLDLAPAN_DivaId, 0);
		}
