...
public class DPDMNBGAONP : IHFBKJMNIPH_NetBaseLoopAction<DKFCEGODKFJ_GetPlayerCounters>
{
	...
	private int HMFFHLPNMPH_count; // 0x14

	//// RVA: 0x1233CE0 Offset: 0x1233CE0 VA: 0x1233CE0 Slot: 4
	protected override void LJDCONBNPBM_Initialize()
	{
		HMFFHLPNMPH_count = 0;
		NFEAMMJIMPG_Result.OJCNJFLJCLA_player_counters = new System.Collections.Generic.Dictionary<int, int>(BIHCCEHLAOD.FAMHAPONILI_PlayerIds.Count);
	}
	...
	protected override DKFCEGODKFJ_GetPlayerCounters JIJACMIFOMB_OnStartAction(PJKLMCGEJMK_NetActionManager _CPHFEPHDJIB_ServerRequester)
	{
		...
		if(BIHCCEHLAOD.FAMHAPONILI_PlayerIds.Count < 101)
		{
			...
			HMFFHLPNMPH_count = res.BIHCCEHLAOD.FAMHAPONILI_PlayerIds.Count;
		}
		else
		{
			DKFCEGODKFJ_GetPlayerCounters.ENKJGMFOJDI data = new DKFCEGODKFJ_GetPlayerCounters.ENKJGMFOJDI();
			data.IHALNOJAMLE_PlayerCounterMasterName = BIHCCEHLAOD.IHALNOJAMLE_PlayerCounterMasterName;
			data.FAMHAPONILI_PlayerIds = new System.Collections.Generic.List<int>(100);
			int c = Mathf.Min(BIHCCEHLAOD.FAMHAPONILI_PlayerIds.Count, HMFFHLPNMPH_count + 100);
			for(int i = HMFFHLPNMPH_count; i < c; i++)
			{
		...
		res.BHFHGFKBOHH_OnSuccess = (CACGCMBKHDI_NetBaseAction _HKICMNAACDA_a) =>
		{
			...
			foreach(var p in req.NFEAMMJIMPG_Result.OJCNJFLJCLA_player_counters)
			{
				NFEAMMJIMPG_Result.OJCNJFLJCLA_player_counters.Add(p.Key, p.Value);
			}
		};
		return res;
	}
	...
	{
		return HMFFHLPNMPH_count < BIHCCEHLAOD.FAMHAPONILI_PlayerIds.Count;
	}
}
