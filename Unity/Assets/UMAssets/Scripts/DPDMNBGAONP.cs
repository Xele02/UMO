...
public class DPDMNBGAONP : IHFBKJMNIPH<DKFCEGODKFJ_GetPlayerCounters>
{
	...
	{
		HMFFHLPNMPH_Count = 0;
		NFEAMMJIMPG_Result.OJCNJFLJCLA = new System.Collections.Generic.Dictionary<int, int>(BIHCCEHLAOD.FAMHAPONILI_PlayerIds.Count);
	}
	...
	protected override DKFCEGODKFJ_GetPlayerCounters JIJACMIFOMB_OnStartAction(PJKLMCGEJMK _CPHFEPHDJIB_ServerRequester)
	{
		...
		res.BHFHGFKBOHH_OnSuccess = (CACGCMBKHDI_Request HKICMNAACDA) =>
		{
			...
			foreach(var p in req.NFEAMMJIMPG_Result.OJCNJFLJCLA)
			{
				NFEAMMJIMPG_Result.OJCNJFLJCLA.Add(p.Key, p.Value);
			}
		};
		return res;
	}
