...
public class AIBFEFOFMFK_LineLinkage : AILHMHMOKKA_BaseLinkage
{
	...
	public override void EMKKJILHOOB_GetLinkageStatus(IMCBBOAFION _BHFHGFKBOHH_OnSuccess, JFDNPFFOACP NIMPEHIECJH, DJBHIFLHJLK _AOCANKOMKFG_OnError)
	{
		...
		LNBLBFPAKPP_GetLineLinkageStatus req = NKGJPJPHLIF_SakashoManager.HHCJCDFCLOB_Instance.IBLPICFDGOF_ServerRequester.IFFNCAFNEAG_AddRequest(new LNBLBFPAKPP_GetLineLinkageStatus());
	...
	{
		OMOHMPMNOJG req = NKGJPJPHLIF_SakashoManager.HHCJCDFCLOB_Instance.IBLPICFDGOF_ServerRequester.IFFNCAFNEAG_AddRequest(new OMOHMPMNOJG());
		req.ICDEFIIADDO_Timeout = 15;
		req.BHFHGFKBOHH_OnSuccess = (CACGCMBKHDI_NetBaseAction JIPCHHHLOMM) =>
		{
			//0xCCB600
	...
	{
		HJMMIMBMFFP req = NKGJPJPHLIF_SakashoManager.HHCJCDFCLOB_Instance.IBLPICFDGOF_ServerRequester.IFFNCAFNEAG_AddRequest(new HJMMIMBMFFP());
		req.ICDEFIIADDO_Timeout = 15;
		req.BHFHGFKBOHH_OnSuccess = (CACGCMBKHDI_NetBaseAction JIPCHHHLOMM) =>
		{
			//0xCCB770
	...
	{
		MOJEDCPFGJJ_IsVersionOk = false;
		AOODABADOGG req = NKGJPJPHLIF_SakashoManager.HHCJCDFCLOB_Instance.IBLPICFDGOF_ServerRequester.IFFNCAFNEAG_AddRequest(new AOODABADOGG());
		req.ICDEFIIADDO_Timeout = 15;
		req.NBFDEFGFLPJ = NPOAJHCHJGF;
		req.BHFHGFKBOHH_OnSuccess = (CACGCMBKHDI_NetBaseAction JIPCHHHLOMM) =>
		{
			//0xCCB8DC
			NKGJPJPHLIF_SakashoManager.HHCJCDFCLOB_Instance.IJMGMJHLGDG((JIPCHHHLOMM as AOODABADOGG).NFEAMMJIMPG_Result.EHGBICNIBKE_player_id);
			_BHFHGFKBOHH_OnSuccess();
		};
		req.MOBEEPPKFLG_OnFail = (CACGCMBKHDI_NetBaseAction JIPCHHHLOMM) =>
		{
