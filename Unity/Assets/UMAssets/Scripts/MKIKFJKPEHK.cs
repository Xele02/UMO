...
public class MKIKFJKPEHK
{
	...
	{
		if ((ENKGMPLGEIJ.MNNHHJBBICA_EventType > 0 && ENKGMPLGEIJ.MNNHHJBBICA_EventType < 4) || ENKGMPLGEIJ.MNNHHJBBICA_EventType == 6)
		{
			...
			if(ev != null)
			{
				if((int)ev.HIDHLFCBIDE_EventType == ENKGMPLGEIJ.MNNHHJBBICA_EventType)
				{
		...
		else if(ENKGMPLGEIJ.MNNHHJBBICA_EventType == 0)
		{
	...
	{
		IKDICBBFBMI_EventBase ev = JEPBIIJDGEF_EventInfo.HHCJCDFCLOB.MKBJOOAILBB_GetEventByStatus(KGCNCBOKCBA.GNENJEHKMHD.EMAMLLFAOJI_6/*6*/, false);
		if(ev != null)
		{
			long t = NKGJPJPHLIF.HHCJCDFCLOB.IBLPICFDGOF_ServerRequester.FJDBNGEPKHL.KMEFBNBFJHI_GetServerTime();
