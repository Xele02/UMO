...
public class JJOELIOGMKK_DivaIntimacyInfo
{
	...
	private bool MOACIBEKLEN(int DNBFMLBNAEE)
	{
		...
		int gainLevel = CIOECGOMILE.HHCJCDFCLOB.AHEFHIMGIBI_PlayerData.DGCJCAHIAPP_Diva.BJBKCMPCPME(AHHJLDLAPAN_DivaId, DNBFMLBNAEE, IMMAOANGPNK.HHCJCDFCLOB.NKEBMCIMJND_Database.KDIALKDKBGE_Intimacy);
		...
		HCDGELDHFHB(false);
		CIOECGOMILE.HHCJCDFCLOB.AHEFHIMGIBI_PlayerData.KCCLEHLLOFG_Common.MJDMEKMGFJA_IntimacyTouchSaveTime = NKGJPJPHLIF.HHCJCDFCLOB.IBLPICFDGOF_ServerRequester.FJDBNGEPKHL.KMEFBNBFJHI_GetServerTime();
		return true;
	}
			...
			if(CIOECGOMILE.HHCJCDFCLOB.AHEFHIMGIBI_PlayerData != null)
			{
				...
				if(saveIntimacy.BFINGCJHOHI_Cnt - HMFFHLPNMPH > -1)
				{
					...
					if(MOACIBEKLEN(HBODCMLFDOB.NLFMNHGEADA))
					{
						...
						ILCCJNDFFOB.HHCJCDFCLOB.DNNLEIKNHKL(AHHJLDLAPAN_DivaId, NHCCINMHEAB_Tension, HBODCMLFDOB.ODHAIDDEFJL_GetExp, HBODCMLFDOB.EOIJEGJDLAN_AfterExp, HBODCMLFDOB.CPDEMMFGKED_Level, HBODCMLFDOB.KBHJOBKOOGC_NextLevel, JpStringLiterals.StringLiteral_12004, _ADJBIEOILPJ_ItemPresentId, HMFFHLPNMPH, JGLMDLKHFGG_FavBonus, MFFJEALABOG_TensionBonus);
						return true;
                    }
				}
			}
	...
	public void HCDGELDHFHB(bool FBBNPFFEJBN/* = false*/)
	{
		...
		for(int i = HBODCMLFDOB.CPDEMMFGKED_Level; i < HBODCMLFDOB.KBHJOBKOOGC_NextLevel; i++)
		{
			...
			{
				DOMNLGKLGEH_SetMaxSkillLevel(CIOECGOMILE.HHCJCDFCLOB.AHEFHIMGIBI_PlayerData, (LPBGKOJDNJK)levelInfo.DMEDKJPOLCH, levelInfo.EIIDPKCBKEK_SkillLevel, FBBNPFFEJBN);
				...
				if (a > 0)
				{
					COOGHGBNEMB(CIOECGOMILE.HHCJCDFCLOB.AHEFHIMGIBI_PlayerData, a, 1, levelInfo.ANAJIAENLNB);
				}
			...
			if(IMMAOANGPNK.HHCJCDFCLOB.NKEBMCIMJND_Database != null)
			{
				if(CIOECGOMILE.HHCJCDFCLOB.AHEFHIMGIBI_PlayerData != null)
				{
					...
					if(d.PLALNIIBLOF == 2)
					{
						...
						AMHBDAMIAJM(CIOECGOMILE.HHCJCDFCLOB.AHEFHIMGIBI_PlayerData, d.GLCLFMGPMAN_FullItemId, 1);
