...
public class HLEBAINCOME_EventScore : IKDICBBFBMI_EventBase
{
	...
	protected override bool IMCMNOPNGHO(long JHNMKKNEENE)
	{
		...
		{
			FEHINJKHDAP_EventScore.ALGDNCMJHGN save = CIOECGOMILE.HHCJCDFCLOB.AHEFHIMGIBI_PlayerData.KOOKJBMGBIG_EventScore.FBCJICEPLED[ev.NGHKJOEDLIP_Setting.MOEKELIIDEO_SaveSlot];
			if(ev.NGHKJOEDLIP_Setting.OCGFKMHNEOF_NameForApi != save.MPCAGEPEJJJ_Key || save.EGBOHDFBAPB_CloseAt == 0 || (!RuntimeSettings.CurrentSettings.UnlimitedEvent && save.EGBOHDFBAPB_CloseAt < ev.NGHKJOEDLIP_Setting.BONDDBOFBND_RankingStart))
			{
				save.LHPDDGIJKNB();
				save.MPCAGEPEJJJ_Key = ev.NGHKJOEDLIP_Setting.OCGFKMHNEOF_NameForApi;
				save.EGBOHDFBAPB_CloseAt = ev.NGHKJOEDLIP_Setting.IDDBFFBPNGI_RewardEnd;
			}
			return true;
		}
		return false;
		...
		if(KBACNOCOANM != null)
		{
			...
			if(ev != null)
			{
					...
					NGOFCFJHOMI_Status == KGCNCBOKCBA.GNENJEHKMHD_EventStatus.EMAMLLFAOJI_Counting_6) //0x6cU
				{
					...
					{
						//0x15EEC84
						return _GHPLINIACBB_x.OCGFKMHNEOF_NameForApi == KBACNOCOANM.OCGFKMHNEOF_NameForApi;
					});
					if(e != null)
					{
