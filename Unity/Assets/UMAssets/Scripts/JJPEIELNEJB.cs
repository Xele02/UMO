...
public class JJPEIELNEJB
{
		...
		public void FBANBDCOEJL()
		{
			if(IMMAOANGPNK.HHCJCDFCLOB.NKEBMCIMJND_Database != null && GNGMCIAIKMA.HHCJCDFCLOB != null)
			{
				...
				if (bingo2 == null || ACKBJLLNLJF < 1)
				{
					for(int i = 0; i < LPLDJDCELAN_RewardsId.Length; i++)
					{
						LPLDJDCELAN_RewardsId[i].DNJEJEANJGL_Value = r.EIOBAIJMNGL[i].PPFNGGCBJKC;
					}
				}
				else
				{
					for (int i = 0; i < LPLDJDCELAN_RewardsId.Length; i++)
					{
						EKLNMHFCAOI.FKGCBLHOOCL_Category cat = EKLNMHFCAOI.BKHFLDMOGBD_GetItemCategory(r.EIOBAIJMNGL[i].PPFNGGCBJKC);
						if(cat == EKLNMHFCAOI.FKGCBLHOOCL_Category.HJNNKCMLGFL_None)
						{
							LPLDJDCELAN_RewardsId[i].DNJEJEANJGL_Value = bingo2.GLCLFMGPMAN[r.EIOBAIJMNGL[i].PPFNGGCBJKC - 1].DNJEJEANJGL_Value;
						}
						else
						{
							LPLDJDCELAN_RewardsId[i].DNJEJEANJGL_Value = r.EIOBAIJMNGL[i].PPFNGGCBJKC;
				...
				FHLGEKGBLLN = OLPBIMOPHLM_ReceiveCount == 8;
				...
				for(int i = 0; i < bl.Count; i++)
				{
					...
					BECEJMHDBBP[i].MPBADMODLOJ = bl[i].PMDLBHLNIDP;
				...
				for(int i = 0; i < r.EIOBAIJMNGL.Count; i++)
				{
					...
					data.GLCLFMGPMAN_ItemId = r.EIOBAIJMNGL[i].PPFNGGCBJKC;
					if (r.EIOBAIJMNGL[i].PPFNGGCBJKC > 0 && r.EIOBAIJMNGL[i].PPFNGGCBJKC < 9 && bingo2 != null)
					{
						data.GLCLFMGPMAN_ItemId = bingo2.GLCLFMGPMAN[r.EIOBAIJMNGL[i].PPFNGGCBJKC - 1].DNJEJEANJGL_Value;
					}
					...
					data.FLGEGLADKHC = BECEJMHDBBP[i].FEMMDNIELFC_MissionText;
					data.LJKMKCOAICL_ItemCount = r.EIOBAIJMNGL[i].BFINGCJHOHI;
	...
	public class JLHHGLANHGE
	{
		...
		public int GLCLFMGPMAN_ItemId { get { return AHGCGHAAHOO ^ FBGGEFFJJHB; } set { AHGCGHAAHOO = value ^ FBGGEFFJJHB; } } //0x1358B50 LNJAENEGDEL 0x1358B60 JHIDBGCHOKL
