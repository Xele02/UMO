...
public class EAJCBFGKKFA_FriendInfo
{
	...
	public void KHEKNNFCAOI_Init(IBIGBMDANNM NIMOGBDCMLJ)
	{
		if(NIMOGBDCMLJ.AHEFHIMGIBI_PlayerData.MHEAEGMIKIE_PublicStatus.AFBMEMCHJCL_MainScene.PPFNGGCBJKC_id == 0)
		{
		...
		else
		{
			...
			JNMFKOHFAFB_PublicStatus.KNHIPBADANI card = NIMOGBDCMLJ.AHEFHIMGIBI_PlayerData.MHEAEGMIKIE_PublicStatus.AFBMEMCHJCL_MainScene;
		...
		MGMFOJPNDGA_AssistData.LDHHJFHGGMA(NIMOGBDCMLJ.AHEFHIMGIBI_PlayerData);
		HDJOHAJPGBA_SubScene.Clear();
		for(int i = 0; i < NIMOGBDCMLJ.AHEFHIMGIBI_PlayerData.MHEAEGMIKIE_PublicStatus.HDJOHAJPGBA_SubScene.Count; i++)
		{
			JNMFKOHFAFB_PublicStatus.KNHIPBADANI card = NIMOGBDCMLJ.AHEFHIMGIBI_PlayerData.MHEAEGMIKIE_PublicStatus.HDJOHAJPGBA_SubScene[i];
		...
		if(NIMOGBDCMLJ.AHEFHIMGIBI_PlayerData.MHEAEGMIKIE_PublicStatus.DIPKCALNIII_DivaId != 0)
		{
			JIGONEMPPNP_Diva = new FFHPBEPOMAK_DivaInfo();
			JIGONEMPPNP_Diva.KHEKNNFCAOI_Init(NIMOGBDCMLJ.AHEFHIMGIBI_PlayerData.MHEAEGMIKIE_PublicStatus.DIPKCALNIII_DivaId,
				NIMOGBDCMLJ.AHEFHIMGIBI_PlayerData.MHEAEGMIKIE_PublicStatus.ANAJIAENLNB_Level,
				NIMOGBDCMLJ.AHEFHIMGIBI_PlayerData.MHEAEGMIKIE_PublicStatus.BEEAIAAJOHD_CostumeId,
				NIMOGBDCMLJ.AHEFHIMGIBI_PlayerData.MHEAEGMIKIE_PublicStatus.AFNIOJHODAG_CostumeColorId,
				AFBMEMCHJCL_MainScene, HDJOHAJPGBA_SubScene, false);
		}
		PCEGKKLKFNO_FriendData = NIMOGBDCMLJ;
		PDIPANKOKOL_FriendStat = NIMOGBDCMLJ.LHMDABPNDDH_state;
		LBODHBDOMGK_PlayerName = NIMOGBDCMLJ.LBODHBDOMGK_PlayerName;
		if(NIMOGBDCMLJ.AHEFHIMGIBI_PlayerData == null)
		{
			FGMPKKOOGCM_Comment = "";
		...
		{
			FGMPKKOOGCM_Comment = NIMOGBDCMLJ.AHEFHIMGIBI_PlayerData.JHFIPCIHJNL_Base.CMKKFCGBILD_Prof;
			LNFMPMEIMPH_SceneCnt = NIMOGBDCMLJ.AHEFHIMGIBI_PlayerData.MHEAEGMIKIE_PublicStatus.BKIFLJAMJGG_SceneCount;
		}
		ILOJAJNCPEC_Rank = NIMOGBDCMLJ.ADFIHAPELAN_PLevel;
		MLPEHNBNOGD_PlayerId = NIMOGBDCMLJ.MLPEHNBNOGD_PlayerId;
		NDOLELKAJNL_Degree = new IAPDFOPPGND();
		if(NIMOGBDCMLJ.AHEFHIMGIBI_PlayerData.MHEAEGMIKIE_PublicStatus == null)
		{
			NDOLELKAJNL_Degree.KHEKNNFCAOI_Init(1, 0);
		}
		else
		{
			NDOLELKAJNL_Degree.KHEKNNFCAOI_Init(NIMOGBDCMLJ.AHEFHIMGIBI_PlayerData.MHEAEGMIKIE_PublicStatus.ABLOIBMGLFD_EmId,
				NIMOGBDCMLJ.AHEFHIMGIBI_PlayerData.MHEAEGMIKIE_PublicStatus.FHCAFLCLGAA_EmCnt);
		}
		...
		PPMGIDEHHDI_ViewHSRatingData.KHEKNNFCAOI_Init(NIMOGBDCMLJ);
		...
		FAABJIHJKEM_DcNm = NIMOGBDCMLJ.AHEFHIMGIBI_PlayerData.MHEAEGMIKIE_PublicStatus.NAKJJBEIION_DcNm;
		BHJLNGEDEGN = NIMOGBDCMLJ.AHEFHIMGIBI_PlayerData.MHEAEGMIKIE_PublicStatus.DALCINDEJLC_DcTm != 0;
	}
	...
	public void KLJNFJJMJMC(IMCBBOAFION _BHFHGFKBOHH_OnSuccess, DJBHIFLHJLK _AOCANKOMKFG_OnError, bool OJEBNBLHPNP/* = false*/)
	{
		...
		if(MLPEHNBNOGD_PlayerId > 9999)
		{
			...
			req.PINPBOCDKLI_OnPlayerCb = (int _BMBBDIAEOMP_i, int _EHGBICNIBKE_PlayerId, long _IFNLEKOILPM_UpdatedAt, List<string> _OHNJJIMGKGK_Names, EDOHBJAPLPF_JsonData _IDLHJIOMJBK_Data) =>
			{
				//0x14F1AEC
			...
			req.BHFHGFKBOHH_OnSuccess = (CACGCMBKHDI_Request JIPCHHHLOMM) =>
			{
				...
				PCEGKKLKFNO_FriendData.AHEFHIMGIBI_PlayerData.MHEAEGMIKIE_PublicStatus.FOFGELKGMAH_CosCnt = KPMOBPNENCD_serverData.BEKHNNCGIEL_Costume.EFFKJGEDONM_GetNumUnlockedCostume(IMMAOANGPNK.HHCJCDFCLOB.NKEBMCIMJND_Database.MFPNGNMFEAL_Costume, KPMOBPNENCD_serverData.DGCJCAHIAPP_Diva, true);
				_BHFHGFKBOHH_OnSuccess();
			};
