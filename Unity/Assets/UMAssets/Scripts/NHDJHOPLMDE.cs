...
public class NHDJHOPLMDE
{
	...
	public bool LAKLFHGMCLI(EPIFHEDDJAE.NGEDJNHECKN CNLDPKNLBND, EPIFHEDDJAE.JFEIHHBGFPF_AbilityCondition OAFPGJLCNFM)
	{
		if (EICJJFIBCEF != CNLDPKNLBND)
			return false;
		if(CNLDPKNLBND == EPIFHEDDJAE.NGEDJNHECKN.FJFMLFPJKNB)
		{
			if(CHOFDPDFPDC == EPIFHEDDJAE.JFEIHHBGFPF_AbilityCondition.PPNNBADDNKB)
			{
				if(OAFPGJLCNFM > EPIFHEDDJAE.JFEIHHBGFPF_AbilityCondition.FHBJEIEPABF)
				{
					return false;
				}
			}
			else if(CHOFDPDFPDC == EPIFHEDDJAE.JFEIHHBGFPF_AbilityCondition.FHBJEIEPABF && OAFPGJLCNFM != EPIFHEDDJAE.JFEIHHBGFPF_AbilityCondition.FHBJEIEPABF)
			{
				return false;
			}
		}
