...
public class IMKNEDJDNGC
{
	...
	public void FCFDHFAJKPB()
	{
		if (EDCBHGECEBE_Read)
			return;
		DDEMMEPBOIA_Sns.EFIFBJGKPJF saveSns = CIOECGOMILE_NetPlayerDataManager.HHCJCDFCLOB_Instance.AHEFHIMGIBI_PlayerData.FLHMJHBOBEA_Sns.HAJEJPFGILG[AIPLIEMLHGC_SnsId - 1];
...
public class GAKAAIHLFKI
{
	...
	public int MCGDHHHFBMO_GetUnreadIndex(long _JHNMKKNEENE_Time, bool BAOMADKMHKP/* = false*/)
	{
		...
		for(int i = 0; i < CNEOPOINCBA.Count; i++)
		{
			if (_JHNMKKNEENE_Time < CNEOPOINCBA[i].BEBJKJKBOGH_date)
				return res;
			if (CNEOPOINCBA[i].OIPCCBHIKIA_index + 1 <= CIOECGOMILE_NetPlayerDataManager.HHCJCDFCLOB_Instance.AHEFHIMGIBI_PlayerData.FLHMJHBOBEA_Sns.HAJEJPFGILG[CNEOPOINCBA[i].AIPLIEMLHGC_SnsId - 1].LDJIMGPHFPA_Cnt)
				res++;
		}
		return res;
	}
	...
	public int BDNLOOAGKKE(long _JHNMKKNEENE_Time, bool BAOMADKMHKP/* = false*/, bool MIENHPPDMNG/* = false*/)
	{
		...
		for(int i = 0; i < CNEOPOINCBA.Count; i++)
		{
			if(!MIENHPPDMNG || !CNEOPOINCBA[i].EDCBHGECEBE_Read)
			{
				if (_JHNMKKNEENE_Time < CNEOPOINCBA[i].BEBJKJKBOGH_date)
					return res;
				if (CIOECGOMILE_NetPlayerDataManager.HHCJCDFCLOB_Instance.AHEFHIMGIBI_PlayerData.FLHMJHBOBEA_Sns.HAJEJPFGILG[CNEOPOINCBA[i].AIPLIEMLHGC_SnsId - 1].LDJIMGPHFPA_Cnt < CNEOPOINCBA[i].OIPCCBHIKIA_index + 1)
					res++;
			}
		}
		return res;
