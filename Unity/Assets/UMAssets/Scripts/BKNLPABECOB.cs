...
public class BKNLPABECOB_FacebookLinkage : AILHMHMOKKA_BaseLinkage
{
	...
	public override void EMKKJILHOOB_GetLinkageStatus(IMCBBOAFION _BHFHGFKBOHH_OnSuccess, JFDNPFFOACP NIMPEHIECJH, DJBHIFLHJLK _AOCANKOMKFG_OnError)
	{
		...
		NCBEKFBAFCL_GetFacebookLinkageStatus req = NKGJPJPHLIF_SakashoManager.HHCJCDFCLOB_Instance.IBLPICFDGOF_ServerRequester.IFFNCAFNEAG_AddRequest(new NCBEKFBAFCL_GetFacebookLinkageStatus());
	...
	{
		NPPLCKMKIFE req = NKGJPJPHLIF_SakashoManager.HHCJCDFCLOB_Instance.IBLPICFDGOF_ServerRequester.IFFNCAFNEAG_AddRequest(new NPPLCKMKIFE());
		req.ICDEFIIADDO_Timeout = 15;
		req.BHFHGFKBOHH_OnSuccess = (CACGCMBKHDI_NetBaseAction JIPCHHHLOMM) =>
		{
			//0xC89D3C
	...
	{
		NIAECEJLJKH req = NKGJPJPHLIF_SakashoManager.HHCJCDFCLOB_Instance.IBLPICFDGOF_ServerRequester.IFFNCAFNEAG_AddRequest(new NIAECEJLJKH());
		req.ICDEFIIADDO_Timeout = 15;
		req.BHFHGFKBOHH_OnSuccess = (CACGCMBKHDI_NetBaseAction JIPCHHHLOMM) =>
		{
			//0xC89E8C
	...
	{
		ANLMHAMGJHP req = NKGJPJPHLIF_SakashoManager.HHCJCDFCLOB_Instance.IBLPICFDGOF_ServerRequester.IFFNCAFNEAG_AddRequest(new ANLMHAMGJHP());
		req.ICDEFIIADDO_Timeout = 15;
		req.NBFDEFGFLPJ = NPOAJHCHJGF;
		req.BHFHGFKBOHH_OnSuccess = (CACGCMBKHDI_NetBaseAction JIPCHHHLOMM) =>
		{
			//0xC89FDC
			NKGJPJPHLIF_SakashoManager.HHCJCDFCLOB_Instance.IJMGMJHLGDG((JIPCHHHLOMM as ANLMHAMGJHP).NFEAMMJIMPG_Result.EHGBICNIBKE_player_id);
			_BHFHGFKBOHH_OnSuccess();
		};
		req.MOBEEPPKFLG_OnFail = (CACGCMBKHDI_NetBaseAction JIPCHHHLOMM) =>
		{
