...
public class KKLGENJKEBN
{
	...
	public void DNJKPPCBINA(string LJNAKDMILMC, IMCBBOAFION KLMFJJCNBIP, DJBHIFLHJLK PDPNECFNLNI, IMCBBOAFION FHKCKGJPFFO, DJBHIFLHJLK PPCMIJIFPGP, DJBHIFLHJLK BFKEGJMPELF, int BLJMMMABHDA = 0, int BBLCHHIGFAE = -1)
	{
        ...
        OKPEFAPPFDH_GetRanksAroundSelf req = CPHFEPHDJIB.IFFNCAFNEAG_AddRequest(new OKPEFAPPFDH_GetRanksAroundSelf(false));
		req.EMPNJPMAKBF_Id = IDHFDLLDCEL.PPFNGGCBJKC_Id;
		...
		req.BHFHGFKBOHH_OnSuccess = (CACGCMBKHDI_Request NHECPMNKEFK) =>
		{
			...
			BBIIHPDBEHP LBHGPLCOCHD = CPHFEPHDJIB.IFFNCAFNEAG_AddRequest(new BBIIHPDBEHP(false));
			LBHGPLCOCHD.EMPNJPMAKBF = IDHFDLLDCEL.PPFNGGCBJKC_Id;
			CACGCMBKHDI_Request.HDHIKGLMOGF DMLJLPMBLCH = (CACGCMBKHDI_Request DLMLKJACBPM) =>
			{
				//0x1A078E0
			...
			LBHGPLCOCHD.BHFHGFKBOHH_OnSuccess = (CACGCMBKHDI_Request DLMLKJACBPM) =>
			{
				...
				GNDMFGGOPII_GetInventoryRecords req2 = CPHFEPHDJIB.IFFNCAFNEAG_AddRequest(new GNDMFGGOPII_GetInventoryRecords());
				if(BBLCHHIGFAE < 0)
				{
					req2.AMOMNBEAHBF = req3.NFEAMMJIMPG.COGMPENEPBD;
				}
				else
				{
					req2.AMOMNBEAHBF.Add(req3.NFEAMMJIMPG.COGMPENEPBD[BBLCHHIGFAE]);
				}
	...
	private IEnumerator ACAJCMEIJCH_Coroutine_AddRankingPlayerListSecond(int CJHEHIMLGGL, int NEFEFHBHFFF, LIOLBKLMMIK KLMFJJCNBIP, DJBHIFLHJLK IDAEHNGOKAE, DJBHIFLHJLK JGKOLBLPMPG, bool EEJADLBBMLH = false)
	{
		...
		{
			PPFGOOFFNMB_GetTopRanks req = NKGJPJPHLIF.HHCJCDFCLOB.IBLPICFDGOF_ServerRequester.IFFNCAFNEAG_AddRequest(new PPFGOOFFNMB_GetTopRanks());
			req.EMPNJPMAKBF_Id = MEMPPLMGNKN;
			req.MJGOBEGONON_Type = EBJDDBPFNPB ? 1 : 0;
			req.MLPLGFLKKLI_Ipp = MLPLGFLKKLI;
