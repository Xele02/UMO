...
public class KKLGENJKEBN
{
	...
	//public void KMBMMCDPKDF(bool JKDJCFEBDHC, int _FJOLNJLLJEJ_Rank) { }
	...
	public void LDOBHAAIDEJ_UpdateRankingScore(string LJNAKDMILMC, int LHJCOPMMIGO, double HOCPLDLAIGL, IMCBBOAFION KLMFJJCNBIP, DJBHIFLHJLK NEFKBBNKNPP, DJBHIFLHJLK JGKOLBLPMPG)
	{
		...
		TodoLogger.LogWarning(TodoLogger.SakashoServer, "Ranking not found : " + LJNAKDMILMC);
		NEFKBBNKNPP();
	}

	//// RVA: 0x1A0528C Offset: 0x1A0528C VA: 0x1A0528C
	public void DNJKPPCBINA(string LJNAKDMILMC, IMCBBOAFION KLMFJJCNBIP, DJBHIFLHJLK PDPNECFNLNI, IMCBBOAFION FHKCKGJPFFO, DJBHIFLHJLK PPCMIJIFPGP, DJBHIFLHJLK _BFKEGJMPELF_OnError, int BLJMMMABHDA/* = 0*/, int BBLCHHIGFAE/* = -1*/)
	{
		...
		if(LJNAKDMILMC == "*")
		{
			...
			IDHFDLLDCEL = JPDPFGFMKHK[BLJMMMABHDA];
        }
		else
		{
			IDHFDLLDCEL = JPDPFGFMKHK.Find((EECOJKDJIFG HKICMNAACDA) =>
			{
				//0x1A07338
				return HKICMNAACDA.OCGFKMHNEOF_NameForApi == LJNAKDMILMC;
			});
		...
		req.BHFHGFKBOHH_OnSuccess = (CACGCMBKHDI_Request NHECPMNKEFK) =>
		{
			...
			BBIIHPDBEHP LBHGPLCOCHD = CPHFEPHDJIB_ServerRequester.IFFNCAFNEAG_AddRequest(new BBIIHPDBEHP(false));
			LBHGPLCOCHD.EMPNJPMAKBF_Id = IDHFDLLDCEL.PPFNGGCBJKC_Id;
			CACGCMBKHDI_Request.HDHIKGLMOGF DMLJLPMBLCH = (CACGCMBKHDI_Request DLMLKJACBPM) =>
			{
				//0x1A078E0
			...
			LBHGPLCOCHD.MOBEEPPKFLG_OnFail = DMLJLPMBLCH;
			LBHGPLCOCHD.BHFHGFKBOHH_OnSuccess = (CACGCMBKHDI_Request DLMLKJACBPM) =>
			{
				//0x1A07D64
				...
				if(BBLCHHIGFAE < 0)
				{
					req2.AMOMNBEAHBF_InventoryIds = req3.NFEAMMJIMPG.COGMPENEPBD_InventoryIds;
				}
				else
				{
					req2.AMOMNBEAHBF_InventoryIds.Add(req3.NFEAMMJIMPG.COGMPENEPBD_InventoryIds[BBLCHHIGFAE]);
				}
				CACGCMBKHDI_Request.HDHIKGLMOGF NDBKOPGCPHJ = (CACGCMBKHDI_Request NGHEOHLOOIH) =>
				{
					...
					for(int i = 0; i < req4.NFEAMMJIMPG.PJJFEAHIPGL.Count; i++)
					{
						PHOPJDPEHJA.Add(req4.NFEAMMJIMPG.PJJFEAHIPGL[i]);
					}
					LNHFLJBGGJB = false;
					KLMFJJCNBIP();
				};
	...
	public void JPNACOLKHLB_AddRankingPlayerListSecond(int CJHEHIMLGGL, int NEFEFHBHFFF, LIOLBKLMMIK KLMFJJCNBIP, DJBHIFLHJLK IDAEHNGOKAE, DJBHIFLHJLK JGKOLBLPMPG, bool EEJADLBBMLH/* = false*/)
	{
		N.a.StartCoroutineWatched(ACAJCMEIJCH_Coroutine_AddRankingPlayerListSecond(CJHEHIMLGGL, NEFEFHBHFFF, KLMFJJCNBIP, IDAEHNGOKAE, JGKOLBLPMPG, EEJADLBBMLH));
	}

	//[IteratorStateMachineAttribute] // RVA: 0x6BA3CC Offset: 0x6BA3CC VA: 0x6BA3CC
	//// RVA: 0x1A05D58 Offset: 0x1A05D58 VA: 0x1A05D58
	private IEnumerator ACAJCMEIJCH_Coroutine_AddRankingPlayerListSecond(int CJHEHIMLGGL, int NEFEFHBHFFF, LIOLBKLMMIK KLMFJJCNBIP, DJBHIFLHJLK IDAEHNGOKAE, DJBHIFLHJLK JGKOLBLPMPG, bool EEJADLBBMLH/* = false*/)
	{
		//0x1A0A328
		if(NEFEFHBHFFF == 0)
		{
		...
		else
		{
			...
			req.MJGOBEGONON_Type = EBJDDBPFNPB ? 1 : 0;
			req.MLPLGFLKKLI_Ipp = MLPLGFLKKLI;
			req.IGNIIEBMFIN_Page = (NEFEFHBHFFF == -1 ? CJHEHIMLGGL - 1 - MLPLGFLKKLI : CJHEHIMLGGL) / MLPLGFLKKLI + 1;
			...
			req.BHFHGFKBOHH_OnSuccess = (CACGCMBKHDI_Request NHECPMNKEFK) =>
			{
				...
				else
				{
					...
					FGEIGGNCGGD_GetPlayerData(l, CJHEHIMLGGL, NEFEFHBHFFF, r.NFEAMMJIMPG.EJDEDOJFOOK, KLMFJJCNBIP, IDAEHNGOKAE, JGKOLBLPMPG, EEJADLBBMLH);
				}
			};
			req.MOBEEPPKFLG_OnFail = (CACGCMBKHDI_Request NHECPMNKEFK) =>
			{
	...
	public void HEOKADCEAGL_GetRanks(string LJNAKDMILMC, IMCBBOAFION KLMFJJCNBIP_OnSuccess, DJBHIFLHJLK IDAEHNGOKAE_OnRankingError, DJBHIFLHJLK _BFKEGJMPELF_OnError, DJBHIFLHJLK OPJANCPEKAB_OnDroppedPlayerError)
	{
		LPPCNCMEDFA_Rank = 0;
		IPNEJCOFBIB_ScoreTrunc = 0;
